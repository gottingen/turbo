...
const (
	...
	// a metric scope for a prometheus reporter.
	DefaultSeparator = "_"
)
...
// Reporter is a Prometheus backed metric reporter.
...
func (m noopMetric) ValueBucket(lower, upper float64) metrics.CachedHistogramBucket {
	return m
}
func (m noopMetric) DurationBucket(lower, upper time.Duration) metrics.CachedHistogramBucket {
	return m
}
...
// Options is a set of options for the metric reporter.
type Options struct {
...
// AllocateCounter implements metrics.CachedStatsReporter.
...
// AllocateGauge implements metrics.CachedStatsReporter.
...
func canonicalMetricID(name string, tagKeys []string) metricID {
	...
	return metricID(metrics.KeyForPrefixedStringMap(name, keySet))
}
