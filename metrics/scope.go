...
type scope struct {
	...
	integerGauges map[string]*integerGauge
...
func newRootScope(opts ScopeOptions, interval time.Duration) *scope {
	...
	s := &scope{
		...
		integerGauges: make(map[string]*integerGauge),
...
func (s *scope) report(r StatsReporter) {
	...
	s.igm.RLock()
	for name, igauge := range s.integerGauges {
		igauge.report(s.fullyQualifiedName(name), s.tags, r)
	}
	s.igm.RUnlock()
...
func (s *scope) cachedReport() {
	...
	s.igm.RLock()
	for _, igauge := range s.integerGauges {
		igauge.cachedReport()
	}
	s.igm.RUnlock()
...
func (s *scope) IntegerGauge(name string) IntegerGauge {
	name = s.sanitizer.Name(name)
	s.igm.RLock()
	val, ok := s.integerGauges[name]
	s.igm.RUnlock()
	if !ok {
		s.igm.Lock()
		val, ok = s.integerGauges[name]
		if !ok {
			...
			val = newIntegerGauge(cachedGauge)
			s.integerGauges[name] = val
		}
		s.igm.Unlock()
	}
	return val
