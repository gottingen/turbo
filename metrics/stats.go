...
func newIntegerGauge(cachedGauge CachedGauge) *integerGauge {
	return &integerGauge{cachedGauge: cachedGauge}
}

func (g *integerGauge) Update(v int64) {
	atomic.StoreInt64(&g.curr, v)
	atomic.StoreUint64(&g.updated, 1)
}

func (g *integerGauge) Inc(v int64) {
	atomic.AddInt64(&g.curr, v)
	atomic.StoreUint64(&g.updated, 1)
}

func (g *integerGauge) Dec(v int64) {
	atomic.AddInt64(&g.curr, -v)
	atomic.StoreUint64(&g.updated, 1)
}

func (g *integerGauge) value() float64 {
	return float64(atomic.LoadInt64(&g.curr))
}

func (g *integerGauge) report(name string, tags map[string]string, r StatsReporter) {
	if atomic.SwapUint64(&g.updated, 0) == 1 {
		r.ReportGauge(name, tags, g.value())
	}
}

func (g *integerGauge) cachedReport() {
	if atomic.SwapUint64(&g.updated, 0) == 1 {
		g.cachedGauge.ReportGauge(g.value())
	}
}

func (g *integerGauge) snapshot() float64 {
	return float64(atomic.LoadInt64(&g.curr))
}
