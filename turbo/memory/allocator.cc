...
namespace {
    ...
    inline void *allocate_pages(size_t count) {
        ...
        TURBO_CHECK(mapping != MAP_FAILED) << "Failed to allocate" << int(count) << " pages";
    ...
    inline void freePages(void *ptr, size_t count) {
        ...
        (void) res;
        TURBO_CHECK(res == 0) << "Failed to free " << int(count) << " pages at " << ptr;
    }

    inline void protect_page(void *addr) {
        auto res = mprotect(addr, page_size(), PROT_NONE);
        (void) res;
        TURBO_CHECK(res == 0) << "Failed to protect page at " << addr;
    }
}  // anonymous namespace
...
inline void* allocate_pages(size_t count) {
  ...
  zx_handle_close(vmo);
  (void)status;
  TURBO_CHECK(status == ZX_OK, "Failed to allocate %d pages", int(count));
...
inline void freePages(void* ptr, size_t count) {
  ...
  (void)status;
  TURBO_CHECK(status == ZX_OK, "Failed to free %d pages at %p", int(count),
              ptr);
}
inline void protect_page(void* addr) {
  ...
  (void)status;
  TURBO_CHECK(status == ZX_OK, "Failed to protect page at %p", addr);
}
}  // anonymous namespace
#elif defined(_WIN32)
...
inline void* allocate_pages(size_t count) {
  ...
  TURBO_CHECK(mapping != nullptr, "Failed to allocate %d pages", int(count));
  return mapping;
}
inline void freePages(void* ptr, size_t count) {
  (void)count;
  ...
  (void)res;
  TURBO_CHECK(res != 0, "Failed to free %d pages at %p", int(count), ptr);
}
inline void protect_page(void* addr) {
  DWORD oldVal = 0;
  auto res = VirtualProtect(addr, page_size(), PAGE_NOACCESS, &oldVal);
  (void)res;
  TURBO_CHECK(res != 0, "Failed to protect page at %p", addr);
}
}  // anonymous namespace
#else
...
// The pointer returned must be freed with pagedFree().
    void *pagedMalloc(size_t alignment,
                      ...
                      bool guardHigh) {
        (void) alignment;
        TURBO_CHECK(alignment < page_size());
    ...
    inline void *aligned_malloc(size_t alignment, size_t size) {
        ...
        auto aligned = reinterpret_cast<uint8_t *>(turbo::base::align_up(
    ...
    class default_allocator : public turbo::allocator {
    public:
        static default_allocator instance;

        virtual turbo::allocation allocate(
                const turbo::allocation::required_info &request) override {
            void *ptr = nullptr;
            ...
            TURBO_CHECK(ptr != nullptr)<<"allocation failed";
            TURBO_CHECK(reinterpret_cast<uintptr_t>(ptr) % request.alignment == 0)<<"allocation gave incorrect alignment";

            turbo::allocation allocation;
            allocation.ptr = ptr;
            allocation.request = request;
            return allocation;
        }

        virtual void free(const turbo::allocation &allocation) override {
