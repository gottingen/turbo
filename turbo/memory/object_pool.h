...
#define TURBO_ENABLE_POOLABLE_ON_THIS                                          \
  template <typename T, size_t S> friend class ::turbo::ObjectPool;            \
...
template <typename T, size_t S = 65536> class ObjectPool {
  ...
  // sizeof(T)); constexpr static size_t M = (S - offsetof(Block, data)) / X;
  ...
  static_assert(
      M >= 128,
      "block size S must be larger enough to pool at least 128 objects");
...
ObjectPool<T, S>::ObjectPool(unsigned t)
    : //_heap_mask   {(_next_pow2(t) << 1) - 1u},
      //_heap_mask   { _next_pow2(t<<1) - 1u },
      //_heap_mask   {(t << 1) - 1},
      _lheap_mask{_next_pow2((t + 1) << 1) - 1}, _lheaps{_lheap_mask + 1} {
...
template <typename T, size_t S>
void ObjectPool<T, S>::_blocklist_add_impl(Blocklist *curr, Blocklist *prev,
                                           Blocklist *next) {
  next->prev = curr;
  curr->next = next;
  curr->prev = prev;
...
template <typename T, size_t S>
bool ObjectPool<T, S>::_blocklist_is_first(const Blocklist *list,
...
// @head: the head of the list
template <typename T, size_t S>
bool ObjectPool<T, S>::_blocklist_is_last(const Blocklist *list,
                                          const Blocklist *head) {
        ...
        if ((h->u + K * M < h->a) && (h->u < ((F - 1) * h->a / F))) {
          for (size_t i = 0; i < F; i++) {
            if (!_blocklist_is_empty(&h->lists[i])) {
              ...
              // printf("transfer a block (x.u=%lu/x.i=%lu) to the global
              // heap\n", x->u, x->i);
...
typename ObjectPool<T, S>::LocalHeap &ObjectPool<T, S>::_this_heap() {
  ...
  // thread_local auto hv =
  // std::hash<std::thread::id>()(std::this_thread::get_id()); return _lheaps[hv
  // & _lheap_mask];

  return _lheaps[std::hash<std::thread::id>()(std::this_thread::get_id()) &
                 _lheap_mask];
}
