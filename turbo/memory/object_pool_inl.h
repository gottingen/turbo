...
#ifndef TURBO_MEMORY_OBJECT_POOL_INL_H_
#define TURBO_MEMORY_OBJECT_POOL_INL_H_
...
#include "turbo/thread/thread.h"           // TURBO_THREAD_LOCAL
...
// for gcc 3.4.5
    ...
    template<typename T>
    class TURBO_CACHELINE_ALIGNMENT ObjectPool {
    public:
        ...
        struct TURBO_CACHELINE_ALIGNMENT Block {
        ...
        class TURBO_CACHELINE_ALIGNMENT LocalPool {
        public:
...
#undef TURBO_OBJECT_POOL_GET
        ...
        void clear_objects() {
            ...
            if (lp) {
                _local_pool = nullptr;
                turbo::thread::atexit_cancel(LocalPool::delete_local_pool, lp);
                delete lp;
            }
        }
    ...
    private:
        ...
        static bool add_block_group(size_t old_ngroup) {
            ...
            TURBO_SCOPED_LOCK(_block_group_mutex);
        ...
        inline LocalPool *get_or_new_local_pool() {
            ...
            TURBO_SCOPED_LOCK(_change_thread_mutex); //avoid race with clear()
            _local_pool = lp;
            turbo::thread::atexit(LocalPool::delete_local_pool, lp);
        ...
        void clear_from_destructor_of_local_pool() {
            ...
            TURBO_SCOPED_LOCK(_change_thread_mutex);  // including acquire fence.
            // Do nothing if there're active threads.
    ...
    private:
        ...
        static turbo::static_atomic<ObjectPool *> _singleton;
        static pthread_mutex_t _singleton_mutex;
        static TURBO_THREAD_LOCAL LocalPool *_local_pool;
        static turbo::static_atomic<long> _nlocal;
        static turbo::static_atomic<size_t> _ngroup;
        static pthread_mutex_t _block_group_mutex;
        static pthread_mutex_t _change_thread_mutex;
        static turbo::static_atomic<BlockGroup *> _block_groups[OP_MAX_BLOCK_NGROUP];

        std::vector<DynamicFreeChunk *> _free_chunks;
        pthread_mutex_t _free_chunks_mutex;

#ifdef TURBO_OBJECT_POOL_NEED_FREE_ITEM_NUM
        static turbo::static_atomic<size_t> _global_nfree;
#endif
    };
...
// Declare template static variables:
    ...
    template<typename T>
    turbo::static_atomic<ObjectPool<T> *> ObjectPool<T>::_singleton = TURBO_STATIC_ATOMIC_INIT(nullptr);

    template<typename T>
    pthread_mutex_t ObjectPool<T>::_singleton_mutex = PTHREAD_MUTEX_INITIALIZER;

    template<typename T>
    turbo::static_atomic<long> ObjectPool<T>::_nlocal = TURBO_STATIC_ATOMIC_INIT(0);

    template<typename T>
    turbo::static_atomic<size_t> ObjectPool<T>::_ngroup = TURBO_STATIC_ATOMIC_INIT(0);
    ...
    template<typename T>
    turbo::static_atomic<typename ObjectPool<T>::BlockGroup *>
            ObjectPool<T>::_block_groups[OP_MAX_BLOCK_NGROUP] = {};

#ifdef TURBO_OBJECT_POOL_NEED_FREE_ITEM_NUM
    template <typename T>
    turbo::static_atomic<size_t> ObjectPool<T>::_global_nfree = TURBO_STATIC_ATOMIC_INIT(0);
#endif
...
#endif  // TURBO_MEMORY_OBJECT_POOL_INL_H_
