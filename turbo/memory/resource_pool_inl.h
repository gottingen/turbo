...
#ifndef TURBO_MEMORY_RESOURCE_POOL_INL_H_
#define TURBO_MEMORY_RESOURCE_POOL_INL_H_
...
template <typename T>
class TURBO_CACHELINE_ALIGNMENT ResourcePool {
public:
...
private:
    ...
    static turbo::static_atomic<ResourcePool*> _singleton;
    ...
    static turbo::static_atomic<BlockGroup*> _block_groups[RP_MAX_BLOCK_NGROUP];
...
template <typename T>
turbo::static_atomic<ResourcePool<T>*> ResourcePool<T>::_singleton =
    TURBO_STATIC_ATOMIC_INIT(nullptr);

template <typename T>
pthread_mutex_t ResourcePool<T>::_singleton_mutex = PTHREAD_MUTEX_INITIALIZER;

template <typename T>
turbo::static_atomic<long> ResourcePool<T>::_nlocal = TURBO_STATIC_ATOMIC_INIT(0);

template <typename T>
turbo::static_atomic<size_t> ResourcePool<T>::_ngroup = TURBO_STATIC_ATOMIC_INIT(0);
...
template <typename T>
turbo::static_atomic<typename ResourcePool<T>::BlockGroup*>
ResourcePool<T>::_block_groups[RP_MAX_BLOCK_NGROUP] = {};

#ifdef TURBO_RESOURCE_POOL_NEED_FREE_ITEM_NUM
template <typename T>
turbo::static_atomic<size_t> ResourcePool<T>::_global_nfree = TURBO_STATIC_ATOMIC_INIT(0);
#endif
...
#endif  // TURBO_MEMORY_RESOURCE_POOL_INL_H_
