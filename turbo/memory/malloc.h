...
#ifndef TURBO_MEMORY_MALLOC_H_
#define TURBO_MEMORY_MALLOC_H_
...
#include "turbo/memory/detail/malloc_impl.h" /* nolint */
...
#define TURBO_MALLOC_CHECKED_MALLOC \
 ...
 * @return bool
 */
#if defined(TURBO_ASSUME_NO_JEMALLOC) || defined(TURBO_SANITIZE)
    inline bool usingJEMalloc() noexcept {
    return false;
  }
#elif defined(USE_JEMALLOC) && !defined(TURBO_SANITIZE)
    inline bool usingJEMalloc() noexcept {
    return true;
  }
#else

    TURBO_NO_INLINE inline bool usingJEMalloc() noexcept {
 ...
 * @return bool
 */
#if defined(TURBO_ASSUME_NO_TCMALLOC) || defined(TURBO_SANITIZE)
    inline bool usingTCMalloc() noexcept {
    return false;
  }
#elif defined(USE_TCMALLOC) && !defined(TURBO_SANITIZE)
    inline bool usingTCMalloc() noexcept {
    return true;
  }
#else

    TURBO_NO_INLINE inline bool usingTCMalloc() noexcept {
        static const bool result = []() noexcept {
            // Some platforms (*cough* OSX *cough*) require weak symbol checks to be
            // in the form if (mallctl != nullptr). Not if (mallctl) or if (!mallctl)
            // (!!). http://goo.gl/xpmctm
...
#if !defined(TURBO_USE_JEMALLOC) && !defined(USE_TCMALLOC) && \
      (!defined(TURBO_ASSUME_NO_JEMALLOC) || !defined(TURBO_ASSUME_NO_TCMALLOC))
...
inline bool canNallocx() noexcept {
...
#if defined(__BMI2__) && __cplusplus >= 202002L
  ...
  if (std::is_constant_evaluated()) {
...
#else
        spacing = turbo::base::prev_pow_two(minSize) >> 2;
        spacing -= 1;
#endif
        return (minSize + spacing) & (~(spacing));
    }
     ...
     * @return size_t
     */
    inline size_t good_malloc_size(size_t minSize) noexcept {
        if (minSize == 0) {
            return 0;
        }
        ...
        // a good_malloc_size if you want minSize
    ...
    inline void *checked_malloc(size_t size) {
        void *p = malloc(size);
        if (!p) {
            turbo::base::throw_std_bad_alloc();
        }
        return p;
    }
    ...
    TURBO_MALLOC_CHECKED_MALLOC TURBO_NO_INLINE inline void *smartRealloc(
        ...
        if (slack * 2 > currentSize) {
            ...
            auto const result = checked_malloc(newCapacity);
