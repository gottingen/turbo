...
#    include <turbo/tlog/details/windows_include.h>
...
#    endif
#    if defined(TLOG_PREVENT_CHILD_FD)
                if (*fp != nullptr)
                {
...
#else // unix
#    if defined(TLOG_PREVENT_CHILD_FD)
                const int mode_flag = mode == TLOG_FILENAME_T("ab") ? O_APPEND : O_TRUNC;
...
// Return file size according to open FILE* object
            size_t filesize(FILE *f) {
                if (f == nullptr) {
                    throw_tlog_ex("Failed getting file size. fd is null");
                }
...
#    endif
#endif
                throw_tlog_ex("Failed getting file size from fd", errno);
