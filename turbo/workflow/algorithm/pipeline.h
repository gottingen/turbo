...
#ifndef TURBO_WORKFLOW_ALGORITHM_PIPELINE_H_
#define TURBO_WORKFLOW_ALGORITHM_PIPELINE_H_
...
/** @private */
    ...
    Pipeflow can only be created privately by the turbo::Pipeline and
...
*/
    ...
    @em parallel direction or @em serial direction (specified by turbo::PipeType)
    and is coupled with a callable to invoke by the pipeline scheduler.
    The callable must take a referenced turbo::Pipeflow object in the first argument:

    @code{.cpp}
    Pipe{PipeType::SERIAL, [](turbo::Pipeflow&){}}
    @endcode
    ...
    */
    template<typename C = std::function<void(turbo::Pipeflow &)>>
    class Pipe {
    ...
    public:
        ...
        */
        ...
        @param d pipe type (turbo::PipeType)
        @param callable callable type

        The constructor constructs a pipe with the given direction
        (turbo::PipeType::SERIAL or turbo::PipeType::PARALLEL) and the given callable.
        The callable must take a referenced turbo::Pipeflow object in the first argument.
        ...
        */
        ...
        @param type a turbo::PipeType variable
        */
        ...
        @param callable a callable object constructible from std::function<void(turbo::Pipeflow&)>
    ...
    <i>pipeline scheduling framework</i> using a module task in a workflow.
    Unlike the conventional pipeline programming frameworks (e.g., Intel TBB),
    %Workflow's pipeline algorithm does not provide any data abstraction,
    ...
    turbo::Pipeline pipeline(num_lines,
    ...
    turbo::Task task = workflow.composed_of(pipeline)
                            .name("pipeline");
    ...
    Internally, turbo::Pipeline uses std::tuple to store the given sequence of pipes.
    ...
    If applications need to change these pipes, please use turbo::ScalablePipeline.
    */
    template<typename... Ps>
    class Pipeline {
    ...
    public:
        ...
        The first pipe must define a serial direction (turbo::PipeType::SERIAL)
...
// Procedure: _build
    ...
    void Pipeline<Ps...>::_build() {
        ...
        for (size_t l = 0; l < num_lines(); l++) {
            ...
            _tasks[l + 1] = fb.emplace([this, l](turbo::Runtime &rt) mutable {
    ...
    <i>pipeline scheduling framework</i> using a module task in a workflow.
    Unlike turbo::Pipeline that instantiates all pipes upon the construction time,
    turbo::ScalablePipeline allows variable assignments of pipes using range iterators.
    ...
    turbo::ScalablePipeline pl(num_lines, pipes.begin(), pipes.end());
    ...
    Each piPipeline.2P(SS).DeferNextToken.2L.1Wpe has the same type of `%turbo::Pipe<%std::function<void(%turbo::Pipeflow&)>>`
...
#endif  // TURBO_WORKFLOW_ALGORITHM_PIPELINE_H_
