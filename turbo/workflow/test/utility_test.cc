...
TEST_CASE("InlinedVector" * doctest::timeout(300)) {
  ...
  //SUBCASE("constructor")
  {
    turbo::InlinedVector<int> vec1;
    REQUIRE(vec1.size() == 0);
    REQUIRE(vec1.empty() == true);

    turbo::InlinedVector<int, 4> vec2;
  ...
  {
    for(int N=0; N<=65536; N = (N ? N << 1 : 1)) {
      turbo::InlinedVector<int> vec1(N);
      for(auto& item : vec1) {
        item = N;
      }

      turbo::InlinedVector<int> vec2(vec1);
      REQUIRE(vec1.size() == N);
      REQUIRE(vec2.size() == N);
  ...
  //SUBCASE("move_constructor")
  {
    for(int N=0; N<=65536; N = (N ? N << 1 : 1)) {
      ...
      turbo::InlinedVector<int> vec2(std::move(vec1));
      REQUIRE(vec1.size() == 0);
      REQUIRE(vec1.empty() == true);
      REQUIRE(vec2.size() == N);
...
//
//  SUBCASE("func1") {
//    using func1_traits = turbo::function_traits<decltype(func1)>;
//    static_assert(std::is_same<func1_traits::return_type, void>::value, "");
//    static_assert(func1_traits::arity == 0, "");
//  }
//
//  SUBCASE("func2") {
//    using func2_traits = turbo::function_traits<decltype(func2)>;
...
//      return 1;
//    };
//    using lambda1_traits = turbo::function_traits<decltype(lambda1)>;
...
//    };
//    using lambda2_traits = turbo::function_traits<decltype(lambda2)>;
...
//    };
//    using foo_traits = turbo::function_traits<foo>;
...
//    using ft1 = turbo::function_traits<std::function<void()>>;
//    static_assert(std::is_same<ft1::return_type, void>::value, "");
//    static_assert(ft1::arity == 0, "");
//
//    using ft2 = turbo::function_traits<std::function<int(int&, double&&)>&>;
...
//
//    using ft3 = turbo::function_traits<std::function<int(int&, double&&)>&&>;
...
//
//    using ft4 = turbo::function_traits<const std::function<void(int)>&>;
