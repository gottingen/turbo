...
#ifndef TURBO_WORKFLOW_CORE_ECECUTOR_H_
#define TURBO_WORKFLOW_CORE_ECECUTOR_H_
...
TURBO_NAMESPACE_BEGIN
    ...
    @brief class to create an executor for running a workflow graph

    An executor manages a set of worker threads to run one or multiple workflows
    using an efficient work-stealing scheduling algorithm.

    @code{.cpp}
    // Declare an executor and a workflow
    turbo::Executor executor;
    turbo::Workflow workflow;

    // Add three tasks into the workflow
    turbo::Task A = workflow.emplace([] () { std::cout << "This is TaskA\n"; });
    turbo::Task B = workflow.emplace([] () { std::cout << "This is TaskB\n"; });
    turbo::Task C = workflow.emplace([] () { std::cout << "This is TaskC\n"; });

    // Build precedence between tasks
    A.precede(B, C);

    turbo::Future<void> fu = executor.run(workflow);
    fu.wait();                // block until the execution completes

    executor.run(workflow, [](){ std::cout << "end of 1 run"; }).wait();
    executor.run_n(workflow, 4);
    executor.wait_for_all();  // block until all associated executions finish
    executor.run_n(workflow, 4, [](){ std::cout << "end of 4 runs"; }).wait();
    executor.run_until(workflow, [cnt=0] () mutable { return ++cnt == 10; });
    @endcode

    All the @c run methods are @em thread-safe. You can submit multiple
    workflows at the same time to an executor from different threads.
    */
    class Executor {
    ...
    public:
        ...
        via deriving an instance from turbo::WorkerInterface.
        */
        explicit Executor(
        ...
        workflows to complete and then notifies all worker threads to stop
        and join these threads.
        */
        ~Executor();

        /**
        @brief runs a workflow once

        @param workflow a turbo::Workflow object

        @return a turbo::Future that holds the result of the execution

        This member function executes the given workflow once and returns a turbo::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        turbo::Future<void> future = executor.run(workflow);
        // do something else
        future.wait();
        @endcode
        ...
        @attention
        The executor does not own the given workflow. It is your responsibility to
        ensure the workflow remains alive during its execution.
        */
        turbo::Future<void> run(Workflow &workflow);

        /**
        @brief runs a moved workflow once

        @param workflow a moved turbo::Workflow object

        @return a turbo::Future that holds the result of the execution

        This member function executes a moved workflow once and returns a turbo::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved workflow.

        @code{.cpp}
        turbo::Future<void> future = executor.run(std::move(workflow));
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        /**
        @brief runs a workflow once and invoke a callback upon completion
        ...
        This member function executes the given workflow once and invokes the given
        callable when the execution completes.
        This member function returns a turbo::Future object that
        eventually holds the result of the execution.

        @code{.cpp}
        turbo::Future<void> future = executor.run(workflow, [](){ std::cout << "done"; });
        // do something else
        future.wait();
        @endcode
        ...
        */
        template<typename C>
        turbo::Future<void> run(Workflow &workflow, C &&callable);

        /**
        @brief runs a moved workflow once and invoke a callback upon completion
        ...
        This member function executes a moved workflow once and invokes the given
        callable when the execution completes.
        This member function returns a turbo::Future object that
        eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved workflow.
        ...
        @code{.cpp}
        turbo::Future<void> future = executor.run(
          std::move(workflow), [](){ std::cout << "done"; }
        );
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        /**
        @brief runs a workflow for @c N times
        ...
        This member function executes the given workflow @c N times and returns a turbo::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        turbo::Future<void> future = executor.run_n(workflow, 2);  // run workflow 2 times
        // do something else
        future.wait();
        @endcode
        ...
        */
        turbo::Future<void> run_n(Workflow &workflow, size_t N);

        /**
        @brief runs a moved workflow for @c N times
        ...
        This member function executes a moved workflow @c N times and returns a turbo::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved workflow.

        @code{.cpp}
        turbo::Future<void> future = executor.run_n(
          std::move(workflow), 2    // run the moved workflow 2 times
        );
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        /**
        @brief runs a workflow for @c N times and then invokes a callback

        @param workflow a turbo::Workflow
        @param N number of runs
        @param callable a callable object to be invoked after this run

        @return a turbo::Future that holds the result of the execution

        This member function executes the given workflow @c N times and invokes the given
        callable when the execution completes.
        This member function returns a turbo::Future object that
        eventually holds the result of the execution.
        ...
        @code{.cpp}
        turbo::Future<void> future = executor.run(
          workflow, 2, [](){ std::cout << "done"; }  // runs workflow 2 times and invoke
        ...
        */
        template<typename C>
        turbo::Future<void> run_n(Workflow &workflow, size_t N, C &&callable);

        /**
        @brief runs a moved workflow for @c N times and then invokes a callback

        @param workflow a moved turbo::Workflow
        @param N number of runs
        @param callable a callable object to be invoked after this run

        @return a turbo::Future that holds the result of the execution

        This member function executes a moved workflow @c N times and invokes the given
        callable when the execution completes.
        This member function returns a turbo::Future object that
        eventually holds the result of the execution.
        ...
        @code{.cpp}
        turbo::Future<void> future = executor.run_n(
          // run the moved workflow 2 times and invoke the lambda to print "done"
          std::move(workflow), 2, [](){ std::cout << "done"; }
        );
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        /**
        @brief runs a workflow multiple times until the predicate becomes true
        ...
        This member function executes the given workflow multiple times until
        the predicate returns @c true.
        This member function returns a turbo::Future object that
        eventually holds the result of the execution.

        @code{.cpp}
        turbo::Future<void> future = executor.run_until(
          workflow, [](){ return rand()%10 == 0 }
        );
        // do something else
        future.wait();
        @endcode
        ...
        */
        template<typename P>
        turbo::Future<void> run_until(Workflow &workflow, P &&pred);

        /**
        @brief runs a moved workflow and keeps running it
        ...
        This member function executes a moved workflow multiple times until
        the predicate returns @c true.
        This member function returns a turbo::Future object that
        eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved workflow.
        ...
        @code{.cpp}
        turbo::Future<void> future = executor.run_until(
          std::move(workflow), [](){ return rand()%10 == 0 }
        );
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        /**
        @brief runs a workflow multiple times until the predicate becomes true and
        ...
        @code{.cpp}
        turbo::Future<void> future = executor.run_until(
          workflow, [](){ return rand()%10 == 0 }, [](){ std::cout << "done"; }
        );
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        turbo::Future<void> run_until(Workflow &workflow, P &&pred, C &&callable);

        /**
        @brief runs a moved workflow and keeps running
        ...
        */
        ...
        @tparam T target type which has `turbo::Graph& T::graph()` defined
        @param target the target task graph object

        The method runs a target graph which has `turbo::Graph& T::graph()` defined
        and waits until the execution completes.
        Unlike the typical flow of calling `turbo::Executor::run` series
        ...
        turbo::Workflow workflow;
        std::array<turbo::Workflow, 1000> others;
        ...
        for(size_t n=0; n<1000; n++) {
          ...
          workflow.emplace([&executor, &tf=others[n]](){
        ...
        @attention
        You must call turbo::Executor::run_and_wait from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename T>
        ...
        @attention
        You must call turbo::Executor::loop_until from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename P>
        ...
        (e.g., workflows, asynchronous tasks) to finish.

        @code{.cpp}
        executor.run(workflow1);
        executor.run_n(workflow2, 10);
        executor.run_n(workflow3, 100);
        executor.wait_for_all();  // wait until the above submitted workflows finish
        @endcode
        */
        void wait_for_all();
        ...
        */
        ...
        When a workflow is submitted to an executor, a topology is created to store
        runtime metadata of the running workflow.
        When the execution of the submitted workflow finishes,
        its corresponding topology will be removed from the executor.

        @code{.cpp}
        executor.run(workflow);
        std::cout << executor.num_topologies();  // 0 or 1 (workflow still running)
        @endcode
        */
        size_t num_topologies() const;

        /**
        @brief queries the number of running workflows with moved ownership

        @code{.cpp}
        executor.run(std::move(workflow));
        std::cout << executor.num_workflows();  // 0 or 1 (workflow still running)
        @endcode
        */
        size_t num_workflows() const;
        ...
        @code{.cpp}
        turbo::Executor executor(4);   // 4 workers in the executor
        ...
        */
        ...
        Unlike std::async, the return here is a @em turbo::Future that holds
        ...
        @code{.cpp}
        turbo::Future<std::optional<int>> future = executor.async([](){
        ...
        */
        ...
        Unlike std::async, the return here is a turbo::Future that holds
        an optional object to the result.
        If the asynchronous task is cancelled before it runs, the return is
        a @c std::nullopt, or the value returned by the callable.

        @code{.cpp}
        turbo::Future<std::optional<int>> future = executor.named_async("name", [](){
        ...
        */
        ...
        /**
        @brief similar to turbo::Executor::async but does not return a future object

        This member function is more efficient than turbo::Executor::async
        ...
        */
        ...
        /**
        @brief similar to turbo::Executor::named_async but does not return a future object

        This member function is more efficient than turbo::Executor::named_async
        and is encouraged to use when there is no data returned.
        ...
        */
        ...
        @tparam Observer observer type derived from turbo::ObserverInterface
        ...
        turbo::ObserverInterface::on_entry and turbo::ObserverInterface::on_exit
    ...
    inline Executor::Executor(size_t N, std::shared_ptr <WorkerInterface> wix) :
        ...
        if (N == 0) {
            TF_THROW("no cpu workers to execute workflows");
        }
...
// Function: num_workflows
    inline size_t Executor::num_workflows() const {
...
// Procedure: _detach_dynamic_task
    inline void Executor::_detach_dynamic_task(
        ...
        {
            std::lock_guard <std::mutex> lock(p->_topology->_workflow._mutex);
            p->_topology->_workflow._graph._merge(std::move(g));
        }
...
// Function: run
    inline turbo::Future<void> Executor::run(Workflow &f) {
        return run_n(f, 1, []() {});
    }
...
// Function: run
    template<typename C>
    turbo::Future<void> Executor::run(Workflow &f, C &&c) {
...
// Function: run_n
    inline turbo::Future<void> Executor::run_n(Workflow &f, size_t repeat) {
...
// Function: run_n
    template<typename C>
    turbo::Future<void> Executor::run_n(Workflow &f, size_t repeat, C &&c) {
        return run_until(
...
// Function: run_until
    template<typename P>
    turbo::Future<void> Executor::run_until(Workflow &f, P &&pred) {
...
// Function: run_until
    template<typename P, typename C>
    turbo::Future<void> Executor::run_until(Workflow &f, P &&p, C &&c) {

        _increment_topology();

        // Need to check the empty under the lock since dynamic task may
        // define detached blocks that modify the workflow at the same time
        bool empty;
        {
        ...
        if (empty || p()) {
            ...
            promise.set_value();
            _decrement_topology_and_notify();
            return turbo::Future<void>(promise.get_future(), std::monostate{});
        }
        ...
        turbo::Future<void> future(t->_promise.get_future(), t);
...
// Function: run_until
    template<typename P, typename C>
    turbo::Future<void> Executor::run_until(Workflow &&f, P &&pred, C &&c) {

        std::list<Workflow>::iterator itr;

        {
            std::scoped_lock <std::mutex> lock(_workflow_mutex);
            itr = _workflows.emplace(_workflows.end(), std::move(f));
            itr->_satellite = itr;
        }
...
// Function: _set_up_topology
    inline void Executor::_set_up_topology(Worker *worker, Topology *tpg) {
        ...
        tpg->_sources.clear();
        tpg->_workflow._graph._clear_detached();

        // scan each node in the graph and build up the links
        for (auto node : tpg->_workflow._graph._nodes) {
    ...
    inline void Executor::_tear_down_topology(Worker &worker, Topology *tpg) {
        ...
        if (!tpg->_is_cancelled && !tpg->_pred()) {
            ...
            if (std::unique_lock <std::mutex> lock(f._mutex); f._topologies.size() > 1) {
                ...
                // Need to back up the promise first here becuz workflow might be
                ...
                // Now we remove the topology from this workflow
                f._topologies.pop();

                //f._mutex.unlock();
                lock.unlock();

                // We set the promise in the end in case workflow leaves the scope.
                // After set_value, the caller will return from wait
                p.set_value();

                _decrement_topology_and_notify();

                // remove the workflow if it is managed by the executor
...
#endif  // TURBO_WORKFLOW_CORE_ECECUTOR_H_
