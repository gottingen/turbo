    ...
    inline const char *to_string(TaskType type) {
        ...
        switch (type) {
            case TaskType::PLACEHOLDER:
                val = "placeholder";
                break;
            case TaskType::CUDAFLOW:
                val = "cudaflow";
                break;
            case TaskType::SYCLFLOW:
                val = "syclflow";
                break;
...
*/
    ...
    */
    ...
    std::function<void(turbo::cudaFlow&)> or std::function<void(turbo::cudaFlowCapturer&)>.
    */
    template<typename C>
    ...
    std::function<void(turbo::syclFlow&)>.
    */
    template<typename C>
    ...
    std::function<void(turbo::Runtime&)>.
    */
    template<typename C>
    ...
    @brief class to create a task handle over a node in a workflow graph

    A task is a wrapper over a node in a workflow graph.
    It provides a set of methods for users to access and modify the attributes of
    the associated node in the workflow graph.
    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
        @brief creates a module task from a workflow
        ...
        */
        ...
        turbo::TaskPriority::HIGH (numerically equivalent to 0),
        turbo::TaskPriority::NORMAL (numerically equivalent to 1), and
        turbo::TaskPriority::LOW (numerically equivalent to 2).
    ...
    inline TaskType Task::type() const {
        switch (_node->_handle.index()) {
            ...
            case Node::SILENT_ASYNC:
                return TaskType::ASYNC;
            case Node::CUDAFLOW:
                return TaskType::CUDAFLOW;
            case Node::SYCLFLOW:
                return TaskType::SYCLFLOW;
            case Node::RUNTIME:
