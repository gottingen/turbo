...
//
#ifndef TURBO_TASKFLOW_ALGORITHM_PARTITIONER_H_
#define TURBO_TASKFLOW_ALGORITHM_PARTITIONER_H_
...
namespace turbo {
    ...
    to schedule parallel iterations (e.g., turbo::Taskflow::for_each).

    An partitioner defines the scheduling method for running parallel algorithms,
    such turbo::Taskflow::for_each, turbo::Taskflow::reduce, and so on.
    By default, we provide the following partitioners:

    + turbo::GuidedPartitioner to enable guided scheduling algorithm of adaptive chunk size
    + turbo::DynamicPartitioner to enable dynamic scheduling algorithm of equal chunk size
    + turbo::StaticPartitioner to enable static scheduling algorithm of static chunk size
    + turbo::RandomPartitioner to enable random scheduling algorithm of random chunk size
    ...
    iteration, turbo::StaticPartitioner can deliver the best performance.
    On the other hand, if the work unit per iteration is irregular and unbalanced,
    turbo::GuidedPartitioner or turbo::DynamicPartitioner can outperform turbo::StaticPartitioner.
    In most situations, turbo::GuidedPartitioner can deliver decent performance and
    ...
    */
    ...
    Similar to turbo::DynamicPartitioner,
    ...
    */
    ...
    /**
    @brief default partitioner set to turbo::GuidedPartitioner
    ...
    */
    ...
    A partitioner is a derived type from turbo::PartitionerBase.
    */
    template<typename C>
    inline constexpr bool is_partitioner_v = std::is_base_of<PartitionerBase, C>::value;

}  // namespace turbo
#endif  // TURBO_TASKFLOW_ALGORITHM_PARTITIONER_H_
