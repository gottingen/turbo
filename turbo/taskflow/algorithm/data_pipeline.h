...
//
#ifndef TURBO_TASKFLOW_ALGORITHM_DATA_PIPE_H_
#define TURBO_TASKFLOW_ALGORITHM_DATA_PIPE_H_
...
namespace turbo {
    ...
    (specified by turbo::PipeType) and is associated with a callable to invoke
    by the pipeline scheduler.

    You need to use the template function, turbo::make_data_pipe, to create
    a data pipe. The input and output types of a turbo::DataPipe should be decayed types
    ...
    @code{.cpp}
    turbo::make_data_pipe<int, std::string>(
    ...
    of turbo::Pipeflow in the second argument to probe the runtime information
    for a stage task, such as its line number and token number:

    @code{.cpp}
    turbo::make_data_pipe<int, std::string>(
      turbo::PipeType::SERIAL,
      [](int& input, turbo::Pipeflow& pf) {
    ...
    */
    ...
    public:
        ...
        */
        ...
        You should use the helper function, turbo::make_data_pipe,
        to create a DataPipe object, especially when you need turbo::DataPipe
        ...
        */
        ...
        A data pipe can be either parallel (turbo::PipeType::PARALLEL) or serial
        (turbo::PipeType::SERIAL).
        */
        PipeType type() const {
            return _type;
...
/**
@brief function to construct a data pipe (turbo::DataPipe)

@tparam Input input data type
@tparam Output output data type
@tparam C callable type

turbo::make_data_pipe is a helper function to create a data pipe (turbo::DataPipe)
in a data-parallel pipeline (turbo::DataPipeline).
The first argument specifies the direction of the data pipe,
either turbo::PipeType::SERIAL or turbo::PipeType::PARALLEL,
...
The callable can additionally take a reference of turbo::Pipeflow,
...
Similar to turbo::Pipeline, a turbo::DataPipeline is a composable graph object
for users to create a <i>data-parallel pipeline scheduling framework</i> 
using a module task in a taskflow.
The only difference is that turbo::DataPipeline provides a data abstraction
...
*/
    ...
    private:
        ...
        std::vector<turbo::CachelineAligned<data_t>> _buffer;
    ...
    void DataPipeline<Ps...>::_on_pipe(Pipeflow &pf, Runtime &) {
        ...
        visit_tuple([&](auto &&pipe) {
            ...
            if constexpr (std::is_invocable_v<callable_t, Pipeflow &>) {
                // [](turbo::Pipeflow&) -> void {}, i.e., we only have one pipe
                if constexpr (std::is_void_v<output_t>) {
                    pipe._callable(pf);
                    // [](turbo::Pipeflow&) -> output_t {}
                } else {
                ...
                // [](input_t&, turbo::Pipeflow&) -> void {}
                if constexpr (std::is_void_v<output_t>) {
                    pipe._callable(std::get<input_t>(_buffer[pf._line].data), pf);
                    // [](input_t&, turbo::Pipeflow&) -> output_t {}
                } else {
                    _buffer[pf._line].data = pipe._callable(
...
#endif  // TURBO_TASKFLOW_ALGORITHM_DATA_PIPE_H_
