...
//
#ifndef TURBO_TASKFLOW_ALGORITHM_TRANSFORM_H_
#define TURBO_TASKFLOW_ALGORITHM_TRANSFORM_H_
...
namespace turbo {
    ...
    TURBO_FORCE_INLINE auto make_transform_task(
    ...
    // Function: make_transform_task
    template<typename B1, typename E1, typename B2, typename O, typename C, typename P = GuidedPartitioner,
            std::enable_if_t<!is_partitioner_v<std::decay_t<C>>, void> * = nullptr>
    TURBO_FORCE_INLINE auto make_transform_task(B1 first1, E1 last1, B2 first2, O d_first, C c, P &&part = P()) {
        ...
        return [first1, last1, first2, d_first, c, part = std::forward<P>(part)]
                (Runtime &rt) mutable {
    ...
    Task FlowBuilder::transform(B first1, E last1, O d_first, C c, P &&part) {
        return emplace(make_transform_task(first1, last1, d_first, c, std::forward<P>(part)));
    }
    ...
    Task FlowBuilder::transform(B1 first1, E1 last1, B2 first2, O d_first, C c, P &&part) {
        return emplace(make_transform_task(first1, last1, first2, d_first, c, std::forward<P>(part)));
    }


}  // namespace turbo
#endif  // TURBO_TASKFLOW_ALGORITHM_TRANSFORM_H_
