...
//
#ifndef TURBO_TASKFLOW_CORE_EXCUTOR_H_
#define TURBO_TASKFLOW_CORE_EXCUTOR_H_
...
namespace turbo {
    ...
    turbo::Task A = taskflow.emplace([] () { std::cout << "This is TaskA\n"; });
    turbo::Task B = taskflow.emplace([] () { std::cout << "This is TaskB\n"; });
    turbo::Task C = taskflow.emplace([] () { std::cout << "This is TaskC\n"; });

    // Build precedence between tasks
    A.precede(B, C);

    turbo::Future<void> fu = executor.run(taskflow);
    ...
    */
    ...
    public:
        ...
        */
        ...
        @param taskflow a turbo::Taskflow object

        @return a turbo::Future that holds the result of the execution

        This member function executes the given taskflow once and returns a turbo::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        turbo::Future<void> future = executor.run(taskflow);
        // do something else
        future.wait();
        @endcode
        ...
        */
        turbo::Future<void> run(Taskflow &taskflow);

        /**
        @brief runs a moved taskflow once

        @param taskflow a moved turbo::Taskflow object

        @return a turbo::Future that holds the result of the execution

        This member function executes a moved taskflow once and returns a turbo::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.

        @code{.cpp}
        turbo::Future<void> future = executor.run(std::move(taskflow));
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        @code{.cpp}
        turbo::Future<void> future = executor.run(taskflow, [](){ std::cout << "done"; });
        // do something else
        future.wait();
        @endcode
        ...
        */
        template<typename C>
        turbo::Future<void> run(Taskflow &taskflow, C &&callable);
        ...
        */
        ...
        This member function executes the given taskflow @c N times and returns a turbo::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        turbo::Future<void> future = executor.run_n(taskflow, 2);  // run taskflow 2 times
        // do something else
        future.wait();
        @endcode
        ...
        */
        turbo::Future<void> run_n(Taskflow &taskflow, size_t N);
        ...
        This member function executes a moved taskflow @c N times and returns a turbo::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.
        ...
        */
        ...
        @param taskflow a turbo::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        template<typename C>
        turbo::Future<void> run_n(Taskflow &taskflow, size_t N, C &&callable);

        /**
        @brief runs a moved taskflow for @c N times and then invokes a callback

        @param taskflow a moved turbo::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        template<typename P>
        turbo::Future<void> run_until(Taskflow &taskflow, P &&pred);
        ...
        */
        ...
        turbo::Future<void> run_until(Taskflow &taskflow, P &&pred, C &&callable);
        ...
        */
        ...
        turbo::Taskflow taskflow;
        std::array<turbo::Taskflow, 1000> others;
        ...
        @attention
        You must call turbo::Executor::corun from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename T>
        void corun(T &target);
        ...
        @attention
        You must call turbo::Executor::corun_until from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename P>
        ...
        */
        ...
        @tparam F callable type
        @tparam Tasks task types convertible to turbo::AsyncTask

        @param func callable object
        @param tasks asynchronous tasks on which this execution depends

        @return a turbo::AsyncTask handle

        This member function is more efficient than turbo::Executor::dependent_async
        and is encouraged to use when you do not want a @std_future to
        acquire the result or synchronize the execution.
        ...
        @code{.cpp}
        turbo::AsyncTask A = executor.silent_dependent_async([](){ printf("A\n"); });
        turbo::AsyncTask B = executor.silent_dependent_async([](){ printf("B\n"); });
        ...
        */
        ...
        >
        turbo::AsyncTask silent_dependent_async(F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        turbo::AsyncTask A = executor.silent_dependent_async("A", [](){ printf("A\n"); });
        turbo::AsyncTask B = executor.silent_dependent_async("B", [](){ printf("B\n"); });
        ...
        */
        template<typename F, typename... Tasks,
                std::enable_if_t<all_same_v<AsyncTask, std::decay_t<Tasks>...>, void> * = nullptr
        >
        turbo::AsyncTask silent_dependent_async(const std::string &name, F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        std::array<turbo::AsyncTask, 2> array {
        ...
        */
        ...
        >
        turbo::AsyncTask silent_dependent_async(F &&func, I first, I last);
        ...
        */
        template<typename F, typename I,
                std::enable_if_t<!std::is_same_v<std::decay_t<I>, AsyncTask>, void> * = nullptr
        >
        turbo::AsyncTask silent_dependent_async(const std::string &name, F &&func, I first, I last);
        ...
        @return a pair of a turbo::AsyncTask handle and
                          a @std_future that holds the result of the execution

        The example below creates three asynchronous tasks, @c A, @c B, and @c C,
        in which task @c C runs after task @c A and task @c B.
        Task @c C returns a pair of its turbo::AsyncTask handle and a std::future<int>
        ...
        You can mixed the use of turbo::AsyncTask handles
    ...
    private:
        ...
        Node *tear_down_invoke(Worker &, Node *);
        ...
        void _schedule(Worker &, const turbo::SmallVector<Node *> &);

        void _schedule(const turbo::SmallVector<Node *> &);

        void _set_up_topology(Worker *, Topology *);

        void _set_up_graph(Graph &, Node *, Topology *, int, turbo::SmallVector<Node *> &);
        ...
        void invoke_static_task(Worker &, Node *);
        ...
        void invoke_condition_task(Worker &, Node *, turbo::SmallVector<int> &);

        void _invoke_multi_condition_task(Worker &, Node *, turbo::SmallVector<int> &);
        ...
        void _process_async_dependent(Node *, turbo::AsyncTask &, size_t &);
        ...
        template<typename P>
        void corun_until_impl(Worker &, P &&);
    };
    ...
    template<typename P>
    void Executor::corun_until_impl(Worker &w, P &&stop_predicate) {
...
// Procedure: _schedule
    ...
    // Procedure: _schedule
    inline void Executor::_schedule(Worker &worker, const turbo::SmallVector<Node *> &nodes) {
    ...
    // Procedure: _schedule
    inline void Executor::_schedule(const turbo::SmallVector<Node *> &nodes) {
    ...
    inline void Executor::_invoke(Worker &worker, Node *node) {
        ...
        switch (node->_handle.index()) {
            // static task
            case Node::STATIC: {
                invoke_static_task(worker, node);
            }
                break;
            ...
            case Node::DYNAMIC: {
                ...
                // condition task
            case Node::CONDITION: {
                invoke_condition_task(worker, node, conds);
            }
                break;
        ...
        if (node->_semaphores && !node->_semaphores->to_release.empty()) {
            _schedule(worker, node->release_all());
        }
    ...
    // Proecdure: tear_down_invoke
    inline Node *Executor::tear_down_invoke(Worker &worker, Node *node) {
    ...
    // Procedure: invoke_static_task
    inline void Executor::invoke_static_task(Worker &worker, Node *node) {
    ...
    inline void Executor::_consume_graph(Worker &w, Node *p, Graph &g) {
        ...
        corun_until_impl(w, [p]() -> bool {
                             return p->_join_counter.load(std::memory_order_acquire) == 0;
                         }
        );
    }

    // Procedure: invoke_condition_task
    inline void Executor::invoke_condition_task(
            Worker &worker, Node *node, turbo::SmallVector<int> &conds) {
        _observer_prologue(worker, node);
        try {
    ...
    // Function: run
    inline turbo::Future<void> Executor::run(Taskflow &f) {
        return run_n(f, 1, []() {});
    }
    ...
    // Function: run
    template<typename C>
    turbo::Future<void> Executor::run(Taskflow &f, C &&c) {
    ...
    // Function: run_n
    inline turbo::Future<void> Executor::run_n(Taskflow &f, size_t repeat) {
    ...
    // Function: run_n
    template<typename C>
    turbo::Future<void> Executor::run_n(Taskflow &f, size_t repeat, C &&c) {
        return run_until(
    ...
    // Function: run_until
    template<typename P>
    turbo::Future<void> Executor::run_until(Taskflow &f, P &&pred) {
    ...
    // Function: run_until
    template<typename P, typename C>
    turbo::Future<void> Executor::run_until(Taskflow &f, P &&p, C &&c) {
        ...
        if (empty || p()) {
            ...
            promise.set_value();
            _decrement_topology();
            return turbo::Future<void>(promise.get_future());
        }
    ...
    // Function: run_until
    template<typename P, typename C>
    turbo::Future<void> Executor::run_until(Taskflow &&f, P &&pred, C &&c) {
    ...
    void Executor::corun_until(P &&predicate) {
        ...
        corun_until_impl(*w, std::forward<P>(predicate));
    }
...
// Procedure: _decrement_topology
    ...
    inline void Executor::_set_up_graph(
            Graph &g, Node *parent, Topology *tpg, int state, turbo::SmallVector<Node *> &src) {
    ...
    void Runtime::corun_until(P &&predicate) {
        _executor.corun_until_impl(_worker, std::forward<P>(predicate));
    }

    // Function: silent_async
    template<typename F>
    void Runtime::silent_async(Worker &w, const std::string &name, F &&f) {
    ...
    void Runtime::silent_async(F &&f) {
        silent_async(*_executor._this_worker(), "", std::forward<F>(f));
    }

    // Function: silent_async
    template<typename F>
    void Runtime::silent_async(const std::string &name, F &&f) {
        silent_async(*_executor._this_worker(), name, std::forward<F>(f));
    }

    // Function: silent_async_unchecked
    template<typename F>
    void Runtime::silent_async_unchecked(const std::string &name, F &&f) {
        silent_async(_worker, name, std::forward<F>(f));
    }

    // Function: async_impl
    template<typename F>
    auto Runtime::async_impl(Worker &w, const std::string &name, F &&f) {
    ...
    auto Runtime::async(F &&f) {
        return async_impl(*_executor._this_worker(), "", std::forward<F>(f));
    }

    // Function: async
    template<typename F>
    auto Runtime::async(const std::string &name, F &&f) {
        return async_impl(*_executor._this_worker(), name, std::forward<F>(f));
    }
...
#endif  // TURBO_TASKFLOW_CORE_EXCUTOR_H_
