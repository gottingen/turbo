...
#ifndef TURBO_BASE_SCOPED_LOCK_H_
#define TURBO_BASE_SCOPED_LOCK_H_
...
#define TURBO_SCOPED_LOCK(ref_of_lock)                                  \
    decltype(::turbo::base::detail::get_lock_guard<decltype(ref_of_lock)>()) \
...
namespace std {
    ...
    public:
        explicit lock_guard(pthread_mutex_t & mutex) : _pmutex(&mutex) {
            ...
            if (rc) {
                TURBO_LOG(FATAL) << "Fail to lock pthread_mutex_t=" << _pmutex << ", " << turbo_error(rc);
                _pmutex = nullptr;
            }
#else
    ...
    public:
        explicit lock_guard(pthread_spinlock_t & spin) : _pspin(&spin) {
            ...
            if (rc) {
                TURBO_LOG(FATAL) << "Fail to lock pthread_spinlock_t=" << _pspin << ", " << turbo_error(rc);
                _pspin = nullptr;
            }
#else
    ...
    public:
        ...
        void lock() {
            ...
            if (rc) {
                TURBO_LOG(FATAL) << "Fail to lock pthread_mutex=" << _mutex << ", " << turbo_error(rc);
                return;
            }
            _owns_lock = true;
#else
    ...
    public:
        ...
        void lock() {
            ...
            if (rc) {
                TURBO_LOG(FATAL) << "Fail to lock pthread_spinlock=" << _mutex << ", " << turbo_error(rc);
                return;
            }
            _owns_lock = true;
#else
...
#endif  // TURBO_BASE_SCOPED_LOCK_H_
