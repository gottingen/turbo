...
#ifndef TURBO_BASE_MATH_BIT_ITERATOR_H_
#define TURBO_BASE_MATH_BIT_ITERATOR_H_
...
namespace turbo::base {
     ...
     */
    template<size_t N>
    static TURBO_FORCE_INLINE size_t get_first_set(const std::bitset<N> &bitset) {
     ...
     */
    template<size_t N>
    static TURBO_FORCE_INLINE size_t get_last_set(const std::bitset<N> &bitset) {
        static_assert(N <= ulong_bits, "bitset too large");
    ...
    template<size_t N>
    TURBO_FORCE_INLINE set_range<N> for_each_set(std::bitset<N> bitset, int offset = 0) {
        return set_range<N>(bitset, offset);
    }

}  // namespace turbo::base

#endif  // TURBO_BASE_MATH_BIT_ITERATOR_H_
