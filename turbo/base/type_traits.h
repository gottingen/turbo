...
namespace turbo {
    ...
    template<typename... T>
    TURBO_INLINE_VARIABLE constexpr tag_t<T...> tag{};
...
//  compiler error message with static_assert.
    template<typename...>
    TURBO_INLINE_VARIABLE constexpr bool always_false = false;
...
//  templates.
        ...
        TURBO_INLINE_VARIABLE constexpr bool is_instantiation_of_v = false;
        template<template<typename...> class C, typename... T>
        TURBO_INLINE_VARIABLE constexpr bool is_instantiation_of_v<C, C<T...>> = true;
        template<template<typename...> class C, typename... T>
        struct is_instantiation_of : bool_constant<is_instantiation_of_v<C, T...>> {
        };

        template<typename, typename>
        TURBO_INLINE_VARIABLE constexpr bool is_similar_instantiation_v = false;
        template<template<typename...> class C, typename... A, typename... B>
        TURBO_INLINE_VARIABLE constexpr bool
...
//  constexpr context.
    template<typename T>
    TURBO_INLINE_VARIABLE constexpr bool is_constexpr_default_constructible_v =
 ...
 *
 *    template <class T>
 *    struct has_value_type<T, turbo::void_t<typename T::value_type>>
 *        : std::true_type {};
 */
...
//  Note: the trait type is_detected differs here by being deferred.
    template<template<typename...> class T, typename... A>
    TURBO_INLINE_VARIABLE constexpr bool is_detected_v =
            detected_or<nonesuch, T, A...>::value_t::value;
    template<template<typename...> class T, typename... A>
...
// Older versions of libstdc++ do not provide std::is_trivially_copyable
    ...
    template<class T>
    TURBO_INLINE_VARIABLE constexpr bool is_trivially_copyable_v =
            is_trivially_copyable<T>::value;

/**
 * is_relocatable<T>::value describes the ability of moving around
 ...
 * namespace turbo and make sure you keep the specialization of
 * is_relocatable<SomeStruct> in the same header as SomeStruct.
 *
 * You may also declare a type to be relocatable by including
 *    `typedef std::true_type is_relocatable;`
 * in the class header.
 *
 * It may be unset in a base class by overriding the typedef to false_type.
 */
/*
 * is_zero_initializable describes the property that default construction is the
 * same as memset(dst, 0, sizeof(T)).
 */

    namespace traits_detail {

#define TURBO_HAS_TRUE_XXX(name)                                             \
  template <typename T>                                                      \
  ...
  struct has_true_##name : std::conditional<                                 \
        ...
        TURBO_HAS_TRUE_XXX(is_relocatable);
        TURBO_HAS_TRUE_XXX(is_zero_initializable);

#undef TURBO_HAS_TRUE_XXX
...
/* using override */ using traits_detail_IsNothrowSwappable::IsNothrowSwappable;
    ...
    struct is_relocatable
            : std::conditional<
                    is_detected_v<traits_detail::detect_is_relocatable, T>,
                    traits_detail::has_true_is_relocatable<T>,
    ...
    template<class T>
    struct is_zero_initializable
            : std::conditional<
                    is_detected_v<traits_detail::detect_is_zero_initializable, T>,
                    traits_detail::has_true_is_zero_initializable<T>,
...
//  heterogeneous access.
    template<typename T>
    TURBO_INLINE_VARIABLE constexpr bool is_transparent_v =
...
/**
 * Use this macro ONLY inside namespace turbo. When using it with a
 * regular type, use it like this:
 *
 * // Make sure you're at namespace ::turbo scope
 * template <> TURBO_ASSUME_RELOCATABLE(MyType)
 *
 * When using it with a template type, use it like this:
 *
 * // Make sure you're at namespace ::turbo scope
 * template <class T1, class T2>
 * TURBO_ASSUME_RELOCATABLE(MyType<T1, T2>)
 */
#define TURBO_ASSUME_RELOCATABLE(...) \
  struct is_relocatable<__VA_ARGS__> : std::true_type {}

/**
 * The TURBO_ASSUME_FBVECTOR_COMPATIBLE* macros below encode the
 * assumption that the type is relocatable per is_relocatable
 ...
 *
 * Use TURBO_ASSUME_FBVECTOR_COMPATIBLE with regular types like this:
 *
 * TURBO_ASSUME_FBVECTOR_COMPATIBLE(MyType)
 *
 * The versions TURBO_ASSUME_FBVECTOR_COMPATIBLE_1, _2, _3, and _4
 ...
 * TURBO_ASSUME_FBVECTOR_COMPATIBLE_2(MyType)
 */

// Use this macro ONLY at global level (no namespace)
#define TURBO_ASSUME_FBVECTOR_COMPATIBLE(...) \
  namespace turbo {                           \
  template <>                                 \
  TURBO_ASSUME_RELOCATABLE(__VA_ARGS__);      \
  }
// Use this macro ONLY at global level (no namespace)
#define TURBO_ASSUME_FBVECTOR_COMPATIBLE_1(...) \
  namespace turbo {                             \
  template <class T1>                           \
  TURBO_ASSUME_RELOCATABLE(__VA_ARGS__<T1>);    \
  }
// Use this macro ONLY at global level (no namespace)
#define TURBO_ASSUME_FBVECTOR_COMPATIBLE_2(...)  \
  namespace turbo {                              \
  template <class T1, class T2>                  \
  TURBO_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2>); \
  }
// Use this macro ONLY at global level (no namespace)
#define TURBO_ASSUME_FBVECTOR_COMPATIBLE_3(...)      \
  namespace turbo {                                  \
  template <class T1, class T2, class T3>            \
  TURBO_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2, T3>); \
  }
// Use this macro ONLY at global level (no namespace)
#define TURBO_ASSUME_FBVECTOR_COMPATIBLE_4(...)          \
  namespace turbo {                                      \
  template <class T1, class T2, class T3, class T4>      \
  TURBO_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2, T3, T4>); \
  }
...
namespace turbo {
    ...
    struct is_relocatable<std::pair<T, U>>
            : bool_constant<is_relocatable<T>::value && is_relocatable<U>::value> {
    };
    ...
    namespace detail {
        ...
        // turbo::to integral specializations can end up generating code
        ...
        TURBO_PUSH_WARNING
        TURBO_GNU_DISABLE_WARNING("-Wsign-compare")
        TURBO_GCC_DISABLE_WARNING("-Wbool-compare")
        TURBO_MSVC_DISABLE_WARNING(4287) // unsigned/negative constant mismatch
        TURBO_MSVC_DISABLE_WARNING(4388) // sign-compare
        TURBO_MSVC_DISABLE_WARNING(4804) // bool-compare
...
#ifndef _MSC_VER
TURBO_ASSUME_FBVECTOR_COMPATIBLE_2(std::unique_ptr)
TURBO_ASSUME_FBVECTOR_COMPATIBLE_1(std::shared_ptr)
#endif
...
//  Import things into turbo and then fill in whatever is missing.
//
//  The aliases:
//    int128_t
//    uint128_t
...
//    make_unsigned_t
    ...
    template<typename T>
    TURBO_INLINE_VARIABLE constexpr bool is_arithmetic_v = is_arithmetic<T>::value;

    template<typename T>
    struct is_integral : std::is_integral<T> {
    };
    template<typename T>
    TURBO_INLINE_VARIABLE constexpr bool is_integral_v = is_integral<T>::value;

    template<typename T>
    struct is_signed : std::is_signed<T> {
    };
    template<typename T>
    TURBO_INLINE_VARIABLE constexpr bool is_signed_v = is_signed<T>::value;

    template<typename T>
    struct is_unsigned : std::is_unsigned<T> {
    };
    template<typename T>
    TURBO_INLINE_VARIABLE constexpr bool is_unsigned_v = is_unsigned<T>::value;
...
#if TURBO_COMPILER_HAS_BUILTIN(__type_pack_element)
