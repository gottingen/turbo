...
namespace turbo::base {
    ...
    int unix_socket_listen(const char *sockname, bool remove_previous_file) {
        ...
        if (fd < 0) {
            TURBO_PLOG(ERROR) << "Fail to create unix socket";
            return -1;
        }
        ...
        if (bind(fd, (struct sockaddr *) &addr, sizeof(addr)) != 0) {
            TURBO_PLOG(ERROR) << "Fail to bind sockfd=" << fd << " as unix socket="
                        << sockname;
            return -1;
        }
        if (listen(fd, SOMAXCONN) != 0) {
            TURBO_PLOG(ERROR) << "Fail to listen to sockfd=" << fd;
            return -1;
        }
        return fd.release();
    }
    ...
    int unix_socket_connect(const char *sockname) {
        ...
        if (connect(fd, (struct sockaddr *) &addr, sizeof(addr)) != 0) {
            TURBO_PLOG(ERROR) << "Fail to connect to unix socket=" << sockname
                        << " via sockfd=" << fd;
            return -1;
        }
        return fd.release();
