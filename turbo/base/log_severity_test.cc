...
using ParseFlagFromIntegerMatchingEnumeratorTest =
    TestWithParam<std::tuple<turbo::string_piece, turbo::LogSeverity>>;
INSTANTIATE_TEST_SUITE_P(
...
TEST_P(ParseFlagFromIntegerMatchingEnumeratorTest, YieldsExpectedValue) {
  const turbo::string_piece to_parse = std::get<0>(GetParam());
...
using ParseFlagFromOtherIntegerTest =
    TestWithParam<std::tuple<turbo::string_piece, int>>;
...
using ParseFlagFromGarbageTest = TestWithParam<turbo::string_piece>;
...
TEST_P(ParseFlagFromGarbageTest, ReturnsError) {
  const turbo::string_piece to_parse = GetParam();
  turbo::LogSeverity value;
  std::string error;
  EXPECT_THAT(turbo::ParseFlag(to_parse, &value, &error), IsFalse()) << value;
}
...
using UnparseFlagToEnumeratorTest =
    TestWithParam<std::tuple<turbo::LogSeverity, turbo::string_piece>>;
INSTANTIATE_TEST_SUITE_P(
...
TEST_P(UnparseFlagToEnumeratorTest, ReturnsExpectedValueAndRoundTrips) {
  ...
  const turbo::string_piece expected = std::get<1>(GetParam());
