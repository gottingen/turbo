...
namespace turbo {
    ...
    TURBO_FORCE_INLINE uuid::uuid(const std::string_view &from) {
        TURBO_CHECK_EQ(from.size(), 36ul);  // 8-4-4-4-12
        auto p = from.data();
    ...
    TURBO_FORCE_INLINE uuid::uuid(uint64_t hi, uint64_t lo) {
        uint64_t* ptr =(uint64_t*)_bytes;
        *ptr++ = hi;
        *ptr = lo;
    }

    TURBO_FORCE_INLINE bool uuid::operator==(const uuid &other) const noexcept {
    ...
    TURBO_FORCE_INLINE int uuid::compare(const uuid &other) const noexcept {
        // `memcmp` is not `constexpr`..
        return __builtin_memcmp(_bytes, other._bytes, sizeof(_bytes));
    }

    TURBO_FORCE_INLINE int32_t uuid::to_decimal(char x) {
        if (x >= '0' && x <= '9') {
            return x - '0';
        } else if (x >= 'a' && x <= 'f') {
            return x - 'a' + 10;
            ...
            TURBO_CHECK(0)<<"Invalid hex digit ["<<x<<"].";
        }
        return 0;
    }

    TURBO_FORCE_INLINE uint8_t uuid::to_uint8(const char *starts_at) {  // `ToUint8`?
