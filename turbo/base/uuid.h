...
struct UUID {
  ...
  template <typename H>
  friend inline H TurboHashValue(H h, const UUID &id) {
    H::combine_contiguous(std::move(h), id.begin(), id.size());
  }
...
// Partial specialization: hash<turbo::UUID>
template <>
struct hash<turbo::UUID> {
  size_t operator()(const turbo::UUID& rhs) const { return rhs.hash_value(); }
};
