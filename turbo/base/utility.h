...
#ifndef TURBO_BASE_UTILITY_H_
#define TURBO_BASE_UTILITY_H_
...
#define TURBO_DECLVAL(...) ::turbo::detail::declval<__VA_ARGS__>()
#endif
    ...
    namespace detail {
        ...
        template<typename T>
        auto decay_0_(int) -> decltype(detail::decay_1_(TURBO_DECLVAL(T &&)));
 ...
 *      void foo() {
 *        MyData data;
 *        something(turbo::copy(data)); // explicit copy
 ...
 *
 *      turbo::copy(std::move(data)); // force-move, not just a cast to &&
 *
 ...
 */
    ...
    TURBO_INLINE_VARIABLE constexpr in_place_t in_place{};
    ...
    template<class T>
    TURBO_INLINE_VARIABLE constexpr in_place_type_t<T> in_place_type{};
    ...
    TURBO_INLINE_VARIABLE constexpr in_place_index_t<I> in_place_index{};
 ...
 *  public:
 *    explicit Something(int);
 *    Something(turbo::initlist_construct_t, std::initializer_list<int>);
 *
 *    operator int();
 *  };
 *
 ...
 *  Something something_two{turbo::initlist_construct, {1}}; // correct
 */
...
//  Example:
//
//    void takes_numbers(turbo::sorted_unique_t, std::vector<int> alist) {
...
//      takes_numbers(turbo::sorted_unique, alist);
//    }
//
...
//  Example:
//
//    void takes_numbers(turbo::sorted_equivalent_t, std::vector<int> alist) {
//      assert(std::is_sorted(alist.begin(), alist.end()));
//      for (i : alist) {
...
//    void takes_numbers(std::vector<int> alist) {
//      std::sort(alist.begin(), alist.end());
//      takes_numbers(turbo::sorted_equivalent, alist);
//    }
//
 ...
 */
    ...
    TURBO_INLINE_VARIABLE constexpr identity_fn identity{};
    ...
    namespace detail {
        ...
        struct inheritable_inherit_ : T {
            ...
            /* implicit */ TURBO_INLINE_VISIBILITY inheritable_inherit_(A &&... a) noexcept(
        ...
        struct inheritable_contain_ {
            ...
            template<
                    typename... A,
                    std::enable_if_t<std::is_constructible<T, A...>::value, int> = 0>
            /* implicit */ TURBO_INLINE_VISIBILITY inheritable_contain_(A &&... a) noexcept(
            noexcept(T(static_cast<A &&>(a)...)))
                    : v(static_cast<A &&>(a)...) {}

            TURBO_INLINE_VISIBILITY operator T &() & noexcept { return v; }

            TURBO_INLINE_VISIBILITY operator T &&() && noexcept { return static_cast<T &&>(v); }

            TURBO_INLINE_VISIBILITY operator T const &() const & noexcept { return v; }

            TURBO_INLINE_VISIBILITY operator T const &&() const && noexcept {
...
//  Example:
//
//      int local = turbo::unsafe_default_initialized;
...
//      use_value(value); // suppresses possible warning
    struct unsafe_default_initialized_cv {
        template<typename T>
        TURBO_INLINE_VISIBILITY constexpr /* implicit */ operator T() const noexcept {
#if defined(__cpp_lib_is_constant_evaluated)
#if __cpp_lib_is_constant_evaluated >= 201811L
            if (!std::is_constant_evaluated()) {
              T uninit;
              TURBO_PUSH_WARNING
              TURBO_MSVC_DISABLE_WARNING(4701)
              TURBO_MSVC_DISABLE_WARNING(4703)
              TURBO_GNU_DISABLE_WARNING("-Wuninitialized")
              // Clang doesn't implement -Wmaybe-uninitialized and warns about it
              TURBO_GCC_DISABLE_WARNING("-Wmaybe-uninitialized")
              return uninit;
              TURBO_POP_WARNING
            }
#endif
    ...
    TURBO_INLINE_VARIABLE constexpr unsafe_default_initialized_cv
    ...
    TURBO_INLINE_VARIABLE constexpr to_signed_fn to_signed{};
    ...
    TURBO_INLINE_VARIABLE constexpr to_unsigned_fn to_unsigned{};

    namespace detail {
        template<typename Src, typename Dst>
        TURBO_INLINE_VARIABLE constexpr bool is_to_narrow_convertible_v =
    ...
    public:
      ...
      to_narrow_convertible(to_narrow_convertible&&) = default;
        ...
        /* implicit */ constexpr operator Dst() const noexcept {
            TURBO_PUSH_WARNING
            TURBO_MSVC_DISABLE_WARNING(4244) // lossy conversion: arguments
            TURBO_MSVC_DISABLE_WARNING(4267) // lossy conversion: variables
            TURBO_GNU_DISABLE_WARNING("-Wconversion")
            return value_;
            TURBO_POP_WARNING
        }
...
//  for warnings which guard against narrowing implicit conversions.
    ...
    TURBO_INLINE_VARIABLE constexpr to_narrow_fn to_narrow{};
...
//  implicit conversions.
    ...
    TURBO_INLINE_VARIABLE constexpr to_integral_fn to_integral{};
...
//  implicit conversions.
    ...
    TURBO_INLINE_VARIABLE constexpr to_floating_point_fn to_floating_point{};
    ...
    TURBO_INLINE_VARIABLE constexpr to_underlying_fn to_underlying{};

    namespace detail {
        template<typename R>
        using invocable_to_detect = decltype(TURBO_DECLVAL(R)());
        ...
        private:
            ...
            template<typename R>
            static constexpr bool nx_v = noexcept(TURBO_DECLVAL(R)());
            template<typename G>
...
//
//      list.emplace_back(turbo::invoke_to([] {
//        stable obj;
//        obj.value = 7;
//        return obj;
//      }));
    struct invocable_to_fn {
        ...
        TURBO_INLINE_VISIBILITY constexpr R operator()(F &&f) const
        noexcept(noexcept(R(static_cast<F &&>(f)))) {
            return R(static_cast<F &&>(f));
        }
    };

    TURBO_INLINE_VARIABLE constexpr invocable_to_fn invocable_to{};
...
// Simulate if constexpr in C++14
    ...
    template<bool Cond>
    TURBO_INLINE_VARIABLE constexpr if_constexpr_fn<Cond> if_constexpr{};
} // namespace turbo


#endif  // TURBO_BASE_UTILITY_H_
