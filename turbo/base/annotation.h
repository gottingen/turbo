...
#ifndef TURBO_BASE_ANNOTATION_H_
#define TURBO_BASE_ANNOTATION_H_
...
#define TURBO_INTERNAL_USE_ASAN 1
#endif  // __has_feature(address_sanitizer)

#if __has_feature(thread_sanitizer)
#define TURBO_INTERNAL_USE_TSAN 1
#endif  // __has_feature(thread_sanitizer)

#define TURBO_INTERNAL_NO_SANITIZE_ADDRESS [[clang::no_sanitize("address")]]
#define TURBO_INTERNAL_NO_SANITIZE_THREAD [[clang::no_sanitize("thread")]]
#endif  // __clang__
...
#define TURBO_INTERNAL_NO_SANITIZE_ADDRESS [[gnu::no_sanitize("address")]]
#define TURBO_INTERNAL_NO_SANITIZE_THREAD [[gnu::no_sanitize("thread")]]

#endif  // defined(__GNUC__) && !defined(__clang__)

#ifdef TURBO_INTERNAL_USE_ASAN
extern "C" {
...
#endif  // TURBO_INTERNAL_USE_ASAN

// FIXME: TSan support does not work yet.
#ifdef TURBO_INTERNAL_USE_TSAN
extern "C" {
...
// TODO(yinbinli): Annotations for fiber, to replace our own implementation.

#define TURBO_INTERNAL_TSAN_MUTEX_CREATE(...) __tsan_mutex_create(__VA_ARGS__)
#define TURBO_INTERNAL_TSAN_MUTEX_DESTROY(...) __tsan_mutex_destroy(__VA_ARGS__)
#define TURBO_INTERNAL_TSAN_MUTEX_PRE_LOCK(...) \
  __tsan_mutex_pre_lock(__VA_ARGS__)
#define TURBO_INTERNAL_TSAN_MUTEX_POST_LOCK(...) \
  __tsan_mutex_post_lock(__VA_ARGS__)
#define TURBO_INTERNAL_TSAN_MUTEX_PRE_UNLOCK(...) \
  __tsan_mutex_pre_unlock(__VA_ARGS__)
#define TURBO_INTERNAL_TSAN_MUTEX_POST_UNLOCK(...) \
  __tsan_mutex_post_unlock(__VA_ARGS__)

#else

#define TURBO_INTERNAL_TSAN_MUTEX_CREATE(...) (void)0
#define TURBO_INTERNAL_TSAN_MUTEX_DESTROY(...) (void)0
#define TURBO_INTERNAL_TSAN_MUTEX_PRE_LOCK(...) (void)0
#define TURBO_INTERNAL_TSAN_MUTEX_POST_LOCK(...) (void)0
#define TURBO_INTERNAL_TSAN_MUTEX_PRE_UNLOCK(...) (void)0
#define TURBO_INTERNAL_TSAN_MUTEX_POST_UNLOCK(...) (void)0

#endif  // TURBO_INTERNAL_USE_TSAN

// Using `initial-exec` prevents binary from being loaded via `dlopen`. turbo is
// not intended to be used that way (via `dlopen`). Yet in case there's such
// need, you can define `TURBO_USE_SLOW_TLS_MODEL` to accomplish that.
//
// @sa: `doc/performance-guide.md` for more details.
#if !defined(TURBO_USE_SLOW_TLS_MODEL)
#define TURBO_INTERNAL_TLS_MODEL [[gnu::tls_model("initial-exec")]]
#else
#define TURBO_INTERNAL_TLS_MODEL /* Nothing, using the default model. */
#endif
...
#define TURBO_INTERNAL_ASAN_CHECK(fp) \
  TURBO_DCHECK(fp, "Your compiler is too old to use ASan with fibers.");
...
// `complete_switch_fiber` with this shadow stack to restore it.
//
...
inline void start_switch_fiber(void** shadow_stack, const void* stack_bottom,
                             std::size_t stack_limit) {
  TURBO_INTERNAL_ASAN_CHECK(__sanitizer_start_switch_fiber);
...
// `start_switch_fiber`, i.e., **this** (the one you're currently, and will be,
...
// sanitizer.
inline void complete_switch_fiber(void* shadow_stack) {
  TURBO_INTERNAL_ASAN_CHECK(__sanitizer_finish_switch_fiber);
...
inline void poison_memory_region(const void* ptr, std::size_t size) {
  TURBO_INTERNAL_ASAN_CHECK(__asan_poison_memory_region);
  __asan_poison_memory_region(ptr, size);
}

inline void UnpoisonMemoryRegion(const void* ptr, std::size_t size) {
  TURBO_INTERNAL_ASAN_CHECK(__asan_unpoison_memory_region);
  __asan_unpoison_memory_region(ptr, size);
}

#undef TURBO_INTERNAL_ASAN_CHECK
...
#define TURBO_INTERNAL_TSAN_CHECK(fp) \
  TURBO_DCHECK(fp, "Your compiler is too old to use TSan with fibers.");
...
inline void* create_fiber() {
  TURBO_INTERNAL_TSAN_CHECK(__tsan_create_fiber);
  return __tsan_create_fiber(0);
}

// Destroy a TSan fiber context.
inline void destroy_fiber(void* fiber) {
  TURBO_INTERNAL_TSAN_CHECK(__tsan_destroy_fiber);
  __tsan_destroy_fiber(fiber);
}

// Switch fiber context in TSan.
inline void switch_to_fiber(void* fiber) {
  TURBO_INTERNAL_TSAN_CHECK(__tsan_switch_to_fiber);
...
inline void* get_current_fiber() {
  TURBO_INTERNAL_TSAN_CHECK(__tsan_get_current_fiber);
...
inline void SetFiberName(void* fiber, const char* name) {
  TURBO_INTERNAL_TSAN_CHECK(__tsan_set_fiber_name);
  __tsan_set_fiber_name(fiber, name);
}

#undef TURBO_INTERNAL_TSAN_CHECK
...
#endif  // TURBO_BASE_ANNOTATION_H_
