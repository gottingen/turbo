...
#ifndef TURBO_BASE_INTERNAL_COMPILER_H_
#define TURBO_BASE_INTERNAL_COMPILER_H_

#include "turbo/base/profile/platform.h"
...
#ifndef TURBO_COMPILER_HAS_INCLUDE
#if defined(__has_include) && !defined(__INTELLISENSE__) && \
    (!defined(__INTEL_COMPILER) || __INTEL_COMPILER >= 1600)
#define TURBO_COMPILER_HAS_INCLUDE(x) __has_include(x)
#else
#define TURBO_COMPILER_HAS_INCLUDE(x) 0
#endif
#endif


// TURBO_COMPILER_HAS_ATTRIBUTE
//
...
#ifndef TURBO_COMPILER_HAS_ATTRIBUTE
    #ifdef __has_attribute
        #define TURBO_COMPILER_HAS_ATTRIBUTE(x) __has_attribute(x)
    #else
        #define TURBO_COMPILER_HAS_ATTRIBUTE(x) 0
    #endif
#endif  // TURBO_COMPILER_HAS_ATTRIBUTE

// TURBO_COMPILER_HAS_CPP_ATTRIBUTE
//
// A function-like feature checking macro that accepts C++11 style attributes.
// It's a wrapper around `__has_cpp_attribute`, defined by ISO C++ SD-6
// (https://en.ccreference.com/w/cpp/experimental/feature_test). If we don't
// find `__has_cpp_attribute`, will evaluate to 0.
#ifndef TURBO_COMPILER_HAS_CPP_ATTRIBUTE
    #if defined(__has_cpp_attribute)
        // NOTE: requiring __cplusplus above should not be necessary, but
        // works around https://bugs.llvm.org/show_bug.cgi?id=23435.
        #define TURBO_COMPILER_HAS_CPP_ATTRIBUTE(x) __has_cpp_attribute(x)
    #else
        #define TURBO_COMPILER_HAS_CPP_ATTRIBUTE(x) 0
    #endif
#endif

#define TURBO_HAS_CPP17_ATTRIBUTE(attribute) \
  (__cplusplus >= 201703L && TURBO_COMPILER_HAS_CPP_ATTRIBUTE(attribute))
...
#define TURBO_INTERNAL_HAS_KEYWORD(x) !(__is_identifier(x))
#else
#define TURBO_INTERNAL_HAS_KEYWORD(x) 0
#endif
#endif


// TURBO_COMPILER_NO_TAIL_CALL
//
// Prevents the compiler from optimizing away stack frames for functions which
// end in a call to another function.
#ifndef TURBO_COMPILER_NO_TAIL_CALL
#if TURBO_COMPILER_HAS_ATTRIBUTE(disable_tail_calls)
#define TURBO_COMPILER_HAS_NO_TAIL_CALL 1
#define TURBO_COMPILER_NO_TAIL_CALL __attribute__((disable_tail_calls))
#elif defined(__GNUC__) && !defined(__clang__)
#define TURBO_COMPILER_HAS_NO_TAIL_CALL 1
#define TURBO_COMPILER_NO_TAIL_CALL \
          __attribute__((optimize("no-optimize-sibling-calls")))
#else
#define TURBO_COMPILER_NO_TAIL_CALL
#define TURBO_COMPILER_HAS_NO_TAIL_CALL 0
#endif
#endif
...
#define TURBO_CLANG_VERSION TURBO_COMPILER_VERSION
...
#define TURBO_GNUC_VERSION  TURBO_COMPILER_VERSION
...
#define TURBO_BORLANDC_VERSION TURBO_COMPILER_VERSION
...
#elif (defined(TURBO_COMPILER_BORLAND) || defined(TURBO_COMPILER_MSVC)) && !defined(_CPPUNWIND)
...
// Clang & GCC
#define TURBO_PUSH_WARNING _Pragma("GCC diagnostic push")
#define TURBO_POP_WARNING _Pragma("GCC diagnostic pop")
#define TURBO_GNU_DISABLE_WARNING_INTERNAL2(warningName) #warningName
#define TURBO_GNU_DISABLE_WARNING(warningName) \
  _Pragma(                                     \
      TURBO_GNU_DISABLE_WARNING_INTERNAL2(GCC diagnostic ignored warningName))
#ifdef __clang__
#define TURBO_CLANG_DISABLE_WARNING(warningName) \
  TURBO_GNU_DISABLE_WARNING(warningName)
#define TURBO_GCC_DISABLE_WARNING(warningName)
#else
#define TURBO_CLANG_DISABLE_WARNING(warningName)
#define TURBO_GCC_DISABLE_WARNING(warningName) \
  TURBO_GNU_DISABLE_WARNING(warningName)
#endif
#define TURBO_MSVC_DISABLE_WARNING(warningNumber)
#elif defined(_MSC_VER)
#define TURBO_PUSH_WARNING __pragma(warning(push))
#define TURBO_POP_WARNING __pragma(warning(pop))
...
#else
#define TURBO_PUSH_WARNING
#define TURBO_POP_WARNING
#define TURBO_GNU_DISABLE_WARNING(warningName)
#define TURBO_GCC_DISABLE_WARNING(warningName)
#define TURBO_CLANG_DISABLE_WARNING(warningName)
#define TURBO_MSVC_DISABLE_WARNING(warningNumber)
...
#if defined(TURBO_HAS_INLINE_VARIABLES)
#error "TURBO_HAS_INLINE_VARIABLES can not be define manully"
#endif
#if __cpp_inline_variables >= 201606L || __cplusplus >= 201703L
#define TURBO_HAS_INLINE_VARIABLES 1
#define TURBO_INLINE_VARIABLE inline
#else
#define TURBO_HAS_INLINE_VARIABLES 0
...
// However, cxxbase provides a portable TURBO_ALIGN_OF which works for all compilers.
//
...
#endif  // TURBO_BASE_INTERNAL_COMPILER_H_
