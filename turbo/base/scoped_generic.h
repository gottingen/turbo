...
#ifndef TURBO_BASE_SCOPED_GENERIC_H_
#define TURBO_BASE_SCOPED_GENERIC_H_

#include <cstdlib>
#include <algorithm>
#include "turbo/base/profile.h"
#include "turbo/memory/move.h"
...
//
// For an object to be able to be put into a scoped_generic, it must support
...
//   };
//
//   typedef scoped_generic<int, FooScopedTraits> ScopedFoo;


    template<typename T, typename Traits>
    class scoped_generic {
    MOVE_ONLY_TYPE_FOR_CPP_03(scoped_generic, RValue)
    ...
    public:
        ...
        scoped_generic() : data_(traits_type::InvalidValue()) {}

        // Constructor. Takes responsibility for freeing the resource associated with
        // the object T.
        explicit scoped_generic(const element_type &value) : data_(value) {}

        // Constructor. Allows initialization of a stateful traits object.
        scoped_generic(const element_type &value, const traits_type &traits)
                : data_(value, traits) {
        }

        // Move constructor for C++03 move emulation.
        scoped_generic(RValue rvalue)
        ...
        element_type release() TURBO_WARN_UNUSED_RESULT {
    ...
    private:
        ...
        bool operator==(
                const scoped_generic<T2, Traits2> &p2) const;
    ...
    void swap(const scoped_generic<T, Traits> &a,
              const scoped_generic<T, Traits> &b) {
        a.swap(b);
    }

    template<class T, class Traits>
    bool operator==(const T &value, const scoped_generic<T, Traits> &scoped) {
...
#endif  // TURBO_BASE_SCOPED_GENERIC_H_
