...
#ifndef TURBO_ENDPOINT_EXTENDED_H_
#define TURBO_ENDPOINT_EXTENDED_H_
...
namespace turbo::detail {
    ...
    public:
        ...
        static extended_end_point *address(const end_point &ep) {
            ...
            ::turbo::ResourceId <extended_end_point> id;
            id.value = ep.ip.s_addr;
            extended_end_point *eep = ::turbo::address_resource<extended_end_point>(id);
            TURBO_CHECK(eep) << "fail to address extended_end_point from EndPoint";
            return eep;
        }
    ...
    private:
        ...
        static extended_end_point *new_extended_endpoint(sa_family_t family) {
            ::turbo::ResourceId <extended_end_point> id;
            extended_end_point *eep = ::turbo::get_resource(&id);
            if (eep) {
                int64_t old_ref = eep->_ref_count.load(std::memory_order_relaxed);
                TURBO_CHECK(old_ref == 0) << "new extended_end_point has reference " << old_ref;
                TURBO_CHECK(eep->_u.sa.sa_family == AF_UNSPEC) << "new extended_end_point has family " << eep->_u.sa.sa_family
                                                         << " set";
        ...
        void embed_to(end_point *ep) const {
            TURBO_CHECK(0 == _id.value >> 32) << "ResourceId beyond index";
            ep->reset();
    ...
    public:
        ...
        void dec_ref(void) {
            ...
            TURBO_CHECK(old_ref >= 1) << "extended_end_point has unexpected reference " << old_ref;
            if (old_ref == 1) {
        ...
        void to(end_point_str *ep_str) const {
            if (_u.sa.sa_family == AF_UNIX) {
                ...
                TURBO_CHECK(ret) << "fail to do inet_ntop";
                snprintf(ep_str->_buf, sizeof(ep_str->_buf), "[%s]:%d", buf, ntohs(_u.in6.sin6_port));
            } else {
                TURBO_CHECK(0) << "family " << _u.sa.sa_family << " not supported";
            }
        }
    ...
    private:
        ...
        turbo::ResourceId <extended_end_point> _id;
...
#endif // TURBO_ENDPOINT_EXTENDED_H_
