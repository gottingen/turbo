...
namespace turbo {
    ...
    public:
        ...
        static result_status from_turbo_error(int err);

        static result_status from_turbo_error(int err, const std::string_view &ext);

        template<typename ...Args>
        static result_status from_turbo_error(int err, const std::string_view &fmt, Args &&...args) {
            auto msg = string_format(fmt, std::forward<Args>(args)...);
            return from_turbo_error(err, msg);
        }
