...
// 
//     "redefinition of `class turbo_errno_helper<30>'"
//
...
class turbo_errno_helper {
};

#define TURBO_REGISTER_ERRNO(error_code, description)                   \
    const int TURBO_ALLOW_UNUSED TURBO_CONCAT(turbo_errno_dummy_, __LINE__) =              \
        ::turbo::base::describe_customized_errno((error_code), #error_code, (description)); \
    template <> class turbo_errno_helper<(int)(error_code)> {};
