...
namespace turbo {
    ...
    end_point::end_point(ip_t ip2, int port2) : ip(ip2), port(port2) {
        ...
        if (extended_end_point::is_extended(*this)) {
            TURBO_CHECK(0) << "EndPoint construct with value that points to an extended EndPoint";
            ip = IP_ANY;
            port = 0;
        }
    }
    ...
    int ip2hostname(ip_t ip, char *host, size_t host_len) {
        ...
        if (turbo::ends_with(host, ".baidu.com")) {
            host[str.size() - 10] = '\0';
        }
        return 0;
    }
...
#endif // defined(TURBO_PLATFORM_APPLE)
    ...
    int tcp_connect(end_point point, int *self_port) {
        ...
        turbo::base::fd_guard sockfd(socket(serv_addr.ss_family, SOCK_STREAM, 0));
        if (sockfd < 0) {
            return -1;
        }
        ...
        if (self_port != nullptr) {
            ...
            if (get_local_side(sockfd, &pt) == 0) {
                *self_port = pt.port;
                ...
                TURBO_CHECK(false) << "Fail to get the local port of sockfd=" << sockfd;
            }
        }
    ...
    int tcp_listen(end_point point) {
...
#else
            TURBO_LOG(ERROR) << "Missing def of SO_REUSEADDR while -reuse_addr is on";
            return -1;
#endif
        }
        ...
        if (FLAGS_reuse_port) {
            ...
            if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEPORT,
                           &on, sizeof(on)) != 0) {
                TURBO_LOG(WARNING) << "Fail to setsockopt SO_REUSEPORT of sockfd=" << sockfd;
            }
#else
            TURBO_LOG(ERROR) << "Missing def of SO_REUSEPORT while -reuse_port is on";
            return -1;
#endif
        }
