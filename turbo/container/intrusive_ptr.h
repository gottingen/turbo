...
#ifndef TURBO_CONTAINER_INTRUSIVE_PTR_H_
#define TURBO_CONTAINER_INTRUSIVE_PTR_H_
...
namespace TURBO_HASH_NAMESPACE {
    ...
    template<typename T>
    struct hash<turbo::container::intrusive_ptr < T> > {
    std::size_t operator()(const turbo::container::intrusive_ptr <T> &p) const {
        return hash<T *>()(p.get());
    }
};
#elif defined(TURBO_COMPILER_MSVC)
template<typename T>
inline size_t hash_value(const turbo::container::intrusive_ptr<T>& sp) {
...
#endif  // TURBO_CONTAINER_INTRUSIVE_PTR_H_
