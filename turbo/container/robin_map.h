...
#ifndef TURBO_CONTAINER_ROBIN_MAP_H_
#define TURBO_CONTAINER_ROBIN_MAP_H_
...
#include <type_traits>
#include <utility>
#include "turbo/container/robin_hash.h"
...
namespace turbo {
     ...
     * as we then compare the stored hashes before comparing the keys. When `turbo::rh::power_of_two_growth_policy` is used
     ...
     * `sizeof(turbo::detail_robin_hash::bucket_entry<ValueType, true>) ==
     * sizeof(turbo::detail_robin_hash::bucket_entry<ValueType, false>)`) and `turbo::rh::power_of_two_growth_policy` is
     ...
     * By default the map uses `turbo::rh::power_of_two_growth_policy`. This policy keeps the number of buckets
     ...
     * check `turbo::rh::power_of_two_growth_policy` for the interface.
     *
    ...
    template<class Key,
            ...
            class GrowthPolicy = turbo::rh::power_of_two_growth_policy<2>>
    class robin_map {
    private:
        template<typename U>
        using has_is_transparent = turbo::detail_robin_hash::has_is_transparent<U>;
    ...
    /**
     * Same as `turbo::robin_map<Key, T, Hash, KeyEqual, Allocator, StoreHash, turbo::rh::prime_growth_policy>`.
     */
    template<class Key,
            class T,
            class Hash = std::hash<Key>,
            ...
            class Allocator = std::allocator<std::pair<Key, T>>,
            bool StoreHash = false>
    using robin_pg_map = robin_map<Key, T, Hash, KeyEqual, Allocator, StoreHash, turbo::rh::prime_growth_policy>;

}  // namespace turbo

#endif  // TURBO_CONTAINER_ROBIN_MAP_H_
