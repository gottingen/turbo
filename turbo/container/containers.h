...
#ifndef TURBO_CONTAINER_CONTAINERS_H_
#define TURBO_CONTAINER_CONTAINERS_H_
...
namespace turbo {
    namespace containers {
        ...
        // Unlike std::vector, turbo::containers::vector keeps the first
        ...
        private:
            using TStorage = typename turbo::aligned_storage<sizeof(T), alignof(T)>::type;
        ...
        vector<T, BASE_CAPACITY>::vector(
                turbo::allocator *allocator /* = allocator::Default */)
                : _allocator(allocator) {}

        template<typename T, int BASE_CAPACITY>
        template<int BASE_CAPACITY_2>
        vector<T, BASE_CAPACITY>::vector(
                const vector<T, BASE_CAPACITY_2> &other, turbo::allocator *alloc /* = allocator::Default */)
                : _allocator(alloc) {
            *this = other;
        }
        ...
        void vector<T, BASE_CAPACITY>::pop_back() {
            TURBO_CHECK(count > 0) << "pop_back() called on empty vector";
            count--;
        ...
        T &vector<T, BASE_CAPACITY>::front() {
            TURBO_CHECK(count > 0) << "front() called on empty vector";
            return reinterpret_cast<T *>(elements)[0];
        }

        template<typename T, int BASE_CAPACITY>
        T &vector<T, BASE_CAPACITY>::back() {
            TURBO_CHECK(count > 0) << "back() called on empty vector";
        ...
        void list<T>::grow(size_t count) {
            ...
            auto const allocChainOffset = turbo::base::align_up(entriesSize, alignof(AllocationChain));
        ...
        void list<T>::link(Entry *entry, Entry *&list) {
            TURBO_CHECK(entry->next == nullptr);
            TURBO_CHECK(entry->prev == nullptr);
            if (list) {
                entry->next = list;
                list->prev = entry;
            }
...
#endif  // TURBO_CONTAINER_CONTAINERS_H_
