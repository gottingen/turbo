...
#ifndef TURBO_CONTAINER_ROBIN_GROWTH_POLICY_H_
#define TURBO_CONTAINER_ROBIN_GROWTH_POLICY_H_
 ...
 */
#ifndef TURBO_THROW_OR_TERMINATE
#    if (defined(__cpp_exceptions) || defined(__EXCEPTIONS) || (defined (_MSC_VER) && defined (_CPPUNWIND))) && !defined(TSL_NO_EXCEPTIONS)
#        define TURBO_THROW_OR_TERMINATE(ex, msg) throw ex(msg)
#    else
#        ifdef NDEBUG
#            define TURBO_THROW_OR_TERMINATE(ex, msg) std::terminate()
#        else
#            include <cstdio>
#            define TURBO_THROW_OR_TERMINATE(ex, msg) do { std::fprintf(stderr, msg); std::terminate(); } while(0)
#        endif
#    endif
#endif
...
public:
    ...
    explicit power_of_two_growth_policy(std::size_t& min_bucket_count_in_out) {
        if(min_bucket_count_in_out > max_bucket_count()) {
            TURBO_THROW_OR_TERMINATE(std::length_error, "The hash table exceeds its maxmimum size.");
        }
...
/**
 * Grow the hash table by using prime numbers as bucket count. Slower than turbo::rh::power_of_two_growth_policy in
...
#endif  // TURBO_CONTAINER_ROBIN_GROWTH_POLICY_H_
