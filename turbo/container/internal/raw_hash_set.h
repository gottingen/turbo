...
#include "turbo/container/internal/map_fwd_decl.h"
        ...
        */
        ...
        public:
            ...
            int LowestBitSet() const {
                return turbo::base::countr_zero(mask_) >> Shift;
            }

            int HighestBitSet() const {
                return (sizeof(T) * CHAR_BIT - turbo::base::countl_zero(mask_) -
                        1) >>
                           Shift;
            }
            ...
            int leading_zeros() const {
                ...
                return turbo::base::countl_zero(mask_ << extra_bits) >> Shift;
            }
...
#ifdef TURBO_MAP_NON_DETERMINISTIC
...
// --------------------------------------------------------------------------
        ...
        struct group_portable_impl {
            ...
            explicit group_portable_impl(const ctrl_t *pos)
                    : ctrl(turbo::base::little_endian::load64(pos)) {}
            ...
            uint32_t count_leading_empty_or_deleted() const {
                ...
                return (uint32_t) ((turbo::base::countr_zero(((~ctrl & (ctrl >> 7)) | gaps) + 1) + 7) >> 3);
            }
            ...
            void convert_special_to_empty_and_full_to_deleted(ctrl_t *dst) const {
                ...
                auto x = ctrl & msbs;
                auto res = (~x + (x >> 7)) & ~lsbs;
                turbo::base::little_endian::store64(dst, res);
            }
...
// --------------------------------------------------------------------------
        inline size_t NormalizeCapacity(size_t n) {
            return n ? ~size_t{} >> turbo::base::countl_zero(n) : 1;
        }
...
// ----------------------------------------------------------------------------
        ...
        private:
            ...
            using Layout = turbo::priv::Layout<ctrl_t, slot_type>;
        ...
        iterator operator--(int) {
            ...
            std::is_nothrow_copy_constructible<allocator_type>::value)
                    : ctrl_(turbo::exchange(that.ctrl_, EmptyGroup())),
                      slots_(turbo::exchange(that.slots_, nullptr)),
                      size_(turbo::exchange(that.size_, 0)),
                      capacity_(turbo::exchange(that.capacity_, 0)),
                      infoz_(turbo::exchange(that.infoz_, HashtablezInfoHandle())),
            ...
            template<class InputIt, typename turbo::enable_if_t<has_difference_operator<InputIt>::value, int> = 0>
...
#if !defined(TURBO_MAP_NON_DETERMINISTIC)

            template<typename OutputArchive>
            bool turbo_map_dump(OutputArchive &) const;

            template<typename InputArchive>
            bool turbo_map_load(InputArchive &);
        ...
        private:
            ...
            friend
            struct turbo::priv::hashtable_debug_internal::HashtableDebugAccess;
            ...
            struct HashElement {
                ...
                size_t operator()(const K &key, Args &&...) const {
                    return turbo_mix<sizeof(size_t)>()(h(key));
                }
            ...
            void drop_deletes_without_resize() TURBO_NO_INLINE {
            ...
            bool has_element(const value_type &elem, size_t hashval) const {
                ...
                while (true) {
                    Group g{ctrl_ + seq.offset()};
                    for (int i : g.match((h2_t) H2(hashval))) {
                        if (TURBO_LIKELY(PolicyTraits::element(slots_ + seq.offset((size_t) i)) ==
                                         elem))
                            return true;
                    }
                    if (TURBO_LIKELY(g.match_empty())) return false;
                    seq.next();
        ...
        protected:
            ...
            std::pair<size_t, bool> find_or_prepare_insert(const K &key, size_t hashval) {
                ...
                while (true) {
                    ...
                    if (TURBO_LIKELY(g.match_empty())) break;
                    seq.next();
                }
            ...
            size_t prepare_insert(size_t hashval) TURBO_NO_INLINE {
                auto target = find_first_non_full(hashval);
                if (TURBO_UNLIKELY(growth_left() == 0 &&
        ...
        private:
            ...
            turbo::priv::CompressedTuple<size_t /* growth_left */, hasher,
...
// --------------------------------------------------------------------------
        ...
        public:
            ...
            MappedReference<P> at(const key_arg<K> &key) {
                ...
                if (it == this->end())
                    turbo::base_internal::ThrowStdOutOfRange("flat_hash_map at(): lookup non-existent key");
...
// ----------------------------------------------------------------------------
        ...
        protected:
            using Lockable = turbo::LockableImpl<Mtx_>;
        ...
        public:
            ...
            explicit parallel_hash_set(size_t bucket_cnt,
                    ...
                    parallel_hash_set(typename Inner::Params{bucket_cnt, hash_param, eq, alloc},
                                      turbo::make_index_sequence<num_tables>{}) {}

            template<std::size_t... i>
            parallel_hash_set(typename Inner::Params const &p,
                              turbo::index_sequence<i...>) : sets_{((void) i, p)...} {}
        ...
        for (auto& inner : sets_)
            ...
            bool if_contains_unsafe(const key_arg<K> &key, F &&f) const {
                return const_cast<parallel_hash_set *>(this)->template
                        modify_if_impl<K, F, LockableBaseImpl<turbo::null_mutex>::DoNothing>(key, std::forward<F>(f));
            }
            ...
            template<typename OutputArchive>
            bool turbo_map_dump(OutputArchive &ar) const;

            template<typename InputArchive>
            bool turbo_map_load(InputArchive &ar);
        ...
        struct flat_hash_set_policy {
            ...
            template<class F, class... Args>
            static decltype(turbo::priv::DecomposeValue(
                    std::declval<F>(), std::declval<Args>()...))
            apply(F &&f, Args &&... args) {
                return turbo::priv::DecomposeValue(
        ...
        struct flat_hash_map_policy {
            ...
            template<class F, class... Args>
            static decltype(turbo::priv::DecomposePair(
                    std::declval<F>(), std::declval<Args>()...))
            apply(F &&f, Args &&... args) {
                return turbo::priv::DecomposePair(std::forward<F>(f),
                                                  std::forward<Args>(args)...);
            }
        ...
        struct node_hash_set_policy
                : turbo::priv::node_hash_policy<T &, node_hash_set_policy<T>> {
            using key_type = T;
            using init_type = T;
            using constant_iterators = std::true_type;
...
// --------------------------------------------------------------------------
        ...
        class node_hash_map_policy
                : public turbo::priv::node_hash_policy<
        ...
        struct hash_eq<T *> {
            struct hash {
                ...
                size_t operator()(const U &ptr) const {
                    return turbo::hash<const T *>{}(hash_eq::ToPtr(ptr));
                }
            };
