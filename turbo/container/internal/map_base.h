...
#ifndef TURBO_CONTAINER_INTERNAL_MAP_BASE_H_
#define TURBO_CONTAINER_INTERNAL_MAP_BASE_H_
...
namespace turbo {
    ...
    struct case_ignored_less<std::string> {
        inline bool operator()(const std::string_view &a, const std::string_view &b) const {
            return turbo::compare_case(a, b) < 0;
        }
    };
...
#define TURBO_MAP_INTERNAL_EXTERN_DECL(type, name) \
...
#define TURBO_MAP_INTERNAL_INLINE_CONSTEXPR(type, name, init) \
  TURBO_MAP_INTERNAL_EXTERN_DECL(type, name)                  \
...
//   types, etc..
#define TURBO_MAP_INTERNAL_INLINE_CONSTEXPR(var_type, name, init)                  \
  template <class /*turbo_map_internal_dummy*/ = void>                               \
  struct map_internal_inline_variable_holder##name {                             \
    static constexpr ::turbo::internal::identity_t<var_type> kInstance = init; \
  };                                                                          \
                                                                              \
  template <class turbo_map_internal_dummy>                                          \
  constexpr ::turbo::internal::identity_t<var_type>                            \
      map_internal_inline_variable_holder##name<turbo_map_internal_dummy>::kInstance;   \
                                                                              \
...
namespace turbo {
    ...
    namespace memory_internal {
        ...
        // Traits to select proper overload and return type for `turbo::make_unique<>`.
        template<typename T>
...
// ExtractOr<E, O, D>::type evaluates to E<O> if possible. Otherwise, D.
    ...
    struct allocator_traits {
        ...
        // turbo::pointer_traits<pointer>::rebind<const value_type>
        using const_pointer =
        memory_internal::ExtractOrT<memory_internal::GetConstPointer, Alloc,
                typename turbo::pointer_traits<pointer>::
                template rebind<const value_type>>;

        // void_pointer:
        // Alloc::void_pointer if present, otherwise
        // turbo::pointer_traits<pointer>::rebind<void>
        using void_pointer = memory_internal::ExtractOrT<
                memory_internal::GetVoidPointer, Alloc,
                typename turbo::pointer_traits<pointer>::template rebind<void>>;

        // const_void_pointer:
        // Alloc::const_void_pointer if present, otherwise
        // turbo::pointer_traits<pointer>::rebind<const void>
        using const_void_pointer = memory_internal::ExtractOrT<
                memory_internal::GetConstVoidPointer, Alloc,
                typename turbo::pointer_traits<pointer>::template rebind<const void>>;

        // difference_type:
        // Alloc::difference_type if present, otherwise
        // turbo::pointer_traits<pointer>::difference_type
        using difference_type = memory_internal::ExtractOrT<
                memory_internal::GetDifferenceType, Alloc,
                typename turbo::pointer_traits<pointer>::difference_type>;
        ...
        // rebind_traits:
        // turbo::allocator_traits<rebind_alloc<T>>
        template<typename T>
        using rebind_traits = turbo::allocator_traits<rebind_alloc<T>>;
...
namespace turbo {
    ...
    namespace priv {
        ...
        template<class T>
        struct IsTransparent<T, turbo::void_t<typename T::is_transparent>>
                : std::true_type {
        };
        ...
        private:
            ...
            mutable turbo::aligned_storage_t<sizeof(slot_type), alignof(slot_type)> slot_space_;
        };
...
// ---------
        ...
        class node_handle<Policy, PolicyTraits, Alloc,
                turbo::void_t<typename Policy::mapped_type>>
...
// https://bugs.llvm.org/show_bug.cgi?id=27538.
    ...
    public:
        ...
        constexpr reference front() const noexcept {
            return TURBO_ASSERT(size() > 0), *data();
        }
        ...
        constexpr reference back() const noexcept {
            return TURBO_ASSERT(size() > 0), *(data() + size() - 1);
        }
...
//
    ...
    Span<T> MakeSpan(T *begin, T *end) noexcept {
        return TURBO_ASSERT(begin <= end), Span<T>(begin, end - begin);
    }
...
//
    ...
    Span<const T> MakeConstSpan(T *begin, T *end) noexcept {
        return TURBO_ASSERT(begin <= end), Span<const T>(begin, end - begin);
    }
...
#define TURBO_MAP_INTERNAL_HAS_CXA_DEMANGLE
#endif

#ifdef TURBO_MAP_INTERNAL_HAS_CXA_DEMANGLE
...
// ------------------------------------------------
            ...
            struct Storage {
                ...
                explicit constexpr Storage(T &&v) : value(turbo::forward<T>(v)) {}

                constexpr const T &get() const &{ return value; }

                T &get() &{ return value; }

                constexpr const T &&get() const &&{ return turbo::move(*this).value; }
            ...
            struct Storage<D, I, true>
                ...
                explicit constexpr Storage(T &&v) : T(turbo::forward<T>(v)) {}

                constexpr const T &get() const &{ return *this; }

                T &get() &{ return *this; }

                constexpr const T &&get() const &&{ return turbo::move(*this); }
            ...
            struct CompressedTupleImpl<CompressedTuple<Ts...>, turbo::index_sequence<I...>>
                ...
                explicit constexpr CompressedTupleImpl(Ts &&... args)
                        : Storage<CompressedTuple<Ts...>, I>(turbo::forward<Ts>(args))... {}
            };
...
//
// Eg:
//   turbo::priv::CompressedTuple<int, T1, T2, T3> value(7, t1, t2,
//                                                                    t3);
...
// ---------------------------------------------------------------------------
        ...
        public:
            ...
            explicit constexpr CompressedTuple(Ts... base)
                    : CompressedTuple::CompressedTupleImpl(turbo::forward<Ts>(base)...) {}
        ...
        public:
            ...
            static void swap(Allocator *alloc, slot_type *a, slot_type *b) {
                if (kMutableKeys::value) {
                    ...
                    turbo::allocator_traits<Allocator>::destroy(*alloc, &a->value);
                    turbo::allocator_traits<Allocator>::construct(*alloc, &a->value,
                                                                  std::move(b->value));
                    turbo::allocator_traits<Allocator>::destroy(*alloc, &b->value);
                    turbo::allocator_traits<Allocator>::construct(*alloc, &b->value,
                                                                  std::move(tmp));
                }
            }
            ...
            static void move(Allocator *alloc, slot_type *src, slot_type *dest) {
                if (kMutableKeys::value) {
                    ...
                    turbo::allocator_traits<Allocator>::destroy(*alloc, &dest->value);
                    turbo::allocator_traits<Allocator>::construct(*alloc, &dest->value,
                                                                  std::move(src->value));
                }
            }
...
namespace turbo {
    ...
    // internal locking (like turbo::parallel_flat_hash_map).
    // -----------------------------------------------------------------------------
    class null_mutex {
    public:
        null_mutex() {}
...
// ------------------------ lockable object used internally -------------------------
    ...
    public:
        ...
        struct DoNothing {
            ...
            DoNothing(mutex_type &, turbo::adopt_lock_t) noexcept {}

            DoNothing(mutex_type &, turbo::defer_lock_t) noexcept {}

            DoNothing(mutex_type &, turbo::try_to_lock_t) {}
    ...
    // -----------------------------------
    // use as:
    //    using Lockable = turbo::LockableImpl<mutex_type>;
    //    Lockable m;
    //
    ...
    // ---------------------------------------------------------------------------
    template<>
    class LockableImpl<turbo::null_mutex> : public turbo::null_mutex {
    public:
        using mutex_type = turbo::null_mutex;
        using Base = LockableBaseImpl<turbo::null_mutex>;
...
#endif  // TURBO_CONTAINER_INTERNAL_MAP_BASE_H_
