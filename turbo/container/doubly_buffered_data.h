...
// Called when thread initializes thread-local wrapper.
    ...
    DoublyBufferedData<T, TLS>::AddWrapper() {
        ...
        try {
            TURBO_SCOPED_LOCK(_wrappers_mutex);
...
// Called when thread quits.
    ...
    DoublyBufferedData<T, TLS>::DoublyBufferedData()
        ...
        if (rc != 0) {
            TURBO_LOG(FATAL) << "Fail to pthread_key_create: " << turbo_error(rc);
        } else {
            _created_key = true;
        }
    ...
    size_t DoublyBufferedData<T, TLS>::Modify(Fn &fn) {
        ...
        TURBO_SCOPED_LOCK(_modify_mutex);
        ...
        TURBO_CHECK_EQ(ret2, ret) << "index=" << _index.load(std::memory_order_relaxed);
        return ret2;
    }
