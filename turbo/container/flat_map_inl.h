...
#ifndef TURBO_CONTAINER_FLAT_MAP_INL_H_
#define TURBO_CONTAINER_FLAT_MAP_INL_H_
...
// Iterate SparseFlatMap
        ...
        if (_buckets == nullptr || is_too_crowded(rhs._size)) {
            ...
            if (_S) {
                ...
                if (nullptr == _thumbnail) {
                    TURBO_LOG(ERROR) << "Fail to new _thumbnail";
                    return;
                }
    ...
    int FlatMap<_K, _T, _H, _E, _S>::init(size_t nbucket, u_int load_factor) {
        ...
        if (load_factor < 10 || load_factor > 100) {
            TURBO_LOG(ERROR) << "Invalid load_factor=" << load_factor;
            return -1;
        }
        _size = 0;
    ...
    bool FlatMap<_K, _T, _H, _E, _S>::resize(size_t nbucket2) {
        ...
        if (new_map.init(nbucket2, _load_factor) != 0) {
            TURBO_LOG(ERROR) << "Fail to init new_map, nbucket=" << nbucket2;
            return false;
        }
...
#endif  // TURBO_CONTAINER_FLAT_MAP_INL_H_
