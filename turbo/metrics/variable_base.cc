...
#include "turbo/base/scoped_lock.h"                   // TURBO_SCOPE_LOCK
...
namespace turbo {
        ...
        if (white_matcher.wildcards().empty() &&
            ...
            for (std::set<std::string>::const_iterator
                ...
                if (!black_matcher.match(name)) {
                    if (turbo::variable_base::describe_exposed(
            ...
            turbo::variable_base::list_exposed(&varnames, opt.filter);
    ...
    int variable_base::dump_metrics(metrics_dumper *dumper, const metrics_dump_options *poptions) {
        ...
        turbo::variable_base::list_metrics(&cm);
...
// Start dumping_thread for only once.
    ...
    // validators (to make these gflags reloadable in turbo)
