...
namespace turbo {
    ...
    // Reloading following gflags does not change names of the corresponding variables.
    // Avoid reloading in practice.
    DEFINE_int32(variable_latency_p1, 80, "First latency percentile");
    DEFINE_int32(variable_latency_p2, 90, "Second latency percentile");
    DEFINE_int32(variable_latency_p3, 99, "Third latency percentile");
    ...
    namespace metrics_detail {
        ...
        static int64_t get_p1(void *arg) {
            ...
            return lr->latency_percentile(FLAGS_variable_latency_p1 / 100.0);
        }

        static int64_t get_p2(void *arg) {
            LatencyRecorder *lr = static_cast<LatencyRecorder *>(arg);
            return lr->latency_percentile(FLAGS_variable_latency_p2 / 100.0);
        }

        static int64_t get_p3(void *arg) {
            LatencyRecorder *lr = static_cast<LatencyRecorder *>(arg);
            return lr->latency_percentile(FLAGS_variable_latency_p3 / 100.0);
        }
        ...
        static Vector<int64_t, 4> get_latencies(void *arg) {
            ...
            result[0] = cb->get_number(FLAGS_variable_latency_p1 / 100.0);
            result[1] = cb->get_number(FLAGS_variable_latency_p2 / 100.0);
            result[2] = cb->get_number(FLAGS_variable_latency_p3 / 100.0);
    ...
    Vector<int64_t, 4> LatencyRecorder::latency_percentiles() const {
        ...
        return metrics_detail::get_latencies(
                const_cast<metrics_detail::PercentileWindow *>(&_latency_percentile_window));
    }
    ...
    int64_t LatencyRecorder::latency_percentile(double ratio) const {
        std::unique_ptr<metrics_detail::CombinedPercentileSamples> cb(
        combine((metrics_detail::PercentileWindow *) &_latency_percentile_window));
