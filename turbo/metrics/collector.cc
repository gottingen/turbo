...
// Combine two circular linked list into one.
    ...
    class Collector : public turbo::variable_reducer<Collected *, CombineCollected> {
    public:
        Collector();
    ...
    void Collector::grab_thread() {
        ...
        // vars
        turbo::status_gauge<int64_t> pending_sampled_data(
                "variable_collector_pending_samples", get_pending_count, this);
        double busy_seconds = 0;
        turbo::status_gauge<double> busy_seconds_var(deref_value<double>, &busy_seconds);
        turbo::per_second<turbo::status_gauge<double> > busy_seconds_second(
                "variable_collector_grab_thread_usage", &busy_seconds_var);

        turbo::status_gauge<int64_t> ngrab_var(deref_value<int64_t>, &_ngrab);
        turbo::per_second<turbo::status_gauge<int64_t> > ngrab_second(
...
// Call user's callbacks in this thread.
    void Collector::dump_thread() {
        ...
        turbo::status_gauge<int64_t> ndumped_var(deref_value<int64_t>, &_ndump);
        turbo::per_second<turbo::status_gauge<int64_t> > ndumped_second(
                "turbo::collector_dump_second", &ndumped_var);
