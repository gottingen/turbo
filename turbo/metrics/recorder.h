...
#ifndef  TURBO_VARIABLE_RECORDER_H_
#define  TURBO_VARIABLE_RECORDER_H_

#include <stdint.h>                              // int64_t uint64_t
#include "turbo/log/logging.h"                        // TURBO_LOG
#include "turbo/metrics/detail/combiner.h"                // detail::agent_combiner
...
namespace turbo {
    ...
    public:
        ...
        typedef metrics_detail::agent_combiner<Stat, uint64_t, AddToStat> combiner_type;
        typedef combiner_type::Agent agent_type;

        IntRecorder() : _sampler(nullptr) {}

        explicit IntRecorder(const std::string_view &name) : _sampler(nullptr) {
            expose(name, "");
        }
...
#endif  // TURBO_VARIABLE_RECORDER_H_
