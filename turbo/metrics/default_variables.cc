...
#define VARIABLE_MEMBER_TYPE(member) decltype(turbo::get_member_type(member))
...
// ======================================
    ...
    static bool read_proc_status(ProcStat &stat) {
        ...
        if (!status.is_ok()) {
            TURBO_PLOG_ONCE(WARNING) << "Fail to open /proc/self/stat";
            return false;
        }
        std::string content;
        file.read(&content);
...
// ==================================================
    ...
    static bool read_proc_memory(ProcMemory &m) {
        ...
        if (!status.is_ok()) {
            TURBO_PLOG_ONCE(WARNING) << "Fail to open /proc/self/statm";
            return false;
        }
        std::string content;
        file.read(&content);
...
// ==================================================
    ...
    static bool read_load_average(LoadAverage &m) {
        ...
        if (!status.is_ok()) {
            TURBO_PLOG_ONCE(WARNING) << "Fail to open /proc/loadavg";
            return false;
        }
        m = LoadAverage();
        errno = 0;
...
// ==================================================
    ...
    static bool read_proc_io(ProcIO *s) {
        ...
        if (!status.is_ok()) {
            TURBO_PLOG_ONCE(WARNING) << "Fail to open /proc/self/io";
            return false;
        }
        errno = 0;
        std::string content;
...
#define VARIABLE_DEFINE_PROC_IO_FIELD(field)                                \
    ...
    static bool read_disk_stat(DiskStat *s) {
        ...
        if (!status.is_ok()) {
            TURBO_PLOG_ONCE(WARNING) << "Fail to open /proc/diskstats";
            return false;
        }
        errno = 0;
        std::string content;
