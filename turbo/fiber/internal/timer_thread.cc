...
void TimerThread::run() {
    ...
    // vars
    size_t nscheduled = 0;
    turbo::status_gauge<size_t> nscheduled_var(deref_value<size_t>, &nscheduled);
    turbo::per_second<turbo::status_gauge<size_t> > nscheduled_second(&nscheduled_var);
    size_t ntriggered = 0;
    turbo::status_gauge<size_t> ntriggered_var(deref_value<size_t>, &ntriggered);
    turbo::per_second<turbo::status_gauge<size_t> > ntriggered_second(&ntriggered_var);
    double busy_seconds = 0;
    turbo::status_gauge<double> busy_seconds_var(deref_value<double>, &busy_seconds);
    turbo::per_second<turbo::status_gauge<double> > busy_seconds_second(&busy_seconds_var);
