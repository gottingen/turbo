...
#include "turbo/fiber/internal/errno.h"                       // EAGAIN
...
// Caller of this function must hold the KeyTable
    ...
    static int get_key_count(void *) {
        TURBO_SCOPED_LOCK(turbo::fiber_internal::s_key_mutex);
...
#ifndef UNIT_TEST
    static turbo::status_gauge<int> s_fiber_key_count(
            "fiber_key_count", get_key_count, nullptr);
    static turbo::status_gauge<size_t> s_fiber_keytable_count(
            "fiber_keytable_count", get_keytable_count, nullptr);
    static turbo::status_gauge<size_t> s_fiber_keytable_memory(
...
int fiber_keytable_pool_destroy(fiber_keytable_pool_t *pool) {
    ...
    turbo::fiber_internal::fiber_worker *const g = turbo::fiber_internal::tls_task_group;
    ...
    if (nullptr == kt) {
        ...
        if (g) {
            ...
            if (!turbo::fiber_internal::tls_ever_created_keytable) {
                ...
                TURBO_CHECK_EQ(0, turbo::thread::atexit(turbo::fiber_internal::cleanup_pthread, kt));
            }
        }
    }
...
void *fiber_getspecific(fiber_local_key key) {
    ...
    turbo::fiber_internal::fiber_worker *const g = turbo::fiber_internal::tls_task_group;
    if (g) {
        turbo::fiber_internal::fiber_entity *const task = g->current_task();
