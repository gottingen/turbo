...
// This queue reduces the chance to allocate memory for deque
    ...
    struct TURBO_CACHELINE_ALIGNMENT token {
        ...
        token() {
            ...
            join_butex = turbo::fiber_internal::waitable_event_create_checked<uint32_t>();
            *event = 0;
            *join_butex = 0;
        }
        ...
        ~token() {
            ...
            turbo::fiber_internal::waitable_event_destroy(join_butex);
        }
        ...
        if (*event == meta->first_ver) {
            ...
            if (range == 0) {
                ...
                TURBO_LOG_IF(FATAL, range < 0) << "range must be positive, actually "
                                         << range;
                TURBO_LOG_IF(FATAL, range > turbo::fiber_internal::TOKEN_MAX_RANGE)
                                << "max range is " << turbo::fiber_internal::TOKEN_MAX_RANGE
                                << ", actually " << range;
            } else {
            ...
            *event = meta->contended_ver();
            ...
            meta->mutex.unlock();
            ever_contended = true;
            if (turbo::fiber_internal::waitable_event_wait(event, expected_ver, nullptr) < 0 &&
...
int fiber_token_join(fiber_token_t tn) {
    ...
    while (1) {
        ...
        if (turbo::fiber_internal::waitable_event_wait(join_butex, expected_ver, nullptr) < 0 &&
            errno != EWOULDBLOCK && errno != EINTR) {
            return errno;
        }
    }
...
int fiber_token_unlock_and_destroy(fiber_token_t tn) {
    ...
    *join_butex = next_ver;
    ...
    turbo::fiber_internal::waitable_event_wake_except(event, 0);
    turbo::fiber_internal::waitable_event_wake_all(join_butex);
    return_resource(turbo::fiber_internal::get_slot(tn));
    return 0;
}
