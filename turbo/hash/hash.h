...
namespace turbo {
    ...
    template<>
    struct hash<uint64_t> : public turbo_unary_function<uint64_t, size_t> {
        inline size_t operator()(uint64_t val) const noexcept { return fold_if_needed<sizeof(size_t)>()(val); }
    };

    template<>
    struct hash<float> : public turbo_unary_function<float, size_t> {
    ...
    template<>
    struct hash<double> : public turbo_unary_function<double, size_t> {
    ...
    H hash_state_base<H>::combine(H seed, const T &v, const Ts &... vs) {
        return hash_state_base<H>::combine(hash_combiner<H, sizeof(H)>()(seed, turbo::hash<T>()(v)), vs...);
    }
    ...
    struct hash<std::pair<T1, T2>> {
        size_t operator()(std::pair<T1, T2> const &p) const noexcept {
            return turbo::hash_state().combine(turbo::hash<T1>()(p.first), p.second);
        }
    };
    ...
    private:
        ...
        hash_impl_helper(const std::tuple<P...> &t) const noexcept {
            ...
            return hash_combiner<size_t, sizeof(size_t)>()(
                    turbo::hash<el_type>()(el), hash_impl_helper<I + 1>(t));
        }
    };
