...
#define uint32_in_expected_order(x) (turbo::base::bit_swap32(x))
#define uint64_in_expected_order(x) (turbo::base::bit_swap64(x))
#else
    ...
    std::swap(a, c);      \
        ...
        uint32_t city_hash32(const char *s, size_t len) {
            ...
            do {
                ...
                h = h * 5 + 0xe6546b64;
                g ^= b4;
                g = turbo::base::bit_swap32(g) * 5;
                h += b4 * 5;
                h = turbo::base::bit_swap32(h);
                f += b0;
                PERMUTE3(f, h, g);
                s += 20;
            } while (--iters != 0);
...
// Return a 16-byte hash for s[0] ... s[31], a, and b.  Quick and dirty.
        ...
        static uint64_t HashLen33to64(const char *s, size_t len) {
            ...
            uint64_t w = turbo::base::bit_swap64((u + v) * mul) + h;
            uint64_t x = Rotate(e + f, 42) + c;
            uint64_t y = (turbo::base::bit_swap64((v + w) * mul) + g) * mul;
            uint64_t z = e + f + c;
            a = turbo::base::bit_swap64((x + z) * mul + y) + b;
