...
// compatible with turbo::Time. Suitable for in-register
// parameter-passing (e.g. syscalls.)
// Constructible from a turbo::Time (for a timeout to be respected) or {}
// (for "no timeout".)
...
class KernelTimeout {
 public:
  ...
  // respected.
  explicit KernelTimeout(turbo::Time t) : ns_(MakeNs(t)) {}
  // No timeout.
  KernelTimeout() : ns_(0) {}
  ...
  // Unify on this and turbo::Time, please.
 ...
 private:
  ...
  static int64_t MakeNs(turbo::Time t) {
    // optimization--InfiniteFuture is common "no timeout" value
    // and cheaper to compare than convert.
    if (t == turbo::InfiniteFuture()) return 0;
  ...
  DWord InMillisecondsFromNow() const {
    ...
    // The use of turbo::Now() to convert from absolute time to
    // relative time means that turbo::Now() cannot use anything that
    // depends on KernelTimeout (for example, Mutex) on Windows.
    int64_t now = ToUnixNanos(turbo::Now());
    if (ns_ >= now) {
