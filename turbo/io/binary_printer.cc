...
namespace turbo {
    ...
    template<typename Appender>
    static void print_cord_buf(Appender *appender, const cord_buf &b, size_t max_length) {
    ...
    void to_printable::print(std::ostream &os) const {
        OStreamAppender appender(os);
        if (_iobuf) {
            print_cord_buf(&appender, *_iobuf, _max_length);
        } else if (!_str.empty()) {
            PrintString(&appender, _str, _max_length);
        }
    }

    std::string to_printable_string(const cord_buf &data, size_t max_length) {
        std::string result;
        StringAppender appender(&result);
        print_cord_buf(&appender, data, max_length);
        return result;
    }
