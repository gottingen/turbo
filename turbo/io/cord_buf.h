...
#ifndef TURBO_IO_IOBUF_H_
#define TURBO_IO_IOBUF_H_
...
#include "turbo/io/snappy/snappy-sinksource.h"
#include "turbo/io/zero_copy_stream_as_streambuf.h"
#include "turbo/io/reader_writer.h"
#include "turbo/io/binary_printer.h"
...
// simultaneously is unsafe and likely to crash.
    ...
    public:
        ...
        // Explicitly declare this overload as error to avoid copy_to(turbo::cord_buf*)
    ...
    inline bool operator==(const turbo::cord_buf &b, const std::string_view &s) { return b.equals(s); }

    inline bool operator==(const std::string_view &s, const turbo::cord_buf &b) { return b.equals(s); }

    inline bool operator!=(const turbo::cord_buf &b, const std::string_view &s) { return !b.equals(s); }

    inline bool operator!=(const std::string_view &s, const turbo::cord_buf &b) { return !b.equals(s); }

    inline bool operator==(const turbo::cord_buf &b1, const turbo::cord_buf &b2) { return b1.equals(b2); }
    ...
    public:
        explicit cord_buf_cutter(turbo::cord_buf *buf);

        ~cord_buf_cutter();

        // Cut off n bytes and APPEND to `out'
        // Returns bytes cut.
        size_t cutn(turbo::cord_buf *out, size_t n);
    ...
    class cord_buf_as_snappy_source : public turbo::snappy::Source {
    public:
        explicit cord_buf_as_snappy_source(const turbo::cord_buf &buf)
    ...
    private:
        const turbo::cord_buf *_buf;
        turbo::cord_buf_as_zero_copy_input_stream _stream;
    };

// Wrap cord_buf into output of snappy compression.
    class cord_buf_as_snappy_sink : public turbo::snappy::Sink {
    public:
        explicit cord_buf_as_snappy_sink(turbo::cord_buf &buf);
    ...
    private:
        ...
        int _cur_len;
        turbo::cord_buf *_buf;
        turbo::cord_buf_as_zero_copy_output_stream _buf_stream;
    };
...
// During iteration, the iobuf should NOT be changed.
    ...
    public:
        explicit cord_buf_bytes_iterator(const turbo::cord_buf &buf);
        ...
        size_t append_and_forward(turbo::cord_buf *buf, size_t n);
...
namespace std {
    template<>
    inline void swap(turbo::cord_buf &a, turbo::cord_buf &b) {
        return a.swap(b);
    }
} // namespace std

#include "turbo/io/cord_buf_inl.h"

#endif  // TURBO_IO_IOBUF_H_
