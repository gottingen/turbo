...
TEST(MarshallingTest, TestInt16Parsing) {
  ...
  EXPECT_FALSE(turbo::ParseFlag("-0x7FFD", &value, &err));
  EXPECT_NE(value, -3);
  EXPECT_FALSE(turbo::ParseFlag("+0x31", &value, &err));
  EXPECT_NE(value, 49);
...
TEST(MarshallingTest, TestInt32Parsing) {
  ...
  EXPECT_EQ(value, 564);
  // TODO(rogeeff): fix below validations
  EXPECT_FALSE(turbo::ParseFlag("-0x7FFFFFFD", &value, &err));
  EXPECT_NE(value, -3);
  EXPECT_FALSE(turbo::ParseFlag("+0x31", &value, &err));
  EXPECT_NE(value, 49);
...
TEST(MarshallingTest, TestInt64Parsing) {
  ...
  EXPECT_FALSE(turbo::ParseFlag("-0x7FFFFFFFFFFFFFFE", &value, &err));
  EXPECT_NE(value, -2);
  EXPECT_FALSE(turbo::ParseFlag("+0x31", &value, &err));
  EXPECT_NE(value, 49);
