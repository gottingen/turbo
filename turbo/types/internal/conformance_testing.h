...
#include "turbo/types/internal/conformance_aliases.h"
#include "turbo/types/internal/conformance_archetype.h"
#include "turbo/types/internal/conformance_profile.h"
#include "turbo/types/internal/conformance_testing_helpers.h"
#include "turbo/types/internal/parentheses.h"
#include "turbo/types/internal/transform_args.h"
...
// object type.
template <class Fun,
          turbo::enable_if_t<IsNullaryCallable<Fun>::value>** = nullptr>
GeneratorType<Fun> Generator(Fun fun, const char* description) {
  return GeneratorType<Fun>{turbo::move(fun), description};
}
...
template <class... Funs, turbo::enable_if_t<AreGeneratorsWithTheSameReturnType<
                             Funs...>::value>** = nullptr>
EquivalenceClassType<Funs...> EquivalenceClass(GeneratorType<Funs>... funs) {
  return {std::make_tuple(turbo::move(funs)...)};
}
...
struct GivenDeclaration {
  std::string outputDeclaration(std::size_t width) const {
    ...
    std::string result = turbo::StrCat("  ", name);

    if (!expression.empty()) {
      // Indent
      result.resize(indent_size + width, ' ');
      turbo::StrAppend(&result, " = ", expression, ";\n");
    } else {
...
#define ABSL_INTERNAL_EXPECT_OP(name, op)                                   \
  struct Expect##name {                                                     \
    ...
    void operator()(turbo::string_view test_name, turbo::string_view context, \
                    const T& lhs, const T& rhs, turbo::string_view lhs_name, \
                    turbo::string_view rhs_name) const {                     \
...
void ExpectOneWayEquality(ConformanceErrors* errors,
                          turbo::string_view test_name,
                          turbo::string_view context, const T& lhs, const T& rhs,
                          turbo::string_view lhs_name,
                          turbo::string_view rhs_name) {
...
template <class T, class Prof>
void ExpectEquality(ConformanceErrors* errors, turbo::string_view test_name,
                    turbo::string_view context, const T& lhs, const T& rhs,
                    turbo::string_view lhs_name, turbo::string_view rhs_name) {
...
struct ExpectMoveConstructOneGenerator {
  ...
  void operator()(const Fun& generator) const {
    ...
    const T moved_object = turbo::move(generator());  // Force no elision.
...
struct ExpectSelfSwap {
  ...
  void operator()(const Fun& generator) const {
    ...
    std::string preliminary_info = turbo::StrCat(
        PrepareGivenContext(
...
template <class T, class Prof>
void ExpectOrdered(ConformanceErrors* errors, turbo::string_view context,
                   const T& small, const T& big, turbo::string_view small_name,
                   turbo::string_view big_name) {
  const turbo::string_view test_name = "Comparison";
...
struct ExpectOrderedEquivalenceClassesComparisons {
  ...
  void operator()(SmallEqClass small_eq_class,
    ...
    (ForEachTupleElement)(
        Impl<BigEqClasses...>{std::make_tuple(turbo::move(big_eq_classes)...),
                              errors},
        small_eq_class.generators);

    // Recurse so that all equivalence class combinations are checked.
    (*this)(turbo::move(big_eq_classes)...);
  }
...
ConformanceErrors ExpectRegularityImpl(
  ...
  using minimal_profile = typename turbo::conditional_t<
  ...
  turbo::apply(ExpectOrderedEquivalenceClasses<T, minimal_profile>{&errors},
              vals.eq_classes);
...
template <class T>
struct ProfileRangeOfImpl<T, turbo::void_t<PropertiesOfT<T>>> {
...
struct ExpectConformanceOf {
  ...
  initializer(GeneratorType<Fun> fun) && {
    return {
        {std::tuple_cat(turbo::move(ordered_vals.eq_classes),
                        std::make_tuple((EquivalenceClass)(turbo::move(fun))))},
  ...
  equivalence_class(GeneratorType<Funs>... funs) && {
    return {{std::tuple_cat(
                turbo::move(ordered_vals.eq_classes),
                std::make_tuple((EquivalenceClass)(turbo::move(funs)...)))},
            std::move(expected_failed_tests)};
  }
  ...
  ABSL_MUST_USE_RESULT ::testing::AssertionResult with_strict_profile(
    ...
    ConformanceErrors test_result =
        (ExpectRegularityImpl<
            ...
            MaxProfileOfT<ProfRange>>)(turbo::move(ordered_vals));
    ...
    ConformanceErrors test_result =
        (ExpectRegularityImpl<
            ...
            CombineProfiles<TriviallyCompleteProfile,
                            NothrowComparableProfile>>)(turbo::
                                                            move(ordered_vals));
