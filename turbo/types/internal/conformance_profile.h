...
constexpr turbo::underlying_type_t<Enum> UnderlyingValue(Enum value) {
  return static_cast<turbo::underlying_type_t<Enum>>(value);
}
...
class ConformanceErrors {
 public:
  ...
  ::testing::AssertionResult expectFailedTests(
    ...
    turbo::c_set_difference(test_names, test_failures_,
    ...
    turbo::c_set_difference(nonfailing_tests, test_successes_,
...
template <class T>
struct PropertiesOfImpl<T, turbo::void_t<typename T::properties>> {
  using type = typename T::properties;
};

template <class T>
struct PropertiesOfImpl<T, turbo::void_t<typename T::profile_alias_of>> {
  ...
  constexpr property property##_support_of() {                              \
    return std::is_##property<T>::value                                     \
               ? std::is_nothrow_##property<T>::value                       \
                     ? turbo::is_trivially_##property<T>::value              \
                           ? property::trivial                              \
                           : property::nothrow                              \
                     : property::yes                                        \
               : property::maybe;                                           \
...
struct IsOpableImpl<T, Op, turbo::void_t<Op<T>>> : std::true_type {};
...
template <class T, template <class...> class Op>
struct IsNothrowOpableImpl<T, Op, turbo::enable_if_t<Op<T>::value>>
    : std::true_type {};
...
template <class T>
struct IsProfileImpl<T, turbo::void_t<PropertiesOfT<T>>> : std::true_type {};
