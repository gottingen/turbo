...
#define LINUX_SYSINFO_TICK2MSEC(s) \
    ...
    static turbo::file_path get_proc_root() {
        if (mock_root) {
            return turbo::file_path(mock_root) / "mock" / "linux" / "proc";
        }
        return turbo::file_path{"/proc"};

    }

    static void tokenize_file_line_by_line(
            const turbo::file_path &name,
        ...
        for (auto line : lines) {
            ...
            if (allowEmpty) {
                toks = turbo::string_split(line, by_char(delim), turbo::allow_empty());
            } else {
                toks = turbo::string_split(line, by_char(delim), turbo::skip_empty());
            }
    ...
    public:
        ...
        result_status get_proc_mem(turbo_pid_t pid, proc_mem_info &procmem) override;

        result_status get_proc_state(turbo_pid_t pid, proc_state_info &procstate) override;

    protected:

        int get_proc_time(turbo_pid_t pid, proc_time_info &proctime) override;

        static result_status proc_stat_read(turbo_pid_t pid, linux_proc_stat_info &stat);

        static result_status parse_stat_file(const turbo::file_path &name, linux_proc_stat_info &ret);
    };
        ...
        sysinfo_tokenize_file_line_by_line(
                ...
                [&mem, &buffers, &cached](const auto &vec) {
                    ...
                    if (vec.front() == "MemTotal") {
                        return turbo::simple_atoi(vec[1], &mem.total);
                    }
                    if (vec.front() == "MemFree") {
                        return turbo::simple_atoi(vec[1], &mem.free);
                    }
                    if (vec.front() == "Buffers") {
                        return turbo::simple_atoi(vec[1], &buffers);
                    }
                    if (vec.front() == "Cached") {
                        return turbo::simple_atoi(vec[1], &cached);
                    }
    ...
    result_status linux_info::get_swap(swap_info &swap) {
        sysinfo_tokenize_file_line_by_line(
                ...
                [&swap](const auto &vec) {
                    ...
                    if (vec.front() == "SwapTotal") {
                        return turbo::simple_atoi(vec[1], &swap.total);
                    }
                    if (vec.front() == "SwapFree") {
                        return turbo::simple_atoi(vec[1], &swap.free);
                    }
                    return true;
                },
                ':');
        ...
        sysinfo_tokenize_file_line_by_line(
                ...
                [&swap](const auto &vec) {
                    ...
                    if (vec.front() == "pswpin") {
                        turbo::simple_atoi(vec[1], &swap.page_in);
                    } else if (vec.front() == "pswpout") {
                        turbo::simple_atoi(vec[1], &swap.page_out);
                    } else if (vec.front() == "allocstall") {
                        turbo::simple_atoi(vec[1], &swap.allocstall);
                    } else if (vec.front() == "allocstall_dma") {
                        turbo::simple_atoi(vec[1], &swap.allocstall_dma);
                    } else if (vec.front() == "allocstall_dma32") {
                        turbo::simple_atoi(vec[1], &swap.allocstall_dma32);
                    } else if (vec.front() == "allocstall_normal") {
                        turbo::simple_atoi(vec[1], &swap.allocstall_normal);
                    } else if (vec.front() == "allocstall_movable") {
                        turbo::simple_atoi(vec[1], &swap.allocstall_movable);
                    }
    ...
    result_status linux_info::get_proc_mem(turbo_pid_t pid, proc_mem_info &procmem) {
    ...
    result_status linux_info::get_proc_state(turbo_pid_t pid, proc_state_info &procstate) {
        linux_proc_stat_info pstat;
    ...
    result_status linux_info::parse_stat_file(const turbo::file_path &name, linux_proc_stat_info &ret) {
        turbo::readline_file file;
        file.open(name);
        auto lines = file.lines();
        if (lines.size() != 1) {
        ...
        auto line = lines[0];
        std::vector<std::string_view> fields = turbo::string_split(line, ' ');
        ...
        if(!turbo::simple_atoi(fields[stat_pid_index], &ret.pid)) {
            return result_status::from_turbo_error(EINVAL);
        }

        ret.name = std::string{fields[stat_name_index].data(),
                               fields[stat_name_index].size()};
        ret.state = fields[stat_state_index].front();
        if(!turbo::simple_atoi(fields[stat_ppid_index], &ret.ppid)) {
            return result_status::from_turbo_error(EINVAL);
        }

        if(!turbo::simple_atoi(fields[stat_tty_index], &ret.tty)) {
            return result_status::from_turbo_error(EINVAL);
        }
        if(!turbo::simple_atoi(fields[stat_minor_faults_index], &ret.minor_faults)) {
            return result_status::from_turbo_error(EINVAL);
        }
        if(!turbo::simple_atoi(fields[stat_major_faults_index], &ret.major_faults)) {
            return result_status::from_turbo_error(EINVAL);
        }
        int64_t utime;
        if(!turbo::simple_atoi(fields[stat_utime_index], &utime)) {
            return result_status::from_turbo_error(EINVAL);
        }
        ret.utime = LINUX_SYSINFO_TICK2MSEC(utime);
        int64_t stime;
        if(!turbo::simple_atoi(fields[stat_stime_index], &stime)) {
            return result_status::from_turbo_error(EINVAL);
        }
        ret.stime = LINUX_SYSINFO_TICK2MSEC(stime);
        if(!turbo::simple_atoi(fields[stat_priority_index], &ret.priority)) {
            return result_status::from_turbo_error(EINVAL);
        }
        if(!turbo::simple_atoi(fields[stat_nice_index], &ret.nice)) {
            return result_status::from_turbo_error(EINVAL);
        }
        if(!turbo::simple_atoi(fields[stat_start_time_index], &ret.start_time)) {
            return result_status::from_turbo_error(EINVAL);
        }
        ...
        if(!turbo::simple_atoi(fields[stat_rss_index], &ret.rss)) {
            return result_status::from_turbo_error(EINVAL);
        }
        if(!turbo::simple_atoi(fields[stat_processor_index], &ret.processor)) {
            return result_status::from_turbo_error(EINVAL);
        }
        return result_status::success();
    }

    result_status linux_info::proc_stat_read(turbo_pid_t pid, linux_proc_stat_info &stat) {
        const auto nm = get_proc_root() / std::to_string(pid) / "stat";
        return parse_stat_file(nm, stat);
    }

    int linux_info::get_proc_time(turbo_pid_t pid, proc_time_info &proctime) {
        linux_proc_stat_info pstat;
