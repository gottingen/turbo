...
namespace turbo {
    ...
    int read_command_output_through_clone(std::ostream& os, const char* cmd) {
        ...
        if (!child_stack_mem) {
            TURBO_LOG(ERROR) << "Fail to alloc stack for the child process";
            rc = -1;
            goto END;
        }
        ...
        if (cpid < 0) {
            TURBO_PLOG(ERROR) << "Fail to clone child process";
            rc = -1;
            goto END;
        }
        close(pipe_fd[1]);
        ...
        for (;;) {
            ...
            if (nr > 0) {
                ...
                TURBO_LOG(ERROR) << "Encountered error while reading for the pipe";
                break;
            }
        }
...
#if defined(TURBO_PLATFORM_LINUX)
        turbo::base::fd_guard fd(open("/proc/self/cmdline", O_RDONLY));
    if (fd < 0) {
        TURBO_LOG(ERROR) << "Fail to open /proc/self/cmdline";
        return -1;
    }
    ssize_t nr = read(fd, buf, len);
    if (nr <= 0) {
        TURBO_LOG(ERROR) << "Fail to read /proc/self/cmdline";
        return -1;
    }
        ...
        if (turbo::read_command_output(oss, cmdbuf) != 0) {
            TURBO_LOG(ERROR) << "Fail to read cmdline";
            return -1;
        }
...
#error Not Implemented
        ...
        if (with_args) {
            ...
            if ((size_t)nr == len) {
                TURBO_LOG(ERROR) << "buf is not big enough";
                return -1;
            }
            return nr;
        }
