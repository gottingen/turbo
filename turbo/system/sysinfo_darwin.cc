...
#define SYSINFO_PROC_STATE_IDLE 'D'
    ...
    int darwin_proc_cpu_type(turbo_pid_t pid, cpu_type_t *type);
    ...
    protected:
        ...
        static std::pair<int, kinfo_proc> get_pinfo(turbo_pid_t pid);

        static int get_proc_threads(turbo_pid_t pid, proc_state_info &procstate);
    ...
    result_status darwin_info::get_mem_info(mem_info &mem) {
        ...
        if (sysctl(mib, NMIB(mib), &pagesize, &len, nullptr, 0) < 0) {
            return result_status::from_turbo_error(errno);
        }
    ...
    result_status darwin_info::get_proc_mem(turbo_pid_t pid, proc_mem_info &procmem) {
    ...
    result_status darwin_info::get_proc_state(turbo_pid_t pid, proc_state_info &procstate) {
        const auto[status, pinfo] = get_pinfo(pid);
        if (status != 0) {
            return turbo::result_status::from_turbo_error(status);
        }
    ...
    std::pair<int, kinfo_proc> darwin_info::get_pinfo(turbo_pid_t pid) {
    ...
    int darwin_info::get_proc_threads(turbo_pid_t pid, proc_state_info &procstate) {
        mach_port_t task, self = mach_task_self();
        kern_return_t status;
    ...
    static int get_proc_times(turbo_pid_t pid, proc_time_info *time) {
        unsigned int count;
    ...
    int darwin_info::get_proc_time(turbo_pid_t pid, proc_time_info &proctime) {
        const auto[status, pinfo] = get_pinfo(pid);
        if (status != 0) {
            return status;
        }
        ...
        proctime.start_time = turbo::time_point::from_timeval(pinfo.kp_proc.p_starttime).to_unix_millis();
        return 0;
    }
