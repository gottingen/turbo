...
namespace turbo {
    ...
    Variable::~Variable() {
        TLOG_CHECK((!hide(), "Subclass of Variable MUST call hide() manually in their"
    ...
    int Variable::expose_impl(const std::string_view &prefix,
                              ...
                              DisplayFilter display_filter) {
        if (name.empty()) {
            TLOG_ERROR("Parameter[name] is empty");
            return -1;
        }
        ...
        TLOG_CHECK(!VariableConfig::get_instance().abort_on_same_name, "Abort due to name conflict");
        if (!s_bvar_may_abort) {
            s_bvar_may_abort = true;
        }

        TLOG_ERROR("Already exposed `{}' whose value is `{}'", _name, describe_exposed(_name));
        _name.clear();
        return -1;
    }
    ...
    bool Variable::hide() {
        ...
        auto it = m.find(_name);
        if (it != m.end()) {
            TLOG_CHECK_EQ(1UL, m.erase(_name));
        } else {
            TLOG_CHECK(false, "`{}' must exist", _name);
        }
        _name.clear();
        return true;
    }
...
// <A href="mailto:jakkhandy@hotmail.com">jakkhandy@hotmail.com</A>
    ...
    public:
        WildcardMatcher(const std::string &wildcards,
            ...
            std::vector<std::string_view> wcs = turbo::str_split(wildcards, ",;");
            for (auto &sp : wcs) {
                name.assign(sp.data(), sp.length());
    ...
    int Variable::dump_exposed(Dumper *dumper, const DumpOptions *poptions) {
        if (nullptr == dumper) {
            TLOG_ERROR("Parameter[dumper] is nullptr");
            return -1;
        }
        DumpOptions opt;
        if (poptions) {
        ...
        if (white_matcher.wildcards().empty() &&
            ...
            for (std::set<std::string>::const_iterator
                ...
                if (!black_matcher.match(name)) {
                    ...
                    if (VariableConfig::get_instance().var_log_dumped) {
                        dumpped_info << '\n' << name << ": " << streambuf.data();
                    }
        ...
        if (log_dummped) {
            TLOG_INFO("Dumpped variables:%s", dumpped_info.str().c_str());
        }
        return count;
    }
...
// The background thread to export all bvar periodically.
    ...
    static void launch_dumping_thread() {
        ...
        TLOG_CHECK_EQ(0, pthread_detach(thread_id));
...
// Start dumping_thread for only once.
    ...
    const bool ALLOW_UNUSED
    dummy_bvar_dump = ::GFLAGS_NS::RegisterFlagValidator(
...
// validators (to make these gflags reloadable in brpc)
    ...
    const bool ALLOW_UNUSED
    dummy_bvar_dump_file = ::GFLAGS_NS::RegisterFlagValidator(
            &FLAGS_bvar_dump_file, wakeup_dumping_thread);
    const bool ALLOW_UNUSED
    dummy_bvar_dump_filter = ::GFLAGS_NS::RegisterFlagValidator(
            &FLAGS_bvar_dump_include, wakeup_dumping_thread);
    const bool ALLOW_UNUSED
    dummy_bvar_dump_exclude = ::GFLAGS_NS::RegisterFlagValidator(
            &FLAGS_bvar_dump_exclude, wakeup_dumping_thread);
    const bool ALLOW_UNUSED
    dummy_bvar_dump_prefix = ::GFLAGS_NS::RegisterFlagValidator(
            &FLAGS_bvar_dump_prefix, wakeup_dumping_thread);
    const bool ALLOW_UNUSED
    dummy_bvar_dump_tabs = ::GFLAGS_NS::RegisterFlagValidator(
            &FLAGS_bvar_dump_tabs, wakeup_dumping_thread);

    const bool ALLOW_UNUSED
    dummy_mbvar_dump = ::GFLAGS_NS::RegisterFlagValidator(
            &FLAGS_mbvar_dump, validate_bvar_dump);
    const bool ALLOW_UNUSED
    dummy_mbvar_dump_prefix = ::GFLAGS_NS::RegisterFlagValidator(
            &FLAGS_mbvar_dump_prefix, wakeup_dumping_thread);
    const bool ALLOW_UNUSED
    dump_mbvar_dump_file = ::GFLAGS_NS::RegisterFlagValidator(
    ...
    const bool ALLOW_UNUSED
    dummy_mbvar_dump_format = ::GFLAGS_NS::RegisterFlagValidator(
