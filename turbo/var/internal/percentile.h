...
#ifndef  TURBO_VAR_DETAIL_PERCENTILE_H_
#define  TURBO_VAR_DETAIL_PERCENTILE_H_
...
# define DEFINE_SMALL_ARRAY(Tp, name, size, maxsize)                    \
    ...
    ::turbo::var_internal::ArrayDeleter<Tp> name##_array_deleter;            \
...
# define DEFINE_SMALL_ARRAY(Tp, name, size, maxsize)                    \
    ...
    ::turbo::var_internal:ArrayDeleter<char> name##_array_deleter;          \
    if (name##_stack_array_size) {                                      \
    ...
    const ::turbo::var_internal::ArrayCtorDtor<Tp> name##_array_ctor_dtor(name, name##_size);
...
namespace turbo::var_internal {
    ...
    inline unsigned long round_of_expectation(unsigned long a, unsigned long b) {
        ...
        return a / b + (turbo::fast_uniform(0u, b) < a % b);
    }
    ...
    public:
        ...
        uint32_t get_sample_at(size_t index) {
            ...
            if(TURBO_UNLIKELY(saved_num != _num_samples)) {
                TURBO_RAW_LOG(FATAL, "You must call get_number() on"
        ...
        void merge(const PercentileInterval<size2> &rhs) {
            ...
            TURBO_ASSERT(rhs._num_samples == rhs._num_added);
            if (_num_added + rhs._num_added <= SAMPLE_SIZE) {
                if(_num_samples != _num_added) {
                    TURBO_RAW_LOG(FATAL, "_num_samples=%u, rhs._num_samples=%u, _num_added=%u, rhs._num_added=%u, SAMPLE_SIZE=%lu, size2=%lu",
                                  _num_samples, rhs._num_samples, _num_added, rhs._num_added, SAMPLE_SIZE, size2);
                }
                ...
                TURBO_ASSERT(num_remain <= SAMPLE_SIZE);
                // Randomly drop samples of this
                for (size_t i = _num_samples; i > num_remain; --i) {
                    _samples[turbo::fast_uniform(0u,i-1)] = _samples[i - 1];
                }
                const size_t num_remain_from_rhs = SAMPLE_SIZE - num_remain;
                TURBO_ASSERT(num_remain_from_rhs <= rhs._num_samples);
                ...
                for (size_t i = 0; i < num_remain_from_rhs; ++i) {
                    const int index = turbo::fast_uniform(0u, rhs._num_samples - i  -1);
                    _samples[num_remain++] = tmp[index];
                    tmp[index] = tmp[rhs._num_samples - i - 1];
                }
                _num_samples = num_remain;
                TURBO_ASSERT(_num_samples== SAMPLE_SIZE);
            }
        ...
        void merge_with_expectation(PercentileInterval<size2> &mutable_rhs, size_t n) {
            TURBO_ASSERT(n <= mutable_rhs._num_samples);
            ...
            for (size_t i = 0; i < n; ++i) {
                size_t index = turbo::fast_uniform<size_t>(0u, mutable_rhs._num_samples - i-1);
                if (_num_samples < SAMPLE_SIZE) {
                    _samples[_num_samples++] = mutable_rhs._samples[index];
                } else {
                    _samples[ turbo::fast_uniform(0u, _num_samples)]
        ...
        bool add32(uint32_t x) {
            if (TURBO_UNLIKELY(_num_samples >= SAMPLE_SIZE)) {
                TURBO_RAW_LOG(ERROR, "This interval was full");
                return false;
            }
            ++_num_added;
...
#undef  DEFINE_SMALL_ARRAY

#endif  // TURBO_VAR_INTERNAL_PERCENTILE_H_
