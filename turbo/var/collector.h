...
#ifndef TURBO_VAR_COLLECTOR_H_
#define TURBO_VAR_COLLECTOR_H_
...
namespace turbo {
    ...
    class Collected : public turbo::intrusive_list_node {
    public:
        virtual ~Collected() {}

        // Sumbit the sample for later dumping, a sample can only be submitted once.
        // submit() is implemented as writing a value to turbo::Reducer which does
    ...
    inline size_t is_collectable(CollectorSpeedLimit *speed_limit) {
        if (speed_limit->ever_grabbed) { // most common case
            ...
            if ((turbo::fast_uniform<uint32_t>() & (COLLECTOR_SAMPLING_BASE - 1)) >= sampling_range) {
                return 0;
            }
    ...
    private:
        turbo::PassiveStatus<double> _var;
    };

}  // namespace turbo

#endif  // TURBO_VAR_COLLECTOR_H_
