...
#ifndef  TURBO_VAR_RECORDER_H_
#define  TURBO_VAR_RECORDER_H_

#include <stdint.h>                              // int64_t uint64_t
#include "turbo/base/internal/raw_logging.h"
#include "turbo/var/internal/combiner.h"                // detail::AgentCombiner
...
namespace turbo {
    ...
    public:
        ...
        typedef var_internal::ReducerSampler<IntRecorder, Stat,
        ...
        typedef var_internal::AgentCombiner<Stat, uint64_t, AddToStat> combiner_type;
        typedef combiner_type::Agent agent_type;

        IntRecorder() : _sampler(nullptr) {}

        explicit IntRecorder(const std::string &name) : _sampler(nullptr) {
            expose(name);
        }

        IntRecorder(const std::string &prefix, const std::string &name)
                : _sampler(nullptr) {
        ...
        Stat get_value() const {
            auto s = _combiner.combine_agents();
            return s;
    ...
    inline IntRecorder &IntRecorder::operator<<(int64_t sample) {
        if (TURBO_UNLIKELY((int64_t) (int) sample != sample)) {
            ...
            if (!name().empty()) {
                TURBO_RAW_LOG(WARNING, "Input=%ld to `%s\' %s", sample, name().c_str(), reason);
            } else if (!_debug_name.empty()) {
                TURBO_RAW_LOG(WARNING, "Input=%ld to `%s\' %s", sample, _debug_name.c_str(), reason);
            } else {
                TURBO_RAW_LOG(WARNING, "Input=%ld to IntRecorder(%p) %s", sample, (void *) this, reason);
            }
        }
