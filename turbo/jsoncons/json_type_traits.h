...
namespace turbo {
    ...
    struct is_json_type_traits_specialized<Json, T,
            typename std::enable_if<!turbo::detail::is_json_type_traits_unspecialized<Json, T>::value
            >::type> : std::true_type {
    };
    ...
    struct json_type_traits<Json, T,
            typename std::enable_if<(traits_extension::is_signed_integer<T>::value && sizeof(T) <= sizeof(int64_t)) ||
                                    (traits_extension::is_unsigned_integer<T>::value && sizeof(T) <= sizeof(uint64_t))
            >::type> {
        using allocator_type = typename Json::allocator_type;
    ...
    template<class Json>
    struct json_type_traits<Json, turbo::null_type> {
        using allocator_type = typename Json::allocator_type;
        ...
        static typename turbo::null_type as(const Json &j) {
            if (!j.is_null()) {
                JSONCONS_THROW(conv_error(conv_errc::not_jsoncons_null_type));
            }
            return turbo::null_type();
        }

        static Json to_json(turbo::null_type) {
            return Json(turbo::null_type{}, semantic_tag::none);
        }

        static Json to_json(turbo::null_type, const allocator_type &) {
            return Json(turbo::null_type{}, semantic_tag::none);
        }
    };
    ...
    struct json_type_traits<Json, T,
            typename std::enable_if<!is_json_type_traits_declared<T>::value &&
                                    turbo::detail::is_compatible_array_type<Json, T>::value &&
        ...
        as(const Json &j) {
            if (j.is_array()) {
                T result;
                visit_reserve_(typename std::integral_constant<bool, traits_extension::has_reserve<T>::value>::type(),
                               result, j.size());
    ...
    struct json_type_traits<Json, std::shared_ptr<ValueType>,
        ...
        static std::shared_ptr<ValueType> as(const Json &j) {
            return j.is_null() ? std::shared_ptr<ValueType>(nullptr) : std::make_shared<ValueType>(
                    j.template as<ValueType>());
        }
    ...
    struct json_type_traits<Json, std::unique_ptr<ValueType>,
        ...
        static std::unique_ptr<ValueType> as(const Json &j) {
            return j.is_null() ? std::unique_ptr<ValueType>(nullptr) : std::make_unique<ValueType>(
                    j.template as<ValueType>());
        }
    ...
    template<class Json, class T>
    struct json_type_traits<Json, std::optional<T>,
            typename std::enable_if<!is_json_type_traits_declared<std::optional<T>>::value>::type> {
    public:
        static bool is(const Json &j) noexcept {
            return j.is_null() || j.template is<T>();
        }

        static std::optional<T> as(const Json &j) {
            return j.is_null() ? std::optional<T>() : std::optional<T>(j.template as<T>());
        }

        static Json to_json(const std::optional<T> &val) {
    ...
    public:
        ...
        static bool is(const Json &j) noexcept {
            switch (j.type()) {
                case json_type::string_value:
                    return turbo::detail::is_base10(j.as_string_view().data(), j.as_string_view().length());
        ...
        static basic_bigint<Allocator> as(const Json &j) {
            switch (j.type()) {
                case json_type::string_value:
                    if (!turbo::detail::is_base10(j.as_string_view().data(), j.as_string_view().length())) {
            ...
            if (j.is_int64() || j.is_uint64()) {
                ...
                switch (j.tag()) {
                    ...
                    case semantic_tag::epoch_milli: {
                        auto sv = j.as_string_view();
                        Rep n{0};
                        auto result = turbo::detail::to_integer_decimal(sv.data(), sv.size(), n);
                        if (!result) {
                            return duration_type();
                        }
    ...
    struct json_type_traits<Json, std::bitset<N>> {
        ...
        static bool is(const Json &j) noexcept {
            if (j.is_byte_string()) {
                ...
                turbo::string_view sv = j.as_string_view();
