...
namespace turbo {
    ...
    public:
        ...
        constexpr key_value(std::piecewise_construct_t /*unused*/, std::tuple<U1...> a,
                                     std::tuple<U2...>
        ...
        noexcept(KeyT(std::forward<U1>(std::get<I1>(
                ...
                std::declval<std::tuple<U2...> &>()))...)))
                : key_(std::forward<U1>(std::get<I1>(a))...), value_(std::forward<U2>(std::get<I2>(b))...) {
    ...
    public:
        ...
        sorted_json_object(InputIt first, InputIt last) {
            ...
            auto it = std::unique(members_.begin(), members_.end(),
                                  [](const key_value_type &a,
                                     const key_value_type &b) -> bool { return !(a.key().compare(b.key())); });
        ...
        order_preserving_json_object(InputIt first, InputIt last) {
            ...
            auto last_unique = std::unique(index_.begin(), index_.end(),
                                           [&](std::size_t a, std::size_t b) {
                                               return !(members_.at(a).key().compare(members_.at(b).key()));
                                           });
    ...
    private:
        ...
        void erase_index_entries(std::size_t pos1, std::size_t pos2) {
            TURBO_ASSERT(pos1 <= pos2);
            TURBO_ASSERT(pos2 <= index_.size());
        ...
        void build_index() {
            ...
            std::stable_sort(index_.begin(), index_.end(),
                             [&](std::size_t a, std::size_t b) -> bool {
                                 return members_.at(a).key().compare(members_.at(b).key()) < 0;
                             });
        }
    };
