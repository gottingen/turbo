...
#include "turbo/jsoncons/jsonpatch/jsonpatch_error.h"

namespace turbo { namespace jsonpatch {
...
namespace detail {
    ...
    jsonpointer::basic_json_pointer<typename Json::char_type> definite_path(const Json& root, jsonpointer::basic_json_pointer<typename Json::char_type>& location)
    {
        ...
        turbo::detail::from_integer(val.size(), last_token);
    ...
    Json from_diff(const Json& source, const Json& target, const typename Json::string_view_type& path)
    {
        ...
        if (source.is_array() && target.is_array())
        {
            ...
            for (std::size_t i = 0; i < common; ++i)
            {
                ...
                ss.push_back('/');
                turbo::detail::from_integer(i,ss);
...
void apply_patch(Json& target, const Json& patch, std::error_code& ec)
{
   ...
   turbo::jsonpatch::detail::operation_unwinder<Json> unwinder(target);
   std::error_code local_ec;
    ...
    {
        unwinder.state =turbo::jsonpatch::detail::state_type::begin;
        ...
        if (it_op == operation.object_range().end())
        {
            ...
            unwinder.state =turbo::jsonpatch::detail::state_type::abort;
            return;
        }
        ...
        if (op ==turbo::jsonpatch::detail::jsonpatch_names<char_type>::test_name())
        {
        ...
        else if (op ==turbo::jsonpatch::detail::jsonpatch_names<char_type>::add_name())
        {
            auto it_value = operation.find(detail::jsonpatch_names<char_type>::value_name());
            if (it_value == operation.object_range().end())
            {
        ...
        else if (op ==turbo::jsonpatch::detail::jsonpatch_names<char_type>::remove_name())
        {
            Json val = jsonpointer::get(target,location,local_ec);
            if (local_ec)
            {
        ...
        else if (op ==turbo::jsonpatch::detail::jsonpatch_names<char_type>::replace_name())
        {
            Json val = jsonpointer::get(target,location,local_ec);
            if (local_ec)
            {
        ...
        else if (op ==turbo::jsonpatch::detail::jsonpatch_names<char_type>::move_name())
        {
            ...
            if (insert_ec) // try a replace
            {
                ...
                unwinder.stack.emplace_back(turbo::jsonpatch::detail::op_type::replace,npath,orig_val);
            }
            else
            {
                unwinder.stack.emplace_back(detail::op_type::remove,npath,Json::null());
        ...
        else if (op ==turbo::jsonpatch::detail::jsonpatch_names<char_type>::copy_name())
        {
            auto it_from = operation.find(detail::jsonpatch_names<char_type>::from_name());
            if (it_from == operation.object_range().end())
            {
    ...
    if (unwinder.state ==turbo::jsonpatch::detail::state_type::begin)
    {
        unwinder.state =turbo::jsonpatch::detail::state_type::commit;
    }
}
...
Json from_diff(const Json& source, const Json& target)
{
    ...
    return turbo::jsonpatch::detail::from_diff(source, target, path);
}
