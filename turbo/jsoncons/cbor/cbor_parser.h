...
#include "turbo/jsoncons/cbor/cbor_options.h"
...
namespace turbo {
    namespace cbor {
        ...
        enum class parse_mode {
            ...
            map_value,
            indefinite_map_key,
            indefinite_map_value,
            multi_dim
        };

        struct mapped_string {
            turbo::cbor::detail::cbor_major_type type;
            std::string s;
            std::vector<uint8_t> bytes;

            mapped_string(const std::string &s)
                    : type(turbo::cbor::detail::cbor_major_type::text_string), s(s) {
            }

            mapped_string(std::string &&s)
                    : type(turbo::cbor::detail::cbor_major_type::text_string), s(std::move(s)) {
            }

            mapped_string(const std::vector<uint8_t> &bytes)
                    : type(turbo::cbor::detail::cbor_major_type::byte_string), bytes(bytes) {
            }

            mapped_string(std::vector<uint8_t> &&bytes)
                    : type(turbo::cbor::detail::cbor_major_type::byte_string), bytes(std::move(bytes)) {
            }
        ...
        private:
            void read_item(item_event_visitor &visitor, std::error_code &ec) {
                ...
                turbo::cbor::detail::cbor_major_type major_type = get_major_type(c.value);
                uint8_t info = get_additional_information_value(c.value);

                switch (major_type) {
                    case turbo::cbor::detail::cbor_major_type::unsigned_integer: {
                        ...
                        if (!stringref_map_stack_.empty() && other_tags_[stringref_tag]) {
                            ...
                            switch (str.type) {
                                case turbo::cbor::detail::cbor_major_type::text_string: {
                                    handle_string(visitor, turbo::basic_string_view<char>(str.s.data(), str.s.length()),
                                                  ec);
                                    if (ec) {
                                        return;
                                    }
                                ...
                                case turbo::cbor::detail::cbor_major_type::byte_string: {
                    ...
                    case turbo::cbor::detail::cbor_major_type::negative_integer: {
                    ...
                    case turbo::cbor::detail::cbor_major_type::text_string: {
                        ...
                        handle_string(visitor,
                                      turbo::basic_string_view<char>(text_buffer_.data(), text_buffer_.length()), ec);
                        if (ec) {
                            return;
                        }
                        break;
                    }
                    case turbo::cbor::detail::cbor_major_type::semantic_tag: {
                        TURBO_UNREACHABLE();
                        break;
                    }
                    case turbo::cbor::detail::cbor_major_type::simple: {
                        switch (info) {
                            case 0x14:
                    ...
                    case turbo::cbor::detail::cbor_major_type::array: {
                        if (other_tags_[item_tag]) {
                            switch (item_tag_) {
                                case 0x04:
                                    text_buffer_.clear();
                    ...
                    case turbo::cbor::detail::cbor_major_type::map: {
            ...
            void begin_array(item_event_visitor &visitor, uint8_t info, std::error_code &ec) {
                ...
                switch (info) {
                    case turbo::cbor::detail::additional_info::indefinite_length: {
            ...
            void read_text_string(string_type &s, std::error_code &ec) {
                ...
                TURBO_ASSERT(major_type == turbo::cbor::detail::cbor_major_type::text_string);
                ...
                if (!stringref_map_stack_.empty() &&
                    info != turbo::cbor::detail::additional_info::indefinite_length &&
                    s.length() >= turbo::cbor::detail::min_length_for_stringref(stringref_map_stack_.back().size())) {
            ...
            bool read_byte_string(std::vector<uint8_t, byte_allocator_type> &v, std::error_code &ec) {
                ...
                TURBO_ASSERT(major_type == turbo::cbor::detail::cbor_major_type::byte_string);
                ...
                switch (info) {
                    ...
                    default: {
                        ...
                        if (!stringref_map_stack_.empty() &&
                            v.size() >=
                            turbo::cbor::detail::min_length_for_stringref(stringref_map_stack_.back().size())) {
            ...
            void iterate_string_chunks(Function &func, turbo::cbor::detail::cbor_major_type type, std::error_code &ec) {
                int nesting_level = 0;
            ...
            int64_t get_int64_value(std::error_code &ec) {
                ...
                turbo::cbor::detail::cbor_major_type major_type = get_major_type(ch.value);
            ...
            void read_decimal_fraction(string_type &result, std::error_code &ec) {
                ...
                switch (get_major_type(c.value)) {
                    ...
                    case turbo::cbor::detail::cbor_major_type::semantic_tag: {
                        ...
                        if (get_major_type(c.value) == turbo::cbor::detail::cbor_major_type::byte_string) {
                            bytes_buffer_.clear();
                ...
                if (s.size() >= static_cast<std::size_t>((std::numeric_limits<int32_t>::max)()) ||
                    ...
                    if (s[0] == '-') {
                        result.push_back('-');
                        turbo::detail::prettify_string(s.c_str() + 1, s.size() - 1, (int) exponent, -4, 17, result);
                    } else {
                        turbo::detail::prettify_string(s.c_str(), s.size(), (int) exponent, -4, 17, result);
                    }
                } else {
                    ec = cbor_errc::invalid_decimal_fraction;
                    more_ = false;
            ...
            void read_bigfloat(string_type &s, std::error_code &ec) {
                ...
                switch (get_major_type(c.value)) {
                    case turbo::cbor::detail::cbor_major_type::unsigned_integer: {
                        ...
                        s.push_back('0');
                        s.push_back('x');
                        turbo::detail::integer_to_string_hex(val, s);
                        break;
                    }
                    case turbo::cbor::detail::cbor_major_type::negative_integer: {
                        int64_t val = get_int64_value(ec);
                        ...
                        s.push_back('0');
                        s.push_back('x');
                        turbo::detail::integer_to_string_hex(static_cast<uint64_t>(-val), s);
                        break;
                    }
                    case turbo::cbor::detail::cbor_major_type::semantic_tag: {
                        uint8_t b;
                ...
                s.push_back('p');
                if (exponent >= 0) {
                    turbo::detail::integer_to_string_hex(static_cast<uint64_t>(exponent), s);
                } else {
                    s.push_back('-');
                    turbo::detail::integer_to_string_hex(static_cast<uint64_t>(-exponent), s);
                }
            ...
            static turbo::cbor::detail::cbor_major_type get_major_type(uint8_t type) {
                static constexpr uint8_t major_type_shift = 0x05;
                uint8_t value = type >> major_type_shift;
                return static_cast<turbo::cbor::detail::cbor_major_type>(value);
            }
            ...
            void read_tags(std::error_code &ec) {
                ...
                while (major_type == turbo::cbor::detail::cbor_major_type::semantic_tag) {
                    uint64_t val = get_uint64_value(ec);
                    if (!more_) {
                        return;
                    }
            ...
            void
            handle_string(item_event_visitor &visitor, const turbo::basic_string_view<char> &v, std::error_code &ec) {
                semantic_tag tag = semantic_tag::none;
                if (other_tags_[item_tag]) {
                    switch (item_tag_) {
                        case 0:
            ...
            static turbo::endian get_typed_array_endianness(const uint8_t tag) {
                return ((tag & detail::cbor_array_tags_e_mask) >> detail::cbor_array_tags_e_shift) == 0
                       ? turbo::endian::big : turbo::endian::little;
            }
                ...
                if (other_tags_[item_tag]) {
                    switch (item_tag_) {
                        ...
                        case 0x45: {
                            ...
                            turbo::endian e = get_typed_array_endianness(tag);
            ...
            void produce_begin_multi_dim(item_event_visitor &visitor,
                ...
                turbo::cbor::detail::cbor_major_type major_type = get_major_type(b);
                TURBO_ASSERT(major_type == turbo::cbor::detail::cbor_major_type::array);
                uint8_t info = get_additional_information_value(b);
