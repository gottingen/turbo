...
#include "turbo/jsoncons/cbor/cbor_reader.h"
#include "turbo/jsoncons/cbor/cbor_cursor.h"
...
namespace cbor {
                ...
                const cbor_decode_options& options = cbor_decode_options())
    {
        ...
        basic_cbor_reader<turbo::bytes_source> reader(v, adaptor, options);
        reader.read();
        if (!decoder.is_valid())
        {
                ...
                const cbor_decode_options& options = cbor_decode_options())
    {
        ...
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_cbor_reader<turbo::bytes_source,TempAllocator> reader(v, adaptor, options, temp_alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
                ...
                const cbor_decode_options& options = cbor_decode_options())
    {
        json_decoder<T,TempAllocator> decoder(temp_alloc);
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_cbor_reader<turbo::binary_stream_source,TempAllocator> reader(is, adaptor, options, temp_alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
