...
namespace cbor {
                ...
                const cbor_encode_options& options = cbor_encode_options())
    {
        ...
        basic_cbor_encoder<turbo::bytes_sink<Container>> encoder(v, options);
    ...
    encode_cbor(temp_allocator_arg_t, const TempAllocator& temp_alloc,
                ...
                const cbor_encode_options& options = cbor_encode_options())
    {
        basic_cbor_encoder<turbo::bytes_sink<Container>,TempAllocator> encoder(v, options, temp_alloc);
        std::error_code ec;
        encode_traits<T,char>::encode(val, encoder, json(), ec);
        if (ec)
        {
