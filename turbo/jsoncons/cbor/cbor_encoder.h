...
#ifndef TURBO_JSONCONS_CBOR_CBOR_ENCODER_H_
#define TURBO_JSONCONS_CBOR_CBOR_ENCODER_H_
...
#include "turbo/jsoncons/json_exception.h" // turbo::ser_error
...
namespace turbo::cbor {
    ...
    enum class cbor_container_type {
    ...
    class basic_cbor_encoder final : public basic_json_visitor<char> {
        ...
        enum class hexfloat_parse_state {
            start, expect_0, expect_x, integer, exp1, exp2, fraction1
        };
    ...
    private:
        ...
        struct stack_item {
            ...
            bool is_object() const {
                return type_ == cbor_container_type::object ||
                       type_ == cbor_container_type::indefinite_length_object;
            }

            bool is_indefinite_length() const {
                return type_ == cbor_container_type::indefinite_length_array ||
                       type_ == cbor_container_type::indefinite_length_object;
            }
    ...
    private:
        ...
        void write_string(const string_view &sv) {
            ...
            if (options_.pack_strings() &&
                sv.size() >= turbo::cbor::detail::min_length_for_stringref(next_stringref_)) {
        ...
        bool write_decimal_value(const string_view_type &sv, const ser_context &context, std::error_code &ec) {
            ...
            if (exponent.length() > 0) {
                int64_t val;
                auto r = turbo::detail::to_integer(exponent.data(), exponent.length(), val);
                if (!r) {
                    ec = r.error_code();
                    return false;
                }
            ...
            int64_t val{0};
            auto r = turbo::detail::to_integer(s.data(), s.length(), val);
            if (r) {
                more = visit_int64(val, semantic_tag::none, context, ec);
                if (!more) { return more; }
            } else if (r.error_code() == turbo::detail::to_integer_errc::overflow) {
        ...
        bool write_hexfloat_value(const string_view_type &sv, const ser_context &context, std::error_code &ec) {
            ...
            if (exponent.length() > 0) {
                int64_t val{0};
                auto r = turbo::detail::base16_to_integer(exponent.data(), exponent.length(), val);
                if (!r) {
                    ec = r.error_code();
                    return false;
                }
            ...
            int64_t val{0};
            auto r = turbo::detail::base16_to_integer(s.data(), s.length(), val);
            if (r) {
                more = visit_int64(val, semantic_tag::none, context, ec);
                if (!more) return more;
            } else if (r.error_code() == turbo::detail::to_integer_errc::overflow) {
        ...
        bool visit_string(const string_view_type &sv, semantic_tag tag, const ser_context &context,
        ...
        bool visit_byte_string(const byte_string_view &b,
            ...
            if (options_.pack_strings() &&
                b.size() >= turbo::cbor::detail::min_length_for_stringref(next_stringref_)) {
        ...
        bool visit_typed_array(const turbo::Span<const uint16_t> &data,
                               ...
                               std::error_code &ec) override {
            if (options_.use_typed_arrays()) {
                write_typed_array_tag(std::integral_constant<bool, turbo::endian::native == turbo::endian::big>(),
                                      uint16_t(),
                                      tag);
    ...
    using cbor_stream_encoder = basic_cbor_encoder<turbo::binary_stream_sink>;
    using cbor_bytes_encoder = basic_cbor_encoder<turbo::bytes_sink<std::vector<uint8_t>>>;

}  // namespace turbo::cbor
#endif  // TURBO_JSONCONS_CBOR_CBOR_ENCODER_H_
