...
namespace turbo {
    ...
    public:
        ...
        uri(turbo::string_view scheme,
            turbo::string_view userinfo,
            turbo::string_view host,
            turbo::string_view port,
            turbo::string_view path,
            turbo::string_view query,
        ...
        string_view base() const noexcept {
            return string_view(uri_.data() + scheme_.first, (path_.second - scheme_.first));
        }

        string_view scheme() const noexcept {
            return string_view(uri_.data() + scheme_.first, (scheme_.second - scheme_.first));
        }

        string_view userinfo() const noexcept {
            return string_view(uri_.data() + userinfo_.first, (userinfo_.second - userinfo_.first));
        }

        string_view host() const noexcept {
            return string_view(uri_.data() + host_.first, (host_.second - host_.first));
        }

        string_view port() const noexcept {
            return string_view(uri_.data() + port_.first, (port_.second - port_.first));
        }

        string_view path() const noexcept {
            return string_view(uri_.data() + path_.first, (path_.second - path_.first));
        }

        string_view query() const noexcept {
            return string_view(uri_.data() + query_.first, (query_.second - query_.first));
        }
        ...
        string_view authority() const noexcept {
            return string_view(uri_.data() + userinfo_.first, (port_.second - userinfo_.first));
        }
    ...
    private:
        ...
        static std::string remove_dot_segments(const turbo::string_view &input) {
...
*/
        ...
        static std::string merge_paths(const uri &base, const uri &relative) {
            ...
            return remove_dot_segments(turbo::string_view(result));
        }
