...
namespace turbo {
    ...
    private:
        ...
        turbo::detail::chars_to to_double_;
    ...
    public:
        ...
        void end_object(basic_json_visitor<char_type> &visitor, std::error_code &ec) {
            if (TURBO_UNLIKELY(nesting_depth_ < 1)) {
        ...
        void parse_true(basic_json_visitor<char_type> &visitor, std::error_code &ec) {
            saved_position_ = position_;
            if (TURBO_LIKELY(end_input_ - input_ptr_ >= 4)) {
        ...
        void parse_false(basic_json_visitor<char_type> &visitor, std::error_code &ec) {
            saved_position_ = position_;
            if (TURBO_LIKELY(end_input_ - input_ptr_ >= 5)) {
                if (*(input_ptr_ + 1) == 'a' && *(input_ptr_ + 2) == 'l' && *(input_ptr_ + 3) == 's' &&
                    *(input_ptr_ + 4) == 'e') {
                    input_ptr_ += 5;
                    position_ += 5;
                    more_ = visitor.bool_value(false, semantic_tag::none, *this, ec);
        ...
        void parse_number(basic_json_visitor<char_type> &visitor, std::error_code &ec) {
            ...
            minus_sign:
            if (TURBO_UNLIKELY(input_ptr_ >= local_input_end)) // Buffer exhausted
            {
                state_ = json_parse_state::minus;
                return;
            }
    ...
    private:
        ...
        void end_negative_value(basic_json_visitor<char_type> &visitor, std::error_code &ec) {
            int64_t val;
            auto result = turbo::detail::to_integer_unchecked(string_buffer_.data(), string_buffer_.length(), val);
            if (result) {
        ...
        void end_string_value(const char_type *s, std::size_t length, basic_json_visitor<char_type> &visitor,
                              std::error_code &ec) {
            ...
            switch (parent()) {
                ...
                case json_parse_state::array: {
                    auto it = std::find_if(string_double_map_.begin(), string_double_map_.end(),
        ...
        json_parse_state pop_state() {
            TURBO_ASSERT(!state_stack_.empty());
