...
private:
    ...
    {
        // nil
        binary::native_to_big(static_cast<uint8_t>(turbo::ubjson::ubjson_type::null_type), std::back_inserter(sink_));
        end_value();
        return true;
    }
                              ...
                              std::error_code&) override
    {
        ...
        sink_.push_back(turbo::ubjson::ubjson_type::start_array_marker);
        binary::native_to_big(static_cast<uint8_t>(turbo::ubjson::ubjson_type::type_marker), std::back_inserter(sink_));
        binary::native_to_big(static_cast<uint8_t>(turbo::ubjson::ubjson_type::uint8_type), std::back_inserter(sink_));
        put_length(length);
    ...
    {
        // true and false
        sink_.push_back(static_cast<uint8_t>(val ? turbo::ubjson::ubjson_type::true_type : turbo::ubjson::ubjson_type::false_type));
