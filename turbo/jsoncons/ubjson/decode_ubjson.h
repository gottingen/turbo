...
#include "turbo/jsoncons/ubjson/ubjson_reader.h"
#include "turbo/jsoncons/ubjson/ubjson_cursor.h"
...
namespace ubjson {
                  ...
                  const ubjson_decode_options& options = ubjson_decode_options())
    {
        ...
        basic_ubjson_reader<turbo::bytes_source> reader(v, adaptor, options);
        reader.read();
        if (!decoder.is_valid())
        {
                  ...
                  const ubjson_decode_options& options = ubjson_decode_options())
    {
        ...
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_ubjson_reader<turbo::bytes_source,TempAllocator> reader(v, adaptor, options, temp_alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
                  ...
                  const ubjson_decode_options& options = ubjson_decode_options())
    {
        json_decoder<T,TempAllocator> decoder(temp_alloc);
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_ubjson_reader<turbo::binary_stream_source,TempAllocator> reader(is, adaptor, options, temp_alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
