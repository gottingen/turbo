...
#include "turbo/jsoncons/detail/grisu3.h"
...
namespace detail {
    ...
    void fill_exponent(int K, Result& result)
    {
        ...
        {
            turbo::detail::from_integer(K, result);
        }
    }
    ...
    bool dtoa_general(double v, char decimal_point, Result& result, std::true_type)
    {
        ...
        if (turbo::detail::grisu3(u, buffer, &length, &k))
        {
            if (std::signbit(v))
            {
                result.push_back('-');
            ...
            turbo::detail::prettify_string(buffer, length, k, -4, std::numeric_limits<double>::max_digits10, result);
            return true;
        }
        else
        {
    ...
    bool dtoa_fixed(double v, char decimal_point, Result& result, std::true_type)
    {
        ...
        if (turbo::detail::grisu3(u, buffer, &length, &k))
        {
            ...
            turbo::detail::prettify_string(buffer, length, k, std::numeric_limits<int>::lowest(), (std::numeric_limits<int>::max)(), result);
            return true;
        }
        else
        {
