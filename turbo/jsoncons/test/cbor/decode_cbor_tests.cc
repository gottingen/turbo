...
TEST_CASE("cbor object comparison")
{
    ...
    serializer3.byte_string_value(turbo::byte_string{});
    serializer3.end_object(); 
    serializer3.flush();
...
TEST_CASE("cbor member tests")
{
    ...
    encoder.byte_string_value(turbo::byte_string{});
...
*/
    ...
    SECTION("j[0].as<T>()")
    {
        ...
        CHECK(j[1].as<turbo::byte_string>() == turbo::byte_string({'b','a','r'}));
        CHECK(j[2].as<std::string>() == std::string("-18446744073709551617"));
        CHECK(bool(j[2].as<turbo::bigint>() == turbo::bigint::from_string("-18446744073709551617")));
