...
TEST_CASE("csv_test1_repeat")
{
    ...
    turbo::csv::detail::parse_column_types(std::string("string,float*"), result);
    ...
    turbo::csv::detail::parse_column_types(std::string("string,[float*]"), result2);
    ...
    turbo::csv::detail::parse_column_types(std::string("string,[float]*"), result3);
...
3e-05
5001173100E95978)";
        turbo::csv::csv_options options;
        options.assume_header(true)
               .field_delimiter(',');

        const auto csv = turbo::csv::decode_csv<turbo::json>(data, options);
...
5001173100E95978)";
        ...
        options.field_delimiter(',')
               .header_lines(1)
               .mapping_kind(turbo::csv::csv_mapping_kind::n_rows);

        auto result = turbo::csv::decode_csv<std::vector<std::vector<double>>>(data, options);
