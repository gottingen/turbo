...
using turbo::json;
namespace msgpack = turbo::msgpack;
...
{
    // fixarray
    check_decode_msgpack({0x90}, json(turbo::json_array_arg));
    check_decode_msgpack({0x80}, json(turbo::json_object_arg));

    check_decode_msgpack({0x91,'\0'},json::parse("[0]"));
    check_decode_msgpack({0x92,'\0','\0'}, json(turbo::json_array_arg, {0,0}));
...
TEST_CASE("Compare msgpack packed item and jsoncons item")
{
    ...
    encoder.byte_string_value(turbo::byte_string{'b','a','r'});
    encoder.end_array();
    encoder.flush();

    json expected(turbo::json_array_arg);

    expected.emplace_back("foo");
    expected.emplace_back(turbo::byte_string{ 'b','a','r' });
