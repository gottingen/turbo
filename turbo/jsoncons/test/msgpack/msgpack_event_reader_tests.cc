...
#include "turbo/jsoncons/msgpack/msgpack_event_reader.h"
...
{
    using event_reader_type = msgpack::msgpack_event_reader<turbo::binary_stream_source>;
                   ...
                   msgpack_stream_cursor2_reset_test_traits)
{
    ...
    SECTION("keeping same source")
    {
        ...
        CHECK(reader.current().template get<turbo::string_view>() ==
              turbo::string_view("Tom"));
        reader.next();
        CHECK(reader.done());
