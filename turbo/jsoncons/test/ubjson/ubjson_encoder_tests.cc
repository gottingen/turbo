...
TEST_CASE("serialize big array to ubjson")
{
    ...
    turbo::ubjson::encode_ubjson(val, data);

    turbo::ubjson::ubjson_options options;
    options.max_items((std::numeric_limits<int32_t>::max)());
    ns::hiking_reputon val2 = turbo::ubjson::decode_ubjson<ns::hiking_reputon>(data, options);
