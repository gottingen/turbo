...
namespace {
    ...
    json resolver(const turbo::uri& uri)
    {
        if (uri.path() == "/draft-07/schema") 
        {
            return turbo::jsonschema::draft7::schema_draft7<json>::get_schema();
        }
        else
        {
...
TEST_CASE("jsonschema-tests")
{
    ...
    SECTION("#417")
    {
        turbo::json schema = turbo::json::parse(R"(
    {
      ...
      "maxItems": 1
            ...
            turbo::json instance = turbo::json::parse(R"(
    [
      {
        "x": 2.5,
        "y": 1.3
      },
      {
        ...
        "z": 6.7
            ...
            auto sch = turbo::jsonschema::make_schema(schema);
            turbo::jsonschema::json_validator<turbo::json> validator(sch);
