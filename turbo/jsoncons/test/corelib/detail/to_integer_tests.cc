...
TEST_CASE("detail::to_integer tests")
{
    SECTION("")
    {
        std::string s = "-";
        int64_t val;
        auto result = turbo::detail::to_integer(s.data(), s.length(), val);
        REQUIRE_FALSE(result);
        CHECK(result.ec == turbo::detail::to_integer_errc::invalid_number);
    }
    SECTION("-")
    {
        std::string s = "-";
    ...
    SECTION("min int64_t - 1")
    {
        ...
        auto result = turbo::detail::to_integer(s.data(), s.length(), val);
        REQUIRE_FALSE(result);
        CHECK(result.ec == turbo::detail::to_integer_errc::overflow);
    }
...
TEST_CASE("detail::to_integer_decimal tests")
{
    SECTION("")
    {
        std::string s = "-";
        int64_t val;
        auto result = turbo::detail::to_integer_decimal(s.data(), s.length(), val);
        REQUIRE_FALSE(result);
        CHECK(result.ec == turbo::detail::to_integer_errc::invalid_number);
    }
    SECTION("-")
...
TEST_CASE("detail::to_integer_unchecked tests")
{
    SECTION("max uint64_t")
    {
        std::string s = "18446744073709551615";
        uint64_t val;
        auto result = turbo::detail::to_integer_unchecked(s.data(), s.length(), val);
        REQUIRE(result);
        CHECK(val == (std::numeric_limits<uint64_t>::max)());
    }
    SECTION("min int64_t")
