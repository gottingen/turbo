...
TEST_CASE("convert_pair_test")
{
    ...
    turbo::encode_json(val, s);

    auto result = turbo::decode_json<std::pair<bool,std::string>>(s);
...
TEST_CASE("convert_vector_test")
{
    ...
    std::string s;
    turbo::encode_json(v,s);

    auto result = turbo::decode_json<std::vector<double>>(s);
...
TEST_CASE("convert_vector_test, temp_allocator")
{
    ...
    auto result = turbo::decode_json<std::vector<double>>(
...
TEST_CASE("convert_map_test")
{
    ...
    std::string s;
    turbo::encode_json(m,s);
    auto result = turbo::decode_json<std::map<std::string,double>>(s);
...
TEST_CASE("convert_map_test, temp_allocator")
{
    ...
    std::string s;
    turbo::encode_json(m,s);
    auto result = turbo::decode_json<std::map<std::string,double>>(
        temp_allocator_arg, MyAlloc<char>(1), s);
    REQUIRE(result.size() == m.size());
    CHECK(m["a"] == result["a"]);
    CHECK(m["b"] == result["b"]);
...
TEST_CASE("convert_array_test")
{
    ...
    std::string s;
    turbo::encode_json(v,s);
    std::array<double, 4> result = turbo::decode_json<std::array<double,4>>(s);
...
TEST_CASE("convert vector of vector test")
{
    ...
    std::string s;
    turbo::encode_json(v,s);
    auto result = turbo::decode_json<std::vector<std::vector<double>>>(s);
    REQUIRE(result.size() == v.size());
...
TEST_CASE("convert vector of vector test, temp_allocator")
{
    ...
    std::string s;
    turbo::encode_json(v,s);
    auto result = turbo::decode_json<std::vector<std::vector<double>>>(
        temp_allocator_arg, MyAlloc<char>(1), s);
    REQUIRE(result.size() == v.size());
    for (const auto& item : result)
    {
...
TEST_CASE("convert_tuple_test")
{
    ...
    std::string s;
    turbo::encode_json_pretty(input, s);
    ...
    auto employees2 = turbo::decode_json<employee_collection>(s);
...
TEST_CASE("convert_tuple_test, temp_allocator")
{
    ...
    std::string s;
    turbo::encode_json_pretty(employees, s);
    auto employees2 = turbo::decode_json<employee_collection>(
        temp_allocator_arg, MyAlloc<char>(1), s);
...
TEST_CASE("encode/decode map with integer key")
{
    ...
    SECTION("string source")
    {
        std::string s;
        turbo::encode_json(m,s);
        auto result = turbo::decode_json<std::map<int,double>>(s);
    ...
    SECTION("stream source")
    {
        ...
        auto result = turbo::decode_json<std::map<int,double>>(is);
    ...
    SECTION("iterator source")
    {
        ...
        auto result = turbo::decode_json<std::map<int,double>>(s.begin(), s.end());
    ...
    SECTION("custom iterator source")
    {
        ...
        auto result = turbo::decode_json<std::map<int,double>>(it, end);
