...
struct own_vector : std::vector<std::int64_t> {
...
TEST_CASE("turbo::json_type_traits<optional>")
{
    std::vector<std::optional<int>> v = {0, 1, std::optional<int>{}};
    json j = v;
    ...
    CHECK(j[0].is<std::optional<int>>());
    CHECK_FALSE(j[0].is<std::optional<double>>());
    CHECK(j[1].is<std::optional<int>>());
    CHECK_FALSE(j[1].is<std::optional<double>>());
    CHECK(j[2].is<std::optional<int>>()); // null can be any optinal
}

TEST_CASE("turbo::json_type_traits<shared_ptr>")
{
...
TEST_CASE("turbo::json_type_traits<unique_ptr>")
{
    std::vector<std::unique_ptr<std::string>> v;

    v.emplace_back(std::make_unique<std::string>("Hello"));
    v.emplace_back(std::make_unique<std::string>("World"));
...
{
    turbo::json j = own_vector({0,9,8,7});
    std::cout << j;
}
*/
    ...
    std::string input = R"(
{
  ...
  "items": [
    ...
    SECTION("std::variant<int, double, bool, std::string, ns::Color> test")
    {
        ...
        std::string buffer1;
        turbo::encode_json(var1, buffer1);
        std::string buffer2;
        turbo::encode_json(var2, buffer2);
        std::string buffer3;
        turbo::encode_json(var3, buffer3);
        std::string buffer4;
        turbo::encode_json(var4, buffer4);
        std::string buffer5;
        turbo::encode_json(var5, buffer5);

        auto v1 = turbo::decode_json<variant_type>(buffer1);
        auto v2 = turbo::decode_json<variant_type>(buffer2);
        auto v3 = turbo::decode_json<variant_type>(buffer3);
        auto v4 = turbo::decode_json<variant_type>(buffer4);
        auto v5 = turbo::decode_json<variant_type>(buffer5);
