...
namespace {
    ...
    class string_locator : public turbo::default_json_visitor
    {
        std::string path_;
        std::string from_;
    ...
    public:
        using turbo::default_json_visitor::string_view_type;
        ...
        bool visit_string(const string_view_type&, turbo::semantic_tag, const turbo::ser_context& context, std::error_code&) override
        {
    ...
    void update_in_place(std::string& input, const std::string& path, std::vector<std::size_t>& positions)
    {
        ...
        turbo::json_string_reader reader(input, updater);
        reader.read();
    }
}
