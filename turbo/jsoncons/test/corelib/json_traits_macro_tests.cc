...
namespace ns {
    ...
    public:
        ...
        const std::optional<std::string>& publisher() const
        {
            return publisher_;
        }
    };
    ...
    public:
        ...
        const std::optional<std::string>& getIsbn() const
        {
            return isbn_;
        }

        void setIsbn(const std::optional<std::string>& value)
        {
            isbn_ = value;
        }
    };
...
TEST_CASE("JSONCONS_N_MEMBER_TRAITS with optional tests")
{
    ...
    {
        ns::book1c book{an_author, a_title, a_price, std::optional<std::string>{}};
...
TEST_CASE("JSONCONS_N_CTOR_GETTER_TRAITS tests")
{
    ...
    SECTION("to_json")
    {
        ns::book2b book(an_author,a_title,a_price,an_isbn,std::optional<std::string>());
...
[
    ...
    {
        auto v = turbo::decode_json<std::vector<std::shared_ptr<ns::Employee>>>(input);
        REQUIRE(v.size() == 2);
    ...
    SECTION("decode vector of unique_ptr test")
    {
        ...
        auto v = turbo::decode_json<std::vector<std::unique_ptr<ns::Employee>>>(input);
        REQUIRE(v.size() == 2);
        CHECK(v[0]->firstName() == firstName0);
        CHECK(v[0]->lastName() == lastName0);
        CHECK(v[0]->calculatePay() == pay0);
...
TEST_CASE("JSONCONS_N_MEMBER_TRAITS pointer and optional test")
{
    SECTION("test 1")
    {
        ...
        val.field2 = std::make_unique<std::string>("Field 2");
        val.field3 = "Field 3";
        ...
        val.field8 = std::make_unique<std::string>("Field 8");
        val.field9 = "Field 9";
