...
#include "turbo/jsoncons/jsonpath/flatten.h"
...
namespace {
    ...
    void compare_match(turbo::json& doc,
                       const std::string& path, 
                       const std::string& value)
    {
        auto result = turbo::jsonpath::json_query(doc, path);
...
TEST_CASE("jsonpath flatten escape")
{
    ...
    turbo::json doc = turbo::json::parse(json);

    auto flat_doc = turbo::jsonpath::flatten(doc);
