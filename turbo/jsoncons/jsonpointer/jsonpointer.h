...
#ifndef TURBO_JSONCONS_JSONPOINTER_JSONPOINTER_H_
#define TURBO_JSONCONS_JSONPOINTER_JSONPOINTER_H_
...
namespace turbo {
    namespace jsonpointer {
        ...
        public:
            ...
            friend basic_json_pointer<CharT>
            operator+(const basic_json_pointer<CharT> &lhs, const basic_json_pointer<CharT> &rhs) {
                basic_json_pointer<CharT> p(lhs);
                p += rhs;
                return p;
            }
        ...
        namespace detail {
            ...
            template<class Json>
            const Json *
            resolve(const Json *current, const typename Json::string_view_type &buffer, std::error_code &ec) {
                if (current->is_array()) {
            ...
            template<class Json>
            Json *resolve(Json *current, const typename Json::string_view_type &buffer, bool create_if_missing,
                          std::error_code &ec) {
                if (current->is_array()) {
                    if (buffer.size() == 1 && buffer[0] == '-') {
                        ec = jsonpointer_errc::index_exceeds_array_size;
...
#endif  // TURBO_JSONCONS_JSONPOINTER_JSONPOINTER_H_
