...
#ifndef TURBO_JSONCONS_MSGPACK_MSGPACK_ENCODER_H_
#define TURBO_JSONCONS_MSGPACK_MSGPACK_ENCODER_H_
...
#include "turbo/jsoncons/msgpack/msgpack_options.h"

namespace turbo::msgpack {

    enum class msgpack_container_type {
        object, array
    };
    ...
    private:
        ...
        visit_begin_object(std::size_t length, semantic_tag, const ser_context &, std::error_code &ec) override {
            ...
            if (length <= 15) {
                // fixmap
                sink_.push_back(turbo::msgpack::msgpack_type::fixmap_base_type | (length & 0xf));
            } else if (length <= 65535) {
                // map 16
                sink_.push_back(turbo::msgpack::msgpack_type::map16_type);
                binary::native_to_big(static_cast<uint16_t>(length),
                                      std::back_inserter(sink_));
            } else if (length <= 4294967295) {
                // map 32
                sink_.push_back(turbo::msgpack::msgpack_type::map32_type);
        ...
        visit_begin_array(std::size_t length, semantic_tag, const ser_context &, std::error_code &ec) override {
            ...
            if (length <= 15) {
                // fixarray
                sink_.push_back(turbo::msgpack::msgpack_type::fixarray_base_type | (length & 0xf));
            } else if (length <= (std::numeric_limits<uint16_t>::max)()) {
                // array 16
                sink_.push_back(turbo::msgpack::msgpack_type::array16_type);
                binary::native_to_big(static_cast<uint16_t>(length), std::back_inserter(sink_));
            } else if (length <= (std::numeric_limits<uint32_t>::max)()) {
                // array 32
                sink_.push_back(turbo::msgpack::msgpack_type::array32_type);
        ...
        bool visit_null(semantic_tag, const ser_context &, std::error_code &) override {
            // nil
            sink_.push_back(turbo::msgpack::msgpack_type::nil_type);
            end_value();
            return true;
        }
        ...
        bool visit_string(const string_view_type &sv, semantic_tag tag, const ser_context &,
            ...
            switch (tag) {
                case semantic_tag::epoch_second: {
                    int64_t seconds;
                    auto result = turbo::detail::to_integer(sv.data(), sv.length(), seconds);
                    if (!result) {
        ...
        void write_string_value(const string_view_type &sv) {
            ...
            if (length <= 31) {
                ...
                sink_.push_back(turbo::msgpack::msgpack_type::fixstr_base_type | static_cast<uint8_t>(length));
            } else if (length <= (std::numeric_limits<uint8_t>::max)()) {
                // str 8 stores a byte array whose length is upto (2^8)-1 bytes
                sink_.push_back(turbo::msgpack::msgpack_type::str8_type);
                sink_.push_back(static_cast<uint8_t>(length));
            } else if (length <= (std::numeric_limits<uint16_t>::max)()) {
                // str 16 stores a byte array whose length is upto (2^16)-1 bytes
                sink_.push_back(turbo::msgpack::msgpack_type::str16_type);
                binary::native_to_big(static_cast<uint16_t>(length), std::back_inserter(sink_));
            } else if (length <= (std::numeric_limits<uint32_t>::max)()) {
                // str 32 stores a byte array whose length is upto (2^32)-1 bytes
                sink_.push_back(turbo::msgpack::msgpack_type::str32_type);
                binary::native_to_big(static_cast<uint32_t>(length), std::back_inserter(sink_));
            }
        ...
        bool visit_byte_string(const byte_string_view &b,
            ...
            if (length <= (std::numeric_limits<uint8_t>::max)()) {
                ...
                sink_.push_back(turbo::msgpack::msgpack_type::bin8_type);
                sink_.push_back(static_cast<uint8_t>(length));
            } else if (length <= (std::numeric_limits<uint16_t>::max)()) {
                // bin 16 stores a byte array whose length is upto (2^16)-1 bytes
                sink_.push_back(turbo::msgpack::msgpack_type::bin16_type);
                binary::native_to_big(static_cast<uint16_t>(length), std::back_inserter(sink_));
            } else if (length <= (std::numeric_limits<uint32_t>::max)()) {
                // bin 32 stores a byte array whose length is upto (2^32)-1 bytes
                sink_.push_back(turbo::msgpack::msgpack_type::bin32_type);
                binary::native_to_big(static_cast<uint32_t>(length), std::back_inserter(sink_));
            }
        ...
        bool visit_byte_string(const byte_string_view &b,
            ...
            switch (length) {
                case 1:
                    sink_.push_back(turbo::msgpack::msgpack_type::fixext1_type);
                    sink_.push_back(static_cast<uint8_t>(ext_tag));
                    break;
                case 2:
                    sink_.push_back(turbo::msgpack::msgpack_type::fixext2_type);
                    sink_.push_back(static_cast<uint8_t>(ext_tag));
                    break;
                case 4:
                    sink_.push_back(turbo::msgpack::msgpack_type::fixext4_type);
                ...
                case 8:
                    ...
                    break;
                case 16:
                    sink_.push_back(turbo::msgpack::msgpack_type::fixext16_type);
                    sink_.push_back(static_cast<uint8_t>(ext_tag));
                    break;
                default:
                    if (length <= (std::numeric_limits<uint8_t>::max)()) {
                        ...
                        sink_.push_back(turbo::msgpack::msgpack_type::ext16_type);
                        binary::native_to_big(static_cast<uint16_t>(length), std::back_inserter(sink_));
                        sink_.push_back(static_cast<uint8_t>(ext_tag));
                    } else if (length <= (std::numeric_limits<uint32_t>::max)()) {
                        sink_.push_back(turbo::msgpack::msgpack_type::ext32_type);
                        binary::native_to_big(static_cast<uint32_t>(length), std::back_inserter(sink_));
                        sink_.push_back(static_cast<uint8_t>(ext_tag));
                    }
                    break;
        ...
        bool visit_double(double val,
            ...
            if ((double) valf == val) {
                // float 32
                sink_.push_back(turbo::msgpack::msgpack_type::float32_type);
                binary::native_to_big(valf, std::back_inserter(sink_));
            } else {
                // float 64
                sink_.push_back(turbo::msgpack::msgpack_type::float64_type);
                ...
                default: {
                    if (val >= 0) {
                        if (val <= 0x7f) {
                            ...
                            sink_.push_back(turbo::msgpack::msgpack_type::uint8_type);
                            sink_.push_back(static_cast<uint8_t>(val));
                        } else if (val <= (std::numeric_limits<uint16_t>::max)()) {
                            // uint 16 stores a 16-bit big-endian unsigned integer
                            sink_.push_back(turbo::msgpack::msgpack_type::uint16_type);
                            binary::native_to_big(static_cast<uint16_t>(val), std::back_inserter(sink_));
                        } else if (val <= (std::numeric_limits<uint32_t>::max)()) {
                            // uint 32 stores a 32-bit big-endian unsigned integer
                            sink_.push_back(turbo::msgpack::msgpack_type::uint32_type);
                            binary::native_to_big(static_cast<uint32_t>(val), std::back_inserter(sink_));
                        } else if (val <= (std::numeric_limits<int64_t>::max)()) {
                            // int 64 stores a 64-bit big-endian signed integer
                            sink_.push_back(turbo::msgpack::msgpack_type::uint64_type);
                        ...
                        if (val >= -32) {
                            ...
                            sink_.push_back(turbo::msgpack::msgpack_type::int8_type);
                            binary::native_to_big(static_cast<int8_t>(val), std::back_inserter(sink_));
                        } else if (val >= (std::numeric_limits<int16_t>::lowest)()) {
                            // int 16 stores a 16-bit big-endian signed integer
                            sink_.push_back(turbo::msgpack::msgpack_type::int16_type);
        ...
        bool visit_bool(bool val, semantic_tag, const ser_context &, std::error_code &) override {
            // true and false
            sink_.push_back(static_cast<uint8_t>(val ? turbo::msgpack::msgpack_type::true_type
                                                     : turbo::msgpack::msgpack_type::false_type));
    ...
    using msgpack_stream_encoder = basic_msgpack_encoder<turbo::binary_stream_sink>;
    using msgpack_bytes_encoder = basic_msgpack_encoder<turbo::bytes_sink<std::vector<uint8_t>>>;


} // namespace turbo::msgpack

#endif  // TURBO_JSONCONS_MSGPACK_MSGPACK_ENCODER_H_
