...
#include "turbo/jsoncons/jmespath/jmespath_error.h"
...
namespace jmespath {
    ...
    struct slice
    {
        ...
        slice(const std::optional<int64_t>& start, const std::optional<int64_t>& end, int64_t step)
    ...
    public:
        ...
        {
            auto temp = std::make_unique<Json>(std::forward<Args>(args)...);
            Json* ptr = temp.get();
    ...
    public:
        ...
        class function_base
        {
            std::optional<std::size_t> arg_count_;
        public:
            function_base(std::optional<std::size_t> arg_count)
        ...
        public:
            ...
            {
                TURBO_ASSERT(args.size() == *this->arity());
        ...
        public:
            merge_function()
                : function_base(std::optional<std::size_t>())
            {
            }
            ...
            reference evaluate(std::vector<parameter>& args, dynamic_resources<Json,JsonReference>& resources, std::error_code& ec) const override
            {
                ...
                switch (arg0.type())
                {
                    ...
                    case json_type::string_value:
                    {
                        ...
                        uint64_t uval{ 0 };
                        auto result1 = turbo::detail::to_integer(sv.data(), sv.length(), uval);
                        if (result1)
                        {
                            return *resources.create_json(uval);
                        }
                        int64_t sval{ 0 };
                        auto result2 = turbo::detail::to_integer(sv.data(), sv.length(), sval);
                        if (result2)
                        {
        ...
        public:
            ...
            std::optional<std::size_t> arity() const
            {
                return type_ == token_kind::function ? function_->arity() : std::optional<std::size_t>();
            }
        ...
        static pointer evaluate_tokens(reference doc, const std::vector<token>& output_stack, dynamic_resources<Json,JsonReference>& resources, std::error_code& ec)
        {
            ...
            for (std::size_t i = 0; i < output_stack.size(); ++i)
            {
                ...
                switch (t.type())
                {
                    ...
                    case token_kind::begin_expression_type:
                    {
                        TURBO_ASSERT(i+1 < output_stack.size());
                        ++i;
                        TURBO_ASSERT(output_stack[i].is_expression());
                        TURBO_ASSERT(!stack.empty());
                        stack.pop_back();
        ...
        public:
            ...
            {
                turbo::jmespath::detail::jmespath_evaluator<Json,const Json&> evaluator;
                std::error_code ec;
            ...
            while (p_ < end_input_)
            {
                switch (state_stack_.back())
                {
                    ...
                    case path_state::lhs_expression: 
                    {
                        switch (*p_)
                        {
                            ...
                            case '{':
                                ...
                                break;
                            case '*': // wildcard
                                push_token(token(std::make_unique<object_projection>()), ec);
                                if (ec) {return jmespath_expression();}
                                state_stack_.pop_back();
                                ++p_;
                                ++column_;
                            ...
                            case '@':
                                ++p_;
                                ++column_;
                                push_token(token(std::make_unique<current_node>()), ec);
                                if (ec) {return jmespath_expression();}
                                state_stack_.pop_back();
                                break;
                            case '[': 
                    ...
                    case path_state::val_expr:
                        push_token(token(std::make_unique<identifier_selector>(buffer)), ec);
                        if (ec) {return jmespath_expression();}
                        buffer.clear();
                        state_stack_.pop_back(); 
                        break;
                    ...
                    case path_state::bracket_specifier:
                        ...
                        {
                            case '*':
                                push_token(token(std::make_unique<list_projection>()), ec);
                                if (ec) {return jmespath_expression();}
                                ...
                                break;
                            case ']': // []
                                push_token(token(std::make_unique<flatten_projection>()), ec);
                                if (ec) {return jmespath_expression();}
                    ...
                    case path_state::index_or_slice_expression:
                        switch(*p_)
                        {
                            case ']':
                            {
                                ...
                                {
                                    int64_t val{ 0 };
                                    auto r = turbo::detail::to_integer(buffer.data(), buffer.size(), val);
                                    if (!r)
                                    {
                                        ec = jmespath_errc::invalid_number;
                                        return jmespath_expression();
                                    }
                                    push_token(token(std::make_unique<index_selector>(val)), ec);
                                    if (ec) {return jmespath_expression();}
                    ...
                    case path_state::rhs_slice_expression_stop :
                    {
                        if (!buffer.empty())
                        {
                            ...
                            slic.stop_ = std::optional<int64_t>(val);
                            buffer.clear();
                        }
                        switch(*p_)
                        {
                            case ']':
                                push_token(token(std::make_unique<slice_projection>(slic)), ec);
                                if (ec) {return jmespath_expression();}
                                slic = slice{};
        ...
        void push_token(token&& tok, std::error_code& ec)
        {
            switch (tok.type())
            {
                case token_kind::end_filter:
                {
                    ...
                    {
                        output_stack_.back().expression_->add_expression(std::make_unique<filter_expression>(std::move(toks)));
                    }
                    else
                    {
                        output_stack_.emplace_back(token(std::make_unique<filter_expression>(std::move(toks))));
                    }
                    break;
                }
                case token_kind::end_multi_select_list:
                {
                    ...
                    if (!output_stack_.empty() && output_stack_.back().is_projection() && 
                        ...
                        (tok.precedence_level() == output_stack_.back().precedence_level() && tok.is_right_associative())))
                    {
                        output_stack_.back().expression_->add_expression(std::make_unique<multi_select_list>(std::move(vals)));
                    }
                    else
                    {
                        output_stack_.emplace_back(token(std::make_unique<multi_select_list>(std::move(vals))));
                    }
                    break;
                }
                case token_kind::end_multi_select_hash:
                {
                    ...
                    if (!output_stack_.empty() && output_stack_.back().is_projection() && 
                        ...
                        (tok.precedence_level() == output_stack_.back().precedence_level() && tok.is_right_associative())))
                    {
                        output_stack_.back().expression_->add_expression(std::make_unique<multi_select_hash>(std::move(key_toks)));
                    }
                    else
                    {
                        output_stack_.emplace_back(token(std::make_unique<multi_select_hash>(std::move(key_toks))));
                    }
                    break;
                }
                case token_kind::end_expression_type:
                {
                    ...
                    std::reverse(toks.begin(), toks.end());
                    output_stack_.erase(it.base(),output_stack_.end());
                    output_stack_.emplace_back(token(std::make_unique<function_expression>(std::move(toks))));
                    break;
                }
                case token_kind::literal:
                ...
                case token_kind::end_function:
                    {
                        ...
                        if (!output_stack_.empty() && output_stack_.back().is_projection() && 
                            ...
                            (tok.precedence_level() == output_stack_.back().precedence_level() && tok.is_right_associative())))
                        {
                            output_stack_.back().expression_->add_expression(std::make_unique<function_expression>(std::move(toks)));
                        }
                        else
                        {
                            output_stack_.emplace_back(token(std::make_unique<function_expression>(std::move(toks))));
    ...
    template <class Json>
    using jmespath_expression = typename turbo::jmespath::detail::jmespath_evaluator<Json,const Json&>::jmespath_expression;
