...
#include "turbo/jsoncons/jsonpath/expression.h"
#include "turbo/jsoncons/jsonpath/jsonpath_selector.h"
...
namespace detail {
    ...
    public:
                ...
                switch (state_stack_.back())
                {
                    ...
                    case path_state::index_or_slice_or_union:
                        switch(*p_)
                        {
                            ...
                            case ']':
                            {
                                ...
                                int64_t n{0};
                                auto r = turbo::detail::to_integer(buffer.data(), buffer.size(), n);
                                if (!r)
                                {
                                    ec = jsonpath_errc::invalid_number;
                                    return path_expression_type(alloc_);
                    ...
                    case path_state::slice_expression_stop:
                    {
                        if (!buffer.empty())
                        {
                            ...
                            slic.stop_ = std::optional<int64_t>(n);
                            buffer.clear();
                        }
                        switch(*p_)
                        {
        ...
        void push_token(turbo::jsonpath::detail::static_resources<value_type,reference>& resources, token_type&& tok, std::error_code& ec)
        {
            //std::cout << tok.to_string() << "\n";
            switch (tok.token_kind())
            {
                ...
                case jsonpath_token_kind::end_argument_expression:
                {
                    ...
                    ++it;
                    output_stack_.erase(it.base(),output_stack_.end());
                    output_stack_.emplace_back(token_type(std::make_unique<expression_type>(std::move(toks))));
                    break;
                }
    ...
    struct jsonpath_traits
    {
        ...
        using allocator_type = typename Json::allocator_type;
        using evaluator_type = typename turbo::jsonpath::detail::jsonpath_evaluator<Json, JsonReference>;
        using json_location_node_type = json_location_node<string_type>;
        using json_location_type = json_location<string_type>;
        using path_expression_type = path_expression<Json,JsonReference>;
    ...
    {
    public:
        using jsonpath_traits_type = turbo::jsonpath::detail::jsonpath_traits<Json, JsonReference>;
    ...
    private:
        allocator_type alloc_;
        std::unique_ptr<turbo::jsonpath::detail::static_resources<value_type,reference>> static_resources_;
        path_expression_type expr_;
    public:
        jsonpath_expression(std::unique_ptr<turbo::jsonpath::detail::static_resources<value_type,reference>>&& resources,
                            path_expression_type&& expr, const allocator_type& alloc = allocator_type())
            : alloc_(alloc),
        ...
        {
            turbo::jsonpath::detail::dynamic_resources<Json,reference> resources{alloc_};
    ...
    jsonpath_expression<Json> make_expression(std::allocator_arg_t, Alloc alloc, const typename Json::string_view_type& path, 
        const custom_functions<Json>& functions = custom_functions<Json>())
    {
        using jsonpath_traits_type = turbo::jsonpath::detail::jsonpath_traits<Json>;
        ...
        auto resources = std::make_unique<turbo::jsonpath::detail::static_resources<value_type,reference>>(functions, alloc);
