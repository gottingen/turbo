...
#include "turbo/jsoncons/jsonpath/jsonpath_utilities.h"
            ...
            for (const auto& node : nodes_)
            {
                switch (node->node_kind())
                {
                    ...
                    case json_location_node_kind::name:
                        buffer.push_back('[');
                        buffer.push_back('\'');
                        turbo::jsonpath::escape_string(node->name().data(), node->name().size(), buffer);
                        buffer.push_back('\'');
                        buffer.push_back(']');
                        break;
                    case json_location_node_kind::index:
                        buffer.push_back('[');
                        turbo::detail::from_integer(node->index(), buffer);
                        buffer.push_back(']');
                        break;
                }
            }
