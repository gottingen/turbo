...
#include "turbo/jsoncons/jsonpath/json_location.h"
#include "turbo/jsoncons/jsonpath/jsonpath_error.h"
...
namespace detail {
    ...
    public:
        ...
        {
            return is_false(val) ? Json(true, turbo::semantic_tag::none) : Json(false, semantic_tag::none);
        }
    };
    ...
    private:
        function_type f_;
    public:
        decorator_function(std::optional<std::size_t> arity,
            const function_type& f)
                            ...
                            std::error_code& ec) const override
        {
            ...
            switch (arg0.type())
            {
                ...
                case json_type::string_value:
                {
                    ...
                    uint64_t un{0};
                    auto result1 = turbo::detail::to_integer(sv.data(), sv.length(), un);
                    if (result1)
                    {
                        return value_type(un, semantic_tag::none);
                    }
                    int64_t sn{0};
                    auto result2 = turbo::detail::to_integer(sv.data(), sv.length(), sn);
                    if (result2)
                    {
    ...
    public:
        ...
        {
            auto temp = std::make_unique<json_location_node_type>(std::forward<Args>(args)...);
    ...
    public:
        using char_type = typename Json::char_type;
        using string_type = typename Json::string_type;
        using string_view_type = turbo::basic_string_view<char_type, std::char_traits<char_type>>;
        using value_type = Json;
        using reference = JsonReference;
    ...
    struct static_resources
    {
            ...
            : static_resources(alloc)
        {
            for (const auto& item : functions)
            {
                custom_functions_.emplace(item.name(),
                                          std::make_unique<decorator_function<Json>>(item.arity(),item.function()));
            }
        }
        ...
        {
            unary_operators_.push_back(std::make_unique<regex_operator<Json,JsonReference>>(std::move(pattern)));
        ...
        {
            selectors_.emplace_back(std::make_unique<T>(std::forward<T>(val)));
    ...
    public:
        ...
        std::optional<std::size_t> arity() const
        {
            return token_kind_ == jsonpath_token_kind::function ? function_->arity() : std::optional<std::size_t>();
        }
        ...
        std::string to_string(int level = 0) const
        {
            ...
            switch (token_kind_)
            {
                ...
                default:
                    ...
                    s.append("token kind: ");
                    s.append(turbo::jsonpath::detail::to_string(token_kind_));
                    break;
            }
            //s.append("\n");
            return s;
