...
#ifndef TURBO_JSONCONS_CSV_CSV_ENCODER_H_
#define TURBO_JSONCONS_CSV_CSV_ENCODER_H_
...
namespace turbo::csv {
    ...
    private:
        static turbo::basic_string_view<CharT> null_constant() {
            static turbo::basic_string_view<CharT> k = JSONCONS_STRING_VIEW_CONSTANT(CharT, "null");
            return k;
        }

        static turbo::basic_string_view<CharT> true_constant() {
            static turbo::basic_string_view<CharT> k = JSONCONS_STRING_VIEW_CONSTANT(CharT, "true");
            return k;
        }

        static turbo::basic_string_view<CharT> false_constant() {
            static turbo::basic_string_view<CharT> k = JSONCONS_STRING_VIEW_CONSTANT(CharT, "false");
            return k;
        }
    ...
    public:
        ...
        basic_csv_encoder(Sink &&sink,
                ...
                : sink_(std::forward<Sink>(sink)),
                  ...
                  column_index_(0) {
            turbo::csv::detail::parse_column_names(options.column_names(), strings_buffer_);
        }
    ...
    private:
        ...
        bool visit_begin_array(semantic_tag, const ser_context &, std::error_code &ec) override {
            ...
            switch (stack_.back().item_kind_) {
                ...
                case stack_item_kind::column: {
                    ...
                    turbo::string_sink<std::basic_string<CharT>> bo(strings_buffer_[row_counts_.back()]);
                    begin_value(bo);
            ...
            switch (stack_.back().item_kind_) {
                ...
                case stack_item_kind::object_multi_valued_field: {
                    ...
                    if (it != buffered_line_.end()) {
                        ...
                        turbo::string_sink<std::basic_string<CharT>> bo(s);
                        write_null_value(bo);
                        bo.flush();
        ...
        bool
        visit_string(const string_view_type &sv, semantic_tag, const ser_context &, std::error_code &) override {
            TURBO_ASSERT(!stack_.empty());
            switch (stack_.back().item_kind_) {
                case stack_item_kind::object:
                case stack_item_kind::object_multi_valued_field: {
        ...
        bool visit_byte_string(const byte_string_view &b,
            ...
            byte_string_chars_format format = turbo::detail::resolve_byte_string_chars_format(encoding_hint,
        ...
        bool do_string_value(const CharT *s, std::size_t length, AnyWriter &sink) {
            bool quote = false;
            if (options_.quote_style() == quote_style_kind::all ||
                options_.quote_style() == quote_style_kind::nonnumeric ||
                (options_.quote_style() == quote_style_kind::minimal &&
                 (std::char_traits<CharT>::find(s, length, options_.field_delimiter()) != nullptr ||
                  std::char_traits<CharT>::find(s, length, options_.quote_char()) != nullptr))) {
                quote = true;
        ...
        void write_int64_value(int64_t val, AnyWriter &sink) {
            ...
            turbo::detail::from_integer(val, sink);
    ...
    using csv_string_encoder = basic_csv_encoder<char, turbo::string_sink<std::string>>;
    using csv_wstream_encoder = basic_csv_encoder<wchar_t>;
    using wcsv_string_encoder = basic_csv_encoder<wchar_t, turbo::string_sink<std::wstring>>;

}  // namespace turbo::csv

#endif  // TURBO_JSONCONS_CSV_CSV_ENCODER_H_
