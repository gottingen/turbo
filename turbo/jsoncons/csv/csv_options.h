...
#ifndef TURBO_JSONCONS_CSV_CSV_OPTIONS_H_
#define TURBO_JSONCONS_CSV_CSV_OPTIONS_H_
...
namespace turbo::csv {
    ...
    namespace detail {
        ...
        void parse_column_types(const std::basic_string<CharT> &types,
            ...
            const std::map<turbo::basic_string_view<CharT>, csv_column_type> type_dictionary =
                    {
                    ...
                    case column_state::sequence: {
                        switch (*p) {
                            ...
                            case '[':
                                ...
                                break;
                            case ']':
                                TURBO_ASSERT(depth > 0);
                                --depth;
                                ++p;
                                break;
                            case '*': {
                                TURBO_ASSERT(column_types.size() != 0);
                                std::size_t offset = 0;
    ...
    public:
        ...
        csv_mapping_kind mapping_kind() const {
            return mapping_ != csv_mapping_kind() ? mapping_ : (assume_header() || this->column_names().size() > 0
...
#endif  // TURBO_JSONCONS_CSV_CSV_OPTIONS_H_
