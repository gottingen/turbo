...
private:
    void initialize()
    {
        turbo::csv::detail::parse_column_names(options_.column_names(), column_names_);
        turbo::csv::detail::parse_column_types(options_.column_types(), column_types_);
        turbo::csv::detail::parse_column_names(options_.column_defaults(), column_defaults_);
    ...
    void end_value_with_numeric_check(std::error_code& ec)
    {
        ...
        switch (state)
        {
            ...
            case numeric_check_state::integer:
            {
                ...
                {
                    int64_t val{ 0 };
                    auto result = turbo::detail::to_integer_decimal(buffer_.data(), buffer_.length(), val);
                    if (result)
                    {
                        more_ = visitor_->int64_value(val, semantic_tag::none, *this, ec);
                    }
                ...
                else
                {
                    ...
                    else if (result.ec == turbo::detail::to_integer_errc::overflow)
                    {
                        more_ = visitor_->string_value(buffer_, semantic_tag::bigint, *this, ec);
                    }
                    else
