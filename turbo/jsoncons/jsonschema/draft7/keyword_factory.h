...
#include "turbo/jsoncons/jsonschema/draft7/schema_draft7.h"
#include "turbo/jsoncons/jsonschema/schema_version.h"
#include <cassert>
#include <set>
#include <sstream>
#include <iostream>
...
namespace draft7 {
    ...
    {
        Json operator()(const turbo::uri& uri)
        {
            ...
            const std::vector<std::string>& keys) override
        {
            ...
            switch (schema.type())
            {
                ...
                case json_type::object_value:
                {
                    ...
                    {
                        auto ref = std::make_unique<type_validator<Json>>(this, schema, new_context);
                        sch = ref.get();
                        subschemas_.emplace_back(std::move(ref));
                    }
                    break;
            ...
            return std::make_unique<reference_validator_type>(sch);
        }
    ...
    private:
        ...
        validator_type get_or_create_reference(const schema_location& uri)
        {
            ...
            if (sch != file.schemas.end())
                return std::make_unique<reference_validator_type>(sch->second);
            ...
            if (ref != file.unresolved.end()) 
            {
                ...
                return std::make_unique<reference_validator_type>(ref->second);
            }
            else 
            {
                auto orig = std::make_unique<reference_schema<Json>>(uri.string());
                ...
                return std::make_unique<reference_validator_type>(p);
            }
        }
