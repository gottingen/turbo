...
#ifndef TURBO_JSONCONS_BSON_BSON_ENCODER_H_
#define TURBO_JSONCONS_BSON_BSON_ENCODER_H_
...
namespace turbo::bson {
    ...
    private:
        struct stack_item {
            turbo::bson::bson_container_type type_;
            std::size_t offset_;
            std::size_t name_offset_;
            std::size_t index_;

            stack_item(turbo::bson::bson_container_type type, std::size_t offset) noexcept
            ...
            bool is_object() const {
                return type_ == turbo::bson::bson_container_type::document;
            }
    ...
    private:
        ...
        bool visit_begin_object(semantic_tag, const ser_context &, std::error_code &ec) override {
            ...
            if (buffer_.size() > 0) {
                ...
                before_value(turbo::bson::bson_type::document_type);
            }

            stack_.emplace_back(turbo::bson::bson_container_type::document, buffer_.size());
        ...
        bool visit_begin_array(semantic_tag, const ser_context &, std::error_code &ec) override {
            ...
            if (buffer_.size() > 0) {
                ...
                before_value(turbo::bson::bson_type::array_type);
            }
            stack_.emplace_back(turbo::bson::bson_container_type::array, buffer_.size());
            buffer_.insert(buffer_.end(), sizeof(int32_t), 0);
            return true;
        }
        ...
        bool visit_null(semantic_tag tag, const ser_context &, std::error_code &ec) override {
            ...
            switch (tag) {
                case semantic_tag::undefined:
                    before_value(turbo::bson::bson_type::undefined_type);
                    break;
                default:
                    before_value(turbo::bson::bson_type::null_type);
                    break;
            }
            return true;
        }
        ...
        bool visit_bool(bool val, semantic_tag, const ser_context &, std::error_code &ec) override {
            ...
            before_value(turbo::bson::bson_type::bool_type);
            if (val) {
        ...
        bool
        visit_string(const string_view_type &sv, semantic_tag tag, const ser_context &, std::error_code &ec) override {
            if (stack_.empty()) {
                ec = bson_errc::expected_bson_document;
                return false;
            }
            ...
            switch (tag) {
                case semantic_tag::float128: {
                    before_value(turbo::bson::bson_type::decimal128_type);
                    decimal128_t dec;
                ...
                case semantic_tag::id: {
                    before_value(turbo::bson::bson_type::object_id_type);
                    oid_t oid(sv);
                    for (auto b: oid) {
                        buffer_.push_back(b);
                    }
                ...
                case semantic_tag::regex: {
                    before_value(turbo::bson::bson_type::regex_type);
                ...
                default:
                    switch (tag) {
                        case semantic_tag::code:
                            before_value(turbo::bson::bson_type::javascript_type);
                            break;
                        default:
                            before_value(turbo::bson::bson_type::string_type);
                            break;
                    }
        ...
        bool visit_byte_string(const byte_string_view &b,
            ...
            before_value(turbo::bson::bson_type::binary_type);
        ...
        bool visit_int64(int64_t val,
            ...
            switch (tag) {
                case semantic_tag::epoch_second:
                    ...
                    before_value(turbo::bson::bson_type::datetime_type);
                ...
                case semantic_tag::epoch_nano:
                    ...
                    return true;
                default: {
                    if (val >= (std::numeric_limits<int32_t>::lowest)() &&
                        val <= (std::numeric_limits<int32_t>::max)()) {
                        before_value(turbo::bson::bson_type::int32_type);
                        binary::native_to_little(static_cast<uint32_t>(val), std::back_inserter(buffer_));
                    } else {
                        before_value(turbo::bson::bson_type::int64_type);
                        binary::native_to_little(static_cast<int64_t>(val), std::back_inserter(buffer_));
                    }
                    return true;
                }
        ...
        bool visit_double(double val,
            ...
            before_value(turbo::bson::bson_type::double_type);
            binary::native_to_little(val, std::back_inserter(buffer_));
            return true;
        }
    ...
    using bson_stream_encoder = basic_bson_encoder<turbo::binary_stream_sink>;
    using bson_bytes_encoder = basic_bson_encoder<turbo::bytes_sink<std::vector<uint8_t>>>;


}  // namespace turbo::bson
#endif  // TURBO_JSONCONS_BSON_BSON_ENCODER_H_
