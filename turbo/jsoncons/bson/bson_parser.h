...
#include "turbo/jsoncons/bson/bson_decimal128.h"
#include "turbo/jsoncons/bson/bson_error.h"
#include "turbo/jsoncons/bson/bson_options.h"
#include "turbo/jsoncons/bson/bson_oid.h"
...
public:
    ...
    {
        if (TURBO_UNLIKELY(source_.is_error()))
        {
        ...
        while (!done_ && more_)
        {
            switch (state_stack_.back().mode)
            {
                ...
                case parse_mode::document:
                {
                    ...
                    if (type != 0x00)
                    {
                        read_e_name(visitor,turbo::bson::bson_container_type::document,ec);
                ...
                case parse_mode::array:
                {
                    ...
                    if (type != 0x00)
                    {
                        read_e_name(visitor,turbo::bson::bson_container_type::array,ec);
...
private:
    ...
    {
        if (TURBO_UNLIKELY(static_cast<int>(state_stack_.size()) > options_.max_nesting_depth()))
        {
        ...
        if (TURBO_UNLIKELY(n != sizeof(int32_t)))
        {
            ec = bson_errc::unexpected_eof;
            more_ = false;
            return;
    ...
    {
        TURBO_ASSERT(state_stack_.size() >= 2);

        more_ = visitor.end_object(*this,ec);
        if (TURBO_UNLIKELY(state_stack_.back().pos != state_stack_.back().length))
        {
    ...
    void read_e_name(json_visitor& visitor, turbo::bson::bson_container_type type, std::error_code& ec)
    {
        text_buffer_.clear();
        read_cstring(ec);
        if (ec)
        ...
        if (type == turbo::bson::bson_container_type::document)
        {
            auto result = unicode_traits::validate(text_buffer_.data(),text_buffer_.size());
            if (TURBO_UNLIKELY(result.ec != unicode_traits::conv_errc()))
            {
    ...
    void read_value(json_visitor& visitor, uint8_t type, std::error_code& ec)
    {
        switch (type)
        {
            case turbo::bson::bson_type::double_type:
            {
                uint8_t buf[sizeof(double)]; 
                std::size_t n = source_.read(buf, sizeof(double));
                state_stack_.back().pos += n;
                if (TURBO_UNLIKELY(n != sizeof(double)))
                {
                    ec = bson_errc::unexpected_eof;
                    more_ = false;
                    return;
            ...
            case turbo::bson::bson_type::symbol_type:
            case turbo::bson::bson_type::min_key_type:
            case turbo::bson::bson_type::max_key_type:
            case turbo::bson::bson_type::string_type:
            {
                text_buffer_.clear();
                read_string(ec);
                if (ec)
            ...
            case turbo::bson::bson_type::javascript_type:
            {
                text_buffer_.clear();
                read_string(ec);
                if (ec)
            ...
            case turbo::bson::bson_type::regex_type:
            {
                text_buffer_.clear();
            ...
            case turbo::bson::bson_type::document_type:
            {
                begin_document(visitor,ec);
                break;
            }

            case turbo::bson::bson_type::array_type:
            {
                begin_array(visitor,ec);
                break;
            }
            case turbo::bson::bson_type::undefined_type:
                {
                    more_ = visitor.null_value(semantic_tag::undefined, *this, ec);
                    break;
                }
            case turbo::bson::bson_type::null_type:
            {
                more_ = visitor.null_value(semantic_tag::none, *this, ec);
                break;
            }
            case turbo::bson::bson_type::bool_type:
            {
                uint8_t c;
            ...
            case turbo::bson::bson_type::int32_type:
            {
                uint8_t buf[sizeof(int32_t)]; 
                std::size_t n = source_.read(buf, sizeof(int32_t));
                state_stack_.back().pos += n;
            ...
            case turbo::bson::bson_type::timestamp_type:
            {
                uint8_t buf[sizeof(uint64_t)]; 
                std::size_t n = source_.read(buf, sizeof(uint64_t));
                state_stack_.back().pos += n;
                if (TURBO_UNLIKELY(n != sizeof(uint64_t)))
                {
                    ec = bson_errc::unexpected_eof;
                    more_ = false;
                    return;
            ...
            case turbo::bson::bson_type::int64_type:
            {
                uint8_t buf[sizeof(int64_t)]; 
                std::size_t n = source_.read(buf, sizeof(int64_t));
                state_stack_.back().pos += n;
                if (TURBO_UNLIKELY(n != sizeof(int64_t)))
                {
                    ec = bson_errc::unexpected_eof;
                    more_ = false;
                    return;
            ...
            case turbo::bson::bson_type::datetime_type:
            {
                uint8_t buf[sizeof(int64_t)]; 
                std::size_t n = source_.read(buf, sizeof(int64_t));
                state_stack_.back().pos += n;
            ...
            case turbo::bson::bson_type::binary_type:
            {
                uint8_t buf[sizeof(int32_t)]; 
                std::size_t n = source_.read(buf, sizeof(int32_t));
                state_stack_.back().pos += n;
                ...
                state_stack_.back().pos += n;
                if (TURBO_UNLIKELY(n != static_cast<std::size_t>(len)))
                {
                    ec = bson_errc::unexpected_eof;
                    more_ = false;
                    return;
            ...
            case turbo::bson::bson_type::decimal128_type:
            {
                uint8_t buf[sizeof(uint64_t)*2]; 
                std::size_t n = source_.read(buf, sizeof(buf));
                state_stack_.back().pos += n;
                if (TURBO_UNLIKELY(n != sizeof(buf)))
                {
                    ec = bson_errc::unexpected_eof;
                    more_ = false;
                    return;
            ...
            case turbo::bson::bson_type::object_id_type:
            {
                uint8_t buf[12]; 
                std::size_t n = source_.read(buf, sizeof(buf));
                state_stack_.back().pos += n;
