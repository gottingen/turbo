...
namespace turbo {
                        ...
                        if (escape_solidus && c == '/') {
                            ...
                            if (is_non_ascii_codepoint(cp) || is_control_character(c)) {
                                if (cp > 0xFFFF) {
                                    ...
                                    sink.push_back('\\');
                                    sink.push_back('u');
                                    sink.push_back(turbo::detail::to_hex_character(first >> 12 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(first >> 8 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(first >> 4 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(first & 0x000F));
                                    sink.push_back('\\');
                                    sink.push_back('u');
                                    sink.push_back(turbo::detail::to_hex_character(second >> 12 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(second >> 8 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(second >> 4 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(second & 0x000F));
                                    count += 12;
                                } else {
                                    sink.push_back('\\');
                                    sink.push_back('u');
                                    sink.push_back(turbo::detail::to_hex_character(cp >> 12 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(cp >> 8 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(cp >> 4 & 0x000F));
                                    sink.push_back(turbo::detail::to_hex_character(cp & 0x000F));
                                    count += 6;
                                }
                            } else {
                                sink.push_back(c);
    ...
    template<class CharT, class Sink=turbo::stream_sink<CharT>, class Allocator=std::allocator<char>>
    class basic_json_encoder final : public basic_json_visitor<CharT> {
        static const turbo::basic_string_view<CharT> null_constant() {
            static const turbo::basic_string_view<CharT> k = JSONCONS_STRING_VIEW_CONSTANT(CharT, "null");
            return k;
        }

        static const turbo::basic_string_view<CharT> true_constant() {
            static const turbo::basic_string_view<CharT> k = JSONCONS_STRING_VIEW_CONSTANT(CharT, "true");
            return k;
        }

        static const turbo::basic_string_view<CharT> false_constant() {
            static const turbo::basic_string_view<CharT> k = JSONCONS_STRING_VIEW_CONSTANT(CharT, "false");
            return k;
        }
    ...
    private:
        ...
        public:
            encoding_context(container_type type, line_split_kind split_lines, bool indent_once,
                    ...
                    : type_(type), count_(0), line_splits_(split_lines), indent_before_(indent_once),
    ...
    private:
        ...
        bool visit_key(const string_view_type &name, const ser_context &, std::error_code &) override {
            ...
            sink_.push_back('\"');
            std::size_t length = turbo::detail::escape_string(name.data(), name.length(),
        ...
        visit_string(const string_view_type &sv, semantic_tag tag, const ser_context &, std::error_code &) override {
            ...
            switch (tag) {
                ...
                default: {
                    sink_.push_back('\"');
                    std::size_t length = turbo::detail::escape_string(sv.data(), sv.length(),
                                                                      options_.escape_all_non_ascii(),
                                                                      options_.escape_solidus(), sink_);
                    sink_.push_back('\"');
                    column_ += (length + 2);
        ...
        bool visit_byte_string(const byte_string_view &b,
            ...
            byte_string_chars_format format = turbo::detail::resolve_byte_string_chars_format(
                    options_.byte_string_format(),
                    encoding_hint,
        ...
        bool visit_int64(int64_t value,
            ...
            std::size_t length = turbo::detail::from_integer(value, sink_);
            column_ += length;
            end_value();
            return true;
        }
    ...
    private:
        ...
        bool visit_key(const string_view_type &name, const ser_context &, std::error_code &) override {
            ...
            sink_.push_back('\"');
            turbo::detail::escape_string(name.data(), name.length(), options_.escape_all_non_ascii(),
                                         options_.escape_solidus(), sink_);
            sink_.push_back('\"');
            sink_.push_back(':');
            return true;
        ...
        visit_string(const string_view_type &sv, semantic_tag tag, const ser_context &, std::error_code &) override {
            ...
            switch (tag) {
                ...
                default: {
                    sink_.push_back('\"');
                    turbo::detail::escape_string(sv.data(), sv.length(), options_.escape_all_non_ascii(),
                                                 options_.escape_solidus(), sink_);
                    sink_.push_back('\"');
                    break;
                }
        ...
        bool visit_double(double value,
            ...
            if (TURBO_UNLIKELY(!std::isfinite(value))) {
                if ((std::isnan)(value)) {
                    if (options_.enable_nan_to_num()) {
                        sink_.append(options_.nan_to_num().data(), options_.nan_to_num().length());
                    } else if (options_.enable_nan_to_str()) {
        ...
        bool visit_int64(int64_t value,
            ...
            turbo::detail::from_integer(value, sink_);
            if (!stack_.empty()) {
                stack_.back().increment_count();
            }
            return true;
    ...
    using json_stream_encoder = basic_json_encoder<char, turbo::stream_sink<char>>;
    using wjson_stream_encoder = basic_json_encoder<wchar_t, turbo::stream_sink<wchar_t>>;
    using compact_json_stream_encoder = basic_compact_json_encoder<char, turbo::stream_sink<char>>;
    using compact_wjson_stream_encoder = basic_compact_json_encoder<wchar_t, turbo::stream_sink<wchar_t>>;

    using json_string_encoder = basic_json_encoder<char, turbo::string_sink<std::string>>;
    using wjson_string_encoder = basic_json_encoder<wchar_t, turbo::string_sink<std::wstring>>;
    using compact_json_string_encoder = basic_compact_json_encoder<char, turbo::string_sink<std::string>>;
    using compact_wjson_string_encoder = basic_compact_json_encoder<wchar_t, turbo::string_sink<std::wstring>>;

#if !defined(JSONCONS_NO_DEPRECATED)
    template<class CharT, class Sink=turbo::stream_sink<CharT>>
    ...
    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_json_stream_encoder") typedef basic_compact_json_encoder<char, turbo::stream_sink<char>> json_compressed_stream_encoder;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_wjson_stream_encoder")typedef basic_compact_json_encoder<wchar_t, turbo::stream_sink<wchar_t>> wjson_compressed_stream_encoder;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_json_string_encoder") typedef basic_compact_json_encoder<char, turbo::string_sink<char>> json_compressed_string_encoder;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_wjson_string_encoder")typedef basic_compact_json_encoder<wchar_t, turbo::string_sink<wchar_t>> wjson_compressed_string_encoder;
    ...
    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_json_stream_encoder") typedef compact_json_stream_encoder compact_json_encoder;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_wjson_stream_encoder") typedef compact_wjson_stream_encoder wcompact_json_encoder;

    JSONCONS_DEPRECATED_MSG(
            "Instead, use json_stream_encoder") typedef basic_json_encoder<char, turbo::stream_sink<char>> json_serializer;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use wjson_stream_encoder") typedef basic_json_encoder<wchar_t, turbo::stream_sink<wchar_t>> wjson_serializer;

    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_json_stream_encoder")  typedef basic_compact_json_encoder<char, turbo::stream_sink<char>> json_compressed_serializer;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_wjson_stream_encoder") typedef basic_compact_json_encoder<wchar_t, turbo::stream_sink<wchar_t>> wjson_compressed_serializer;

    JSONCONS_DEPRECATED_MSG(
            "Instead, use json_string_encoder")  typedef basic_json_encoder<char, turbo::string_sink<std::string>> json_string_serializer;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use wjson_string_encoder") typedef basic_json_encoder<wchar_t, turbo::string_sink<std::wstring>> wjson_string_serializer;

    JSONCONS_DEPRECATED_MSG(
            "Instead, use compact_json_string_encoder")  typedef basic_compact_json_encoder<char, turbo::string_sink<std::string>> json_compressed_string_serializer;
    JSONCONS_DEPRECATED_MSG(
            "Instead, use wcompact_json_string_encoder") typedef basic_compact_json_encoder<wchar_t, turbo::string_sink<std::wstring>> wjson_compressed_string_serializer;
#endif
