...
#include "turbo/log/logging.h"          // To pick up flag settings etc.
...
// Helper for RawLog__ below.
    ...
    void RawLog__(log_severity severity, const char *file, int line,
                  const char *format, ...) {
        if (!(FLAGS_turbo_logtostderr || severity >= FLAGS_turbo_stderrthreshold ||
              FLAGS_turbo_also_logtostderr || !is_logging_initialized())) {
        ...
        DoRawLog(&buf, &size, "%c00000000 00:00:00.000000 %5u %s:%d] RAW: ",
                 ...
                 static_cast<unsigned int>(turbo::sysinfo::get_tid()),
        ...
        if (no_chop) {
            ...
            DoRawLog(&buf, &size, "TURBO_RAW_LOG ERROR: The Message was too long!\n");
        }
...
#ifdef HAVE_STACKTRACE
                crash_reason.depth =
                        turbo::debugging::get_stack_trace(crash_reason.stack, TURBO_ARRAY_SIZE(crash_reason.stack), 1);
#else
