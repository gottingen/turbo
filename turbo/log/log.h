...
// Example:
//
//   LOG(LEVEL(stale ? turbo::LogSeverity::kWarning : turbo::LogSeverity::kInfo))
...
//
//   * .AtLocation(turbo::string_view file, int line)
//     .AtLocation(turbo::SourceLocation loc)
...
//
//   LOG(INFO) << "Found " << num_cookies << " cookies";
#define LOG(severity) TURBO_LOG_INTERNAL_LOG_IMPL(_##severity)

// PLOG()
//
// `PLOG` behaves like `LOG` except that a description of the current state of
// `errno` is appended to the streamed message.
#define PLOG(severity) TURBO_LOG_INTERNAL_PLOG_IMPL(_##severity)
...
#define DLOG(severity) TURBO_LOG_INTERNAL_DLOG_IMPL(_##severity)
...
// using `DVLOG` instead.
#define VLOG(severity) TURBO_LOG_INTERNAL_VLOG_IMPL(severity)

// `DVLOG` behaves like `VLOG` in debug mode (i.e. `#ifndef NDEBUG`).
// Otherwise, it compiles away and does nothing.
#define DVLOG(severity) TURBO_LOG_INTERNAL_DVLOG_IMPL(severity)
