...
namespace turbo::tlog {
    namespace details {
        ...
        void file_helper::open(const filename_t &fname, bool truncate) {
            ...
            auto *mode = TLOG_FILENAME_T("ab");
            auto *trunc_mode = TLOG_FILENAME_T("wb");
            ...
            throw_tlog_ex("Failed opening file " + os::filename_to_str(filename_) + " for writing", errno);
        }

        void file_helper::reopen(bool truncate) {
            if (filename_.empty()) {
                throw_tlog_ex("Failed re opening file - was not opened before");
            }
        ...
        void file_helper::flush() {
            if (std::fflush(fd_) != 0) {
                throw_tlog_ex("Failed flush to file " + os::filename_to_str(filename_), errno);
            }
        }
        ...
        void file_helper::write(const memory_buf_t &buf) {
            ...
            if (std::fwrite(data, 1, msg_size, fd_) != msg_size) {
                throw_tlog_ex("Failed writing to file " + os::filename_to_str(filename_), errno);
            }
        }

        size_t file_helper::size() const {
            if (fd_ == nullptr) {
                throw_tlog_ex("Cannot use size() on closed file " + os::filename_to_str(filename_));
            }
