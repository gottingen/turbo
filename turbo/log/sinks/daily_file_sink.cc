...
DailyFileSink::DailyFileSink(const turbo::filesystem::path &file_name,
    ...
    : base_filename_(file_name), rotation_h_(rotation_hour),
      rotation_m_(rotation_minute), truncate_(truncate), max_files_(max_files) {
...
turbo::filesystem::path DailyFileSink::calc_filename(const turbo::Time &t) {
  ...
  return turbo::StrFormat("%s_%4d-%02d-%02d.%s", base_filename_.filename(),
                          civil_day.year(), civil_day.month(), civil_day.day(),
                          base_filename_.extension());
}

turbo::Status DailyFileSink::init() {
  if (rotation_h_ < 0 || rotation_h_ > 23 || rotation_m_ < 0 ||
      rotation_m_ > 59) {
    return turbo::Status(turbo::kInvalidArgument,
  ...
  CivilSecond next_tp(civil_now.year(), civil_now.month(), civil_now.day(),
                      rotation_h_, rotation_m_, 0);
...
void DailyFileSink::Flush() { log_file_->flush(); }
