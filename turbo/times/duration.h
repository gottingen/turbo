...
// Additive Operators
    ...
    // duration
    //
    // The `turbo::duration` class represents a signed, fixed-length span of time.
    // A `duration` is generated using a unit-specific factory function, or is
    // the result of subtracting one `turbo::time_point` from another. Durations behave
    ...
    public:
        ...
        //   auto z = turbo::infinite_duration().to_chrono_seconds();
        ...
        // Example:
        //
        //   double d = turbo::float_div_duration(turbo::milliseconds(1500), turbo::seconds(1));
        //   // d == 1.5
        ...
        //   turbo::duration a = turbo::trunc(turbo::microseconds(1));  // 123456us
        ...
        //
        //   turbo::duration d = turbo::nanoseconds(123456789);
        //   turbo::duration b = turbo::floor(turbo::microseconds(1));  // 123456us
        ...
        //
        //   turbo::duration d = turbo::nanoseconds(123456789);
        //   turbo::duration c = turbo::ceil(turbo::microseconds(1));   // 123457us
    ...
    public:
        ...
        // `turbo::duration::hours(24)`. If you actually want a civil day, use turbo::chrono_day
        // from civil_time.h.
        //
        // Example:
        //
    ...
    public:
        ...
        //note that these interface use by turbo internal
    ...
    namespace times_internal {
        TURBO_FORCE_INLINE bool is_valid_divisor(double d) {
        ...
        TURBO_FORCE_INLINE void normalize_ticks(int64_t *sec, int64_t *ticks) {
            if (*ticks < 0) {
                --*sec;
        ...
        TURBO_FORCE_INLINE uint64_t encode_twos_comp(int64_t v) {
            return turbo::base::bit_cast<uint64_t>(v);
        }

        TURBO_FORCE_INLINE int64_t decode_twos_comp(uint64_t v) { return turbo::base::bit_cast<int64_t>(v); }
    ...
    TURBO_FORCE_INLINE duration operator+(duration lhs, duration rhs) { return lhs += rhs; }
    ...
    TURBO_FORCE_INLINE int64_t operator/(duration lhs, duration rhs) {
    ...
    //   int64_t q = turbo::integer_div_duration(a, b, &rem);
    //   // q == std::numeric_limits<int64_t>::max(), rem == a - b * q
    TURBO_FORCE_INLINE int64_t duration::integer_div_duration(duration num, duration den, duration *rem) {
    ...
    TURBO_FORCE_INLINE duration abs_duration(duration d) {
    ...
    TURBO_FORCE_INLINE std::ostream &operator<<(std::ostream &os, duration d) {
...
// Makes a uint128 count of ticks out of the absolute value of the duration.
    TURBO_FORCE_INLINE uint128 duration::make_uint128_ticks() const {
...
// Breaks a uint128 of ticks into a duration.
    TURBO_FORCE_INLINE duration duration::make_duration_from_uint128(uint128 u128, bool is_neg) {
        int64_t rep_hi;
        uint32_t rep_lo;
...
// Converts a std::chrono::duration to an turbo::duration.
    ...
    TURBO_FORCE_INLINE bool duration::safe_add_rep_hi(double a_hi, double b_hi) {
        double c = a_hi + b_hi;
    ...
    template<template<typename> class Operation>
    TURBO_FORCE_INLINE duration duration::scale_fixed(int64_t r) {
    ...
    template<template<typename> class Operation>
    TURBO_FORCE_INLINE duration duration::scale_double(double r) const {
        Operation<double> op;
        ...
        int64_t lo64 = turbo::base::round(lo_frac * times_internal::kTicksPerSecond);
        duration ans;
    ...
    TURBO_FORCE_INLINE bool duration::i_div_fast_path(const duration den, int64_t *q,
                                                      duration *rem) const {
...
// Fastpath implementations for the 6 common duration units.
    TURBO_FORCE_INLINE int64_t duration::to_int64(std::nano) const {
        return to_int64_nanoseconds();
    }

    TURBO_FORCE_INLINE int64_t duration::to_int64(std::micro) const {
        return to_int64_microseconds();
    }

    TURBO_FORCE_INLINE int64_t duration::to_int64(std::milli) const {
        return to_int64_milliseconds();
    }

    TURBO_FORCE_INLINE int64_t duration::to_int64(std::ratio<1>) const {
        return to_int64_seconds();
    }

    TURBO_FORCE_INLINE int64_t duration::to_int64(std::ratio<60>) const {
        return to_int64_minutes();
    }

    TURBO_FORCE_INLINE int64_t duration::to_int64(std::ratio<3600>) const {
