...
#include "turbo/times/internal/time_zone_libc.h"
#include <chrono>
#include <ctime>
#include <limits>
#include <utility>
...
// Adapt to different spellings of the struct std::tm extension fields.
        ...
        TURBO_FORCE_INLINE std::tm *gm_time(const std::time_t *timep, std::tm *result) {
#if defined(_WIN32) || defined(_WIN64)
        ...
        TURBO_FORCE_INLINE std::tm *local_time(const std::time_t *timep, std::tm *result) {
#if defined(_WIN32) || defined(_WIN64)
...
// (1) lo doesn't match, (2) hi does, and (3) there is only one transition.
    ...
    time_zone::civil_lookup time_zone_libc::make_time(const civil_second &cs) const {
        ...
        std::time_t t0, t1;
        int offset0, offset1;
        if (::turbo::times_internal::make_time(cs, 0, &t0, &offset0) &&
            ::turbo::times_internal::make_time(cs, 1, &t1, &offset1)) {
            if (t0 == t1) {
