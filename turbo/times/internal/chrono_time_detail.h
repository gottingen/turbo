 ...
 *****************************************************************/
#ifndef TURBO_TIMES_INTERNAL_CHRONO_TIME_DETAIL_H_
#define TURBO_TIMES_INTERNAL_CHRONO_TIME_DETAIL_H_
...
namespace turbo::times_internal {
    ...
    namespace times_detail {
        ...
        namespace impl {
            ...
            constexpr int days_per_century(year_t y, month_t m) noexcept {
                const int yi = year_index(y, m);
                return 36524 + (yi == 0 || yi > 300);
            }

            constexpr int days_per_4years(year_t y, month_t m) noexcept {
                const int yi = year_index(y, m);
        ...
        namespace impl {
            ...
            template<typename H>
            H turbo_hash_value_impl(second_tag, H h, fields f) {
                return H::combine(std::move(h), f.y, f.m, f.d, f.hh, f.mm, f.ss);
            }

            template<typename H>
            H turbo_hash_value_impl(minute_tag, H h, fields f) {
                return H::combine(std::move(h), f.y, f.m, f.d, f.hh, f.mm);
            }

            template<typename H>
            H turbo_hash_value_impl(hour_tag, H h, fields f) {
                return H::combine(std::move(h), f.y, f.m, f.d, f.hh);
            }

            template<typename H>
            H turbo_hash_value_impl(day_tag, H h, fields f) {
                return H::combine(std::move(h), f.y, f.m, f.d);
            }

            template<typename H>
            H turbo_hash_value_impl(month_tag, H h, fields f) {
                return H::combine(std::move(h), f.y, f.m);
            }

            template<typename H>
            H turbo_hash_value_impl(year_tag, H h, fields f) {
...
#endif  // TURBO_TIMES_INTERNAL_CHRONO_TIME_DETAIL_H_
