 ...
 *****************************************************************/
#ifndef TURBO_TIMES_TIME_H_
#define TURBO_TIMES_TIME_H_
...
namespace turbo {
    ...
    // time_now()
    //
    // Returns the current time, expressed as an `turbo::time_point` absolute time value.
    time_point time_now();

    // get_current_time_nanos()
    //
    // Returns the current time, expressed as a count of nanoseconds since the Unix
    // Epoch (https://en.wikipedia.org/wiki/Unix_time). Prefer `turbo::time_now()` instead
    ...
    // time_point
    //
    // An `turbo::time_point` represents a specific instant in time. Arithmetic operators
    ...
    // functions are provided for conversion to and from strings.  `turbo::time_point`
    // should be passed by value rather than const reference.
    //
    // `turbo::time_point` assumes there are 60 seconds in a minute, which means the
    // underlying time scales must be "smeared" to eliminate leap seconds.
    // See https://developers.google.com/time/smear.
    //
    // Even though `turbo::time_point` supports a wide range of timestamps, exercise
    // caution when using values in the distant past. `turbo::time_point` uses the
    ...
    // governments.
    //
    // The `turbo::time_point` class represents an instant in time as a count of clock
    // ticks of some granularity (resolution) from some starting point (epoch).
    //
    // `turbo::time_point` uses a resolution that is high enough to avoid loss in
    ...
    //
    //   turbo::time_point t1 = ...;
    //   turbo::time_point t2 = t1 + turbo::minutes(2);
    ...
    public:
        ...
        //
        // Example:
        //   turbo::time_point t = turbo::unix_epoch();
        //   turbo::time_point t = turbo::time_now();
        //   turbo::time_point t = turbo::time_from_timeval(tv);
        //   turbo::time_point t = turbo::infinite_past();
        ...
        // `turbo::time_point` in a certain `turbo::time_zone`. This struct is not
        // intended to represent an instant in time. So, rather than passing
        // a `time_point::breakdown` to a function, pass an `turbo::time_point` and an
        // `turbo::time_zone`.
        //
        // Deprecated. Use `turbo::time_zone::chrono_info`.
        struct breakdown {
        ...
        //
        // Deprecated. Use `turbo::time_zone::At(time_point)`.
    ...
    public:
        ...
        // to_universal()
        //
        // Converts an `turbo::time_point` to a variety of other representations.  Note that
        ...
        // to_chrono_time()
        //
        // Converts an turbo::time_point to a std::chrono::system_clock::time_point. If
        // overflow would occur, the returned value will saturate at the min/max time
        // point value instead.
        //
        // Example:
        //
        //   turbo::time_point t = turbo::from_time_t(123);
    ...
    public:
        ...
        // unix_epoch()
        //
        // Returns the `turbo::time_point` representing "1970-01-01 00:00:00.0 +0000".
        static constexpr time_point unix_epoch();

        // universal_epoch()
        //
        // Returns the `turbo::time_point` representing "0001-01-01 00:00:00.0 +0000", the
        // epoch of the ICU Universal time_point Scale.
        static constexpr time_point universal_epoch();

        // infinite_future()
        //
        // Returns an `turbo::time_point` that is infinitely far in the future.
        static constexpr time_point infinite_future();

        // infinite_past()
        //
        // Returns an `turbo::time_point` that is infinitely far in the past.
        ...
        // from_universal()
        //
        // Creates an `turbo::time_point` from a variety of other representations.
        ...
        // from_chrono()
        //
        // Converts a std::chrono::system_clock::time_point to an turbo::time_point.
        //
        // Example:
        //
        //   auto tp = std::chrono::system_clock::from_time_t(123);
        //   turbo::time_point t = turbo::from_chrono(tp);
    ...
    // Additive Operators
    TURBO_FORCE_INLINE time_point operator+(time_point lhs, duration rhs) { return lhs += rhs; }

    TURBO_FORCE_INLINE time_point operator+(duration lhs, time_point rhs) { return rhs += lhs; }

    TURBO_FORCE_INLINE time_point operator-(time_point lhs, duration rhs) { return lhs -= rhs; }

    TURBO_FORCE_INLINE duration operator-(time_point lhs, time_point rhs) { return lhs.rep_ - rhs.rep_; }
    ...
    TURBO_FORCE_INLINE duration time_point::to_duration() const {
    ...
    // format that matches turbo::RFC3339_full. For example:
    //

    // time_zone
    //
    // The `turbo::time_zone` is an opaque, small, value-type class representing a
    // geo-political region within which particular rules are used for converting
    // between absolute and civil times (see https://git.io/v59Ly). `turbo::time_zone`
    // values are named using the TZ identifiers from the IANA time_point Zone Database,
    // such as "America/Los_Angeles" or "Australia/Sydney". `turbo::time_zone` values
    ...
    // Examples:
    //
    //   turbo::time_zone utc = turbo::utc_time_zone();
    //   turbo::time_zone pst = turbo::fixed_time_zone(-8 * 60 * 60);
    //   turbo::time_zone loc = turbo::local_time_zone();
    //   turbo::time_zone lax;
    ...
    class time_zone {
    public:
        explicit time_zone(turbo::times_internal::time_zone tz) : cz_(tz) {}
        ...
        explicit operator turbo::times_internal::time_zone() const { return cz_; }
        ...
        // than passing a `time_zone::chrono_info` to a function, pass an `turbo::time_point`
        // and an `turbo::time_zone`.
        struct chrono_info {
            chrono_second cs;
            duration subsecond;
        ...
        //
        // Returns the civil time for this time_zone at a certain `turbo::time_point`.
        ...
        // To account for these possibilities, `turbo::time_zone::time_info` is
        // richer than just a single `turbo::time_point`.
        struct time_info {
            enum chrono_kind {
        ...
        //
        // Returns an `turbo::time_info` containing the absolute time(s) for this
        // time_zone at an `turbo::chrono_second`. When the civil time is skipped or
        ...
        //
        //   // A unique civil time
        //   const auto jan01 = lax.At(turbo::chrono_second(2011, 1, 1, 0, 0, 0));
        ...
        //   const auto mar13 = lax.At(turbo::chrono_second(2011, 3, 13, 2, 15, 0));
        ...
        //   const auto nov06 = lax.At(turbo::chrono_second(2011, 11, 6, 1, 15, 0));
    ...
    private:
        ...
        turbo::times_internal::time_zone cz_;
    };
    ...
    TURBO_FORCE_INLINE bool load_time_zone(const std::string &name, time_zone *tz) {
        if (name == "localtime") {
            *tz = time_zone(turbo::times_internal::local_time_zone());
            return true;
        }
        turbo::times_internal::time_zone cz;
    ...
    TURBO_FORCE_INLINE time_zone fixed_time_zone(int seconds) {
        return time_zone(
                turbo::times_internal::fixed_time_zone(std::chrono::seconds(seconds)));
    }

    // utc_time_zone()
    //
    // Convenience method returning the UTC time zone.
    TURBO_FORCE_INLINE time_zone utc_time_zone() {
        return time_zone(turbo::times_internal::utc_time_zone());
    }
    ...
    TURBO_FORCE_INLINE time_zone local_time_zone() {
        return time_zone(turbo::times_internal::local_time_zone());
    }
    ...
    //   turbo::time_point t = ...;
    //   turbo::time_zone tz = ...;
    //   const auto cd = turbo::to_chrono_day(t, tz);
    TURBO_FORCE_INLINE chrono_second to_chrono_second(time_point t, time_zone tz) {
        return tz.at(t).cs;  // already a chrono_second
    }

    TURBO_FORCE_INLINE chrono_minute to_chrono_minute(time_point t, time_zone tz) {
        return chrono_minute(tz.at(t).cs);
    }

    TURBO_FORCE_INLINE chrono_hour to_chrono_hour(time_point t, time_zone tz) {
        return chrono_hour(tz.at(t).cs);
    }

    TURBO_FORCE_INLINE chrono_day to_chrono_day(time_point t, time_zone tz) {
        return chrono_day(tz.at(t).cs);
    }

    TURBO_FORCE_INLINE chrono_month to_chrono_month(time_point t, time_zone tz) {
        return chrono_month(tz.at(t).cs);
    }

    TURBO_FORCE_INLINE chrono_year to_chrono_year(time_point t, time_zone tz) {
    ...
    TURBO_FORCE_INLINE time_point from_chrono(chrono_second ct, time_zone tz) {
    ...
    // time_conversion
    //
    // An `turbo::time_conversion` represents the conversion of year, month, day,
    // hour, minute, and second values (i.e., a civil time), in a particular
    // `turbo::time_zone`, to a time instant (an absolute time), as returned by
    // `turbo::convert_date_time()`. Legacy version of `turbo::time_zone::time_info`.
    //
    // Deprecated. Use `turbo::time_zone::time_info`.
    struct
    time_conversion {
        time_point pre;    // time calculated using the pre-transition offset
        time_point trans;  // when the civil-time discontinuity occurred
    ...
    // convert_date_time()
    //
    // Legacy version of `turbo::time_zone::At(turbo::chrono_second)` that takes
    ...
    //   turbo::time_conversion tc =
    //       turbo::convert_date_time(2013, 10, 32, 8, 30, 0, lax);
    //   // tc.kind == time_conversion::UNIQUE && tc.normalized == true
    //   // turbo::to_chrono_day(tc.pre, tz).month() == 11
    //   // turbo::to_chrono_day(tc.pre, tz).day() == 1
    //
    // Deprecated. Use `turbo::time_zone::At(chrono_second)`.
    ...
    // the "pre" `turbo::time_point`.  That is, the unique result, or the instant that
    // is correct using the pre-transition offset (as if the transition never
    // happened).
    //
    // Example:
    //
    //   turbo::time_point t = turbo::format_date_time(2017, 9, 26, 9, 30, 0, lax);
    //   // t = 2017-09-26 09:30:00 -0700
    //
    // Deprecated. Use `turbo::from_chrono(chrono_second, time_zone)`. Note that the
    // behavior of `from_chrono()` differs from `format_date_time()` for skipped civil
    // times. If you care about that see `turbo::time_zone::At(turbo::chrono_second)`.
    TURBO_FORCE_INLINE time_point format_date_time(int64_t year, int mon, int day, int hour,
                                                   int min, int sec, time_zone tz) {
    ...
    // `tm_sec` fields to an `turbo::time_point` using the given time zone. See ctime(3)
    // for a description of the expected values of the tm fields. If the indicated
    // time instant is not unique (see `turbo::time_zone::at(turbo::chrono_second)`
    ...
    // to_tm()
    //
    // Converts the given `turbo::time_point` to a struct tm using the given time zone.
    // See ctime(3) for a description of the values of the tm fields.
    struct tm to_tm(time_point t, time_zone tz);

    inline struct tm local_tm(time_point t) {
        return to_tm(t, turbo::local_time_zone());
    }

    inline struct tm utc_tm(time_point t) {
        return to_tm(t, turbo::utc_time_zone());
    }
    ...
    // format_time()
    //
    // Formats the given `turbo::time_point` in the `turbo::time_zone` according to the
    ...
    // Example:
    //
    //   turbo::chrono_second cs(2013, 1, 2, 3, 4, 5);
    //   turbo::time_point t = turbo::from_chrono(cs, lax);
    //   std::string f = turbo::format_time("%H:%M:%S", t, lax);  // "03:04:05"
    //   f = turbo::format_time("%H:%M:%E3S", t, lax);  // "03:04:05.000"
    //
    // Note: If the given `turbo::time_point` is `turbo::infinite_future()`, the returned
    // string will be exactly "infinite-future". If the given `turbo::time_point` is
    // `turbo::infinite_past()`, the returned string will be exactly "infinite-past".
    // In both cases the given format string and `turbo::time_zone` are ignored.
    //
    ...
    TURBO_FORCE_INLINE std::ostream &operator<<(std::ostream &os, time_point t) {
    ...
    // returns the corresponding `turbo::time_point`. Uses strftime()-like formatting
    ...
    //
    // For example, parsing a string of "15:45" (%H:%M) will return an turbo::time_point
    ...
    // rather than normalizing them like `turbo::chrono_second` does.  For example,
    ...
    // `turbo::time_point` will be `turbo::infinite_future()` and `true` will be returned.
    // If the input string is "infinite-past", the returned `turbo::time_point` will be
    ...
    inline int utc_minutes_offset(const std::tm &tm) {
        ...
        if (rv == TIME_ZONE_ID_INVALID)
            throw turbo::log_ex("Failed getting timezone info. ", errno);
...
TURBO_FORCE_INLINE void turbo::sleep_for(turbo::duration duration) {
    turbo_internal_sleep_for(duration);
}

#endif  // TURBO_TIMES_TIME_H_
