...
//   const turbo::chrono_day cd(1969, 07, 20);
//
//   // Construct a civil-time object for a specific second
//   const turbo::chrono_second cd(2018, 8, 1, 12, 0, 1);
//
...
//
//   // Valid in C++14
//   constexpr turbo::chrono_day cd(1969, 07, 20);

#ifndef TURBO_TIMES_CIVIL_TIME_H_
#define TURBO_TIMES_CIVIL_TIME_H_
...
namespace turbo {
    ...
    namespace times_internal {
        struct second_tag : turbo::times_internal::times_detail::second_tag {
        };
        struct minute_tag : second_tag, turbo::times_internal::times_detail::minute_tag {
        };
        struct hour_tag : minute_tag, turbo::times_internal::times_detail::hour_tag {
        };
        struct day_tag : hour_tag, turbo::times_internal::times_detail::day_tag {
        };
        struct month_tag : day_tag, turbo::times_internal::times_detail::month_tag {
        };
        struct year_tag : month_tag, turbo::times_internal::times_detail::year_tag {
        };
    }  // namespace times_internal
    ...
    // Examples:
    //
    //   turbo::chrono_day default_value;               // 1970-01-01 00:00:00
    //
    //   turbo::chrono_day a(2015, 2, 3);               // 2015-02-03 00:00:00
    //   turbo::chrono_day b(2015, 2, 3, 4, 5, 6);      // 2015-02-03 00:00:00
    //   turbo::chrono_day c(2015);                     // 2015-01-01 00:00:00
    //
    //   turbo::chrono_second ss(2015, 2, 3, 4, 5, 6);  // 2015-02-03 04:05:06
    //   turbo::chrono_minute mm(ss);                   // 2015-02-03 04:05:00
    //   turbo::chrono_hour hh(mm);                     // 2015-02-03 04:00:00
    //   turbo::chrono_day d(hh);                       // 2015-02-03 00:00:00
    //   turbo::chrono_month m(d);                      // 2015-02-01 00:00:00
    //   turbo::chrono_year y(m);                       // 2015-01-01 00:00:00
    //
    //   m = turbo::chrono_month(y);                    // 2015-01-01 00:00:00
    //   d = turbo::chrono_day(m);                      // 2015-01-01 00:00:00
    //   hh = turbo::chrono_hour(d);                    // 2015-01-01 00:00:00
    //   mm = turbo::chrono_minute(hh);                 // 2015-01-01 00:00:00
    //   ss = turbo::chrono_second(mm);                 // 2015-01-01 00:00:00
    //
    ...
    //
    //   turbo::chrono_second   : 2015-11-22 12:34:56
    //   turbo::chrono_minute   : 2015-11-22 12:34:00
    //   turbo::chrono_hour     : 2015-11-22 12:00:00
    //   turbo::chrono_day      : 2015-11-22 00:00:00
    //   turbo::chrono_month    : 2015-11-01 00:00:00
    //   turbo::chrono_year     : 2015-01-01 00:00:00
    //
    // Each civil-time type performs arithmetic on the field to which it is
    // aligned. This means that adding 1 to an turbo::chrono_day increments the day
    // field (normalizing as necessary), and subtracting 7 from an turbo::chrono_month
    ...
    // alignment. For example, the difference between two turbo::chrono_hour objects
    ...
    // precision (e.g., turbo::chrono_day -> turbo::chrono_second), the conversion may be
    ...
    // Examples:
    //
    //   void UseDay(turbo::chrono_day day);
    //
    //   turbo::chrono_second cs;
    //   UseDay(cs);                  // Won't compile because data may be discarded
    //   UseDay(turbo::chrono_day(cs));  // OK: explicit conversion
    //
    //   turbo::chrono_day cd;
    //   UseDay(cd);                  // OK: no conversion needed
    //
    //   turbo::chrono_month cm;
    //   UseDay(cm);                  // OK: implicit conversion to turbo::chrono_day
    //
    // NORMALIZATION
    //
    ...
    //   turbo::chrono_day d(2016, 10, 32);
    //   // Out-of-range, negative: normalized to 2016-10-30T23
    //   turbo::chrono_hour h1(2016, 10, 31, -1);
    //   // Normalization is cumulative: normalized to 2016-10-30T23
    //   turbo::chrono_hour h2(2016, 10, 32, -25);
    //
    ...
    // Examples:
    //
    //   turbo::chrono_day feb_3(2015, 2, 3);  // 2015-02-03 00:00:00
    //   turbo::chrono_day mar_4(2015, 3, 4);  // 2015-03-04 00:00:00
    //   // feb_3 < mar_4
    //   // turbo::chrono_year(feb_3) == turbo::chrono_year(mar_4)
    //
    //   turbo::chrono_second feb_3_noon(2015, 2, 3, 12, 0, 0);  // 2015-02-03 12:00:00
    //   // feb_3 < feb_3_noon
    //   // feb_3 == turbo::chrono_day(feb_3_noon)
    //
    //   // Iterates all the days of February 2015.
    //   for (turbo::chrono_day d(2015, 2, 1); d < turbo::chrono_month(2015, 3); ++d) {
    //     // ...
    //   }
    //
    // ARITHMETIC
    ...
    // Example:
    //
    //   turbo::chrono_day a(2015, 2, 3);
    //   ++a;                              // 2015-02-04 00:00:00
    //   --a;                              // 2015-02-03 00:00:00
    //   turbo::chrono_day b = a + 1;         // 2015-02-04 00:00:00
    //   turbo::chrono_day c = 1 + b;         // 2015-02-05 00:00:00
    //   int n = c - a;                    // n = 2 (civil days)
    //   int m = c - turbo::chrono_month(c);  // Won't compile: different types.
    //
    // ACCESSORS
    //
    ...
    // Example:
    //
    //   const turbo::chrono_day d(2015, 1, 31);
    //
    //   // Answer 1:
    //   // Add 1 to the month field in the constructor, and rely on normalization.
    //   const auto normalized = turbo::chrono_day(d.year(), d.month() + 1, d.day());
    //   // normalized == 2015-03-03 (aka Feb 31)
    //
    //   // Answer 2:
    //   // Add 1 to month field, capping to the end of next month.
    //   const auto next_month = turbo::chrono_month(d) + 1;
    //   const auto last_day_of_next_month = turbo::chrono_day(next_month + 1) - 1;
    //   const auto capped = std::min(normalized, last_day_of_next_month);
    //   // capped == 2015-02-28
    //
    //   // Answer 3:
    //   // signal an error if the normalized answer is not in next month.
    //   if (turbo::chrono_month(normalized) != next_month) {
    //     // error, month overflow
    //   }
    //
    using chrono_second =
    turbo::times_internal::times_detail::civil_time<times_internal::second_tag>;
    using chrono_minute =
    turbo::times_internal::times_detail::civil_time<times_internal::minute_tag>;
    using chrono_hour =
    turbo::times_internal::times_detail::civil_time<times_internal::hour_tag>;
    using chrono_day =
    turbo::times_internal::times_detail::civil_time<times_internal::day_tag>;
    using chrono_month =
    turbo::times_internal::times_detail::civil_time<times_internal::month_tag>;
    using chrono_year =
    turbo::times_internal::times_detail::civil_time<times_internal::year_tag>;
    ...
    //
    //   turbo::chrono_second cs = ...;
    //   turbo::chrono_year_t y = cs.year();
    //   cs = turbo::chrono_second(y, 1, 1, 0, 0, 0);  // chrono_second(chrono_year(cs))
    //
    using chrono_year_t = turbo::times_internal::year_t;
    ...
    // Example:
    //
    //   turbo::chrono_diff_t n_sec = cs1 - cs2;             // cs1 == cs2 + n_sec;
    //
    using chrono_diff_t = turbo::times_internal::diff_t;
    ...
    //
    //   turbo::chrono_weekday wd = turbo::chrono_weekday::thursday;
    //::
    using chrono_weekday = turbo::times_internal::times_detail::weekday;

    // get_weekday()
    //
    // Returns the turbo::chrono_weekday for the given (realigned) civil-time value.
    //
    // Example:
    //
    //   turbo::chrono_day a(2015, 8, 13);
    //   turbo::chrono_weekday wd = turbo::get_weekday(a);  // wd == turbo::chrono_weekday::thursday
    //
    TURBO_FORCE_INLINE chrono_weekday get_weekday(chrono_second cs) {
        return turbo::times_internal::times_detail::get_weekday(cs);
    }

    // next_weekday()
    // prev_weekday()
    //
    // Returns the turbo::chrono_day that strictly follows or precedes a given
    // turbo::chrono_day, and that falls on the given turbo::chrono_weekday.
    //
    ...
    //
    //   turbo::chrono_day a(2015, 8, 13);
    //   // turbo::get_weekday(a) == turbo::chrono_weekday::thursday
    //   turbo::chrono_day b = turbo::next_weekday(a, turbo::chrono_weekday::thursday);
    //   // b = 2015-08-20
    //   turbo::chrono_day c = turbo::prev_weekday(a, turbo::chrono_weekday::thursday);
    //   // c = 2015-08-06
    //
    //   turbo::chrono_day d = ...
    //   // Gets the following Thursday if d is not already Thursday
    //   turbo::chrono_day thurs1 = turbo::next_weekday(d - 1, turbo::chrono_weekday::thursday);
    //   // Gets the previous Thursday if d is not already Thursday
    //   turbo::chrono_day thurs2 = turbo::prev_weekday(d + 1, turbo::chrono_weekday::thursday);
    //
    TURBO_FORCE_INLINE chrono_day next_weekday(chrono_day cd, chrono_weekday wd) {
        return chrono_day(turbo::times_internal::times_detail::next_weekday(cd, wd));
    }

    TURBO_FORCE_INLINE chrono_day prev_weekday(chrono_day cd, chrono_weekday wd) {
        return chrono_day(turbo::times_internal::times_detail::prev_weekday(cd, wd));
    }
    ...
    // Example:
    //
    //   turbo::chrono_day a(2015, 1, 1);
    //   int yd_jan_1 = turbo::get_yearday(a);   // yd_jan_1 = 1
    //   turbo::chrono_day b(2015, 12, 31);
    //   int yd_dec_31 = turbo::get_yearday(b);  // yd_dec_31 = 365
    //
    TURBO_FORCE_INLINE int get_yearday(chrono_second cs) {
        return turbo::times_internal::times_detail::get_yearday(cs);
    }
    ...
    // Example:
    //
    //   turbo::chrono_day d = turbo::chrono_day(1969, 7, 20);
    //   std::string day_string = turbo::format_chrono_time(d);  // "1969-07-20"
    //
    ...
    //
    //   turbo::chrono_day d;
    //   bool ok = turbo::parse_chrono_time("2018-01-02", &d); // OK
    //
    ...
    //
    // Parses any of the formats accepted by `turbo::parse_chrono_time()`, but is more
...
#endif  // TURBO_TIMES_CIVIL_TIME_H_
