...
// The implementation of the turbo::time_point class, which is declared in
// //turbo/times/time.h.
//
// The representation for an turbo::time_point is an turbo::duration offset from the
...
//
// tz: An turbo::time_zone
// ci: An turbo::time_zone::chrono_info
// ti: An turbo::time_zone::time_info
// cd: An turbo::chrono_day or a turbo::times_internal::civil_day
// cs: An turbo::chrono_second or a turbo::times_internal::civil_second
// bd: An turbo::time_point::breakdown
// cl: A turbo::times_internal::time_zone::civil_lookup
// al: A turbo::times_internal::time_zone::absolute_lookup
...
namespace turbo {
    ...
    namespace {
        ...
        TURBO_FORCE_INLINE turbo::times_internal::time_point<turbo::times_internal::seconds> internal_unix_epoch() {
        ...
        TURBO_FORCE_INLINE int64_t FloorToUnit(turbo::duration d, turbo::duration unit) {
            turbo::duration rem;
        ...
        TURBO_FORCE_INLINE turbo::time_point::breakdown InfiniteFutureBreakdown() {
            turbo::time_point::breakdown bd;
        ...
        TURBO_FORCE_INLINE turbo::time_point::breakdown InfinitePastBreakdown() {
        ...
        TURBO_FORCE_INLINE turbo::time_zone::chrono_info InfiniteFutureCivilInfo() {
        ...
        TURBO_FORCE_INLINE turbo::time_zone::chrono_info InfinitePastCivilInfo() {
            time_zone::chrono_info ci;
        ...
        TURBO_FORCE_INLINE turbo::time_conversion InfiniteFutureTimeConversion() {
            turbo::time_conversion tc;
            tc.pre = tc.trans = tc.post = turbo::time_point::infinite_future();
            tc.kind = turbo::time_conversion::UNIQUE;
            tc.normalized = true;
            return tc;
        }

        TURBO_FORCE_INLINE time_conversion InfinitePastTimeConversion() {
            turbo::time_conversion tc;
            tc.pre = tc.trans = tc.post = turbo::time_point::infinite_past();
            tc.kind = turbo::time_conversion::UNIQUE;
            tc.normalized = true;
            return tc;
        }
...
// necessary. If sec is min/max, then consult cs+tz to check for overlow.
        time_point MakeTimeWithOverflow(const turbo::times_internal::time_point<turbo::times_internal::seconds> &sec,
                                        const turbo::times_internal::civil_second &cs,
                                        const turbo::times_internal::time_zone &tz,
                                        bool *normalized = nullptr) {
            const auto max = turbo::times_internal::time_point<turbo::times_internal::seconds>::max();
            const auto min = turbo::times_internal::time_point<turbo::times_internal::seconds>::min();
            if (sec == max) {
                const auto al = tz.lookup(max);
                if (cs > al.cs) {
                    if (normalized)
                        *normalized = true;
                    return turbo::time_point::infinite_future();
                }
            }
            if (sec == min) {
                ...
                if (cs < al.cs) {
                    if (normalized)
                        *normalized = true;
                    return turbo::time_point::infinite_past();
                }
            }
...
// Returns Mon=1..Sun=7.
        TURBO_FORCE_INLINE int MapWeekday(const turbo::times_internal::weekday &wd) {
            switch (wd) {
                case turbo::times_internal::weekday::monday:
                    return 1;
                case turbo::times_internal::weekday::tuesday:
                    return 2;
                case turbo::times_internal::weekday::wednesday:
                    return 3;
                case turbo::times_internal::weekday::thursday:
                    return 4;
                case turbo::times_internal::weekday::friday:
                    return 5;
                case turbo::times_internal::weekday::saturday:
                    return 6;
                case turbo::times_internal::weekday::sunday:
                    return 7;
            }
            return 1;
        }

        bool FindTransition(const turbo::times_internal::time_zone &tz,
                            bool (turbo::times_internal::time_zone::*find_transition)(
                                    const turbo::times_internal::time_point<turbo::times_internal::seconds> &tp,
                                    turbo::times_internal::time_zone::civil_transition *trans) const,
                            time_point t, time_zone::chrono_transition *trans) {
            // Transitions are second-aligned, so we can discard any fractional part.
            const auto tp = internal_unix_epoch() + turbo::times_internal::seconds(t.to_unix_seconds());
            turbo::times_internal::time_zone::civil_transition tr;
    ...
    turbo::time_point::breakdown time_point::in(turbo::time_zone tz) const {
        if (*this == turbo::time_point::infinite_future())
            return InfiniteFutureBreakdown();
        if (*this == turbo::time_point::infinite_past())
            return InfinitePastBreakdown();

        const auto tp = internal_unix_epoch() + turbo::times_internal::seconds(duration::get_rep_hi(rep_));
        const auto al = turbo::times_internal::time_zone(tz).lookup(tp);
        const auto cs = al.cs;
        const auto cd = turbo::times_internal::civil_day(cs);
        ...
        bd.weekday = MapWeekday(turbo::times_internal::get_weekday(cd));
        bd.yearday = turbo::times_internal::get_yearday(cd);
        bd.offset = al.offset;
        bd.is_dst = al.is_dst;
    ...
    turbo::time_point time_point::from_date(double udate) {
        return time_point::from_unix_duration(duration::milliseconds(udate));
    }

    turbo::time_point time_point::from_universal(int64_t universal) {
        return turbo::time_point::universal_epoch() + 100 * duration::nanoseconds(universal);
    }
    ...
    int64_t time_point::to_universal() const {
        return turbo::FloorToUnit(*this - turbo::time_point::universal_epoch(), duration::nanoseconds(100));
    }

    turbo::time_point time_point::from_timespec(timespec ts) {
        return from_unix_duration(duration::from_timespec(ts));
    }

    turbo::time_point time_point::from_timeval(timeval tv) {
        return from_unix_duration(duration::from_timeval(tv));
    }

    timespec time_point::to_timespec() const {
        timespec ts{0, 0};
        turbo::duration d = to_unix_duration(*this);
...
//
    ...
    turbo::time_zone::chrono_info time_zone::at(time_point t) const {
        ...
        const auto tp = internal_unix_epoch() + turbo::times_internal::seconds(duration::get_rep_hi(ud));
    ...
    turbo::time_zone::time_info time_zone::at(chrono_second ct) const {
        const turbo::times_internal::civil_second cs(ct);
        const auto cl = cz_.lookup(cs);

        time_zone::time_info ti;
        switch (cl.kind) {
            case turbo::times_internal::time_zone::civil_lookup::UNIQUE:
                ti.kind = time_zone::time_info::UNIQUE;
                break;
            case turbo::times_internal::time_zone::civil_lookup::SKIPPED:
                ti.kind = time_zone::time_info::SKIPPED;
                break;
            case turbo::times_internal::time_zone::civil_lookup::REPEATED:
    ...
    bool time_zone::next_transition(time_point t, chrono_transition *trans) const {
        return FindTransition(cz_, &turbo::times_internal::time_zone::next_transition, t, trans);
    }

    bool time_zone::prev_transition(time_point t, chrono_transition *trans) const {
        return FindTransition(cz_, &turbo::times_internal::time_zone::prev_transition, t, trans);
    }
...
//
    ...
    turbo::time_conversion convert_date_time(int64_t year, int mon, int day, int hour,
    ...
    turbo::time_point from_tm(const struct tm &tm, turbo::time_zone tz) {
    ...
    struct tm to_tm(turbo::time_point t, turbo::time_zone tz) {
        struct tm tm = {};
...
namespace turbo {
    ...
    time_point time_now() {
        ...
        return time_point::from_unix_duration(turbo::duration::nanoseconds(n));
    }
...
#include "turbo/times/internal/chrono_time.h"

#else
#include "turbo/times/internal/chrono_posix_time.h"
#endif

// Allows override by test.
#ifndef GET_CURRENT_TIME_NANOS_FROM_SYSTEM
#define GET_CURRENT_TIME_NANOS_FROM_SYSTEM() \
  ::turbo::times_internal::get_current_time_nanos_from_system()
#endif
...
#define GET_CURRENT_TIME_NANOS_CYCLECLOCK_NOW() \
  ::turbo::unscaled_cycle_clock_wrapper_for_get_current_time::now()
#endif
...
// Acquire seqlock (*seq) and return the value to be written to unlock.
    static TURBO_FORCE_INLINE uint64_t SeqAcquire(std::atomic<uint64_t> *seq) {
...
// SeqAcquire.
    static TURBO_FORCE_INLINE void SeqRelease(std::atomic<uint64_t> *seq, uint64_t x) {
...
// preserve precision without overflow.
    ...
    TURBO_NO_INLINE
    static int64_t get_current_time_nanos_slow_path() TURBO_LOCKS_EXCLUDED(lock) {
    ...
    static uint64_t update_last_sample(uint64_t now_cycles, uint64_t now_ns,
                                       ...
                                       const struct time_sample *sample) TURBO_EXCLUSIVE_LOCKS_REQUIRED(lock) {
...
// Returns the maximum duration that SleepOnce() can sleep for.
        constexpr turbo::duration max_sleep() {
#ifdef _WIN32
...
// REQUIRES: to_sleep <= max_sleep().
        void sleep_once(turbo::duration to_sleep) {
#ifdef _WIN32
            Sleep(to_sleep / turbo::milliseconds(1));
#else
...
TURBO_WEAK void turbo_internal_sleep_for(turbo::duration duration) {
    while (duration > turbo::zero_duration()) {
        turbo::duration to_sleep = std::min(duration, turbo::max_sleep());
        turbo::sleep_once(to_sleep);
        duration -= to_sleep;
    }
}
