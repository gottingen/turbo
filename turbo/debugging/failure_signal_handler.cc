...
#define MAP_ANONYMOUS MAP_ANON
        ...
        if (sigstk.ss_sp == MAP_FAILED) {
             TURBO_DLOG(FATAL)<<"mmap() for alternate signal stack failed";
        }
#else
        sigstk.ss_sp = malloc(sigstk.ss_size);
        if (sigstk.ss_sp == nullptr) {
            TURBO_DLOG(FATAL) << "malloc() for alternate signal stack failed";
        }
#endif

        if (sigaltstack(&sigstk, nullptr) != 0) {
            TURBO_DLOG(FATAL) << "sigaltstack() failed with errno={}" << errno;
        }
        return true;
    }
...
// if the system supports using an alternate stack.
    ...
    static void install_one_failure_handler(failure_signal_data *data,
        ...
        TURBO_DCHECK(sigaction(data->signo, &act, &data->previous_action) == 0) << "sigaction() failed";
    }
    ...
    static void install_one_failure_handler(failure_signal_data* data,
      ...
      TURBO_CHECK(data->previous_handler != SIG_ERR)<< "signal() failed";
    }
...
// `void*` might not be big enough to store `void(*)(const char*)`.
    ...
    // Many of the turbo::debugging::::debugging_internal::Dump* functions in
    ...
    TURBO_NO_INLINE static void write_stack_trace(
        ...
        int depth = turbo::debugging::get_stack_frames_with_context(
                stack, frame_sizes, kNumStackFrames,
                1,  // Do not include this function in stack trace.
                ucontext, &min_dropped_frames);
        turbo::debugging::debugging_internal::DumpPCAndFrameSizesAndStackTrace(
                turbo::debugging::debugging_internal::GetProgramCounter(ucontext), stack, frame_sizes,
                depth, min_dropped_frames, symbolize_stacktrace, writerfn, writerfn_arg);
    }

// Called by turbo_failure_signal_handler() to write the failure info. It is
...
// with writerfn set to the user provided function.
    ...
    // turbo_failure_signal_handler() installs this as a signal handler for
    // SIGALRM, then sets an alarm to be delivered to the program after a
    // set amount of time. If turbo_failure_signal_handler() hangs for more than
...
// turbo::debugging::get_tid() returns pid_t on most platforms, but
// returns turbo::debugging::base_internal::pid_t on Windows.
    using GetTidType = decltype(turbo::thread::thread_index());

    static std::atomic<GetTidType> failed_tid(0);

#ifndef TURBO_HAVE_SIGACTION
    static void turbo_failure_signal_handler(int signo) {
      void* ucontext = nullptr;
#else

    static void turbo_failure_signal_handler(int signo, siginfo_t *, void *ucontext) {
#endif

        const GetTidType this_tid = turbo::thread::thread_index();
        ...
        if (!failed_tid.compare_exchange_strong(
            ...
            TURBO_DLOG(ERROR) <<
                              ...
                              << turbo::debugging::debugging_internal::GetProgramCounter(ucontext)
                              << " while already in turbo_failure_signal_handler()";

            if (this_tid != previous_failed_tid) {
                // Another thread is already in turbo_failure_signal_handler(), so wait
    ...
    void install_failure_signal_handler(const failure_signal_handler_options &options) {
        ...
        for (auto &it : failure_signal_array) {
            install_one_failure_handler(&it, turbo_failure_signal_handler);
        }
    }
