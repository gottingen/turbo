...
#include "turbo/base/dynamic_annotations/dynamic_annotations.h"
...
// memset() is not guaranteed to be async-signal-safe.
            ...
            private:
                char *CopyString(const char *s) {
                    ...
                    TURBO_CHECK(dst != nullptr) << "out of memory";
                    memcpy(dst, s, len + 1);
                    return dst;
                }
        ...
        static ssize_t ReadPersistent(int fd, void *buf, size_t count) {
            ...
            while (num_bytes < count) {
                ...
                if (len < 0) {  // There was an error other than EINTR.
                    TURBO_LOG(WARNING) << "read failed: errno=" << errno;
                    return -1;
                }
...
// return the number of bytes read.  Otherwise, return -1.
        static ssize_t ReadFromOffset(const int fd, void *buf, const size_t count,
            ...
            if (off == (off_t) -1) {
                TURBO_LOG(WARNING) << "lseek(" << fd << "), " << static_cast<uintmax_t>(offset)
...
// inlined.
        static TURBO_NO_INLINE bool GetSectionHeaderByType(const int fd, ElfW(Half) sh_num, const off_t sh_offset,
                                                           ElfW(Word) type,
                                                           ElfW(Shdr)
...
bool GetSectionHeaderByName(int fd, const char *name, size_t name_len, ElfW(Shdr)* out) {
    ...
    if (sizeof(header_name) < name_len) {
        TURBO_LOG(WARNING)<<"Section name "<<name<<" is too long ("<<name_len<<"); ""section will not be found (even if present).";
...
TURBO_CHECK(n_read <= out_size)<< "ReadFromOffset read too much data.";
...
static TURBO_NO_INLINE bool ReadAddrMap(
    ...
    if (wrapped_maps_fd.get() < 0) {
        TURBO_LOG(WARNING) << maps_path << "error: " << errno;
        return false;
    }
    ...
    while (true) {
        ...
        if (cursor == eol || *cursor != '-') {
            TURBO_LOG(WARNING) << "Corrupt /proc/self/maps line: " << line;
            return false;
        }
        ++cursor;  // Skip '-'.
        ...
        if (cursor == eol || cursor < flags_start + 4) {
            TURBO_LOG(WARNING) << "Corrupt /proc/self/maps: " << line;
            return false;
        }
...
bool Symbolizer::RegisterObjFile(const char *filename,
    ...
    if (addr_map_size != 0) {
        ...
        if (old->end_addr > end_addr) {
            TURBO_LOG(ERROR)<<"Unsorted addr map entry: 0x"<<reinterpret_cast<uintptr_t>(end_addr)<<": "<<filename
            ...
            if (old->start_addr != start_addr ||
                ...
                TURBO_LOG(ERROR)<<"Duplicate addr 0x"<<reinterpret_cast<uintptr_t>(end_addr)<<": "<<filename
                          <<" <-> 0x"<< reinterpret_cast<uintptr_t>(old->end_addr)<<": "<<old->filename;
            }
            return true;
        }
...
// get inlined.
static TURBO_NO_INLINE void DemangleInplace(char *out, int out_size,
                                            char *tmp_buf,
                                            int tmp_buf_size) {
...
const char *Symbolizer::InsertSymbolInCache(const void *const pc,
    ...
    AgeSymbols(line);
    TURBO_CHECK(oldest_index >= 0) << "Corrupt cache";
...
static bool MaybeInitializeObjFile(ObjFile *obj) {
    if (obj->fd < 0) {
        ...
        if (obj->fd < 0) {
            TURBO_LOG(WARNING) << obj->filename << "open failed: errno=" << errno;
            return false;
        }
        obj->elf_type = FileGetElfType(obj->fd);
        if (obj->elf_type < 0) {
            TURBO_LOG(WARNING) << obj->filename << ": wrong elf type: " << obj->elf_type;
            return false;
        }

        if (!ReadFromOffsetExact(obj->fd, &obj->elf_header, sizeof(obj->elf_header),
                                 0)) {
            TURBO_LOG(WARNING) << obj->filename << ": failed to read elf header";
            return false;
        }
    }
    return true;
    ...
    if (obj != nullptr) {
        ...
        if (vdso.IsPresent()) {
          ...
          if (vdso.LookupSymbolByAddress(pc, &symbol_info)) {
            ...
            TURBO_CHECK(len + 1 < sizeof(symbol_buf_))<<
