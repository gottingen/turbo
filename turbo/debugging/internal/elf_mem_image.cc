...
int ElfBind(const ElfW(Sym) *) {
  TURBO_LOG(FATAL)<<"Unexpected word size";
  return 0;
}
...
ElfMemImage::ElfMemImage(const void *base) {
  TURBO_CHECK(base != kInvalidBase)<< "bad pointer";
  Init(base);
}
...
const ElfW(Sym) *ElfMemImage::GetDynsym(int index) const {
  TURBO_CHECK(index < GetNumSymbols())<< "index out of range";
...
const ElfW(Phdr) *ElfMemImage::GetPhdr(int index) const {
  TURBO_CHECK(index < ehdr_->e_phnum)<< "index out of range";
...
const char *ElfMemImage::GetDynstr(ElfW(Word) offset) const {
  TURBO_CHECK(offset < strsize_)<< "offset out of range";
...
const void *ElfMemImage::GetSymAddr(const ElfW(Sym) *sym) const {
  ...
  TURBO_CHECK(link_base_ < sym->st_value)<< "symbol out of range";
  return GetTableElement<char>(ehdr_, 0, 1, sym->st_value - link_base_);
}

const ElfW(Verdef) *ElfMemImage::GetVerdef(int index) const {
  TURBO_CHECK(0 <= index && static_cast<size_t>(index) <= verdefnum_)<<
                 "index out of range";
...
void ElfMemImage::SymbolIterator::Update(int increment) {
  ...
  TURBO_DCHECK(image->IsPresent() || increment == 0);
  if (!image->IsPresent()) {
    return;
  }
  index_ += increment;
  ...
  TURBO_DCHECK(symbol && version_symbol);
