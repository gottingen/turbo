...
inline turbo::optional<turbo::Cord> Status::GetPayload(
    turbo::string_view type_url) const {
  if (IsInlined(rep_)) return turbo::nullopt;
  return RepToPointer(rep_)->GetPayload(type_url);
}

inline void Status::SetPayload(turbo::string_view type_url, turbo::Cord payload) {
  if (ok()) return;
...
inline bool Status::ErasePayload(turbo::string_view type_url) {
