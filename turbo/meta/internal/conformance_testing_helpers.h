...
struct IsNullaryCallableImpl<
    Fun, turbo::void_t<decltype(std::declval<const Fun&>()())>>
    : std::true_type {
...
struct AreGeneratorsWithTheSameReturnTypeImpl<
    typename std::enable_if<turbo::conjunction<std::is_same<
...
void ForEachTupleElement(const Fun& fun, const Tup& tup) {
  turbo::apply(ForEachParameterFun<Fun>{fun}, tup);
}
