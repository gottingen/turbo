...
// platforms, they are suppressed by TURBO_ATTRIBUTE_UNUSED, but that doesn't
// work on MSVC.
...
void CountAll(turbo::index_sequence<Is...>) {
  ...
  TURBO_ATTRIBUTE_UNUSED Counter<turbo::make_index_sequence<Is>...> seq;
}
...
TEST(IntegerSequenceTest, MakeIndexSequencePerformance) {
  ...
  // We only need an alias here, but instantiate a variable to silence warnings
  // for unused typedefs in some compilers.
  TURBO_ATTRIBUTE_UNUSED turbo::make_index_sequence<(1 << 16) - 1> seq;
