...
// -----------------------------------------------------------------------------
//
// This header file defines an `turbo::variant` type for holding a type-safe
// value of some prescribed set of types (noted as alternative types), and
// associated functions for managing variants.
//
// The `turbo::variant` type is a form of type-safe union. An `turbo::variant`
// should always hold a value of one of its alternative types (except in the
// "valueless by exception state" -- see below). A default-constructed
// `turbo::variant` will hold the value of its first alternative type, provided
// it is default-constructible.
//
// In exceptional cases due to error, an `turbo::variant` can hold no
...
//
// `turbo::variant` is a C++11 compatible version of the C++17 `std::variant`
...
#include "turbo/meta/internal/variant.h"
...
// -----------------------------------------------------------------------------
//
// An `turbo::variant` type is a form of type-safe union. An `turbo::variant` --
...
//   turbo::variant<int, std::string> v = std::string("abc");
//
//   // A default-constructed variant will hold a value-initialized value of
//   // the first alternative type.
//   auto a = turbo::variant<int, std::string>();   // Holds an int of value '0'.
//
//   // variants are assignable.
//
//   // copy assignment
//   auto v1 = turbo::variant<int, std::string>("abc");
//   auto v2 = turbo::variant<int, std::string>(10);
//   v2 = v1;  // copy assign
//
//   // move assignment
//   auto v1 = turbo::variant<int, std::string>("abc");
//   v1 = turbo::variant<int, std::string>(10);
//
//   // assignment through type conversion
//   a = 128;         // variant contains int
//   a = "128";       // variant contains std::string
//
// An `turbo::variant` holding a value of one of its alternative types `T` holds
// an allocation of `T` directly within the variant itself. An `turbo::variant`
...
// swap()
//
// Swaps two `turbo::variant` values. This function is equivalent to `v.swap(w)`
// where `v` and `w` are `turbo::variant` types.
//
...
template <
    typename... Ts,
    turbo::enable_if_t<
        turbo::conjunction<std::is_move_constructible<Ts>...,
...
// variant_size
//
// Returns the number of alternative types available for a given `turbo::variant`
// type as a compile-time constant expression. As this is a class template, it
// is not generally useful for accessing the number of alternative types of
// any given `turbo::variant` instance.
//
// Example:
//
//   auto a = turbo::variant<int, std::string>;
//   constexpr int num_types =
//       turbo::variant_size<turbo::variant<int, std::string>>();
//
//   // You can also use the member constant `value`.
//   constexpr int num_types =
//       turbo::variant_size<turbo::variant<int, std::string>>::value;
//
//   // `turbo::variant_size` is more valuable for use in generic code:
//   template <typename Variant>
//   constexpr bool IsVariantMultivalue() {
//       return turbo::variant_size<Variant>() > 1;
//   }
//
...
// variant_alternative
//
// Returns the alternative type for a given `turbo::variant` at the passed
...
// value of any given `turbo::variant` variable.
//
// Example:
//
//   // The type of the 0th alternative is "int".
//   using alternative_type_0
//     = turbo::variant_alternative<0, turbo::variant<int, std::string>>::type;
//
//   static_assert(std::is_same<alternative_type_0, int>::value, "");
//
//   // `turbo::variant_alternative` is more valuable for use in generic code:
//   template <typename Variant>
...
//
//   using alternative_type_0
//     = turbo::variant_alternative_t<0, turbo::variant<int, std::string>>;
//   static_assert(std::is_same<alternative_type_0, int>::value, "");
template <std::size_t I, class T>
...
// Example:
//
//   turbo::variant<int, std::string> foo = 42;
//   if (turbo::holds_alternative<int>(foo)) {
...
//   int i = turbo::get<int>(a);
//
//   auto b = turbo::variant<int, int>;
//
//   // Getting the value by a type that is not unique is ill-formed.
//   int j = turbo::get<int>(b);     // Compile Error!
//
...
// Note: `turbo::move()` is required to allow use of constexpr in C++11.
template <class T, class... Types>
constexpr T&& get(variant<Types...>&& v) {
  return variant_internal::VariantCoreAccess::CheckedAccess<
      variant_internal::IndexOf<T, Types...>::value>(turbo::move(v));
}
...
constexpr variant_alternative_t<I, variant<Types...>>&& get(
    variant<Types...>&& v) {
  return variant_internal::VariantCoreAccess::CheckedAccess<I>(turbo::move(v));
}
...
// index.
template <std::size_t I, class... Types>
constexpr turbo::add_pointer_t<variant_alternative_t<I, variant<Types...>>>
...
// variant by index.
template <std::size_t I, class... Types>
constexpr turbo::add_pointer_t<const variant_alternative_t<I, variant<Types...>>>
...
// type.
template <class T, class... Types>
constexpr turbo::add_pointer_t<T> get_if(variant<Types...>* v) noexcept {
  return turbo::get_if<variant_internal::IndexOf<T, Types...>::value>(v);
}

// Overload for getting a pointer to the const value stored in the given variant
// by type.
template <class T, class... Types>
constexpr turbo::add_pointer_t<const T> get_if(
...
// visit()
//
// Calls a provided functor on a given set of variants. `turbo::visit()` is
...
//   };
//
//   // Declare our variant, and call `turbo::visit()` on it.
//   // Note that `GetVariant()` returns void in either case.
//   turbo::variant<int, std::string> foo = std::string("foo");
//   GetVariant visitor;
//   turbo::visit(visitor, foo);  // Prints `The variant's value is: foo'
template <typename Visitor, typename... Variants>
variant_internal::VisitResult<Visitor, Variants...> visit(Visitor&& vis,
                                                          Variants&&... vars) {
  return variant_internal::
      VisitIndices<variant_size<turbo::decay_t<Variants> >::value...>::Run(
          variant_internal::PerformVisitation<Visitor, Variants...>{
              std::forward_as_tuple(turbo::forward<Variants>(vars)...),
              turbo::forward<Visitor>(vis)},
          vars.index()...);
}
...
// `turbo::monostate` Relational Operators
...
//------------------------------------------------------------------------------
// `turbo::variant` Template Definition
//------------------------------------------------------------------------------
template <typename T0, typename... Tn>
class variant<T0, Tn...> : private variant_internal::VariantBase<T0, Tn...> {
  static_assert(turbo::conjunction<std::is_object<T0>,
                                  std::is_object<Tn>...>::value,
                "Attempted to instantiate a variant containing a non-object "
                "type.");
  // Intentionally not qualifying `negation` with `turbo::` to work around a bug
  // in MSVC 2015 with inline namespace and variadic template.
  static_assert(turbo::conjunction<negation<std::is_array<T0> >,
                                  negation<std::is_array<Tn> >...>::value,
                "Attempted to instantiate a variant containing an array type.");
  static_assert(turbo::conjunction<std::is_nothrow_destructible<T0>,
 ...
 public:
  ...
  // 2018. It will be implemented and integrated into `turbo::variant`.
  template <
      class T,
      std::size_t I = std::enable_if<
          variant_internal::IsNeitherSelfNorInPlace<variant,
                                                    turbo::decay_t<T>>::value,
          variant_internal::IndexOfConstructedType<variant, T>>::type::value,
      class Tj = turbo::variant_alternative_t<I, variant>,
      turbo::enable_if_t<std::is_constructible<Tj, T>::value>* =
          nullptr>
  constexpr variant(T&& t) noexcept(std::is_nothrow_constructible<Tj, T>::value)
      : Base(variant_internal::EmplaceTag<I>(), turbo::forward<T>(t)) {}
  ...
  constexpr explicit variant(in_place_index_t<I>, Args&&... args)
      : Base(variant_internal::EmplaceTag<I>(), turbo::forward<Args>(args)...) {}
  ...
  // `turbo::valueless_by_exception()` is false.
  ~variant() = default;
  ...
  template <
      class T,
      std::size_t I = std::enable_if<
          !std::is_same<turbo::decay_t<T>, variant>::value,
          variant_internal::IndexOfConstructedType<variant, T>>::type::value,
      class Tj = turbo::variant_alternative_t<I, variant>,
  ...
  // `turbo::valueless_by_exception()` is false). Requires that T is unambiguous
  // in the variant.
  //
  // Example:
  //
  //   turbo::variant<std::vector<int>, int, std::string> v;
  //   v.emplace<int>(99);
  //   v.emplace<std::string>("abc");
  template <
      class T, class... Args,
      typename std::enable_if<std::is_constructible<
          turbo::variant_alternative_t<
              variant_internal::UnambiguousIndexOf<variant, T>::value, variant>,
          Args...>::value>::type* = nullptr>
  T& emplace(Args&&... args) {
    return variant_internal::VariantCoreAccess::Replace<
        variant_internal::UnambiguousIndexOf<variant, T>::value>(
        this, turbo::forward<Args>(args)...);
  }

  // Constructs a value of the given alternative type T within the variant using
  // an initializer list. The existing value of the variant is destroyed first
  // (provided that `turbo::valueless_by_exception()` is false). Requires that T
  ...
  T& emplace(std::initializer_list<U> il, Args&&... args) {
    return variant_internal::VariantCoreAccess::Replace<
        variant_internal::UnambiguousIndexOf<variant, T>::value>(
        this, il, turbo::forward<Args>(args)...);
  }

  // Destroys the current value of the variant (provided that
  // `turbo::valueless_by_exception()` is false) and constructs a new value at
  // the given index.
  //
  // Example:
  //
  //   turbo::variant<std::vector<int>, int, int> v;
  //   v.emplace<1>(99);
  //   v.emplace<2>(98);
  //   v.emplace<int>(99);  // Won't compile. 'int' isn't a unique type.
  template <std::size_t I, class... Args,
            typename std::enable_if<
                std::is_constructible<turbo::variant_alternative_t<I, variant>,
                                      Args...>::value>::type* = nullptr>
  turbo::variant_alternative_t<I, variant>& emplace(Args&&... args) {
  ...
  template <std::size_t I, class U, class... Args,
            typename std::enable_if<std::is_constructible<
                turbo::variant_alternative_t<I, variant>,
                std::initializer_list<U>&, Args...>::value>::type* = nullptr>
  turbo::variant_alternative_t<I, variant>& emplace(std::initializer_list<U> il,
                                                   Args&&... args) {
    return variant_internal::VariantCoreAccess::Replace<I>(
        this, il, turbo::forward<Args>(args)...);
  }
  ...
  constexpr bool valueless_by_exception() const noexcept {
    return this->index_ == turbo::variant_npos;
  }
...
// hash()
template <>  // NOLINT
struct hash<turbo::monostate> {
  std::size_t operator()(turbo::monostate) const { return 0; }
};

template <class... T>  // NOLINT
struct hash<turbo::variant<T...>>
    : turbo::variant_internal::VariantHashBase<turbo::variant<T...>, void,
                                              turbo::remove_const_t<T>...> {};
...
// ConvertVariantTo()
//
// Helper functions to convert an `turbo::variant` to a variant of another set of
// types, provided that the alternative type of the new variant type can be
// converted from any type in the source variant.
//
// Example:
//
//   turbo::variant<name1, name2, float> InternalReq(const Req&);
//
//   // name1 and name2 are convertible to name
//   turbo::variant<name, float> ExternalReq(const Req& req) {
//     return turbo::ConvertVariantTo<turbo::variant<name, float>>(
//              InternalReq(req));
//   }
template <typename To, typename Variant>
To ConvertVariantTo(Variant&& variant) {
  return turbo::visit(variant_internal::ConversionVisitor<To>{},
