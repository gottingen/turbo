...
void ToValuelessByException(turbo::variant<H, T...>& v) {  // NOLINT
  try {
...
TEST(VariantTest, TestDefaultConstructor) {
...

  {
    ...
    ASSERT_FALSE(x.valueless_by_exception());
    ASSERT_EQ(0u, x.index());
    EXPECT_EQ(5, turbo::get<0>(x).value);
...
TYPED_TEST(VariantTypesTest, TestCopyCtor) {
  ...
  using value_type1 = turbo::variant_alternative_t<0, Variant>;
  using value_type2 = turbo::variant_alternative_t<1, Variant>;
  using value_type3 = turbo::variant_alternative_t<2, Variant>;
  using value_type4 = turbo::variant_alternative_t<3, Variant>;
  const TypeParam value(TypeParam::kIndex);
  Variant original(value);
  Variant copied(original);
  EXPECT_TRUE(turbo::holds_alternative<value_type1>(copied) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(turbo::holds_alternative<value_type2>(copied) ||
              TypeParam::kIndex != 2);
  EXPECT_TRUE(turbo::holds_alternative<value_type3>(copied) ||
              TypeParam::kIndex != 3);
  EXPECT_TRUE(turbo::holds_alternative<value_type4>(copied) ||
              TypeParam::kIndex != 4);
  EXPECT_TRUE((turbo::get_if<value_type1>(&original) ==
               turbo::get_if<value_type1>(&copied)) ||
              TypeParam::kIndex == 1);
  EXPECT_TRUE((turbo::get_if<value_type2>(&original) ==
               turbo::get_if<value_type2>(&copied)) ||
              TypeParam::kIndex == 2);
  EXPECT_TRUE((turbo::get_if<value_type3>(&original) ==
               turbo::get_if<value_type3>(&copied)) ||
              TypeParam::kIndex == 3);
  EXPECT_TRUE((turbo::get_if<value_type4>(&original) ==
               turbo::get_if<value_type4>(&copied)) ||
              TypeParam::kIndex == 4);
  EXPECT_TRUE((turbo::get_if<value_type1>(&original) ==
               turbo::get_if<value_type1>(&copied)) ||
              TypeParam::kIndex == 1);
  ...
  EXPECT_TRUE((turbo::get_if<value_type4>(&original) ==
               turbo::get_if<value_type4>(&copied)) ||
              TypeParam::kIndex == 4);
  const TypeParam* ovalptr = turbo::get_if<TypeParam>(&original);
  const TypeParam* cvalptr = turbo::get_if<TypeParam>(&copied);
  ASSERT_TRUE(ovalptr != nullptr);
  ASSERT_TRUE(cvalptr != nullptr);
  EXPECT_EQ(*ovalptr, *cvalptr);
  TypeParam* mutable_ovalptr = turbo::get_if<TypeParam>(&original);
  TypeParam* mutable_cvalptr = turbo::get_if<TypeParam>(&copied);
...
TEST(VariantTest, TestMoveConstruct) {
  ...
  V v2 = turbo::move(v);
  EXPECT_EQ(10, turbo::get<1>(v2).value);
}
...
struct is_trivially_move_assignable
    : turbo::is_move_assignable<SingleUnion<T>>::type {};
...
TYPED_TEST(VariantTypesTest, TestValueCtor) {
  ...
  const TypeParam value(TypeParam::kIndex);
  Variant v(value);
  EXPECT_TRUE(turbo::holds_alternative<value_type1>(v) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(turbo::holds_alternative<value_type2>(v) ||
              TypeParam::kIndex != 2);
  EXPECT_TRUE(turbo::holds_alternative<value_type3>(v) ||
              TypeParam::kIndex != 3);
  EXPECT_TRUE(turbo::holds_alternative<value_type4>(v) ||
              TypeParam::kIndex != 4);
  EXPECT_TRUE(nullptr != turbo::get_if<value_type1>(&v) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(nullptr != turbo::get_if<value_type2>(&v) ||
              TypeParam::kIndex != 2);
  EXPECT_TRUE(nullptr != turbo::get_if<value_type3>(&v) ||
              TypeParam::kIndex != 3);
  EXPECT_TRUE(nullptr != turbo::get_if<value_type4>(&v) ||
              TypeParam::kIndex != 4);
  EXPECT_TRUE(nullptr != turbo::get_if<value_type1>(&v) ||
              TypeParam::kIndex != 1);
  EXPECT_TRUE(nullptr != turbo::get_if<value_type2>(&v) ||
  ...
  EXPECT_TRUE(nullptr != turbo::get_if<value_type4>(&v) ||
              TypeParam::kIndex != 4);
  const TypeParam* valptr = turbo::get_if<TypeParam>(&v);
  ASSERT_TRUE(nullptr != valptr);
  EXPECT_EQ(value.value, valptr->value);
  const TypeParam* mutable_valptr = turbo::get_if<TypeParam>(&v);
  ASSERT_TRUE(nullptr != mutable_valptr);
  EXPECT_EQ(value.value, mutable_valptr->value);
}

TEST(VariantTest, AmbiguousValueConstructor) {
  EXPECT_FALSE((std::is_convertible<int, turbo::variant<int, int>>::value));
  EXPECT_FALSE((std::is_constructible<turbo::variant<int, int>, int>::value));
}
...
TEST(VariantTest, InPlaceType) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<int>(v1));
  EXPECT_EQ(7, turbo::get<int>(v1));

  Var v2(in_place_type_t<std::string>(), "ABC");
  ASSERT_TRUE(turbo::holds_alternative<std::string>(v2));
  EXPECT_EQ("ABC", turbo::get<std::string>(v2));

  Var v3(in_place_type_t<std::string>(), "ABC", 2u);
  ASSERT_TRUE(turbo::holds_alternative<std::string>(v3));
  EXPECT_EQ("AB", turbo::get<std::string>(v3));

  Var v4(in_place_type_t<NonCopyable>{});
  ASSERT_TRUE(turbo::holds_alternative<NonCopyable>(v4));

  Var v5(in_place_type_t<std::vector<int>>(), {1, 2, 3});
  ASSERT_TRUE(turbo::holds_alternative<std::vector<int>>(v5));
  EXPECT_THAT(turbo::get<std::vector<int>>(v5), ::testing::ElementsAre(1, 2, 3));
}
...
TEST(VariantTest, InPlaceTypeInitializerList) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<MoveOnlyWithListConstructor>(v1));
  EXPECT_EQ(6, turbo::get<MoveOnlyWithListConstructor>(v1).value);
}
...
TEST(VariantTest, InPlaceIndex) {
  ...
  EXPECT_TRUE(turbo::holds_alternative<NonCopyable>(v4));

  // Verify that a variant with only non-copyables can still be constructed.
  EXPECT_TRUE(turbo::holds_alternative<NonCopyable>(
...
TEST(VariantTest, TestSelfAssignment) {
  ...
  int counter = 0;
  IncrementInDtor counter_adjuster(&counter);
  turbo::variant_alternative_t<0, Variant> value(counter_adjuster);
  ...
  EXPECT_EQ(long_str, turbo::get<1>(so));
  so = *&so;
...
TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValueSameTypes) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<TypeParam>(source));
  ASSERT_TRUE(turbo::holds_alternative<TypeParam>(target));
  ASSERT_NE(turbo::get<TypeParam>(source), turbo::get<TypeParam>(target));
  target = source;
  ASSERT_TRUE(turbo::holds_alternative<TypeParam>(source));
  ASSERT_TRUE(turbo::holds_alternative<TypeParam>(target));
  EXPECT_EQ(turbo::get<TypeParam>(source), turbo::get<TypeParam>(target));
}
...
TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValuesVaryingSourceType) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<value_type1>(target));
  target = source;
  EXPECT_TRUE(turbo::holds_alternative<TypeParam>(source));
  EXPECT_TRUE(turbo::holds_alternative<TypeParam>(target));
  EXPECT_EQ(turbo::get<TypeParam>(source), turbo::get<TypeParam>(target));
}
...
TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValuesVaryingTargetType) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<value_type1>(source));
  const TypeParam value(TypeParam::kIndex);
  Variant target(value);
  ASSERT_TRUE(turbo::holds_alternative<TypeParam>(target));
  target = source;
  EXPECT_TRUE(turbo::holds_alternative<value_type1>(target));
  EXPECT_TRUE(turbo::holds_alternative<value_type1>(source));
  EXPECT_EQ(turbo::get<value_type1>(source), turbo::get<value_type1>(target));
}
...
TEST(VariantTest, TestEmplaceBasic) {
  ...
  Variant v(turbo::in_place_index<0>, 0);

  {
    char& emplace_result = v.emplace<char>();
    ASSERT_TRUE(turbo::holds_alternative<char>(v));
    EXPECT_EQ(turbo::get<char>(v), 0);
    EXPECT_EQ(&emplace_result, &turbo::get<char>(v));
  }
  ...
  v.emplace<char>('b');
  ASSERT_TRUE(turbo::holds_alternative<char>(v));
  EXPECT_EQ(turbo::get<char>(v), 'b');

  {
    int& emplace_result = v.emplace<int>();
    EXPECT_TRUE(turbo::holds_alternative<int>(v));
    EXPECT_EQ(turbo::get<int>(v), 0);
    EXPECT_EQ(&emplace_result, &turbo::get<int>(v));
  }
}
...
TEST(VariantTest, TestEmplaceInitializerList) {
  ...
  Var v1(turbo::in_place_index<0>, 555);
  MoveOnlyWithListConstructor& emplace_result =
      v1.emplace<MoveOnlyWithListConstructor>({1, 2, 3, 4, 5}, 6);
  ASSERT_TRUE(turbo::holds_alternative<MoveOnlyWithListConstructor>(v1));
  EXPECT_EQ(6, turbo::get<MoveOnlyWithListConstructor>(v1).value);
  EXPECT_EQ(&emplace_result, &turbo::get<MoveOnlyWithListConstructor>(v1));
}

TEST(VariantTest, TestEmplaceIndex) {
  using Variant = variant<int, char>;

  Variant v(turbo::in_place_index<0>, 555);
...
TEST(VariantTest, IndexValuelessByException) {
  ...
  Var v(turbo::in_place_index<0>);
  EXPECT_EQ(0u, v.index());
  ToValuelessByException(v);
  EXPECT_EQ(turbo::variant_npos, v.index());
  v = "str";
  EXPECT_EQ(1u, v.index());
}
...
TEST(VariantTest, MemberSwap) {
  ...
  EXPECT_TRUE(turbo::get<SpecialSwap>(a).special_swap);
...
TEST(VariantTest, VariantSize) {
  {
    using Size1Variant = turbo::variant<int>;
    EXPECT_EQ(1u, turbo::variant_size<Size1Variant>::value);
    EXPECT_EQ(1u, turbo::variant_size<const Size1Variant>::value);
    EXPECT_EQ(1u, turbo::variant_size<volatile Size1Variant>::value);
    EXPECT_EQ(1u, turbo::variant_size<const volatile Size1Variant>::value);
  }

  {
    using Size3Variant = turbo::variant<int, float, int>;
    EXPECT_EQ(3u, turbo::variant_size<Size3Variant>::value);
    EXPECT_EQ(3u, turbo::variant_size<const Size3Variant>::value);
    EXPECT_EQ(3u, turbo::variant_size<volatile Size3Variant>::value);
    EXPECT_EQ(3u, turbo::variant_size<const volatile Size3Variant>::value);
  }
}

TEST(VariantTest, VariantAlternative) {
  {
    using V = turbo::variant<float, int, const char*>;
    EXPECT_TRUE(
        (std::is_same<float, turbo::variant_alternative_t<0, V>>::value));
    EXPECT_TRUE((std::is_same<const float,
                              turbo::variant_alternative_t<0, const V>>::value));
    EXPECT_TRUE(
        (std::is_same<volatile float,
                      turbo::variant_alternative_t<0, volatile V>>::value));
    EXPECT_TRUE((
        std::is_same<const volatile float,
                     turbo::variant_alternative_t<0, const volatile V>>::value));

    EXPECT_TRUE((std::is_same<int, turbo::variant_alternative_t<1, V>>::value));
    EXPECT_TRUE((std::is_same<const int,
                              turbo::variant_alternative_t<1, const V>>::value));
    EXPECT_TRUE(
        (std::is_same<volatile int,
                      turbo::variant_alternative_t<1, volatile V>>::value));
    EXPECT_TRUE((
        std::is_same<const volatile int,
                     turbo::variant_alternative_t<1, const volatile V>>::value));

    EXPECT_TRUE(
        (std::is_same<const char*, turbo::variant_alternative_t<2, V>>::value));
    EXPECT_TRUE((std::is_same<const char* const,
                              turbo::variant_alternative_t<2, const V>>::value));
    EXPECT_TRUE(
        (std::is_same<const char* volatile,
                      turbo::variant_alternative_t<2, volatile V>>::value));
    EXPECT_TRUE((
        std::is_same<const char* const volatile,
                     turbo::variant_alternative_t<2, const volatile V>>::value));
  }

  {
    using V = turbo::variant<float, volatile int, const char*>;
    EXPECT_TRUE(
        (std::is_same<float, turbo::variant_alternative_t<0, V>>::value));
    EXPECT_TRUE((std::is_same<const float,
                              turbo::variant_alternative_t<0, const V>>::value));
    ...
    EXPECT_TRUE(
        (std::is_same<volatile int, turbo::variant_alternative_t<1, V>>::value));
...
TEST(VariantTest, HoldsAlternative) {
  ...
  Var v = 1;
  EXPECT_TRUE(turbo::holds_alternative<int>(v));
  EXPECT_FALSE(turbo::holds_alternative<std::string>(v));
  EXPECT_FALSE(turbo::holds_alternative<double>(v));
  v = "str";
  EXPECT_FALSE(turbo::holds_alternative<int>(v));
  EXPECT_TRUE(turbo::holds_alternative<std::string>(v));
  EXPECT_FALSE(turbo::holds_alternative<double>(v));
  v = 0.;
  EXPECT_FALSE(turbo::holds_alternative<int>(v));
  EXPECT_FALSE(turbo::holds_alternative<std::string>(v));
  EXPECT_TRUE(turbo::holds_alternative<double>(v));

  Var v2 = v;
  EXPECT_FALSE(turbo::holds_alternative<int>(v2));
  EXPECT_FALSE(turbo::holds_alternative<std::string>(v2));
  EXPECT_TRUE(turbo::holds_alternative<double>(v2));
  v2.emplace<int>(3);
  EXPECT_TRUE(turbo::holds_alternative<int>(v2));
  EXPECT_FALSE(turbo::holds_alternative<std::string>(v2));
  EXPECT_FALSE(turbo::holds_alternative<double>(v2));
}
...
TEST(VariantTest, GetIndex) {
  ...
  {
    Var v(turbo::in_place_index<0>, 0);

    using LValueGetType = decltype(turbo::get<0>(v));
    using RValueGetType = decltype(turbo::get<0>(turbo::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(turbo::get<0>(v), 0);
    EXPECT_EQ(turbo::get<0>(turbo::move(v)), 0);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<0>(const_v));
    using ConstRValueGetType = decltype(turbo::get<0>(turbo::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(turbo::get<0>(const_v), 0);
    EXPECT_EQ(turbo::get<0>(turbo::move(const_v)), 0);
  }

  {
    Var v = std::string("Hello");

    using LValueGetType = decltype(turbo::get<1>(v));
    using RValueGetType = decltype(turbo::get<1>(turbo::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, std::string&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, std::string&&>::value));
    EXPECT_EQ(turbo::get<1>(v), "Hello");
    EXPECT_EQ(turbo::get<1>(turbo::move(v)), "Hello");

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<1>(const_v));
    using ConstRValueGetType = decltype(turbo::get<1>(turbo::move(const_v)));
    ...
    EXPECT_EQ(turbo::get<1>(const_v), "Hello");
    EXPECT_EQ(turbo::get<1>(turbo::move(const_v)), "Hello");
  }

  {
    Var v = 2.0;

    using LValueGetType = decltype(turbo::get<2>(v));
    using RValueGetType = decltype(turbo::get<2>(turbo::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, double&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, double&&>::value));
    EXPECT_EQ(turbo::get<2>(v), 2.);
    EXPECT_EQ(turbo::get<2>(turbo::move(v)), 2.);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<2>(const_v));
    using ConstRValueGetType = decltype(turbo::get<2>(turbo::move(const_v)));
    ...
    EXPECT_EQ(turbo::get<2>(const_v), 2.);
    EXPECT_EQ(turbo::get<2>(turbo::move(const_v)), 2.);
  }

  {
    ...
    using LValueGetType = decltype(turbo::get<3>(v));
    using RValueGetType = decltype(turbo::get<3>(turbo::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(turbo::get<3>(v), 1);
    EXPECT_EQ(turbo::get<3>(turbo::move(v)), 1);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<3>(const_v));
    using ConstRValueGetType = decltype(turbo::get<3>(turbo::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(turbo::get<3>(const_v), 1);
    EXPECT_EQ(turbo::get<3>(turbo::move(const_v)), 1);  // NOLINT
  }
}
...
TEST(VariantTest, GetType) {
...

  {
    ...
    using LValueGetType = decltype(turbo::get<int>(v));
    using RValueGetType = decltype(turbo::get<int>(turbo::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(turbo::get<int>(v), 1);
    EXPECT_EQ(turbo::get<int>(turbo::move(v)), 1);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<int>(const_v));
    using ConstRValueGetType = decltype(turbo::get<int>(turbo::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(turbo::get<int>(const_v), 1);
    EXPECT_EQ(turbo::get<int>(turbo::move(const_v)), 1);
  }

  {
    ...
    EXPECT_TRUE((std::is_same<LValueGetType, std::string&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, std::string&&>::value));
    EXPECT_EQ(turbo::get<std::string>(v), "Hello");
    EXPECT_EQ(turbo::get<std::string>(turbo::move(v)), "Hello");
    ...
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const std::string&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const std::string&&>::value));
    EXPECT_EQ(turbo::get<std::string>(const_v), "Hello");
    EXPECT_EQ(turbo::get<std::string>(turbo::move(const_v)), "Hello");
  }

  {
    ...
    EXPECT_TRUE((std::is_same<LValueGetType, double&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, double&&>::value));
    EXPECT_EQ(turbo::get<double>(v), 2.);
    EXPECT_EQ(turbo::get<double>(turbo::move(v)), 2.);
    ...
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const double&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const double&&>::value));
    EXPECT_EQ(turbo::get<double>(const_v), 2.);
    EXPECT_EQ(turbo::get<double>(turbo::move(const_v)), 2.);
  }
}
...
TEST(VariantTest, GetIfIndex) {
...

  {
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(turbo::get_if<0>(&const_v)));

    {
      auto* elem = turbo::get_if<0>(&const_v);
      EXPECT_TRUE((std::is_same<decltype(elem), const int*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, 0);
      {
        auto* bad_elem = turbo::get_if<1>(&const_v);
        EXPECT_TRUE(
            (std::is_same<decltype(bad_elem), const std::string*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
        auto* bad_elem = turbo::get_if<2>(&const_v);
        EXPECT_TRUE((std::is_same<decltype(bad_elem), const double*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
        auto* bad_elem = turbo::get_if<3>(&const_v);
        EXPECT_EQ(bad_elem, nullptr);
  ...
  {
    Var v = std::string("Hello");
    EXPECT_TRUE(noexcept(turbo::get_if<1>(&v)));

    {
      auto* elem = turbo::get_if<1>(&v);
      EXPECT_TRUE((std::is_same<decltype(elem), std::string*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, "Hello");
      {
        auto* bad_elem = turbo::get_if<0>(&v);
        EXPECT_TRUE((std::is_same<decltype(bad_elem), int*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(turbo::get_if<1>(&const_v)));

    {
      auto* elem = turbo::get_if<1>(&const_v);
      EXPECT_TRUE((std::is_same<decltype(elem), const std::string*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, "Hello");
      {
        auto* bad_elem = turbo::get_if<0>(&const_v);
        EXPECT_TRUE((std::is_same<decltype(bad_elem), const int*>::value));
        EXPECT_EQ(bad_elem, nullptr);
      }
      {
  ...
  {
    Var v = 2.0;
    EXPECT_TRUE(noexcept(turbo::get_if<2>(&v)));

    {
      auto* elem = turbo::get_if<2>(&v);
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(turbo::get_if<2>(&const_v)));

    {
      auto* elem = turbo::get_if<2>(&const_v);
...

  {
    Var v(turbo::in_place_index<0>, 0);
    v.emplace<3>(1);
    EXPECT_TRUE(noexcept(turbo::get_if<3>(&v)));

    {
      auto* elem = turbo::get_if<3>(&v);
      EXPECT_TRUE((std::is_same<decltype(elem), int*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, 1);
      {
    ...
    const Var& const_v = v;
    EXPECT_TRUE(noexcept(turbo::get_if<3>(&const_v)));

    {
      auto* elem = turbo::get_if<3>(&const_v);
      EXPECT_TRUE((std::is_same<decltype(elem), const int*>::value));
      ASSERT_NE(elem, nullptr);
      EXPECT_EQ(*elem, 1);
      {
...
TEST(VariantTest, ValuelessOperatorEquals) {
  variant<MoveCanThrow, std::string> int_v(1), string_v("Hello"),
      valueless(turbo::in_place_index<0>),
      other_valueless(turbo::in_place_index<0>);
  ToValuelessByException(valueless);
...
TEST(VariantTest, VisitSimple) {
  ...
  std::string str = turbo::visit(ConvertTo<std::string>{}, v);
  EXPECT_EQ("A", str);

  v = std::string("B");

  std::string_view piece = turbo::visit(ConvertTo<std::string_view>{}, v);
  EXPECT_EQ("B", piece);
  ...
  v = "SomeStr";
  EXPECT_EQ(7u, turbo::visit(StrLen{}, v));
  v = std::string("VeryLargeThisTime");
  EXPECT_EQ(17u, turbo::visit(StrLen{}, v));
}
...
TEST(VariantTest, VisitRValue) {
  ...
  EXPECT_FALSE(turbo::visit(Visitor{}, v));
  EXPECT_TRUE(turbo::visit(Visitor{}, turbo::move(v)));

  // Also test the variadic overload.
  EXPECT_EQ(0, turbo::visit(Visitor{}, v, v));
  EXPECT_EQ(1, turbo::visit(Visitor{}, v, turbo::move(v)));
  EXPECT_EQ(2, turbo::visit(Visitor{}, turbo::move(v), v));
  EXPECT_EQ(3, turbo::visit(Visitor{}, turbo::move(v), turbo::move(v)));
}
...
TEST(VariantTest, VisitRValueVisitor) {
  ...
  Visitor visitor;
  EXPECT_FALSE(turbo::visit(visitor, v));
  EXPECT_TRUE(turbo::visit(Visitor{}, v));
}
...
TEST(VariantTest, VisitResultTypeDifferent) {
  ...
  EXPECT_TRUE(
      (std::is_same<LValue_LValue, decltype(turbo::visit(visitor, v))>::value));
  EXPECT_TRUE(
      (std::is_same<RValue_LValue,
                    decltype(turbo::visit(visitor, turbo::move(v)))>::value));
  EXPECT_TRUE((
      std::is_same<LValue_RValue, decltype(turbo::visit(Visitor{}, v))>::value));
  EXPECT_TRUE(
...
TEST(VariantTest, VisitVariadic) {
  ...
  using B = variant<std::unique_ptr<int>, std::string_view>;

  struct Visitor {
    std::pair<int, int> operator()(int a, std::unique_ptr<int> b) const {
      return {a, *b};
    }
    std::pair<int, int> operator()(std::string_view a,
                                   std::unique_ptr<int> b) const {
      return {static_cast<int>(a.size()), static_cast<int>(*b)};
    }
    std::pair<int, int> operator()(int a, std::string_view b) const {
  ...
  EXPECT_THAT(turbo::visit(Visitor(), A(1), B(std::unique_ptr<int>(new int(7)))),
              ::testing::Pair(1, 7));
  EXPECT_THAT(turbo::visit(Visitor(), A(1), B(std::string_view("ABC"))),
              ::testing::Pair(1, 3));
...
TEST(VariantTest, VisitNoArgs) {
  EXPECT_EQ(5, turbo::visit([] { return 5; }));
}
...
TEST(VariantTest, VisitReferenceWrapper) {
  ...
  EXPECT_EQ(1, turbo::visit(std::cref(cf), three, two));
  EXPECT_EQ(1, turbo::visit(std::ref(cf), three, two));
  EXPECT_EQ(1, turbo::visit(std::ref(mf), three, two));
}
...
TEST(VariantTest, VisitMemberFunction) {
  ...
  EXPECT_EQ(1, turbo::visit(&Class::Method, p, three, two));
  EXPECT_EQ(1, turbo::visit(&Class::ConstMethod, p, three, two));
  EXPECT_EQ(1, turbo::visit(&Class::ConstMethod, cp, three, two));
}
...
TEST(VariantTest, VisitDataMember) {
  ...
  EXPECT_EQ(5, turbo::visit(&Class::member, p));
...
TEST(VariantTest, MonostateBasic) {
  turbo::monostate mono;
  (void)mono;

  // TODO(mattcalabrese) Expose move triviality metafunctions in turbo.
  EXPECT_TRUE(turbo::is_trivially_default_constructible<turbo::monostate>::value);
  EXPECT_TRUE(is_trivially_move_constructible<turbo::monostate>::value);
  EXPECT_TRUE(turbo::is_trivially_copy_constructible<turbo::monostate>::value);
  EXPECT_TRUE(is_trivially_move_assignable<turbo::monostate>::value);
  EXPECT_TRUE(turbo::is_trivially_copy_assignable<turbo::monostate>::value);
  EXPECT_TRUE(turbo::is_trivially_destructible<turbo::monostate>::value);
}

TEST(VariantTest, VariantMonostateDefaultConstruction) {
  turbo::variant<turbo::monostate, NonDefaultConstructible> var;
...
TEST(VariantTest, MonostateComparisons) {
  ...
  EXPECT_TRUE(noexcept(std::declval<turbo::monostate>() ==
                       std::declval<turbo::monostate>()));
  EXPECT_TRUE(noexcept(std::declval<turbo::monostate>() !=
                       std::declval<turbo::monostate>()));
  EXPECT_TRUE(noexcept(std::declval<turbo::monostate>() <
                       std::declval<turbo::monostate>()));
...
TEST(VariantTest, NonmemberSwap) {
  ...
  EXPECT_THAT(a, VariantWith<SpecialSwap>(v1));
  EXPECT_THAT(b, VariantWith<SpecialSwap>(v2));
  EXPECT_TRUE(turbo::get<SpecialSwap>(b).special_swap);
}
...
TEST(VariantTest, BadAccess) {
  EXPECT_TRUE(noexcept(turbo::bad_variant_access()));
  turbo::bad_variant_access exception_obj;
...
TEST(VariantTest, MonostateHash) {
  turbo::monostate mono, other_mono;
  std::hash<turbo::monostate> const hasher{};
...
// result on the same value as different alternative.
...

  {
    ...
    std::hash<turbo::variant<const int>> c_hash;
    for (int i = 0; i < 100; ++i) {
...
TEST(VariantTest, TestVariantWithNonCopyableType) {
  ...
  const int kValue = 1;
  Variant v(kValue);
  ASSERT_TRUE(turbo::holds_alternative<int>(v));
  EXPECT_EQ(kValue, turbo::get<int>(v));
}
...
TEST(VariantTest, TestEmplace) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(0, turbo::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(turbo::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(turbo::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(1, turbo::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(turbo::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(turbo::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(3, turbo::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(turbo::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(turbo::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(6, turbo::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(turbo::holds_alternative<int>(v));
  ...
  ASSERT_TRUE(turbo::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(10, turbo::get<NonCopyable>(v).value);
  v = kValue;
  ASSERT_TRUE(turbo::holds_alternative<int>(v));
}
...
TEST(VariantTest, TestEmplaceDestroysCurrentValue) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<IncrementInDtor>(v));
  ASSERT_EQ(0, counter);
  v.emplace<NonCopyable>();
  ASSERT_TRUE(turbo::holds_alternative<NonCopyable>(v));
  EXPECT_EQ(1, counter);
...
TEST(VariantTest, TestMoveSemantics) {
  ...
  Variant v(turbo::WrapUnique(new int(10)));
  EXPECT_TRUE(turbo::holds_alternative<std::unique_ptr<int>>(v));

  // Construct a variant by moving from another variant.
  Variant v2(turbo::move(v));
  ASSERT_TRUE(turbo::holds_alternative<std::unique_ptr<int>>(v2));
  ASSERT_NE(nullptr, turbo::get<std::unique_ptr<int>>(v2));
  EXPECT_EQ(10, *turbo::get<std::unique_ptr<int>>(v2));

  // Moving from a variant object leaves it holding moved-from value of the
  // same element type.
  EXPECT_TRUE(turbo::holds_alternative<std::unique_ptr<int>>(v));
  ASSERT_NE(nullptr, turbo::get_if<std::unique_ptr<int>>(&v));
  ...
  v2 = turbo::move(v);
  ASSERT_TRUE(turbo::holds_alternative<std::unique_ptr<std::string>>(v2));
  EXPECT_EQ("foo", *turbo::get<std::unique_ptr<std::string>>(v2));
  EXPECT_TRUE(turbo::holds_alternative<std::unique_ptr<std::string>>(v));
}
...
TEST(VariantTest, TestImplicitConversion) {
  EXPECT_TRUE(turbo::holds_alternative<int>(PassThrough(0)));

  // We still need the explicit cast for std::string, because C++ won't apply
  // two user-defined implicit conversions in a row.
  EXPECT_TRUE(
      turbo::holds_alternative<std::string>(PassThrough(std::string("foo"))));
}
...
TEST(VariantTest, TestRvalueConversion) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<double>(var));
  EXPECT_EQ(0.0, turbo::get<double>(var));

  var = ConvertVariantTo<variant<double, std::string>>(
      variant<const char*, float>("foo"));
  ASSERT_TRUE(turbo::holds_alternative<std::string>(var));
  EXPECT_EQ("foo", turbo::get<std::string>(var));

  variant<double> singleton(
      ConvertVariantTo<variant<double>>(variant<int, float>(42)));
  ASSERT_TRUE(turbo::holds_alternative<double>(singleton));
  EXPECT_EQ(42.0, turbo::get<double>(singleton));

  singleton = ConvertVariantTo<variant<double>>(variant<int, float>(3.14f));
  ASSERT_TRUE(turbo::holds_alternative<double>(singleton));
  EXPECT_FLOAT_EQ(3.14f, static_cast<float>(turbo::get<double>(singleton)));

  singleton = ConvertVariantTo<variant<double>>(variant<int>(0));
  ASSERT_TRUE(turbo::holds_alternative<double>(singleton));
  EXPECT_EQ(0.0, turbo::get<double>(singleton));

  variant<int32_t, uint32_t> variant2(
      ConvertVariantTo<variant<int32_t, uint32_t>>(variant<int32_t>(42)));
  ASSERT_TRUE(turbo::holds_alternative<int32_t>(variant2));
  EXPECT_EQ(42, turbo::get<int32_t>(variant2));
  ...
  ASSERT_TRUE(turbo::holds_alternative<Convertible1>(variant3));

  variant3 = ConvertVariantTo<variant<Convertible1, Convertible2>>(
      variant<Convertible2, Convertible1>(Convertible2()));
  ASSERT_TRUE(turbo::holds_alternative<Convertible2>(variant3));
}
...
TEST(VariantTest, TestLvalueConversion) {
  ...
  ASSERT_TRUE(turbo::holds_alternative<double>(destination));
  EXPECT_EQ(0.0, turbo::get<double>(destination));

  variant<const char*, float> source2 = "foo";
  destination = ConvertVariantTo<variant<double, std::string>>(source2);
  ASSERT_TRUE(turbo::holds_alternative<std::string>(destination));
  EXPECT_EQ("foo", turbo::get<std::string>(destination));
...
TEST(VariantTest, DoesNotMoveFromLvalues) {
  ...
  Variant v2(v1);
  EXPECT_EQ(turbo::get<std::shared_ptr<const int>>(v1),
            turbo::get<std::shared_ptr<const int>>(v2));

  // Test copy-assignment operator
  v1 = std::make_shared<const std::string>("foo");
  v2 = v1;
  EXPECT_EQ(turbo::get<std::shared_ptr<const std::string>>(v1),
            turbo::get<std::shared_ptr<const std::string>>(v2));

  // Test converting copy constructor
  OtherVariant other(std::make_shared<int>(0));
  Variant v3(ConvertVariantTo<Variant>(other));
  EXPECT_EQ(turbo::get<std::shared_ptr<int>>(other),
            turbo::get<std::shared_ptr<const int>>(v3));

  other = std::make_shared<std::string>("foo");
  v3 = ConvertVariantTo<Variant>(other);
  EXPECT_EQ(turbo::get<std::shared_ptr<std::string>>(other),
            turbo::get<std::shared_ptr<const std::string>>(v3));
}
...
TEST(VariantTest, TestRvalueConversionViaConvertVariantTo) {
  ...
  EXPECT_THAT(turbo::get_if<double>(&var), Pointee(3.0));

  var = ConvertVariantTo<variant<double, std::string>>(
      variant<const char*, float>("foo"));
  EXPECT_THAT(turbo::get_if<std::string>(&var), Pointee(std::string("foo")));

  variant<double> singleton(
      ConvertVariantTo<variant<double>>(variant<int, float>(42)));
  EXPECT_THAT(turbo::get_if<double>(&singleton), Pointee(42.0));

  singleton = ConvertVariantTo<variant<double>>(variant<int, float>(3.14f));
  EXPECT_THAT(turbo::get_if<double>(&singleton), Pointee(DoubleEq(3.14f)));

  singleton = ConvertVariantTo<variant<double>>(variant<int>(3));
  EXPECT_THAT(turbo::get_if<double>(&singleton), Pointee(3.0));

  variant<int32_t, uint32_t> variant2(
      ConvertVariantTo<variant<int32_t, uint32_t>>(variant<int32_t>(42)));
  EXPECT_THAT(turbo::get_if<int32_t>(&variant2), Pointee(42));
...
TEST(VariantTest, TestLvalueConversionViaConvertVariantTo) {
  ...
  variant<double, std::string> destination(
      ConvertVariantTo<variant<double, std::string>>(source1));
  EXPECT_THAT(turbo::get_if<double>(&destination), Pointee(3.0));

  variant<const char*, float> source2 = "foo";
  destination = ConvertVariantTo<variant<double, std::string>>(source2);
  EXPECT_THAT(turbo::get_if<std::string>(&destination),
...
TEST(VariantTest, TestCopyAndMoveTypeTraits) {
  ...
  EXPECT_TRUE(turbo::is_copy_assignable<variant<std::string>>::value);
  EXPECT_TRUE(std::is_move_constructible<variant<std::string>>::value);
  EXPECT_TRUE(turbo::is_move_assignable<variant<std::string>>::value);
  EXPECT_TRUE(std::is_move_constructible<variant<std::unique_ptr<int>>>::value);
  EXPECT_TRUE(turbo::is_move_assignable<variant<std::unique_ptr<int>>>::value);
  EXPECT_FALSE(
      std::is_copy_constructible<variant<std::unique_ptr<int>>>::value);
  EXPECT_FALSE(turbo::is_copy_assignable<variant<std::unique_ptr<int>>>::value);
...
TEST(VariantTest, TestVectorOfMoveonlyVariant) {
  ...
  EXPECT_EQ(42, *turbo::get<std::unique_ptr<int>>(another_vec[0]));
  EXPECT_EQ("Hello", turbo::get<std::string>(another_vec[1]));
}
...
TEST(VariantTest, NestedVariant) {
  ...
  static_assert(turbo::is_trivially_copy_constructible<variant<variant<int>>>(),
                "");
  static_assert(turbo::is_trivially_copy_assignable<variant<variant<int>>>(),
                "");
  ...
  EXPECT_TRUE(turbo::holds_alternative<variant<int>>(z));
  EXPECT_EQ(x, turbo::get<variant<int>>(z));
}
...
TEST(VariantTest, TestTriviality) {
  {
    ...
    EXPECT_FALSE(turbo::is_trivially_copy_constructible<TrivDestVar>::value);
    EXPECT_FALSE(is_trivially_move_assignable<TrivDestVar>::value);
    EXPECT_FALSE(turbo::is_trivially_copy_assignable<TrivDestVar>::value);
    EXPECT_TRUE(turbo::is_trivially_destructible<TrivDestVar>::value);
  }

  {
    using TrivMoveVar = turbo::variant<TriviallyMovable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivMoveVar>::value);
    EXPECT_FALSE(turbo::is_trivially_copy_constructible<TrivMoveVar>::value);
    EXPECT_FALSE(is_trivially_move_assignable<TrivMoveVar>::value);
    EXPECT_FALSE(turbo::is_trivially_copy_assignable<TrivMoveVar>::value);
    EXPECT_TRUE(turbo::is_trivially_destructible<TrivMoveVar>::value);
  }

  {
    using TrivCopyVar = turbo::variant<TriviallyCopyable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivCopyVar>::value);
    EXPECT_TRUE(turbo::is_trivially_copy_constructible<TrivCopyVar>::value);
    EXPECT_FALSE(is_trivially_move_assignable<TrivCopyVar>::value);
    EXPECT_FALSE(turbo::is_trivially_copy_assignable<TrivCopyVar>::value);
    EXPECT_TRUE(turbo::is_trivially_destructible<TrivCopyVar>::value);
  }

  {
    using TrivMoveAssignVar = turbo::variant<TriviallyMoveAssignable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivMoveAssignVar>::value);
    EXPECT_FALSE(
        turbo::is_trivially_copy_constructible<TrivMoveAssignVar>::value);
    EXPECT_TRUE(is_trivially_move_assignable<TrivMoveAssignVar>::value);
    EXPECT_FALSE(turbo::is_trivially_copy_assignable<TrivMoveAssignVar>::value);
    EXPECT_TRUE(turbo::is_trivially_destructible<TrivMoveAssignVar>::value);
  }

  {
    using TrivCopyAssignVar = turbo::variant<TriviallyCopyAssignable>;

    EXPECT_TRUE(is_trivially_move_constructible<TrivCopyAssignVar>::value);
    EXPECT_TRUE(
        turbo::is_trivially_copy_constructible<TrivCopyAssignVar>::value);
    EXPECT_TRUE(is_trivially_move_assignable<TrivCopyAssignVar>::value);
    EXPECT_TRUE(turbo::is_trivially_copy_assignable<TrivCopyAssignVar>::value);
...
TEST(VariantTest, MoveCtorBug) {
  ...
  {
    using V = turbo::variant<TrivialCopyNontrivialMove, int>;
    V v1(turbo::in_place_index<0>);
    // this should invoke the move ctor, rather than the trivial copy ctor.
    V v2(std::move(v1));
    EXPECT_TRUE(turbo::get<0>(v2).called);
  }
  {
    // this case failed to compile before our fix due to a GCC bug.
    using V = turbo::variant<int, TrivialCopyNontrivialMove>;
    V v1(turbo::in_place_index<1>);
    // this should invoke the move ctor, rather than the trivial copy ctor.
    V v2(std::move(v1));
    EXPECT_TRUE(turbo::get<1>(v2).called);
  }
}
