...
#ifndef TURBO_META_FUNCTION_H_
#define TURBO_META_FUNCTION_H_
...
#ifndef NDEBUG
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define FU2_DETAIL_UNREACHABLE() ::turbo::detail::unreachable_debug()
#else
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
...
/// Configuration trait to configure the function_base class.
            ...
            namespace invocation {
                ...
                struct is_noexcept_correct<true, T, identity<Args...>>
                        : std::integral_constant<bool,
                                noexcept(::turbo::detail::invocation::invoke(
            ...
            namespace type_erasure {
                ...
                static constexpr void write_empty(data_accessor *accessor,
                                                                   bool empty) noexcept {
                ...
                constexpr auto retrieve(std::true_type /*is_inplace*/,
                                                         Accessor from,
...
/// control and vtable.
                ...
                public:
                    ...
                    constexpr data_accessor volatile *
...
/// An owning erasure
                ...
                public:
                    ...
                    constexpr erasure(std::false_type /*use_bool_op*/,
                                                       T &&callable,
                    ...
                    template<typename T, typename Allocator = std::allocator<std::decay_t<T>>>
                    constexpr erasure(std::true_type /*use_bool_op*/,
                                                       T &&callable,
                                                       Allocator &&allocator_ = Allocator{}) {
                        if (!!callable) {
                            vtable_t::init(vtable_,
...
// A non owning erasure
            ...
            /// `turbo::detail::function` types only.
            template <typename T>
            struct use_bool_op : std::false_type {};
...
/// SFINAES out if the given function2 is not owning correct to this one
            ...
            class function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>
                ...
                template<std::size_t, typename, typename...>
                friend
                class type_erasure::invocation_table::operator_impl;
            ...
            public:
                ...
                template<typename T, typename Allocator, //
                        ...
                        assert_wrong_copy_assign_t<T> * = nullptr,
                        assert_no_strong_except_guarantee_t<T> * = nullptr>
                constexpr function(T &&callable, Allocator &&allocator_)
                        : erasure_(use_bool_op<unrefcv_t<T>>{}, std::forward<T>(callable),
                                   std::forward<Allocator>(allocator_)) {
                }
                ...
                function &operator=(T &&callable) {
                    erasure_.assign(use_bool_op < unrefcv_t<T>>
                    {}, std::forward<T>(callable));
                    return *this;
                }
...
///
    ...
    ///
    ///   ```cpp
    ///   auto overloaded = turbo::overload([](std::true_type) { return true; },
