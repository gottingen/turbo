...
struct VariantOfAlternativesImpl<turbo::index_sequence<Indices...>> {
  using type = turbo::variant<VariantAlternative<Indices>...>;
};

template <std::size_t NumAlternatives>
using VariantOfAlternatives = typename VariantOfAlternativesImpl<
    turbo::make_index_sequence<NumAlternatives>>::type;
...
struct MakeWithIndex {
  ...
  static Variant Run(std::size_t index) {
    return index == CurrIndex
               ? Variant(turbo::in_place_index_t<CurrIndex>())
...
struct MakeVariantTuple<NumIndices, turbo::index_sequence<Dimensions...>> {
  using VariantTuple =
...
struct VisitTestBody {
  ...
  static bool Run(Vars& vars, State& state) {
    if (state.KeepRunning()) {
      turbo::apply(VisitorApplier(), vars[I]);
...
void BM_RedundantVisit(benchmark::State& state) {
  auto vars =
      MakeVariantTuple<NumIndices, turbo::make_index_sequence<NumDimensions>>::
          Run(static_cast<std::size_t>(state.range(0)));

  for (auto _ : state) {  // NOLINT
    benchmark::DoNotOptimize(vars);
    turbo::apply(VisitorApplier(), vars);
  }
}
