...
namespace turbo::detail {
    ...
    namespace {
        ...
        flags_registry *get_registry() {
            static turbo::resident<flags_registry> registry;
            return registry.get();
        }
    ...
    void register_flags_overrider(const std::string &name, const std::string &to,
        ...
        if (!value.first.empty()) {
            TURBO_LOG(FATAL)<<"Duplicate override for flag "<<name<<", was ["<<value.first<<"], now ["<<to<<"]";
        }
    ...
    void apply_flags_overrider() {
        for (auto&&[k, v] : *get_registry()) {
            ...
            if (current.is_default || v.second) {
                ...
                TURBO_VLOG(10)<<"Overriding flag ["<<k<<"] with ["<<v.first<<"].";
            }
        }
    }
