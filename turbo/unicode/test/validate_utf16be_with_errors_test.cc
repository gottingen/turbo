...
TEST(validate_utf16be__returns_true_for_valid_input__single_words) {
  ...
  for(size_t trial = 0; trial < 1000; trial++) {
    ...
    turbo::result res = implementation.ValidateUtf16BeWithErrors(reinterpret_cast<const char16_t*>(flipped.data()), flipped.size());
...
TEST(validate_utf16be__returns_true_for_empty_string) {
  ...
  turbo::result res = implementation.ValidateUtf16BeWithErrors(reinterpret_cast<const char16_t*>(buf), 0);
  ASSERT_EQUAL(res.error, turbo::error_code::SUCCESS);
