...
avx2_convert_utf32_to_utf16(const char32_t *buf, size_t len, char16_t *utf16_output) {
    ...
    while (buf + 8 + safety_margin <= end) {
        ...
        if (saturation_bitmask == 0xffffffff) {
            ...
            forbidden_bytemask = _mm256_or_si256(forbidden_bytemask,
            ...
            for (; k < forward; k++) {
                ...
                if ((word & 0xFFFF0000) == 0) {
                    ...
                    *utf16_output++ = is_big_endian(big_endian) ? char16_t((uint16_t(word) >> 8) | (uint16_t(word) << 8)) : char16_t(
                            word);
                } else {
...
avx2_convert_utf32_to_utf16_with_errors(const char32_t *buf, size_t len, char16_t *utf16_output) {
    ...
    while (buf + 8 + safety_margin <= end) {
        ...
        if (saturation_bitmask == 0xffffffff) {
            ...
            if (static_cast<uint32_t>(_mm256_movemask_epi8(forbidden_bytemask)) != 0x0) {
                return std::make_pair(result(error_code::SURROGATE, static_cast<size_t>(buf - start)), utf16_output);
            }
            ...
            for (; k < forward; k++) {
                ...
                if ((word & 0xFFFF0000) == 0) {
                    ...
                    if (word >= 0xD800 && word <= 0xDFFF) {
                        return std::make_pair(result(error_code::SURROGATE, static_cast<size_t>(buf - start) + k),
                                              utf16_output);
                    }
                    *utf16_output++ = is_big_endian(big_endian) ? char16_t((uint16_t(word) >> 8) | (uint16_t(word) << 8)) : char16_t(
                            word);
                    ...
                    // will generate a surrogate pair
                    if (word > 0x10FFFF) {
                        return std::make_pair(result(error_code::TOO_LARGE, static_cast<size_t>(buf - start) + k),
                                              utf16_output);
                    }
                    word -= 0x10000;
                    uint16_t high_surrogate = uint16_t(0xD800 + (word >> 10));
    ...
    return std::make_pair(result(error_code::SUCCESS, static_cast<size_t>(buf - start)), utf16_output);
}
