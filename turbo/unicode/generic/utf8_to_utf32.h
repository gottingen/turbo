...
// Bit 7 = Two Continuations
  ...
  struct validating_transcoder {
    ...
    TURBO_FORCE_INLINE size_t convert(const char* in, size_t size, char32_t* utf32_output) {
      size_t pos = 0;
    ...
    TURBO_FORCE_INLINE result convert_with_errors(const char* in, size_t size, char32_t* utf32_output) {
      size_t pos = 0;
      char32_t* start{utf32_output};
      const size_t safety_margin = 16; // to avoid overruns!
      while(pos + 64 + safety_margin <= size) {
