...
#ifndef TURBO_UNICODE_AVX2_CONVERT_MASKED_UTF8_TO_UTF32_H_
#define TURBO_UNICODE_AVX2_CONVERT_MASKED_UTF8_TO_UTF32_H_
...
namespace turbo::unicode::simd {
    ...
    inline size_t convert_masked_utf8_to_utf32(const char *input,
    ...
    inline size_t convert_masked_utf8_to_utf16(const char *input,
                                        uint64_t utf8_end_of_code_point_mask,
        ...
        if (((utf8_end_of_code_point_mask & 0xffff) == 0xaaaa)) {
            ...
            if (turbo::is_big_endian(big_endian)) composed = _mm_shuffle_epi8(composed, swap);
        ...
        if (input_utf8_end_of_code_point_mask == 0x924) {
            ...
            if (turbo::is_big_endian(big_endian)) composed_repacked = _mm_shuffle_epi8(composed_repacked, swap);
        ...
        if (idx < 64) {
            ...
            for (size_t i = 0; i < 3; i++) {
                if (basic_buffer[i] > 0x3c00000) {
                    ...
                    utf16_output[0] = turbo::is_big_endian(big_endian) ? uint16_t(basic_buffer_swap[i]) : uint16_t(
                            basic_buffer[i]);
                    utf16_output++;
                }
            }
...
#endif  // TURBO_UNICODE_AVX2_CONVERT_MASKED_UTF8_TO_UTF32_H_
