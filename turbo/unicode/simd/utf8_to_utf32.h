...
namespace turbo::unicode::simd::utf8_to_utf32 {
    ...
    TURBO_FORCE_INLINE simd8<uint8_t, Engine> check_special_cases(const simd8<uint8_t, Engine> input, const simd8<uint8_t, Engine> prev1) {
        ...
        const simd8<uint8_t, Engine> byte_1_high = prev1.template shr<4>().template lookup_16<uint8_t>(
        ...
        const simd8<uint8_t, Engine> byte_2_high = input.template shr<4>().template lookup_16<uint8_t>(
    ...
    TURBO_FORCE_INLINE simd8<uint8_t, Engine> check_multibyte_lengths(const simd8<uint8_t, Engine> input,
                            const simd8<uint8_t, Engine> prev_input, const simd8<uint8_t, Engine> sc) {
    ...
    struct validating_transcoder {
        ...
        convert(const char *in, size_t size, char32_t *utf32_output) {
            size_t pos = 0;
            ...
            while (pos + 64 + safety_margin <= size) {
                ...
                if (input.is_ascii()) {
                    ...
                    auto zero = simd8<uint8_t, Engine>{uint8_t(0)};
            ...
            if (errors()) { return 0; }
            if (pos < size) {
                size_t howmany = turbo::unicode::utf8_to_utf32::convert(in + pos, size - pos, utf32_output);
