...
namespace turbo::unicode::simd::utf8_to_utf16 {
    ...
    struct validating_transcoder {
        ...
        check_utf8_bytes(const simd8 <uint8_t, Engine> input, const simd8 <uint8_t, Engine> prev_input) {
        ...
        TURBO_FORCE_INLINE size_t convert(const char *in, size_t size, char16_t *utf16_output) {
            ...
            while (pos + 64 + safety_margin <= size) {
                ...
                if (input.is_ascii()) {
                    input.template store_ascii_as_utf16<endian>(utf16_output);
                    utf16_output += 64;
                    pos += 64;
                } else {
                    // you might think that a for-loop would work, but under Visual Studio, it is not good enough.
                    static_assert((simd8x64<uint8_t, Engine>::NUM_CHUNKS == 2) ||
                                  (simd8x64<uint8_t, Engine>::NUM_CHUNKS == 4),
            ...
            if (errors()) { return 0; }
            if (pos < size) {
                size_t howmany = turbo::unicode::utf8_to_utf16::convert<endian>(in + pos, size - pos, utf16_output);
