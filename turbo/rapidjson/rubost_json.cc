...
#include "turbo/rapidjson/ostreamwrapper.h"
...
RAPIDJSON_NAMESPACE_BEGIN
    ...
    turbo::result_status robust_parse(const std::string &file_path, rapidjson::Document *doc) {
        std::error_code ec;
        auto r = turbo::exists(file_path, ec);
        if (ec) {
            return turbo::result_status::from_error_code(ec);
        }
        if(!r) {
            return turbo::result_status(-1, "not exists file");
        }
        ...
        if (!static_cast<rapidjson::ParseResult>(doc->Parse(content.c_str()))) {
            return turbo::result_status(-2, "json parse error");
        }
        return turbo::result_status::success();
    }


    turbo::result_status robust_write(const rapidjson::Document &d, const std::string &file_path) {
        std::ofstream ofs(file_path);
        if (!ofs) {
            return turbo::result_status(-2, "open file error");
        }
        rapidjson::OStreamWrapper osw(ofs);
        rapidjson::Writer<rapidjson::OStreamWrapper> writer(osw);
        if (!d.Accept(writer)) {
            return  turbo::result_status(-2, "write file error");
        }
        return turbo::result_status::success();
    }
    ...
    std::optional<uint64_t> as_uint64(OptionalRefRapidJsonValue v_) {
        if (v_.has_value()) {
            ...
            if (v.IsString()) {
                uint64_t n;
                if (!turbo::simple_atoi(std::string_view{v.GetString(), v.GetStringLength()}, &n)) {
                    return std::nullopt;
                }
                return n;
            }
    ...
    std::optional<double> as_double(OptionalRefRapidJsonValue v_) {
        if (v_.has_value()) {
            ...
            if (v.IsString()) {
                double n;
                if (!turbo::simple_atod(std::string_view{v.GetString(), v.GetStringLength()}, &n)) {
                    return std::nullopt;
                }
                return n;
            }
