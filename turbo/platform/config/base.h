...
#ifndef TURBO_PLATFORM_CONFIG_BASE_H_
#define TURBO_PLATFORM_CONFIG_BASE_H_


// Identify the compiler and declare the TURBO_COMPILER_xxxx defines
...
// Identify the platform and declare the TURBO_xxxx defines
...
// is defined.
#if defined(TURBO_COMPILER_CLANG) && defined(TURBO_PLATFORM_APPLE)
	#include <stddef.h>
#endif
...
// would have put this into the compilertraits header.
#if !defined(TURBO_COMPILER_HAS_INTTYPES) && (!defined(_MSC_VER) || (_MSC_VER > 1500)) && (defined(TURBO_COMPILER_IS_C99) || defined(INT8_MIN) || defined(TURBO_COMPILER_HAS_C99_TYPES) || defined(_SN_STDINT_H))
	#define TURBO_COMPILER_HAS_INTTYPES
#endif

#ifdef TURBO_COMPILER_HAS_INTTYPES // If the compiler supports inttypes...
	// ------------------------------------------------------------------------
	...
	#if !defined(TURBO_COMPILER_MSVC) || (defined(TURBO_COMPILER_MSVC) && TURBO_COMPILER_VERSION >= 1800)
	...
	#if defined(__MINGW32__) || (defined(TURBO_PLATFORM_ANDROID) && !(defined(TURBO_ANDROID_SDK_LEVEL) && TURBO_ANDROID_SDK_LEVEL >= 21))
...
// type sizes
...
#if defined(TURBO_COMPILER_HAS_INTTYPES) && (!defined(TURBO_COMPILER_MSVC) || (defined(TURBO_COMPILER_MSVC) && TURBO_COMPILER_VERSION >= 1800))
	#define TURBO_COMPILER_HAS_C99_FORMAT_MACROS
#endif

#ifndef TURBO_COMPILER_HAS_C99_FORMAT_MACROS
	// ------------------------------------------------------------------------
	...
	#if defined(TURBO_COMPILER_MSVC) // VC++ 7.1+ understands long long as a data type but doesn't accept %ll as a printf specifier.
		#define TURBO_PRI_64_LENGTH_SPECIFIER "I64"
		#define TURBO_SCN_64_LENGTH_SPECIFIER "I64"
	#else
		#define TURBO_PRI_64_LENGTH_SPECIFIER "ll"
		#define TURBO_SCN_64_LENGTH_SPECIFIER "ll"
	...
	#if defined(TURBO_COMPILER_IS_C99) || defined(TURBO_COMPILER_GNUC)
		#define PRId8     "hhd"
		#define PRIi8     "hhi"
		#define PRIo8     "hho"
		#define PRIu8     "hhu"
	...
	#define PRId64        TURBO_PRI_64_LENGTH_SPECIFIER "d"
	#define PRIi64        TURBO_PRI_64_LENGTH_SPECIFIER "i"
	#define PRIo64        TURBO_PRI_64_LENGTH_SPECIFIER "o"
	#define PRIu64        TURBO_PRI_64_LENGTH_SPECIFIER "u"
	#define PRIx64        TURBO_PRI_64_LENGTH_SPECIFIER "x"
	#define PRIX64        TURBO_PRI_64_LENGTH_SPECIFIER "X"
	...
	#elif (TURBO_PLATFORM_PTR_SIZE == 8)
	...
	#define SCNd64        TURBO_SCN_64_LENGTH_SPECIFIER "d"
	#define SCNi64        TURBO_SCN_64_LENGTH_SPECIFIER "i"
	#define SCNo64        TURBO_SCN_64_LENGTH_SPECIFIER "o"
	#define SCNu64        TURBO_SCN_64_LENGTH_SPECIFIER "u"
	#define SCNx64        TURBO_SCN_64_LENGTH_SPECIFIER "x"

	#if defined(TURBO_COMPILER_MSVC) && (TURBO_COMPILER_VERSION >= 1900)
		#define SCNdPTR       PRIdPTR
		#define SCNiPTR       PRIiPTR
		#define SCNoPTR       PRIoPTR
		#define SCNuPTR       PRIuPTR
		#define SCNxPTR       PRIxPTR
	#elif (TURBO_PLATFORM_PTR_SIZE == 4)
...
//
#if !defined(_INTPTR_T_DEFINED) && !defined(_intptr_t_defined) && !defined(TURBO_COMPILER_HAS_C99_TYPES)
	#if (TURBO_PLATFORM_PTR_SIZE == 4)
...
#if !defined(_UINTPTR_T_DEFINED) && !defined(_uintptr_t_defined) && !defined(TURBO_COMPILER_HAS_C99_TYPES)
	#if (TURBO_PLATFORM_PTR_SIZE == 4)
...
#if !defined(TURBO_COMPILER_HAS_INTTYPES)
...
//
	...
	// so for all compilers, as size_t may be based on int, long, or long long.
		...
		#if defined(_MSC_VER) && (TURBO_PLATFORM_PTR_SIZE == 8)
...
//
#if defined(TBBASE_ENABLE_EATEXT_MACROS)
	#define EAText8(x)   x
	#define EAChar8(x)   x

	#define EAText16(x)  TURBO_CHAR16(x)
	#define EAChar16(x)  TURBO_CHAR16(x)
#endif
...
//
...
#elif !defined(__cplusplus) && defined(TURBO_PLATFORM_ANDROID) && ((defined(__STDC_VERSION__) && __STDC_VERSION__ < 201100L) || !defined(__STDC_VERSION__))
...
#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 401) && defined(TURBO_COMPILER_CPP11_ENABLED)
	// static_assert is defined by the compiler.
	...
	#define NEED_CUSTOM_STATIC_ASSERT
		...
		#define TURBO_STATIC_ASSERT_UNUSED_ATTRIBUTE        __attribute__((unused))
	#else
		#define TURBO_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#endif
	#define TURBO_STATIC_ASSERT_TOKEN_PASTE(a,b)        a ## b
	#define TURBO_STATIC_ASSERT_CONCATENATE_HELPER(a,b) TURBO_STATIC_ASSERT_TOKEN_PASTE(a,b)

	#if defined(__COUNTER__) // If this extension is available, which allows multiple statements per line...
		#define static_assert(expression, description) typedef char TURBO_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert,__COUNTER__) [((expression) != 0) ? 1 : -1] TURBO_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#else
		#define static_assert(expression, description) typedef char TURBO_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert,__LINE__) [((expression) != 0) ? 1 : -1] TURBO_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#endif
...
// TURBO_IS_ENABLED
//
// TURBO_IS_ENABLED is intended to be used for detecting if compile time features are enabled or disabled.
//
...
// must be set to either TURBO_ENABLED or TURBO_DISABLED.  (Do not try to set the feature define directly to some other
// value.)
//
...
//      // TURBO_ENABLED or TURBO_DISABLED.
//      #define USER_PROVIDED_FEATURE_DEFINE TURBO_ENABLED
//
//      #if TURBO_IS_ENABLED(USER_PROVIDED_FEATURE_DEFINE)
//          // USER_PROVIDED_FEATURE_DEFINE is enabled
//      #else
//          // USER_PROVIDED_FEATURE_DEFINE is disabled
//      #endif
//
#define TURBO_ENABLED              111-
#define TURBO_DISABLED             333-
...
#define TURBO_IS_ENABLED(x) (333 == 333 * 111 / ((x 0) * (((x 0) == 333 ? 1 : 0) + ((x 0) == 111 ? 1 : 0))))

#endif  // TURBO_PLATFORM_CONFIG_BASE_H_
