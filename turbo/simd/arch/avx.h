...
#ifndef TURBO_SIMD_ARCH_AVX_H_
#define TURBO_SIMD_ARCH_AVX_H_
...
namespace turbo::simd {
    ...
    namespace kernel {
        ...
        namespace detail {
            ...
            template<class A>
            inline batch<float, A>
            fast_cast(batch<uint32_t, A> const &v, batch<float, A> const &, requires_arch<avx>) noexcept {
                ...
                __m256i v_lo = bitwise_and(batch<uint32_t, A>(v), batch<uint32_t, A>(
                        msk_lo)); /* extract the 16 lowest significant bits of self                             */
                __m256i v_hi = bitwise_rshift(batch<uint32_t, A>(v), 16,
                                              avx{}); /* 16 most significant bits of v                                                 */
            ...
            fast_cast(batch<float, A> const &self, batch<uint32_t, A> const &, requires_arch<avx>) noexcept {
                return _mm256_castps_si256(
                        _mm256_blendv_ps(_mm256_castsi256_ps(_mm256_cvttps_epi32(self)),
                                         _mm256_xor_ps(
                                                 _mm256_castsi256_ps(_mm256_cvttps_epi32(
                                                         _mm256_sub_ps(self, _mm256_set1_ps(1u << 31)))),
