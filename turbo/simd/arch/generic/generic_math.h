...
#ifndef TURBO_SIMD_ARCH_GENERIC_MATH_H_
#define TURBO_SIMD_ARCH_GENERIC_MATH_H_

#include "turbo/simd/arch/scalar.h"
#include "turbo/simd/arch/generic/generic_details.h"
#include "turbo/simd/arch/generic/generic_trigo.h"
    ...
    namespace kernel {
...
#ifndef TURBO_SIMD_NO_DENORMALS
...
#ifndef TURBO_NO_INFINITIES
            r1 = select(turbo::simd::isinf(self), sign(self), r1);
#endif
            return r1;
        }
...
#ifndef TURBO_NO_INFINITIES
            z = select(turbo::simd::isinf(self), sign(self), z);
#endif
        ...
        namespace detail {
            ...
            private:
                ...
                inline B eval(::turbo::simd::detail::index_sequence<Is...>, const Tuple &tuple) {
                    return estrin{x * x}(std::get<Is>(tuple)...);
                }

                template<class... Args>
                inline B eval(const std::tuple<Args...> &tuple) noexcept {
                    return eval(::turbo::simd::detail::make_index_sequence<sizeof...(Args)>(), tuple);
                }
        ...
        namespace detail {
            ...
            static inline batch<float, A> expm1(const batch<float, A> &a) noexcept {
                ...
                using i_type = as_integer_t<batch_type>;
                i_type ik = to_int(k);
                batch_type two2mk = ::turbo::simd::bitwise_cast<float>(
            ...
            static inline batch<double, A> expm1(const batch<double, A> &a) noexcept {
                ...
                using i_type = as_integer_t<batch_type>;
                i_type ik = to_int(k);
                batch_type two2mk = ::turbo::simd::bitwise_cast<double>(
                        (constants::maxexponent<batch_type>() - ik) << constants::nmb<batch_type>());
        ...
        frexp(const batch<T, A> &self, batch<as_integer_t<T>, A> &exp, requires_arch<generic>) noexcept {
            ...
            i_type r1 = m1f & ::turbo::simd::bitwise_cast<int_type>(self);
            batch_type x = self & ::turbo::simd::bitwise_cast<T>(~m1f);
            ...
            return select((self != batch_type(0.)), x | ::turbo::simd::bitwise_cast<T>(constants::mask2frexp<batch_type>()),
                          batch_type(0.));
        }
        ...
        inline batch<T, A> ipow(batch<T, A> const &self, ITy other, requires_arch<generic>) noexcept {
            return ::turbo::simd::detail::ipow(self, other);
        }
        ...
        ldexp(const batch<T, A> &self, const batch<as_integer_t<T>, A> &other, requires_arch<generic>) noexcept {
            ...
            return self * ::turbo::simd::bitwise_cast<T>(ik);
        }
...
#endif
            i_type ix = ::turbo::simd::bitwise_cast<int_type>(x);
            ix += 0x3f800000 - 0x3f3504f3;
            k += (ix >> 23) - 0x7f;
            ix = (ix & i_type(0x007fffff)) + 0x3f3504f3;
            x = ::turbo::simd::bitwise_cast<float>(ix);
            batch_type f = --x;
        ...
        inline batch<double, A> log(batch<double, A> const &self, requires_arch<generic>) noexcept {
            ...
            batch_type x = self;
            i_type hx = ::turbo::simd::bitwise_cast<int_type>(x) >> 32;
            i_type k(0);
            auto isnez = (self != batch_type(0.));
#ifndef TURBO_SIMD_NO_DENORMALS
            auto test = (self < constants::smallestposval<batch_type>()) && isnez;
            ...
            x = ::turbo::simd::bitwise_cast<double>(hx << 32 | (i_type(0xffffffff) & ::turbo::simd::bitwise_cast<int_type>(x)));
        ...
        inline batch<double, A> log2(batch<double, A> const &self, requires_arch<generic>) noexcept {
            ...
            hi = hi & ::turbo::simd::bitwise_cast<double>((constants::allbits<i_type>() << 32));
        ...
        inline batch<float, A> log10(batch<float, A> const &self, requires_arch<generic>) noexcept {
            ...
            hibits &= ::turbo::simd::bitwise_cast<float>(i_type(0xfffff000));
        ...
        inline batch<float, A> log1p(batch<float, A> const &self, requires_arch<generic>) noexcept {
            ...
            i_type iu = ::turbo::simd::bitwise_cast<int_type>(uf);
            iu += 0x3f800000 - 0x3f3504f3;
            i_type k = (iu >> 23) - 0x7f;
            iu = (iu & i_type(0x007fffff)) + 0x3f3504f3;
            batch_type f = --(::turbo::simd::bitwise_cast<float>(iu));
            batch_type s = f / (batch_type(2.) + f);
            batch_type z = s * s;
            batch_type w = z * z;
            batch_type t1 = w * detail::horner<batch_type, 0x3eccce13, 0x3e789e26>(w);
        ...
        inline batch<double, A> log1p(batch<double, A> const &self, requires_arch<generic>) noexcept {
            ...
            const batch_type uf = self + batch_type(1.);
            auto isnez = (uf != batch_type(0.));
            i_type hu = ::turbo::simd::bitwise_cast<int_type>(uf) >> 32;
            ...
            batch_type f = ::turbo::simd::bitwise_cast<double>(
                    (hu << 32) | (i_type(0xffffffff) & ::turbo::simd::bitwise_cast<int_type>(uf)));
            f = --f;
            batch_type hfsq = batch_type(0.5) * f * f;
            batch_type s = f / (batch_type(2.) + f);
            batch_type z = s * s;
        ...
        nearbyint_as_int(batch<float, A> const &self, requires_arch<generic>) noexcept {
            ...
            return kernel::detail::apply_transform<U>([](float x) noexcept -> U { return std::lroundf(x); },
                                                      self);
        }
        ...
        nearbyint_as_int(batch<double, A> const &self, requires_arch<generic>) noexcept {
            ...
            return kernel::detail::apply_transform<U>([](double x) noexcept -> U { return std::llround(x); },
                                                      self);
        }
        ...
        namespace detail {
            ...
            struct nextafter_kernel<T, A, false> {
                ...
                static inline batch_type next(const batch_type &b) noexcept {
                    batch_type n = ::turbo::simd::bitwise_cast<T>(::turbo::simd::bitwise_cast<int_type>(b) + int_type(1));
                    return select(b == constants::infinity<batch_type>(), b, n);
                }

                static inline batch_type prev(const batch_type &b) noexcept {
                    batch_type p = ::turbo::simd::bitwise_cast<T>(::turbo::simd::bitwise_cast<int_type>(b) - int_type(1));
        ...
        inline batch<std::complex<T>, A> sqrt(batch<std::complex<T>, A> const &z, requires_arch<generic>) noexcept {
            ...
            real_batch t0 = select(condxp, turbo::simd::sqrt(0.5 * (r + x)), turbo::simd::sqrt(0.5 * (r - x)));
...
#endif  // TURBO_SIMD_ARCH_GENERIC_MATH_H_
