...
#include "turbo/simd/arch/generic/trigo.h"
...
namespace turbo::simd::kernel {
    ...
    inline batch<float, A> cbrt(batch<float, A> const &self, requires_arch<generic>) noexcept {
        ...
        x = x | bitofsign(self);
#endif
#ifndef TURBO_SIMD_NO_INFINITIES
    ...
    frexp(const batch<T, A> &self, batch<as_integer_t<T>, A> &exp, requires_arch<generic>) noexcept {
        ...
        return select((self != batch_type(0.)),
                      x | ::turbo::simd::bitwise_cast<T>(constants::mask2frexp<batch_type>()), batch_type(0.));
    }
    ...
    inline batch<double, A> log(batch<double, A> const &self, requires_arch<generic>) noexcept {
        ...
        x = ::turbo::simd::bitwise_cast<double>(
                hx << 32 | (i_type(0xffffffff) & ::turbo::simd::bitwise_cast<int_type>(x)));
    ...
    namespace detail {
        ...
        struct nextafter_kernel<T, A, false> {
            ...
            static inline batch_type next(const batch_type &b) noexcept {
                batch_type n = ::turbo::simd::bitwise_cast<T>(
                        ::turbo::simd::bitwise_cast<int_type>(b) + int_type(1));
                return select(b == constants::infinity<batch_type>(), b, n);
            }

            static inline batch_type prev(const batch_type &b) noexcept {
                batch_type p = ::turbo::simd::bitwise_cast<T>(
                        ::turbo::simd::bitwise_cast<int_type>(b) - int_type(1));
