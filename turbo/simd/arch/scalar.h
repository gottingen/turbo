...
namespace turbo::simd {
    ...
    namespace detail {
#define TURBO_SIMD_HASSINCOS_TRAIT(func)                                                                                              \
    template <class S>                                                                                                           \
    struct has##func                                                                                                             \
    {                                                                                                                            \
        template <class T>                                                                                                       \
...
#define TURBO_SIMD_HASSINCOS(func, T) has##func<T>::value

        TURBO_SIMD_HASSINCOS_TRAIT(sincos);

        TURBO_SIMD_HASSINCOS_TRAIT(sincosf);

        TURBO_SIMD_HASSINCOS_TRAIT(__sincos);

        TURBO_SIMD_HASSINCOS_TRAIT(__sincosf);

        struct generic_sincosf {
            template<class T>
            typename std::enable_if<TURBO_SIMD_HASSINCOS(sincosf, T), void>::type
            operator()(float val, T &s, T &c) {
                sincosf(val, &s, &c);
            }

            template<class T>
            typename std::enable_if<!TURBO_SIMD_HASSINCOS(sincosf, T) && TURBO_SIMD_HASSINCOS(__sincosf, T), void>::type
            operator()(float val, T &s, T &c) {
                __sincosf(val, &s, &c);
            }
            ...
            template<class T>
            typename std::enable_if<
                    !TURBO_SIMD_HASSINCOS(sincosf, T) && !TURBO_SIMD_HASSINCOS(__sincosf, T), void>::type
            operator()(float val, T &s, T &c) {
                s = std::sin(val);
                c = std::cos(val);
            }
        ...
        struct generic_sincos {
            template<class T>
            typename std::enable_if<TURBO_SIMD_HASSINCOS(sincos, T), void>::type
            operator()(double val, T &s, T &c) {
                sincos(val, &s, &c);
            }

            template<class T>
            typename std::enable_if<!TURBO_SIMD_HASSINCOS(sincos, T) && TURBO_SIMD_HASSINCOS(__sincos, T), void>::type
            operator()(double val, T &s, T &c) {
                __sincos(val, &s, &c);
            }
...
#undef TURBO_SIMD_HASSINCOS_TRAIT
#undef TURBO_SIMD_HASSINCOS
    }
