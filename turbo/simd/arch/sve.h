...
#ifndef TURBO_SIMD_ARCH_SVE_H_
#define TURBO_SIMD_ARCH_SVE_H_
...
namespace turbo::simd
{
    ...
    namespace kernel
    {
        namespace detail
        {
            using turbo::simd::index;
            using turbo::simd::types::detail::sve_vector_type;
        ...
        namespace detail
        {
            ...
            inline batch<T, A> sve_extract_pair(batch<T, A> const&, batch<T, A> const& /*rhs*/, std::size_t, ::turbo::simd::detail::index_sequence<>) noexcept
            {
            ...
            inline batch<T, A> sve_extract_pair(batch<T, A> const& lhs, batch<T, A> const& rhs, std::size_t n, ::turbo::simd::detail::index_sequence<I, Is...>) noexcept
            {
                if (n == I)
                {
                ...
                {
                    return sve_extract_pair(lhs, rhs, n, ::turbo::simd::detail::index_sequence<Is...>());
                }
            }

            template <class A, class T, size_t... Is>
            inline batch<T, A> sve_extract_pair_impl(batch<T, A> const& lhs, batch<T, A> const& rhs, std::size_t n, ::turbo::simd::detail::index_sequence<0, Is...>) noexcept
            {
                if (n == 0)
                {
                    return rhs;
        ...
        inline batch<T, A> extract_pair(batch<T, A> const& lhs, batch<T, A> const& rhs, std::size_t n, requires_arch<sve>) noexcept
        {
            ...
            return detail::sve_extract_pair_impl(lhs, rhs, n, ::turbo::simd::detail::make_index_sequence<size>());
        }
