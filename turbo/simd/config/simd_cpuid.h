...
#ifndef TURBO_SIMD_CONFIG_SIMD_CPUID_H_
#define TURBO_SIMD_CONFIG_SIMD_CPUID_H_
...
#error "Unsupported configuration"
                ...
                if (sse4_2)
                    best = std::max(best, fma3<turbo::simd::sse4_2>::version() * fma3_sse);

                avx = regs1[2] >> 28 & 1;
                best = std::max(best, avx::version() * avx);

                fma3_avx = avx && fma3_sse;
                best = std::max(best, fma3<turbo::simd::avx>::version() * fma3_avx);
                ...
                // sse4a = regs[2] >> 6 & 1;
                // best = std::max(best, TURBO_SIMD_X86_AMD_SSE4A_VERSION * sse4a);

                // xop = regs[2] >> 11 & 1;
                // best = std::max(best, TURBO_SIMD_X86_AMD_XOP_VERSION * xop);
                ...
                best = std::max(best, fma3<turbo::simd::avx2>::version() * fma3_avx2);
...
#endif  // TURBO_SIMD_CONFIG_SIMD_CPUID_H_
