...
namespace turbo::profiling_internal {
    ...
    template<typename T>
    class ElementContainer<T, typename std::enable_if<profiling_internal::is_atomical<T>::value>::type> {
    public:
    ...
    template <typename T> struct add_cr_non_integral {
        typedef typename std::conditional<std::is_integral<T>::value, T,
                typename std::add_lvalue_reference_t<typename std::add_const<T>::type>::type> type;
    };
    ...
    public:
        ...
        explicit AgentCombiner(const ResultTp result_identity = ResultTp(),
                ...
                : _id(AgentGroup::create_new_id()), _op(op), _global_result(result_identity),
