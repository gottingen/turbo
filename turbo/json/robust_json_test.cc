  ...
  "arr": [
    ...
    TEST(RobustJsonTest, UsageBasic) {
...

        {
            ...
            EXPECT_EQ(robust_json["hello"]["subhello"].cast<turbo::string_view>(), "world");
            EXPECT_FALSE(robust_json["notexist"]["subhello"]);
            EXPECT_EQ(robust_json["notexist"]["subhello"].cast<turbo::string_view>(), "");
        }

        {
            rapidjson::robust_json<> rbdoc{root};
            rapidjson::robust_json<> rbhello = rbdoc["hello"];
            EXPECT_EQ(rbhello["subhello"].cast<turbo::string_view>(), "world");
    ...
    TEST(RobustJsonTest, UsageObject) {
        ...
        EXPECT_EQ(rbhello["newkey"].cast<turbo::string_view>(), "newvalue");
    }
    ...
    TEST(RobustJsonTest, UsageArray) {
        ...
        EXPECT_EQ(rbdoc["arr"][3]["inside_array_key"].cast<turbo::string_view>(), "inside_array_value");
    ...
    TEST(RobustJsonTest, UsageRaw) {
        ...
        EXPECT_EQ(robust_json[0]["subhello"].cast<turbo::string_view>(), "world");
    }
