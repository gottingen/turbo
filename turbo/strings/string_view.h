...
// -----------------------------------------------------------------------------
//
// This file contains the definition of the `turbo::string_view` class. A
...
// `std::string`, or another `turbo::string_view` argument with no need to copy
// the string data. Systematic use of `string_view` within function arguments
// reduces data copies and `strlen()` calls.
//
// Because of its small size, prefer passing `string_view` by value:
//
//   void MyFunction(turbo::string_view arg);
//
// If circumstances require, you may also pass one by const reference:
//
//   void MyFunction(const turbo::string_view& arg);  // not preferred
//
...
//   turbo::string_view sv = obj.ReturnAString();
//
//   // GOOD use of string_view: str outlives sv
//   std::string str = obj.ReturnAString();
//   turbo::string_view sv = str;
//
...
// example, when splitting a string, `std::vector<turbo::string_view>` is a
...
//   turbo::string_view sv(nullptr, 0);
//
...
//   turbo::string_view();
//   turbo::string_view(nullcp, 0);
//   turbo::string_view("");
//   turbo::string_view("", 0);
//   turbo::string_view("abcdef", 0);
//   turbo::string_view("abcdef" + 6, 0);
//
// All empty `string_view` objects whether null or not, are equal:
//
//   turbo::string_view() == turbo::string_view("", 0)
//   turbo::string_view(nullptr, 0) == turbo::string_view("abcdef"+6, 0)
class string_view {
 public:
  using traits_type = std::char_traits<char>;
  using value_type = char;
  using pointer = turbo::Nullable<char*>;
  using const_pointer = turbo::Nullable<const char*>;
  using reference = char&;
  using const_reference = const char&;
  using const_iterator = turbo::Nullable<const char*>;
  ...
  constexpr string_view(  // NOLINT(runtime/explicit)
      turbo::Nonnull<const char*> str)
      : ptr_(str), length_(str ? StrlenInternal(str) : 0) {}

  // Implicit constructor of a `string_view` from a `const char*` and length.
  constexpr string_view(turbo::Nullable<const char*> data, size_type len)
  ...
  // 'string_view` and another `turbo::string_view`.
  constexpr int compare(size_type pos1, size_type count1, string_view v) const {
    return substr(pos1, count1).compare(v);
  }

  // Overload of `string_view::compare()` for comparing a substring of the
  // `string_view` and a substring of another `turbo::string_view`.
  ...
  constexpr int compare(turbo::Nonnull<const char*> s) const {
  ...
  constexpr int compare(size_type pos1, size_type count1,
                        turbo::Nonnull<const char*> s) const {
  ...
  constexpr int compare(size_type pos1, size_type count1,
                        turbo::Nonnull<const char*> s, size_type count2) const {
  ...
  size_type find(turbo::Nonnull<const char*> s, size_type pos,
  ...
  size_type find(turbo::Nonnull<const char *> s, size_type pos = 0) const {
  ...
  // C-style string `s` within the `string_view`.
  size_type rfind(turbo::Nonnull<const char*> s, size_type pos,
                  size_type count) const {
  ...
  // `s` within the `string_view`.
  size_type rfind(turbo::Nonnull<const char*> s, size_type pos = npos) const {
  ...
  size_type find_first_of(turbo::Nonnull<const char*> s, size_type pos,
                          size_type count) const {
  ...
  size_type find_first_of(turbo::Nonnull<const char*> s,
  ...
  // different C-style string `s` within the `string_view`.
  size_type find_last_of(turbo::Nonnull<const char*> s, size_type pos,
                         size_type count) const {
  ...
  // string `s` within the `string_view`.
  size_type find_last_of(turbo::Nonnull<const char*> s,
  ...
  // different C-style string `s` within the `string_view`.
  size_type find_first_not_of(turbo::Nonnull<const char*> s, size_type pos,
                              size_type count) const {
  ...
  // C-style string `s` within the `string_view`.
  size_type find_first_not_of(turbo::Nonnull<const char*> s,
                              size_type pos = 0) const {
  ...
  // different C-style string `s` within the `string_view`.
  size_type find_last_not_of(turbo::Nonnull<const char*> s, size_type pos,
                             size_type count) const {
  ...
  // C-style string `s` within the `string_view`.
  size_type find_last_not_of(turbo::Nonnull<const char*> s,
                             size_type pos = npos) const {
  ...
  constexpr bool starts_with(string_view s) const noexcept {
    return s.empty() ||
           (size() >= s.size() &&
            TURBO_INTERNAL_STRING_VIEW_MEMCMP(data(), s.data(), s.size()) == 0);
  }
 ...
 private:
  ...
  string_view(turbo::Nullable<const char*> data, size_type len,
  ...
  static constexpr size_type StrlenInternal(turbo::Nonnull<const char*> str) {
  ...
  turbo::Nullable<const char*> ptr_;
  size_type length_;
};
...
// NullSafeStringView()
//
// Creates an `turbo::string_view` from a pointer `p` even if it's null-valued.
// This function should be used where an `turbo::string_view` can be created from
// a possibly-null pointer.
constexpr string_view NullSafeStringView(turbo::Nullable<const char*> p) {
