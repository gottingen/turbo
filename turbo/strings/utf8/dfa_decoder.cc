...
#include "turbo/strings/utf8/dfa_decoder.h"
...
namespace turbo::utf8_detail {
    ...
    //
    dfa_decoder::lookup_tables const dfa_decoder::smTables = {
    ...
    //
    char const *dfa_decoder::smClassNames[12] =
            {
    ...
    //
    char const *dfa_decoder::smStateNames[9] =
            {
    ...
    //--------------------------------------------------------------------------------------------------
    //
    TURBO_ALIGN_FUNCTION std::ptrdiff_t

    dfa_decoder::convert(char8_t const *pSrc,
        ...
        while (pSrc < pSrcEnd) {
            ...
            if (unit < 0x80) {
                *pDst++ = unit;
                ...
                first_unit_info info = smTables.maFirstUnitTable[unit - 0x80];
                dfa_state state = info.mNextState;
                cdpt = info.mFirstOctet;
                switch (info.size) {
                    case 2: {
                        // advance_with_table<2>(pSrc, state, cdpt);
                        advance_with_table_2(pSrc, state, cdpt);
                    }
                        break;
                    case 3: {
                        // advance_with_table<3>(pSrc, state, cdpt);
                        advance_with_table_3(pSrc, state, cdpt);
                    }
                        break;
                    case 4: {
                        // advance_with_table<4>(pSrc, state, cdpt);
                        advance_with_table_4(pSrc, state, cdpt);
                    }
                        break;
                    default: {
    ...
    //--------------------------------------------------------------------------------------------------
    //
    std::ptrdiff_t dfa_decoder::convert_with_trace(char8_t const *pSrc,
                                                   char8_t const *pSrcEnd,
                                                   char32_t *pDst) noexcept {
        char32_t *pDstOrig = pDst;
        char32_t cdpt;
        ...
        while (pSrc < pSrcEnd) {
            if (advance_with_trace(pSrc, pSrcEnd, cdpt) != ERR) {
                *pDst++ = cdpt;
            } else {
                return -1;
            }
    ...
    //--------------------------------------------------------------------------------------------------
    //
    void dfa_decoder::print_state_data(dfa_state curr, char_class type, uint32_t unit, dfa_state next) {
