...
namespace turbo::utf8_detail {
    greedy_table_decoder::lookup_tables const greedy_table_decoder::smTables = {
    ...
    TURBO_ALIGN_FUNCTION std::ptrdiff_t greedy_table_decoder::convert(const char8_t* pSrc,
        ...
        while (pSrc < pSrcEnd) {
            ...
            if (unit < 0x80) {
                *pDst++ = unit;
                ...
                switch (info.size) {
                    case 2: {
                        advance_by_greedy_2(pSrc, cdpt);
                    } break;
                    case 3: {
                        advance_by_greedy_3(pSrc, cdpt);
                    } break;
                    case 4: {
                        advance_by_greedy_4(pSrc, cdpt);
                    } break;
                    default: {
    ...
    size_t greedy_table_decoder::count_unit_size(const char8_t* pSrc, const char8_t* pSrcEnd) noexcept {
        size_t count = 0;
        ...
        while (pSrc < pSrcEnd) {
            ...
            if (unit >= 0x80) {
                ...
                switch (info.size) {
                    case 2: {
                        skip_by_greedy_2(pSrc);
                    } break;
                    case 3: {
                        skip_by_greedy_3(pSrc);
                    } break;
                    case 4: {
                        skip_by_greedy_4(pSrc);
                    } break;
                    default: {
                        // https://unicodebook.readthedocs.io/unicode_encodings.html
                        // UTF-8 encoding supports longer byte sequences, up to 6 bytes,
