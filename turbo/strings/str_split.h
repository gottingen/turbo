...
// (`const char*`, `std::string`, or `turbo::string_piece`) is passed in place of
// an explicit `Delimiter` object, `StrSplit()` treats it the same way as if it
// were passed a `ByString` delimiter.
//
// A `Delimiter` is an object with a `Find()` function that knows how to find
// the first occurrence of itself in a given `turbo::string_piece`.
//
...
// in `text`. The returned turbo::string_piece should refer to the next occurrence
// (after `pos`) of the represented delimiter; this returned turbo::string_piece
// represents the next location where the input `text` should be broken.
//
// The returned turbo::string_piece may be zero-length if the Delimiter does not
// represent a part of the string (e.g., a fixed-length delimiter). If no
// delimiter is found in the input `text`, a zero-length turbo::string_piece
// referring to `text.end()` should be returned (e.g.,
// `text.substr(text.size())`). It is important that the returned
// turbo::string_piece always be within the bounds of the input `text` given as an
...
//     turbo::string_piece Find(turbo::string_piece text, size_t pos) {
//       auto found = text.find(c_, pos);
//       if (found == turbo::string_piece::npos)
...
class ByString {
 public:
  explicit ByString(turbo::string_piece sp);
  turbo::string_piece Find(turbo::string_piece text, size_t pos) const;
...
class ByAnyChar {
 public:
  explicit ByAnyChar(turbo::string_piece sp);
  turbo::string_piece Find(turbo::string_piece text, size_t pos) const;
...
template <>
struct SelectDelimiter<turbo::string_piece> {
  using type = ByString;
};
template <>
...
class MaxSplitsImpl {
 public:
  ...
  turbo::string_piece Find(turbo::string_piece text, size_t pos) {
    if (count_++ == limit_) {
      return turbo::string_piece(text.data() + text.size(),
                               0);  // No more matches.
    }
...
// `turbo::string_piece` argument and return a bool indicating whether the
...
struct AllowEmpty {
  bool operator()(turbo::string_piece) const { return true; }
};

// SkipEmpty()
//
// Returns `false` if the given `turbo::string_piece` is empty, indicating that
...
// predicate.
struct SkipEmpty {
  bool operator()(turbo::string_piece sp) const { return !sp.empty(); }
};

// SkipWhitespace()
//
// Returns `false` if the given `turbo::string_piece` is empty *or* contains only
...
struct SkipWhitespace {
  bool operator()(turbo::string_piece sp) const {
...
// may contain `std::string`, `turbo::string_piece` (in which case the original
// string being split must ensure that it outlives the collection), or any
// object that can be explicitly created from an `turbo::string_piece`. This
// behavior works for:
//
...
// Example:
//
//   // The results are returned as `turbo::string_piece` objects. Note that we
...
//   // each element will be of type `turbo::string_piece`.
...
//
//   turbo::StrSplit(turbo::string_piece(""), '-');  // {""}
//   turbo::StrSplit(turbo::string_piece(), '-');    // {}, but should be {""}
//
