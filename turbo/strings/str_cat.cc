...
AlphaNum::AlphaNum(Hex hex) {
  ...
  if (real_width >= hex.width) {
    piece_ = turbo::string_view(end - real_width, real_width);
  } else {
    ...
    piece_ = turbo::string_view(end - hex.width, hex.width);
  }
}
...
std::string StrCat(const AlphaNum& a, const AlphaNum& b) {
  std::string result;
  turbo::strings_internal::STLStringResizeUninitialized(&result,
                                                       a.size() + b.size());
...
std::string CatPieces(std::initializer_list<turbo::string_view> pieces) {
  std::string result;
  size_t total_size = 0;
  for (turbo::string_view piece : pieces) total_size += piece.size();
