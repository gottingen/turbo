...
TURBO_NO_INLINE
bool NonInlinedEq(turbo::string_piece a, turbo::string_piece b) { return a == b; }
...
TURBO_NO_INLINE
void DoEqualityComparisons(benchmark::State& state, turbo::string_piece a,
...
// made with turbo::string_piece comparisons against constant strings. The idea is
...
void BM_CompareSame(benchmark::State& state) {
  ...
  std::string y = x;
  turbo::string_piece a = x;
  turbo::string_piece b = y;
...
struct EasyMap : public std::map<turbo::string_piece, uint64_t> {
