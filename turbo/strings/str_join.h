...
// elements to be joined, using the same formatting that `turbo::string_cat()` uses.
...
//
// Because the default formatter uses the `turbo::alpha_num` class,
// `turbo::string_join()`, like `turbo::string_cat()`, will work out-of-the-box on
// collections of strings, ints, floats, doubles, etc.
//
// Example:
//
//   std::vector<std::string> v = {"foo", "bar", "baz"};
//   std::string s = turbo::string_join(v, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
// See comments on the `turbo::string_join()` function for more examples.
...
// functions. You may provide your own Formatter to enable `turbo::string_join()` to
...
// argument to `turbo::string_join()`:
//
//   std::vector<int> v = {1, 2, 3, 4};
//   std::string s = turbo::string_join(v, "-", MyFormatter());
//   EXPECT_EQ("1-2-3-4", s);
//
...
// alpha_num_formatter()
//
// Default formatter used if none is specified. Uses `turbo::alpha_num` to convert
// numeric arguments to strings.
TURBO_FORCE_INLINE strings_internal::alpha_num_formatter_impl alpha_num_formatter() {
...
TURBO_FORCE_INLINE strings_internal::stream_formatter_impl stream_formatter() {
...
TURBO_FORCE_INLINE strings_internal::pair_formatter_impl<FirstFormatter, SecondFormatter>
...
TURBO_FORCE_INLINE strings_internal::pair_formatter_impl<
...
// `alpha_num_formatter()`.
TURBO_FORCE_INLINE strings_internal::dereference_formatter_impl<
        strings_internal::alpha_num_formatter_impl>
dereference_formatter() {
...
// `turbo::string_join()` takes a range, a separator string to use between the
...
//   std::string s = turbo::string_join({"foo", "bar", "baz"}, "-");
//   EXPECT_EQ("foo-bar-baz", s);
//
// Example 3:
...
//       std::make_pair("b", 2),
//       std::make_pair("c", 3)};
//   std::string s = turbo::string_join(m, ",", turbo::pair_formatter("="));
//   EXPECT_EQ("a=1,b=2,c=3", s);
//
// Example 7:
//   // These examples show how `turbo::string_join()` handles a few common edge
//   // cases:
//   std::vector<std::string> v_empty;
//   EXPECT_EQ("", turbo::string_join(v_empty, "-"));
//
//   std::vector<std::string> v_one_item = {"foo"};
//   EXPECT_EQ("foo", turbo::string_join(v_one_item, "-"));
//
//   std::vector<std::string> v_empty_string = {""};
//   EXPECT_EQ("", turbo::string_join(v_empty_string, "-"));
//
//   std::vector<std::string> v_one_item_empty_string = {"a", ""};
//   EXPECT_EQ("a-", turbo::string_join(v_one_item_empty_string, "-"));
//
//   std::vector<std::string> v_two_empty_string = {"", ""};
//   EXPECT_EQ("-", turbo::string_join(v_two_empty_string, "-"));
//
// Example 8:
//   // Joins a `std::tuple<T...>` of heterogeneous types, converting each to
//   // a std::string using the `turbo::alpha_num` class.
