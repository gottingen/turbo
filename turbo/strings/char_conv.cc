 ...
 *****************************************************************/
#include "turbo/strings/char_conv.h"
...
struct FloatTraits<double> {
    ...
    static double Make(uint64_t mantissa, int exponent, bool sign) {
        ...
        dbl += mantissa;
        return turbo::base::bit_cast<double>(dbl);
...
struct FloatTraits<float> {
    ...
    return sign ? -ldexpf(mantissa, exponent) : ldexpf(mantissa, exponent);
        ...
        flt += mantissa;
        return turbo::base::bit_cast<float>(flt);
#endif  // TURBO_BIT_PACK_FLOATS
    }
};
...
int BitWidth(turbo::uint128 value) {
    if (uint128_high64(value) == 0) {
        return 64 - turbo::base::countl_zero(uint128_low64(value));
    }
    return 128 - turbo::base::countl_zero(uint128_high64(value));
}
...
int TruncateToBitWidth(int bit_width, turbo::uint128 *value) {
...
uint64_t ShiftRightAndRound(turbo::uint128 value, int shift, bool input_exact,
                            bool *output_exact) {
    if (shift <= 0) {
    ...
    *output_exact = true;
    const turbo::uint128 shift_mask = (turbo::uint128(1) << shift) - 1;
    const turbo::uint128 halfway_point = turbo::uint128(1) << (shift - 1);

    const turbo::uint128 shifted_bits = value & shift_mask;
    value >>= shift;
...
bool MustRoundUp(uint64_t guess_mantissa, int guess_exponent,
    ...
    turbo::strings_internal::big_unsigned<84> exact_mantissa;
    ...
    guess_exponent -= 1;
    turbo::strings_internal::big_unsigned<84> &lhs = exact_mantissa;
    int comparison;
    if (exact_exponent >= 0) {
        lhs.MultiplyByFiveToTheNth(exact_exponent);
        turbo::strings_internal::big_unsigned<84> rhs(guess_mantissa);
        ...
        turbo::strings_internal::big_unsigned<84> rhs =
                turbo::strings_internal::big_unsigned<84>::FiveToTheNth(-exact_exponent);
...
CalculatedFloat CalculateFromParsedHexadecimal(
    ...
    int mantissa_width = 64 - turbo::base::countl_zero(mantissa);
...
CalculatedFloat CalculateFromParsedDecimal(
    ...
    turbo::uint128 wide_binary_mantissa = parsed_decimal.mantissa;
