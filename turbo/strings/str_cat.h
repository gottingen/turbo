...
//
// Note that `turbo_stringify()` also supports use with `turbo::StrFormat()` and
// `turbo::Substitute()`.
//
// Example:
//
...
inline void StrAppend(turbo::Nonnull<std::string*>) {}
...
// Support 5 or more arguments
template <typename... AV>
inline void StrAppend(turbo::Nonnull<std::string*> dest, const AlphaNum& a,
                      const AlphaNum& b, const AlphaNum& c, const AlphaNum& d,
                      const AlphaNum& e, const AV&... args) {
