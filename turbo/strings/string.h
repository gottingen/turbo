...
#ifndef TURBO_STRINGS_STRING_H_
#define TURBO_STRINGS_STRING_H_
...
#ifdef TURBO_SANITIZE_ADDRESS
#define TBSTRING_DISABLE_SSO true
#else
#define TBSTRING_DISABLE_SSO false
#endif

    namespace tbstring_detail {
 ...
 */
    ...
    /**
     * Defines a special acquisition method for constructing tbstring
     * objects. AcquireMallocatedString means that the user passes a
     * pointer to a malloc-allocated string that the tbstring object will
     * take into custody.
     */
    ...
    /*
     * tbstring_core_model is a mock-up type that defines all required
     * signatures of a tbstring core. The tbstring class itself uses such
     ...
     * implement the primitives. Then plug the core into basic_tbstring as
     * a template argument.

    template <class Char>
    class tbstring_core_model {
     public:
      tbstring_core_model();
      tbstring_core_model(const tbstring_core_model &);
      tbstring_core_model& operator=(const tbstring_core_model &) = delete;
      ~tbstring_core_model();
    ...
    */
     ...
     *   and big-endian tbstring_core equivalent with merely different ops used
     *   to extract capacity/category.
     */
    template<class Char>
    class tbstring_core {
    public:
        tbstring_core() noexcept { reset(); }

        tbstring_core(const tbstring_core &rhs) {
            assert(&rhs != this);
            switch (rhs.category()) {
                ...
                case Category::isLarge:
                    ...
                    break;
                default:
                    turbo::base::assume_unreachable();
            }
            assert(size() == rhs.size());
            assert(memcmp(data(), rhs.data(), size() * sizeof(Char)) == 0);
        }

        tbstring_core &operator=(const tbstring_core &rhs) = delete;

        tbstring_core(tbstring_core &&goner) noexcept {
            // Take goner's guts
            ml_ = goner.ml_;
        ...
        tbstring_core(
                ...
                const size_t size,
                bool disableSSO = TBSTRING_DISABLE_SSO) {
        ...
        TURBO_NO_INLINE
        void reserve(size_t minCapacity, bool disableSSO = TBSTRING_DISABLE_SSO) {
            TURBO_PUSH_WARNING
            TURBO_CLANG_DISABLE_WARNING("-Wcovered-switch-default")
            switch (category()) {
                case Category::isSmall:
        ...
        size_t size() const {
            ...
            if /* constexpr */ (turbo::base::is_little_endian()) {
    ...
    private:
        ...
        struct RefCounted {
            ...
            static RefCounted *create(size_t *size) {
                ...
                if (!turbo::checked_add(&capacityBytes, *size, size_t(1))) {
                    turbo::base::throw_std_length_error("");
                }
                if (!turbo::checked_muladd(
                        &capacityBytes, capacityBytes, sizeof(Char), getDataOffset())) {
                    turbo::base::throw_std_length_error("");
                }
                const size_t allocSize = good_malloc_size(capacityBytes);
                auto result = static_cast<RefCounted *>(checked_malloc(allocSize));
            ...
            static RefCounted *create(const Char *data, size_t *size) {
                ...
                if (TURBO_LIKELY(effectiveSize > 0)) {
                    tbstring_detail::podCopy(data, data + effectiveSize, result->data_);
                }
                return result;
            }
            ...
            static RefCounted *reallocate(
                ...
                const size_t allocNewCapacity = good_malloc_size(capacityBytes);
        ...
        enum class Category : category_type {
            isSmall = 0,
            isMedium = turbo::base::is_little_endian() ? 0x80 : 0x2,
            isLarge = turbo::base::is_little_endian() ? 0x40 : 0x1,
        };
        ...
        struct MediumLarge {
            ...
            size_t capacity() const {
                return turbo::base::is_little_endian() ? capacity_ & capacityExtractMask : capacity_ >> 2;
            }

            void setCapacity(size_t cap, Category cat) {
                capacity_ = turbo::base::is_little_endian()
        ...
        constexpr static uint8_t categoryExtractMask = turbo::base::is_little_endian() ? 0xC0 : 0x3;
        constexpr static size_t kCategoryShift = (sizeof(size_t) - 1) * 8;
        constexpr static size_t capacityExtractMask = turbo::base::is_little_endian()
                                                      ? ~(size_t(categoryExtractMask) << kCategoryShift)
                                                      : 0x0 /* unused */;

        static_assert(
                !(sizeof(MediumLarge) % sizeof(Char)),
                "Corrupt memory layout for tbstring.");

        size_t smallSize() const {
            assert(category() == Category::isSmall);
            constexpr auto shift = turbo::base::is_little_endian() ? 0 : 2;
        ...
        void copySmall(const tbstring_core &);

        void copyMedium(const tbstring_core &);

        void copyLarge(const tbstring_core &);
    ...
    template<class Char>
    inline void tbstring_core<Char>::copySmall(const tbstring_core &rhs) {
        static_assert(offsetof(MediumLarge, data_) == 0, "tbstring layout failure");
        static_assert(
                offsetof(MediumLarge, size_) == sizeof(ml_.data_),
                "tbstring layout failure");
        static_assert(
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::copyMedium(const tbstring_core &rhs) {
        // Medium strings are copied eagerly. Don't forget to allocate
        // one extra Char for the null terminator.
        auto const allocSize = good_malloc_size((1 + rhs.ml_.size_) * sizeof(Char));
        ml_.data_ = static_cast<Char *>(checked_malloc(allocSize));
        // Also copies terminator.
        tbstring_detail::podCopy(
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::copyLarge(const tbstring_core &rhs) {
...
// Small strings are bitblitted
    template<class Char>
    inline void tbstring_core<Char>::initSmall(
            const Char *const data, const size_t size) {
        // Layout is: Char* data_, size_t size_, size_t capacity_
        static_assert(
                sizeof(*this) == sizeof(Char *) + 2 * sizeof(size_t),
                "tbstring has unexpected size");
        static_assert(
                sizeof(Char *) == sizeof(size_t), "tbstring size assumption violation");
        // sizeof(size_t) must be a power of 2
        static_assert(
                (sizeof(size_t) & (sizeof(size_t) - 1)) == 0,
                "tbstring size assumption violation");
...
// compiling with ASan.
        ...
        {
            if (size != 0) {
                tbstring_detail::podCopy(data, data + size, small_);
            }
        }
        setSmallSize(size);
    }
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::initMedium(
            const Char *const data, const size_t size) {
        // Medium strings are allocated normally. Don't forget to
        // allocate one extra Char for the terminating null.
        auto const allocSize = good_malloc_size((1 + size) * sizeof(Char));
        ml_.data_ = static_cast<Char *>(checked_malloc(allocSize));
        if (TURBO_LIKELY(size > 0)) {
            tbstring_detail::podCopy(data, data + size, ml_.data_);
        }
        ml_.size_ = size;
        ml_.setCapacity(allocSize / sizeof(Char) - 1, Category::isMedium);
        ml_.data_[size] = '\0';
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::initLarge(
            const Char *const data, const size_t size) {
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::unshare(size_t minCapacity) {
        ...
        // Also copies terminator.
        tbstring_detail::podCopy(ml_.data_, ml_.data_ + ml_.size_ + 1, newRC->data_);
        RefCounted::decrementRefs(ml_.data_);
        ml_.data_ = newRC->data_;
        ml_.setCapacity(effectiveCapacity, Category::isLarge);
    ...
    template<class Char>
    inline Char *tbstring_core<Char>::mutableDataLarge() {
        assert(category() == Category::isLarge);
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::reserveLarge(size_t minCapacity) {
        assert(category() == Category::isLarge);
        if (RefCounted::refs(ml_.data_) > 1) { // Ensure unique
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::reserveMedium(
        ...
        if (minCapacity <= maxMediumSize) {
            ...
            size_t capacityBytes = good_malloc_size((1 + minCapacity) * sizeof(Char));
            // Also copies terminator.
    ...
    template<class Char>
    TURBO_NO_INLINE void tbstring_core<Char>::reserveSmall(
        ...
        if (!disableSSO && minCapacity <= maxSmallSize) {
            ...
            auto const allocSizeBytes =
                    good_malloc_size((1 + minCapacity) * sizeof(Char));
            auto const pData = static_cast<Char *>(checked_malloc(allocSizeBytes));
            auto const size = smallSize();
            // Also copies terminator.
            tbstring_detail::podCopy(small_, small_ + size + 1, pData);
            ml_.data_ = pData;
            ml_.size_ = size;
            ...
            auto const size = smallSize();
            // Also copies terminator.
            tbstring_detail::podCopy(small_, small_ + size + 1, newRC->data_);
            ml_.data_ = newRC->data_;
            ml_.size_ = size;
            ml_.setCapacity(minCapacity, Category::isLarge);
            assert(capacity() >= minCapacity);
    ...
    template<class Char>
    inline Char *tbstring_core<Char>::expandNoinit(
            const size_t delta,
            bool expGrowth, /* = false */
            bool disableSSO /* = TBSTRING_DISABLE_SSO */) {
    ...
    template<class Char>
    inline void tbstring_core<Char>::shrinkSmall(const size_t delta) {
        // Check for underflow
    ...
    template<class Char>
    inline void tbstring_core<Char>::shrinkMedium(const size_t delta) {
    ...
    template<class Char>
    inline void tbstring_core<Char>::shrinkLarge(const size_t delta) {
        assert(ml_.size_ >= delta);
        // Shared large string, must make unique. This is because of the
        // durn terminator must be written, which may trample the shared
        // data.
        if (delta) {
            tbstring_core(ml_.data_, ml_.size_ - delta).swap(*this);
        }
        // No need to write the terminator.
    }

    /**
     * Dummy tbstring core that uses an actual std::string. This doesn't
     * make any sense - it's just for testing purposes.
     */
    template<class Char>
    class dummy_tbstring_core {
    public:
        dummy_tbstring_core() {}

        dummy_tbstring_core(const dummy_tbstring_core &another)
                : backend_(another.backend_) {}

        dummy_tbstring_core(const Char *s, size_t n) : backend_(s, n) {}

        void swap(dummy_tbstring_core &rhs) { backend_.swap(rhs.backend_); }
     ...
     * basic_tbstring takes the same template parameters, plus the last
    ...
    template<
            ...
            class Storage = tbstring_core<E>>
    class basic_tbstring {
        static_assert(
                std::is_same<A, std::allocator<E>>::value,
                "tbstring ignores custom allocators");
        ...
        struct Invariant {
            ...
            explicit Invariant(const basic_tbstring &s) noexcept: s_(s) {
                assert(s_.is_sane());
            }

            ~Invariant() noexcept { assert(s_.is_sane()); }

        private:
            const basic_tbstring &s_;
        };
    ...
    public:
        ...
        //
        //   explicit basic_tbstring(const A& a = A()) noexcept { }
        //
        ...
        basic_tbstring() noexcept: basic_tbstring(A()) {}

        explicit basic_tbstring(const A &) noexcept {}

        basic_tbstring(const basic_tbstring &str) : store_(str.store_) {}

        // Move constructor
        basic_tbstring(basic_tbstring &&goner) noexcept
                : store_(std::move(goner.store_)) {}

        // This is defined for compatibility with std::string
        template<typename A2>
        /* implicit */ basic_tbstring(const std::basic_string<E, T, A2> &str)
                : store_(str.data(), str.size()) {}

        basic_tbstring(
                const basic_tbstring &str,
                size_type pos,
                size_type n = npos,
                const A & /* a */ = A()) {
            assign(str, pos, n);
        ...
        TURBO_NO_INLINE
        /* implicit */ basic_tbstring(const value_type *s, const A & /*a*/ = A())
                : store_(s, traitsLength(s)) {}

        TURBO_NO_INLINE
        basic_tbstring(const value_type *s, size_type n, const A & /*a*/ = A())
                : store_(s, n) {}

        TURBO_NO_INLINE
        basic_tbstring(size_type n, value_type c, const A & /*a*/ = A()) {
            auto const pData = store_.expandNoinit(n);
            tbstring_detail::podFill(pData, pData + n, c);
        }

        template<class InIt>
        TURBO_NO_INLINE basic_tbstring(
                InIt begin,
                InIt end,
        ...
        TURBO_NO_INLINE
        basic_tbstring(const value_type *b, const value_type *e, const A & /*a*/ = A())
        ...
        TURBO_NO_INLINE
        basic_tbstring(std::initializer_list<value_type> il) {
            assign(il.begin(), il.end());
        }

        ~basic_tbstring() noexcept {}

        basic_tbstring &operator=(const basic_tbstring &lhs);

        // Move assignment
        basic_tbstring &operator=(basic_tbstring &&goner) noexcept;

        // Compatibility with std::string
        template<typename A2>
        basic_tbstring &operator=(const std::basic_string<E, T, A2> &rhs) {
            return assign(rhs.data(), rhs.size());
        }

        operator std::basic_string<E, T, A>() {
        ...
        // Compatibility with std::string
        std::basic_string<E, T, A> to_std_string() const {
            return std::basic_string<E, T, A>(data(), size());
        }

        basic_tbstring &operator=(const value_type *s) { return assign(s); }

        basic_tbstring &operator=(value_type c);
        ...
        typename std::enable_if<
                std::is_convertible<
                        TP,
                        typename basic_tbstring<E, T, A, Storage>::value_type>::value &&
                !std::is_same<
                        typename std::decay<TP>::type,
                        typename basic_tbstring<E, T, A, Storage>::value_type>::value,
                basic_tbstring<E, T, A, Storage> &>::type
        operator=(TP c) = delete;

        basic_tbstring &operator=(std::initializer_list<value_type> il) {
            return assign(il.begin(), il.end());
        }

#if TURBO_HAS_STRING_VIEW
...
return {data(), size()};
        ...
        void shrink_to_fit() {
            ...
            basic_tbstring(cbegin(), cend()).swap(*this);
        }
        ...
        const_reference at(size_type n) const {
            if (!(n < size())) {
                turbo::base::throw_std_out_of_range("");
            }
            return (*this)[n];
        }
        ...
        basic_tbstring &operator+=(const basic_tbstring &str) { return append(str); }

        basic_tbstring &operator+=(const value_type *s) { return append(s); }

        basic_tbstring &operator+=(const value_type c) {
            push_back(c);
            return *this;
        }
        ...
        basic_tbstring &append(const basic_tbstring &str);

        basic_tbstring &append(
                const basic_tbstring &str, const size_type pos, size_type n);

        basic_tbstring &append(const value_type *s, size_type n);

        basic_tbstring &append(const value_type *s) {
            return append(s, traitsLength(s));
        }

        basic_tbstring &append(size_type n, value_type c);

        template<class InputIterator>
        basic_tbstring &append(InputIterator first, InputIterator last) {
            insert(end(), first, last);
            return *this;
        }

        basic_tbstring &append(std::initializer_list<value_type> il) {
        ...
        basic_tbstring &assign(const basic_tbstring &str) {
            if (&str == this) {
                return *this;
            }
        ...
        basic_tbstring &assign(basic_tbstring &&str) {
            return *this = std::move(str);
        }

        basic_tbstring &assign(
                const basic_tbstring &str, const size_type pos, size_type n);

        basic_tbstring &assign(const value_type *s, const size_type n);

        basic_tbstring &assign(const value_type *s) {
            return assign(s, traitsLength(s));
        }

        basic_tbstring &assign(std::initializer_list<value_type> il) {
            return assign(il.begin(), il.end());
        }

        template<class ItOrLength, class ItOrChar>
        basic_tbstring &assign(ItOrLength first_or_n, ItOrChar last_or_c) {
            return replace(begin(), end(), first_or_n, last_or_c);
        }

        basic_tbstring &insert(size_type pos1, const basic_tbstring &str) {
            return insert(pos1, str.data(), str.size());
        }

        basic_tbstring &insert(
                size_type pos1, const basic_tbstring &str, size_type pos2, size_type n) {
            if (!(pos2 <= str.length())) {
                turbo::base::throw_std_out_of_range("");
            }
        ...
        basic_tbstring &insert(size_type pos, const value_type *s, size_type n) {
            if (!(pos <= length())) {
                turbo::base::throw_std_out_of_range("");
            }
        ...
        basic_tbstring &insert(size_type pos, const value_type *s) {
            return insert(pos, s, traitsLength(s));
        }

        basic_tbstring &insert(size_type pos, size_type n, value_type c) {
            if (!(pos <= length())) {
                turbo::base::throw_std_out_of_range("");
            }
    ...
    public:
        friend inline istream_type &getline(
                istream_type &is, basic_tbstring &str, value_type delim) {
            return str.getlineImpl(is, delim);
        }

        friend inline istream_type &getline(istream_type &is, basic_tbstring &str) {
    ...
    public:
        ...
        basic_tbstring &erase(size_type pos = 0, size_type n = npos) {
        ...
        // content of str
        basic_tbstring &replace(
                size_type pos1, size_type n1, const basic_tbstring &str) {
        ...
        basic_tbstring &replace(size_type pos, size_type n1, const value_type *s) {
        ...
        basic_tbstring &replace(iterator i1, iterator i2, const basic_tbstring &str) {
            return replace(i1, i2, str.data(), str.length());
        }

        basic_tbstring &replace(iterator i1, iterator i2, const value_type *s) {
            return replace(i1, i2, s, traitsLength(s));
        }

    private:
        basic_tbstring &replaceImplDiscr(
                iterator i1,
                iterator i2,
                const value_type *s,
                size_type n,
    ...
    public:
        ...
        size_type copy(value_type *s, size_type n, size_type pos = 0) const {
            ...
            if (n != 0) {
                tbstring_detail::podCopy(data() + pos, data() + pos + n, s);
            }
            return n;
        }

        void swap(basic_tbstring &rhs) { store_.swap(rhs.store_); }
        ...
        size_type find(const basic_tbstring &str, size_type pos = 0) const {
        ...
        size_type rfind(const basic_tbstring &str, size_type pos = npos) const {
        ...
        size_type find_first_of(const basic_tbstring &str, size_type pos = 0) const {
        ...
        size_type find_last_of(
                const basic_tbstring &str, size_type pos = npos) const {
        ...
        size_type find_first_not_of(
                const basic_tbstring &str, size_type pos = 0) const {
        ...
        basic_tbstring substr(size_type pos = 0, size_type n = npos) const &{
            if (!(pos <= size())) {
                turbo::base::throw_std_out_of_range("");
            }
            return basic_tbstring(data() + pos, std::min(n, size() - pos));
        }

        basic_tbstring substr(size_type pos = 0, size_type n = npos) &&{
            if (!(pos <= size())) {
                turbo::base::throw_std_out_of_range("");
            }
            erase(0, pos);
        ...
        int compare(const basic_tbstring &str) const {
            // FIX due to Goncalo N M de Carvalho July 18, 2005
            return compare(0, size(), str);
        }

        int compare(size_type pos1, size_type n1, const basic_tbstring &str) const {
    ...
    TURBO_NO_INLINE typename basic_tbstring<E, T, A, S>::size_type
    basic_tbstring<E, T, A, S>::traitsLength(const value_type *s) {
        return s ? traits_type::length(s)
                 : (turbo::base::throw_std_logic_error(
                        "basic_tbstring: null pointer initializer not valid"),
                        0);
    }

    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> &basic_tbstring<E, T, A, S>::operator=(
            const basic_tbstring &lhs) {
        Invariant checker(*this);
...
// Move assignment
    ...
    template<typename E, class T, class A, class S>
    inline void basic_tbstring<E, T, A, S>::resize(
        ...
        if (n <= size) {
            ...
            tbstring_detail::podFill(pData, pData + delta, c);
        }
        assert(this->size() == n);
    }

    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> &basic_tbstring<E, T, A, S>::append(
            const basic_tbstring &str) {
#ifndef NDEBUG
    ...
    template<typename E, class T, class A, class S>
    TURBO_NO_INLINE basic_tbstring<E, T, A, S> &basic_tbstring<E, T, A, S>::append(
        ...
        if (TURBO_UNLIKELY(le(oldData, s) && !le(oldData + oldSize, s))) {
            ...
            s = data() + (s - oldData);
            tbstring_detail::podMove(s, s + n, pData);
        } else {
            tbstring_detail::podCopy(s, s + n, pData);
        }
    ...
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> &basic_tbstring<E, T, A, S>::assign(
            const basic_tbstring &str, const size_type pos, size_type n) {
        const size_type sz = str.size();
        if (!(pos <= sz)) {
            turbo::base::throw_std_out_of_range("");
    ...
    template<typename E, class T, class A, class S>
    TURBO_NO_INLINE basic_tbstring<E, T, A, S> &basic_tbstring<E, T, A, S>::assign(
        ...
        if (n == 0) {
            ...
            tbstring_detail::podMove(s, s + n, store_.mutableData());
            store_.shrink(size() - n);
            assert(size() == n);
        } else {
            ...
            tbstring_detail::podCopy(s, s + n, store_.expandNoinit(n));
        }
    ...
    template<typename E, class T, class A, class S>
    inline typename basic_tbstring<E, T, A, S>::istream_type &
    basic_tbstring<E, T, A, S>::getlineImpl(istream_type &is, value_type delim) {
        Invariant checker(*this);
        ...
        while (true) {
            ...
            // tbstring has 1 byte extra capacity for the null terminator,
    ...
    template<typename E, class T, class A, class S>
    inline typename basic_tbstring<E, T, A, S>::size_type
    basic_tbstring<E, T, A, S>::find(
    ...
    template<typename E, class T, class A, class S>
    inline typename basic_tbstring<E, T, A, S>::iterator
    basic_tbstring<E, T, A, S>::insertImplDiscr(
            const_iterator i, size_type n, value_type c, std::true_type) {
        Invariant checker(*this);
        ...
        auto b = begin();
        tbstring_detail::podMove(b + pos, b + oldSize, b + pos + n);
        tbstring_detail::podFill(b + pos, b + pos + n, c);
    ...
    template<class FwdIterator>
    inline typename basic_tbstring<E, T, A, S>::iterator
    basic_tbstring<E, T, A, S>::insertImpl(
            const_iterator i,
            FwdIterator s1,
            FwdIterator s2,
            std::forward_iterator_tag) {
    ...
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> &basic_tbstring<E, T, A, S>::replaceImplDiscr(
            iterator i1,
            iterator i2,
            const value_type *s,
            size_type n,
    ...
    template<typename E, class T, class A, class S>
    template<class FwdIterator>
    inline bool basic_tbstring<E, T, A, S>::replaceAliased(
            iterator i1, iterator i2, FwdIterator s1, FwdIterator s2, std::true_type) {
        std::less_equal<const value_type *> le{};
    ...
    template<typename E, class T, class A, class S>
    template<class FwdIterator>
    inline void basic_tbstring<E, T, A, S>::replaceImpl(
            iterator i1,
            iterator i2,
            FwdIterator s1,
            FwdIterator s2,
        ...
        using Sel = std::integral_constant<bool,
        ...
        if (n1 > n2) {
            ...
            // grows
            s1 = tbstring_detail::copy_n(s1, n1, i1).first;
            insert(i2, s1, s2);
        }
        assert(is_sane());
    }
    ...
    template<typename E, class T, class A, class S>
    inline typename basic_tbstring<E, T, A, S>::size_type
    basic_tbstring<E, T, A, S>::find_first_of(
            const value_type *s, size_type pos, size_type n) const {
        if (pos > length() || n == 0) {
            return npos;
        }
    ...
    template<typename E, class T, class A, class S>
    inline typename basic_tbstring<E, T, A, S>::size_type
    basic_tbstring<E, T, A, S>::find_last_of(
            const value_type *s, size_type pos, size_type n) const {
        if (!empty() && n > 0) {
    ...
    template<typename E, class T, class A, class S>
    inline typename basic_tbstring<E, T, A, S>::size_type
    basic_tbstring<E, T, A, S>::find_first_not_of(
            const value_type *s, size_type pos, size_type n) const {
        if (pos < length()) {
            const_iterator i(begin() + pos), finish(end());
            for (; i != finish; ++i) {
    ...
    template<typename E, class T, class A, class S>
    inline typename basic_tbstring<E, T, A, S>::size_type
    basic_tbstring<E, T, A, S>::find_last_not_of(
            const value_type *s, size_type pos, size_type n) const {
        if (!this->empty()) {
...
// C++11 21.4.8.1/1
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            const basic_tbstring<E, T, A, S> &lhs,
            const basic_tbstring<E, T, A, S> &rhs) {
        basic_tbstring <E, T, A, S> result;
    ...
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            basic_tbstring<E, T, A, S> &&lhs, const basic_tbstring<E, T, A, S> &rhs) {
    ...
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            const basic_tbstring<E, T, A, S> &lhs, basic_tbstring<E, T, A, S> &&rhs) {
...
// C++11 21.4.8.1/4
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            basic_tbstring<E, T, A, S> &&lhs, basic_tbstring<E, T, A, S> &&rhs) {
        return std::move(lhs.append(rhs));
    }
...
// C++11 21.4.8.1/5
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            const E *lhs, const basic_tbstring<E, T, A, S> &rhs) {
        //
        basic_tbstring <E, T, A, S> result;
        const auto len = basic_tbstring<E, T, A, S>::traits_type::length(lhs);
    ...
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            const E *lhs, basic_tbstring<E, T, A, S> &&rhs) {
        //
        const auto len = basic_tbstring<E, T, A, S>::traits_type::length(lhs);
...
// C++11 21.4.8.1/7
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            E lhs, const basic_tbstring<E, T, A, S> &rhs) {
        basic_tbstring <E, T, A, S> result;
...
// C++11 21.4.8.1/8
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            E lhs, basic_tbstring<E, T, A, S> &&rhs) {
        //
...
// C++11 21.4.8.1/9
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            const basic_tbstring<E, T, A, S> &lhs, const E *rhs) {
        typedef typename basic_tbstring<E, T, A, S>::size_type size_type;
        typedef typename basic_tbstring<E, T, A, S>::traits_type traits_type;
    ...
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            basic_tbstring<E, T, A, S> &&lhs, const E *rhs) {
        //
    ...
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            const basic_tbstring<E, T, A, S> &lhs, E rhs) {
        basic_tbstring <E, T, A, S> result;
...
// C++11 21.4.8.1/12
    template<typename E, class T, class A, class S>
    inline basic_tbstring<E, T, A, S> operator+(
            basic_tbstring<E, T, A, S> &&lhs, E rhs) {
        //
        return std::move(lhs += rhs);
    }
    ...
    template<typename E, class T, class A, class S>
    inline bool operator==(
            const typename basic_tbstring<E, T, A, S>::value_type *lhs,
            const basic_tbstring<E, T, A, S> &rhs) {
        return rhs == lhs;
    }
    ...
    inline bool operator==(
            const basic_tbstring<E, T, A, S> &lhs,
            const typename basic_tbstring<E, T, A, S>::value_type *rhs) {
...
// C++11 21.4.8.8
    template<typename E, class T, class A, class S>
    void swap(basic_tbstring<E, T, A, S> &lhs, basic_tbstring<E, T, A, S> &rhs) {
        lhs.swap(rhs);
    }
    ...
    inline std::basic_istream<
            typename basic_tbstring<E, T, A, S>::value_type,
            typename basic_tbstring<E, T, A, S>::traits_type> &
    operator>>(
            std::basic_istream<
                    typename basic_tbstring<E, T, A, S>::value_type,
                    typename basic_tbstring<E, T, A, S>::traits_type> &is,
            basic_tbstring<E, T, A, S> &str) {
    ...
    operator<<(
            std::basic_ostream<
                    typename basic_tbstring<E, T, A, S>::value_type,
                    typename basic_tbstring<E, T, A, S>::traits_type> &os,
            const basic_tbstring<E, T, A, S> &str) {
    ...
    constexpr typename basic_tbstring<E1, T, A, S>::size_type
            basic_tbstring<E1, T, A, S>::npos;
...
// basic_string compatibility routines
    ...
    typedef basic_tbstring<char> tbstring;

    // tbstring is relocatable
    template<class T, class R, class A, class S>
    TURBO_ASSUME_RELOCATABLE(basic_tbstring<T, R, A, S>);

    // Compatibility function, to make sure to_std_string(s) can be called
    // to convert a std::string or tbstring variable s into type std::string
    // with very little overhead if s was already std::string
    inline std::string to_std_string(const turbo::tbstring &s) {
        return std::string(s.data(), s.size());
    }

    inline const std::string &to_std_string(const std::string &s) {
        return s;
    }
    ...
    inline std::string &&to_std_string(std::string &&s) {
        return std::move(s);
    }


#undef TBSTRING_DISABLE_SSO

    template<class T>
    struct is_some_string;

    template<>
    struct is_some_string<tbstring> : ::std::true_type {
    };

/*
template<>
struct fmt::formatter<turbo::tbstring> : private formatter<fmt::string_view> {
    using formatter<fmt::string_view>::parse;

    template<typename Context>
    typename Context::iterator format(
            const turbo::tbstring &s, Context &ctx) const {
...
namespace std {
    template<>
    struct hash<turbo::tbstring> {
        size_t operator()(const turbo::tbstring &v) {
