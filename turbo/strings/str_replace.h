...
// Examples:
//
//   std::map<const turbo::string_piece, const turbo::string_piece> replacements;
...
//   std::vector<std::pair<const turbo::string_piece, std::string>> replacements;
...
struct ViableSubstitution {
  turbo::string_piece old;
  turbo::string_piece replacement;
  size_t offset;

  ViableSubstitution(turbo::string_piece old_str,
                     turbo::string_piece replacement_str, size_t offset_val)
...
std::vector<ViableSubstitution> FindSubstitutions(
    turbo::string_piece s, const StrToStrMapping& replacements) {
  std::vector<ViableSubstitution> subs;
  subs.reserve(replacements.size());

  for (const auto& rep : replacements) {
    using std::get;
    turbo::string_piece old(get<0>(rep));
...
int ApplySubstitutions(turbo::string_piece s,
