...
#ifndef TURBO_UNICODE_H_
#define TURBO_UNICODE_H_
...
TURBO_FORCE_INLINE void append_utf8(std::string &str, uint32_t unicode) {
    if (unicode <= 0x7f) {
...
TURBO_FORCE_INLINE bool is_valid_utf8(const std::string &utf8String) {
...
TURBO_FORCE_INLINE int unicode_to_utf8_bytes(uint32_t ui) {
    if (ui <= 0x7f) {
        return 1;
    } else if (ui <= 0x7ff) {
        return 2;
