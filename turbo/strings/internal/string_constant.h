...
// It can be accessed via its `turbo::string_piece value` static member.
// It is guaranteed that the `string_piece` returned has constant `.data()`,
...
struct StringConstant {
 private:
  static constexpr bool TryConstexprEval(turbo::string_piece view) {
    return view.empty() || 2 * view[0] != 1;
  }

 public:
  static constexpr turbo::string_piece value = T{}();
  constexpr turbo::string_piece operator()() const { return value; }
...
// constexpr operator().
// It must return an `turbo::string_piece` or `const char*` pointing to constant
