...
void ReducePadding(string_piece s, size_t *capacity) {
...
class IntDigits {
 public:
  ...
  string_piece with_neg_and_zero() const { return {start_, size_}; }

  // The printed value not including the '-' sign.
  // For inputs of value `0`, this will return "".
  string_piece without_neg_or_zero() const {
...
string_piece BaseIndicator(const IntDigits &as_digits,
...
string_piece SignColumn(bool neg, const FormatConversionSpecImpl conv) {
...
bool ConvertIntImplInnerSlow(const IntDigits &as_digits,
  ...
  string_piece formatted = as_digits.without_neg_or_zero();
  ReducePadding(formatted, &fill);

  string_piece sign = SignColumn(as_digits.is_negative(), conv);
  ReducePadding(sign, &fill);

  string_piece base_indicator = BaseIndicator(as_digits, conv);
...
inline bool ConvertStringArg(string_piece v, const FormatConversionSpecImpl conv,
                             FormatSinkImpl *sink) {
  if (conv.is_basic()) {
    sink->Append(v);
    return true;
...
FormatConvertImpl(const char *v, const FormatConversionSpecImpl conv,
  ...
  return {ConvertStringArg(string_piece(v, len), conv, sink)};
}
