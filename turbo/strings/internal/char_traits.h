...
#ifndef TURBO_STRINGS_INTERNAL_CHAR_TRAITS_H_
#define TURBO_STRINGS_INTERNAL_CHAR_TRAITS_H_
#include <cstddef>
#include <cstring>
#include "turbo/base/profile.h"  // disable some warnings on Windows
...
TURBO_FORCE_INLINE char *char_cat(char *dest, size_t destlen, const char *src,
                                 size_t srclen) {
...
const char *int_char_match(const char *haystack, size_t haylen,
    ...
    for (; haystack < hayend; ++haystack) {
        char hay = case_sensitive
                   ? *haystack
                   : turbo::ascii::to_lower(static_cast<unsigned char>(*haystack));
        char nee = case_sensitive
                   ? *needle
                   : turbo::ascii::to_lower(static_cast<unsigned char>(*needle));
        if (hay == nee) {
            if (++needle == needleend) {
...
TURBO_FORCE_INLINE const char *char_str(const char *phaystack, size_t haylen,
                                       const char *pneedle) {
    return int_char_match<true>(phaystack, haylen, pneedle, strlen(pneedle));
}

TURBO_FORCE_INLINE const char *char_case_str(const char *phaystack, size_t haylen,
                                            const char *pneedle) {
    return int_char_match<false>(phaystack, haylen, pneedle, strlen(pneedle));
}

TURBO_FORCE_INLINE const char *char_mem(const char *phaystack, size_t haylen,
                                       const char *pneedle, size_t needlelen) {
    return int_char_match<true>(phaystack, haylen, pneedle, needlelen);
}

TURBO_FORCE_INLINE const char *char_case_mem(const char *phaystack, size_t haylen,
                                            const char *pneedle, size_t needlelen) {
...
#endif  // TURBO_STRINGS_INTERNAL_CHAR_TRAITS_H_
