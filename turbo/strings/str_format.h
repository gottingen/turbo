...
// Example:
//
//   std::string s = turbo::StrFormat(
//                      "%s %s You have $%d!", "Hello", name, dollars);
//
// The library consists of the following basic utilities:
//
//   * `turbo::StrFormat()`, a type-safe replacement for `std::sprintf()`, to
//     write a format string to a `string` value.
//   * `turbo::StrAppendFormat()` to append a format string to a `string`
//   * `turbo::StreamFormat()` to more efficiently write a format string to a
...
// Example:
//
//   turbo::StrFormat("%s", turbo::FormatStreamed(obj));
template <typename T>
...
//
//   int n = 0;
//   std::string s = turbo::StrFormat("%s%d%n", "hello", 123,
...
// either:
//
// * A `constexpr` literal or `std::string_view`, which is how it most often
//   used.
...
//   turbo::StrFormat(formatString, "TheVillage", 6);
//
...
// supported; please use `std:string` and `std::string_view`. `char` is also not
...
// any string-like argument, so `std::string`, `std::string_view`, and
...
//   if (format_runtime) {
//     value = turbo::StrFormat(*format_runtime, i);
//   } else {
//     ... error case ...
//   }
...
// `turbo:: StrFormat()`. The returned object must be streamed before the full
// expression ends.
//
// Example:
//
...
// appended to it.
//
// `turbo::Format()` is a generic version of `turbo::StrAppendFormat()`, for
...
//     if (!turbo::FormatUntyped(&out, format, args)) {
//       return std::nullopt;
//     }
//     return std::move(out);
//   }
...
class FormatSink {
 public:
  ...
  friend void TurboFormatFlush(FormatSink* sink, std::string_view v) {
    sink->Append(v);
  }
