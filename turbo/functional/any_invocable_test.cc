...
struct Wrapper {
  template <class U,
            class = turbo::enable_if_t<std::is_convertible<U, T>::value>>
  Wrapper(U&&);  // NOLINT
};
...
struct QualifiersForThisImpl {
  ...
  using type =
      turbo::conditional_t<std::is_const<Qualifiers>::value, const This, This>&;
};
...
struct GiveQualifiersToFunImpl<T, R(P...)> {
  using type =
      turbo::conditional_t<std::is_const<T>::value, R(P...) const, R(P...)>;
};
...
struct GiveQualifiersToFunImpl<T, R(P...) noexcept> {
  using type = turbo::conditional_t<std::is_const<T>::value,
...
struct GiveQualifiersToFunImpl<T&, R(P...) noexcept> {
  using type =
      turbo::conditional_t<std::is_const<T>::value, R(P...) const & noexcept,
                          R(P...) & noexcept>;
};
...
struct TestParams {
  ...
  using CompatibleAnyInvocableFunType =
      turbo::conditional_t<std::is_rvalue_reference<Qual>::value,
...
struct IsMemberSwappableImpl<
    T, turbo::void_t<decltype(std::declval<T&>().swap(std::declval<T&>()))>>
    : std::true_type {
...
TYPED_TEST_P(AnyInvTestNonRvalue, NonMoveableResultType) {
  ...
  using UnqualifiedFun =
      turbo::conditional_t<TypeParam::kIsNoexcept, Result() noexcept, Result()>;
