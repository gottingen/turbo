...
std::string FormatTime(turbo::string_piece format, turbo::Time t,
                       turbo::TimeZone tz) {
...
bool ParseTime(turbo::string_piece format, turbo::string_piece input,
...
bool ParseTime(turbo::string_piece format, turbo::string_piece input,
  ...
  auto strip_leading_space = [](turbo::string_piece* sv) {
    while (!sv->empty()) {
  ...
  for (const auto& lit : literals) {
    if (turbo::StartsWith(input, turbo::string_piece(lit.name, lit.size))) {
      turbo::string_piece tail = input;
...
bool TurboParseFlag(turbo::string_piece text, turbo::Time* t, std::string* error) {
