...
namespace turbo {
    ...
    dag_node_builder<T> dag_builder<T>::node(
            F &&work,
            std::initializer_list<dag_node_builder<T>> after) {
        TURBO_CHECK(numIns.size() == dag->nodes.size()) << "node_builder vectors out of sync";
