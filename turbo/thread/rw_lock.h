...
#ifndef TURBO_THREAD_RW_LOCK_H_
#define TURBO_THREAD_RW_LOCK_H_
...
namespace std {
    ...
    template<>
    class lock_guard<turbo::read_lock> {
    public:
        explicit lock_guard(turbo::read_lock &mutex) : _pmutex(&mutex) {
            _pmutex->lock();
        }
    ...
    private:
        ...
        turbo::read_lock *_pmutex;
    };

    template<>
    class lock_guard<turbo::write_lock> {
    public:
        explicit lock_guard(turbo::write_lock &mutex) : _pmutex(&mutex) {
            _pmutex->lock();
        }
    ...
    private:
        ...
        turbo::write_lock *_pmutex;
    };

}  // namespace std

#endif // TURBO_THREAD_RW_LOCK_H_
