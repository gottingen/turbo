...
#ifndef TURBO_THREAD_THREAD_H_
#define TURBO_THREAD_THREAD_H_
...
#include <memory>
#include <functional>
#include "turbo/thread/affinity.h"
...
namespace turbo {
    ...
    public:
        ...
        template<typename F, typename = typename std::enable_if<std::is_same<thread,
                typename turbo::remove_cvref<F>::type>::value == false>>
        explicit thread(F &&f) {
        ...
        template<typename F,
                typename = typename std::enable_if<
                        std::is_same<thread, typename turbo::remove_cvref<F>::type>::value == false>>
        void initialize(F &&f) {
        ...
        static void set_name(const std::string_view &fmt, Args &&... args) {
            std::string name = turbo::string_format(fmt, std::forward<Args>(args)...);
            set_name(name);
        }
