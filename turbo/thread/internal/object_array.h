...
#ifndef TURBO_THREAD_INTERNAL_OBJECT_ARRAY_H_
#define TURBO_THREAD_INTERNAL_OBJECT_ARRAY_H_
...
#include "turbo/base/annotation.h"
    ...
    namespace thread_internal {
        ...
        public:
            ...
            void initialize_at(std::size_t index, F &&f) {
                TURBO_CHECK_LT(index, initialized_.size());
                TURBO_CHECK(!initialized_[index]);
        ...
        public:
            ...
            void registery(object_array<T> *array) {
                ...
                TURBO_CHECK(std::find(arrays_.begin(), arrays_.end(), array) ==
                       arrays_.end());
        ...
        public:
            ...
            std::size_t create_entry(std::function<void(void *)> *initializer, F &&cb) {
                ...
                if (unused_.empty()) {
                    TURBO_CHECK_EQ(max_index_, initializers_.size());
            ...
            void free_entry(std::size_t index, F &&cb) noexcept {
                ...
                TURBO_CHECK(initializers_[index]);
        ...
        object_array<T> *get_newest_local_object_array() {
            ...
            object_array_layout<T>::instance()->with_wewest_layout_locked([&](auto &&layout) {
                ...
                TURBO_CHECK_LT(array.objects.size(), layout.size());
...
// would fail the `TURBO_CHECK` in `reload_local_object_array_cache`, due to inconsistency
...
// method is in hot path, it's explicitly marked as `noinline`.
    ...
    [[gnu::noinline]] T* get_local_object_array_at(std::ptrdiff_t offset) {
            ...
            TURBO_CHECK_EQ(static_cast<size_t>(offset) % sizeof(T), 0ul);
            TURBO_CHECK_GE(static_cast<size_t>(offset), 0ul);
            if (TURBO_LIKELY(static_cast<size_t>(offset) < cache.limit)) {
...
#endif  // TURBO_THREAD_INTERNAL_OBJECT_ARRAY_H_
