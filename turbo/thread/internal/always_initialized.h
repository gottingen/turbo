...
#ifndef TURBO_THREAD_INTERNAL_ALWAYS_INITIALIZED_H_
#define TURBO_THREAD_INTERNAL_ALWAYS_INITIALIZED_H_


#include <utility>
#include <functional>
#include "turbo/thread/internal/object_array.h"
...
namespace turbo::thread_internal {
    ...
    template<class T>
    thread_local_always_initialized<T>::~thread_local_always_initialized() {
        TURBO_CHECK_EQ(static_cast<size_t>(offset_) % sizeof(T), 0ul);
    ...
    void thread_local_always_initialized<T>::for_each(F &&f) const {
        TURBO_DCHECK_EQ(static_cast<size_t>(offset_) % sizeof(T), 0ul);
        auto index = offset_ / sizeof(T);
...
#endif  // TURBO_THREAD_INTERNAL_ALWAYS_INITIALIZED_H_
