...
#ifndef TURBO_THREAD_THREAD_LOCAL_H_
#define TURBO_THREAD_THREAD_LOCAL_H_
...
#include "turbo/thread/internal/always_initialized.h"
#include "turbo/memory/atomic_ptr.h"
...
namespace turbo {
    ...
    public:
        ...
        T *get() const {
            ...
            return TURBO_LIKELY(!!ptr) ? ptr : get_slow();
        }
    ...
    private:
        ...
        mutable thread_internal::thread_local_always_initialized<turbo::atomic_scoped_ptr<T>> raw_tls_;
        mutable std::mutex init_lock_;
        std::function<std::unique_ptr<T>()> creator_;
    };

}  // namespace turbo
#endif  // TURBO_THREAD_THREAD_LOCAL_H_
