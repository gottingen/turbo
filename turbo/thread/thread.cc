...
namespace turbo {
    ...
    void *thread::thread_func(void *arg) {
        ...
        impl->name = turbo::string_format("{}#{}", impl->prefix, impl->index);
        thread::set_name(impl->name);
        TURBO_LOG(INFO) << "start thread: " << impl->name;
        ptr->set_affinity();
        impl->start_latch.count_down();
        impl->func();
        local_impl = nullptr;
        TURBO_LOG(INFO) << "exit thread: " << impl->name;
        return nullptr;
    }
...
#else
        TURBO_CHECK(!turbo::core_affinity::supported) << "Attempting to use thread affinity on a unsupported platform";
#endif
         */
    }
    ...
    thread::~thread() {
        if (_impl) {
            TURBO_LOG(WARNING) << "thread: " << _impl->name
    ...
    int32_t thread::thread_index() {
        if (TURBO_UNLIKELY(local_thread_id == -1)) {
            if (turbo::sysinfo::get_tid() != turbo::sysinfo::get_main_thread_pid()) {
    ...
    thread::native_handler_type thread::native_handler() {
        ...
        // in main thread or create not by turbo thread
        pthread_t mid = pthread_self();
        return mid;
    }
    // for main thread will index '0'
    TURBO_BOOTSTRAP(0, [] { thread::thread_index(); });
