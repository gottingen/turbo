...
namespace flare::base {
    ...
    namespace digest_detail {
        ...
        static FLARE_FORCE_INLINE void store64(uint64_t x, unsigned char *y) {
            for (int i = 0; i != 8; ++i)
                y[i] = (x >> ((7 - i) * 8)) & 255;
        }

        static FLARE_FORCE_INLINE uint64_t load64(const unsigned char *y) {
            uint64_t res = 0;
            for (int i = 0; i != 8; ++i)
        ...
        static FLARE_FORCE_INLINE uint64_t Sh(uint64_t x, uint64_t n) {
            return x >> n;
        }

        static FLARE_FORCE_INLINE uint64_t Sigma0(uint64_t x) {
            return rotr(x, 28) ^ rotr(x, 34) ^ rotr(x, 39);
        }

        static FLARE_FORCE_INLINE uint64_t Sigma1(uint64_t x) {
            return rotr(x, 14) ^ rotr(x, 18) ^ rotr(x, 41);
        }

        static FLARE_FORCE_INLINE uint64_t Gamma0(uint64_t x) {
            return rotr(x, 1) ^ rotr(x, 8) ^ Sh(x, 7);
        }

        static FLARE_FORCE_INLINE uint64_t Gamma1(uint64_t x) {
