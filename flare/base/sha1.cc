...
namespace flare::base {
    ...
    namespace digest_detail {
        ...
        static FLARE_FORCE_INLINE void store64h(uint64_t x, unsigned char *y) {
            for (int i = 0; i != 8; ++i)
                y[i] = (x >> ((7 - i) * 8)) & 255;
        }

        static FLARE_FORCE_INLINE uint32_t load32h(const uint8_t *y) {
            return (uint32_t(y[0]) << 24) | (uint32_t(y[1]) << 16) |
                   (uint32_t(y[2]) << 8) | (uint32_t(y[3]) << 0);
        }

        static FLARE_FORCE_INLINE void store32h(uint32_t x, uint8_t *y) {
            for (int i = 0; i != 4; ++i)
                y[i] = (x >> ((3 - i) * 8)) & 255;
        }
