...
#ifndef FLARE_BASE_ERRNO_H_
#define FLARE_BASE_ERRNO_H_
...
// FLARE_REGISTER_ERRNO(the_error_number, its_description) in global scope of
...
//     flare_error(error_code) returns the description.
//     flare_error() returns description of last system error code.
//
// %m in printf-alike functions does NOT recognize errors defined by
// FLARE_REGISTER_ERRNO, you have to explicitly print them by %s.
//
//     errno = ESTOP;
...
namespace flare::base {
    // You should not call this function, use FLARE_REGISTER_ERRNO instead.
    extern int describe_customized_errno(int, const char *, const char *);
}  // namespace flare::base

template<int error_code>
class flare_errno_helper {
};
