...
namespace flare::base {
    ...
    namespace {
        ...
        static FLARE_FORCE_INLINE u32 min(u32 x, u32 y) {
            return x < y ? x : y;
        }

        static FLARE_FORCE_INLINE u32 load32(const uint8_t *y) {
            return (u32(y[0]) << 24) | (u32(y[1]) << 16) |
                   (u32(y[2]) << 8) | (u32(y[3]) << 0);
        }

        static FLARE_FORCE_INLINE void store64(u64 x, uint8_t *y) {
            for (int i = 0; i != 8; ++i)
                y[i] = (x >> ((7 - i) * 8)) & 255;
        }

        static FLARE_FORCE_INLINE void store32(u32 x, uint8_t *y) {
            for (int i = 0; i != 4; ++i)
                y[i] = (x >> ((3 - i) * 8)) & 255;
        }

        static FLARE_FORCE_INLINE u32 Ch(u32 x, u32 y, u32 z) {
            return z ^ (x & (y ^ z));
        }

        static FLARE_FORCE_INLINE u32 Maj(u32 x, u32 y, u32 z) {
            return ((x | y) & z) | (x & y);
        }

        static FLARE_FORCE_INLINE u32 Sh(u32 x, u32 n) {
            return x >> n;
        }

        static FLARE_FORCE_INLINE u32 Sigma0(u32 x) {
            return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22);
        }

        static FLARE_FORCE_INLINE u32 Sigma1(u32 x) {
            return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25);
        }

        static FLARE_FORCE_INLINE u32 Gamma0(u32 x) {
            return rotr(x, 7) ^ rotr(x, 18) ^ Sh(x, 3);
        }

        static FLARE_FORCE_INLINE u32 Gamma1(u32 x) {
