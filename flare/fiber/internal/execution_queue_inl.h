...
#ifndef  FLARE_FIBER_INTERNAL_EXECUTION_QUEUE_INL_H_
#define  FLARE_FIBER_INTERNAL_EXECUTION_QUEUE_INL_H_
...
namespace flare::fiber_internal {
    ...
    struct FLARE_CACHELINE_ALIGNMENT TaskNode {
        TaskNode()
...
// Specialize TaskNodeAllocator for types with different sizes
    ...
    class FLARE_CACHELINE_ALIGNMENT ExecutionQueueBase {
        FLARE_DISALLOW_COPY_AND_ASSIGN(ExecutionQueueBase);
    ...
    private:
        ...
        std::atomic<TaskNode *> FLARE_CACHELINE_ALIGNMENT _head;
        std::atomic<uint64_t> FLARE_CACHELINE_ALIGNMENT _versioned_ref;
        std::atomic<bool> FLARE_CACHELINE_ALIGNMENT _stopped;
    ...
    public:
        ...
        typedef flare::fiber_internal::ExecutionQueueId<T> id_t;
        ...
        int execute(typename flare::add_const_reference<T>::type task,
            ...
            if (FLARE_UNLIKELY(node == nullptr)) {
                return ENOMEM;
            }
            void *const mem = allocator::allocate(node);
            if (FLARE_UNLIKELY(!mem)) {
...
// ---------------------ExecutionQueueBase--------------------
    inline bool ExecutionQueueBase::_more_tasks(
        ...
        FLARE_CHECK(old_head->next == nullptr);
...
#endif  // FLARE_FIBER_INTERNAL_EXECUTION_QUEUE_INL_H_
