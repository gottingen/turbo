...
// Warm up backtrace before main().
    void *dummy_buf[4];
    const int FLARE_ALLOW_UNUSED
    dummy_bt = backtrace(dummy_buf, FLARE_ARRAY_SIZE(dummy_buf));
...
// instance. nullptr otherwise.
    static ContentionProfiler *FLARE_CACHELINE_ALIGNMENT
    g_cp = nullptr;
...
// Start profiling contention.
    bool ContentionProfilerStart(const char *filename) {
        if (filename == nullptr) {
            FLARE_LOG(ERROR) << "Parameter [filename] is nullptr";
            return false;
        }
...
#define FIBER_MUTEX_LOCKED (*(const unsigned*)&flare::fiber_internal::MUTEX_LOCKED_RAW)
    ...
    inline int mutex_lock_contended(fiber_mutex_t *m) {
        ...
        while (whole->exchange(FIBER_MUTEX_CONTENDED) & FIBER_MUTEX_LOCKED) {
            if (flare::fiber_internal::waitable_event_wait(whole, FIBER_MUTEX_CONTENDED, nullptr) < 0 &&
