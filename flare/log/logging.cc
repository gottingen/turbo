...
#include "flare/log/config.h"        // to get the program name
...
// all other threads.
    ...
    log_message::log_message(const char *file, int line)
            : allocated_(nullptr) {
        init(file, line, FLARE_INFO, &log_message::send_to_log);
    }
    ...
    void log_message::init(const char *file,
        ...
        if (!FLAGS_flare_log_backtrace_at.empty()) {
            char fileline[128];
            snprintf(fileline, sizeof(fileline), "%s:%d", data_->basename_, line);
#ifdef HAVE_STACKTRACE
            if (!strcmp(FLAGS_flare_log_backtrace_at.c_str(), fileline)) {
                string stacktrace;
...
# define ATTRIBUTE_NORETURN
    ...
    FLARE_EXPORT logging_fail_func_t g_logging_fail_func = &logging_fail;
...
// log_stream.
    ...
    static void GetTempDirectories(vector<string> *list) {
        ...
        for (size_t i = 0; i < FLARE_ARRAY_SIZE(candidates); i++) {
