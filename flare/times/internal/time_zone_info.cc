...
namespace flare::times_internal {
    ...
    namespace {
        ...
        FLARE_FORCE_INLINE bool is_leap(year_t year) {
...
// Single-byte, unsigned numeric values are encoded directly.
        FLARE_FORCE_INLINE std::uint_fast8_t decode8(const char *cp) {
...
// Generate a year-relative offset for a posix_transition.
        ...
        FLARE_FORCE_INLINE time_zone::civil_lookup make_unique(const time_point<seconds> &tp) {
        ...
        FLARE_FORCE_INLINE time_zone::civil_lookup make_unique(std::int_fast64_t unix_time) {
            return make_unique(from_unix_seconds(unix_time));
        }

        FLARE_FORCE_INLINE time_zone::civil_lookup make_skipped(const transition &tr,
        ...
        FLARE_FORCE_INLINE time_zone::civil_lookup make_repeated(const transition &tr,
                                                                 const civil_second &cs) {
            time_zone::civil_lookup cl;
        ...
        FLARE_FORCE_INLINE civil_second year_shift(const civil_second &cs, year_t shift) {
...
// fopen(3) adaptor.
        FLARE_FORCE_INLINE FILE *FOpen(const char *path, const char *mode) {
#if defined(_MSC_VER)
            FILE* fp;
