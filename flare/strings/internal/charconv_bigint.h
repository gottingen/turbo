...
#ifndef FLARE_STRINGS_INTERNAL_CHARCONV_BIGINT_H_
#define FLARE_STRINGS_INTERNAL_CHARCONV_BIGINT_H_
...
class big_unsigned {
  public:
    ...
    big_unsigned() : size_(0), words_{} {}

    explicit constexpr big_unsigned(uint64_t v)
            : size_((v >> 32) ? 2 : v ? 1 : 0),
              words_{static_cast<uint32_t>(v & 0xffffffffu),
                     static_cast<uint32_t>(v >> 32)} {}

    // Constructs a big_unsigned from the given string_view containing a decimal
    // value.  If the input std::string is not a decimal integer, constructs a 0
    // instead.
    explicit big_unsigned(std::string_view sv) : size_(0), words_{} {
    ...
    // type.
    //
    // Analagous to std::numeric_limits<big_unsigned>::digits10.
    ...
    // a lookup table, and is faster then seeding a big_unsigned with 1 and calling
    // MultiplyByFiveToTheNth().
    static big_unsigned FiveToTheNth(int n);

    // Multiplies by another big_unsigned, in-place.
    template<int M>
    void MultiplyBy(const big_unsigned<M> &other) {
    ...
    // Returns the value of the nth word of this big_unsigned.  This is
  ...
  private:
    ...
    //
    // When size_ is 0, this big_unsigned stores the value 0.
...
int Compare(const big_unsigned<N> &lhs, const big_unsigned<M> &rhs) {
...
template<int N, int M>
bool operator==(const big_unsigned<N> &lhs, const big_unsigned<M> &rhs) {
    int limit = (std::max)(lhs.size(), rhs.size());
...
// Output operator for big_unsigned, for testing purposes only.
template<int N>
std::ostream &operator<<(std::ostream &os, const big_unsigned<N> &num) {
    return os << num.ToString();
}

// Explicit instantiation declarations for the sizes of big_unsigned that we
// are using.
//
...
extern template
class big_unsigned<4>;

extern template
class big_unsigned<84>;
