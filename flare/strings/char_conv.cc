...
#ifdef FLARE_BIT_PACK_FLOATS
#error FLARE_BIT_PACK_FLOATS cannot be directly set
...
struct FloatTraits<double> {
    ...
    static double Make(uint64_t mantissa, int exponent, bool sign) {
        ...
        dbl += mantissa;
        return flare::base::bit_cast<double>(dbl);
#endif  // FLARE_BIT_PACK_FLOATS
    }
};
...
struct FloatTraits<float> {
    ...
    return sign ? -ldexpf(mantissa, exponent) : ldexpf(mantissa, exponent);
        ...
        flt += mantissa;
        return flare::base::bit_cast<float>(flt);
#endif  // FLARE_BIT_PACK_FLOATS
    }
};
...
int BitWidth(flare::uint128 value) {
    ...
    return 128 - flare::base::countl_zero(uint128_high64(value));
}
...
CalculatedFloat CalculateFromParsedHexadecimal(
    ...
    int mantissa_width = 64 - flare::base::countl_zero(mantissa);
