...
#ifndef FLARE_STRINGS_NUMBERS_H_
#define FLARE_STRINGS_NUMBERS_H_
...
// state.
    ...
    FLARE_MUST_USE_RESULT bool simple_atoi(std::string_view str, int_type *out);
...
// returns `false`, leaving `out` in an unspecified state.
    FLARE_MUST_USE_RESULT bool simple_atof(std::string_view str, float *out);
...
// always use the "C" locale. If any errors are encountered, this function
// returns `false`, leaving `out` in an unspecified state.
    FLARE_MUST_USE_RESULT bool simple_atod(std::string_view str, double *out);
...
//   // buf[0] == '4'
//   // buf[1] == '2'
        FLARE_FORCE_INLINE void put_two_digits(size_t i, char *buf) {
            assert(i < 100);
...
// since 0 has one digit).
        FLARE_FORCE_INLINE size_t
        fast_hex_to_buffer_zero_pad16(uint64_t
        val,
        char *out
        ) {
#ifdef __SSE4_2__
        uint64_t be = flare::base::big_endian::from_host64(val);
    ...
    return 16 -
    flare::base::countl_zero(val| 0x1) / 4;
}
