...
#ifndef FLARE_CONTAINER_ROBIN_MAP_H_
#define FLARE_CONTAINER_ROBIN_MAP_H_
...
#include <type_traits>
#include <utility>
#include "flare/container/robin_hash.h"
...
namespace flare {
     ...
     * as we then compare the stored hashes before comparing the keys. When `flare::rh::power_of_two_growth_policy` is used
     ...
     * `sizeof(flare::detail_robin_hash::bucket_entry<ValueType, true>) ==
     * sizeof(flare::detail_robin_hash::bucket_entry<ValueType, false>)`) and `flare::rh::power_of_two_growth_policy` is
     ...
     * By default the map uses `flare::rh::power_of_two_growth_policy`. This policy keeps the number of buckets
     ...
     * check `flare::rh::power_of_two_growth_policy` for the interface.
     *
    ...
    template<class Key,
            ...
            class GrowthPolicy = flare::rh::power_of_two_growth_policy<2>>
    class robin_map {
    private:
        template<typename U>
        using has_is_transparent = flare::detail_robin_hash::has_is_transparent<U>;
    ...
    /**
     * Same as `flare::robin_map<Key, T, Hash, KeyEqual, Allocator, StoreHash, flare::rh::prime_growth_policy>`.
     */
    template<class Key,
            class T,
            class Hash = std::hash<Key>,
            ...
            class Allocator = std::allocator<std::pair<Key, T>>,
            bool StoreHash = false>
    using robin_pg_map = robin_map<Key, T, Hash, KeyEqual, Allocator, StoreHash, flare::rh::prime_growth_policy>;

}  // namespace flare

#endif  // FLARE_CONTAINER_ROBIN_MAP_H_
