...
#ifndef  FLARE_CONTAINER_SPARSE_HASH_H_
#define  FLARE_CONTAINER_SPARSE_HASH_H_
...
#include "flare/container/sparse_growth_policy.h"
...
#ifdef TSL_DEBUG
#define flare_sh_assert(expr) assert(expr)
#else
#define flare_sh_assert(expr) (static_cast<void>(0))
#endif
...
// From Wikipedia: https://en.wikipedia.org/wiki/Hamming_weight
    ...
    namespace detail_sparse_hash {
        ...
        struct is_power_of_two_policy<flare::sh::power_of_two_growth_policy<GrowthFactor>>
                : std::true_type {
        };
 ...
 */
        template<typename T, typename Allocator, flare::sh::sparsity Sparsity>
        class sparse_array {
        public:
            using value_type = T;
        ...
        private:
            static const size_type CAPACITY_GROWTH_STEP =
                    (Sparsity == flare::sh::sparsity::high) ? 2
                                                          : (Sparsity == flare::sh::sparsity::medium)
                                                            ? 4
                                                            : 8;  // (Sparsity == flare::sh::sparsity::low)
        ...
        public:
            ...
            static std::size_t nb_sparse_buckets(std::size_t bucket_count) noexcept {
                ...
                return std::max<std::size_t>(
                        1, sparse_ibucket(flare::detail_sparse_hash::round_up_to_power_of_two(
                                bucket_count)));
            }
        ...
        public:
            ...
            sparse_array(size_type capacity, Allocator &alloc)
                ...
                if (m_capacity > 0) {
                    ...
                    flare_sh_assert(m_values !=nullptr);  // allocate should throw if there is a failure
                }
            }
            ...
            sparse_array(const sparse_array &other, Allocator &alloc)
                ...
                flare_sh_assert(other.m_capacity >= other.m_nb_elements);
                if (m_capacity == 0) {
                    return;
                }

                m_values = alloc.allocate(m_capacity);
                flare_sh_assert(m_values !=
            ...
            ~sparse_array() noexcept {
                ...
                flare_sh_assert(m_capacity == 0 && m_nb_elements == 0 && m_values == nullptr);
            }
            ...
            bool has_value(size_type index) const noexcept {
                flare_sh_assert(index < BITMAP_NB_BITS);
            ...
            iterator value(size_type index) noexcept {
                flare_sh_assert(has_value(index));
            ...
            iterator set(allocator_type &alloc, size_type index, Args &&...value_args) {
                ...
                flare_sh_assert(has_value(index));
                flare_sh_assert(!has_deleted_value(index));
        ...
        private:
            ...
            static size_type popcount(bitmap_type val) noexcept {
                if (sizeof(bitmap_type) <= sizeof(unsigned int)) {
                    return static_cast<size_type>(
                            flare::detail_popcount::popcount(static_cast<unsigned int>(val)));
                } else {
                    return static_cast<size_type>(flare::detail_popcount::popcountll(val));
                }
            }
            ...
            size_type offset_to_index(size_type offset) const noexcept {
                flare_sh_assert(offset < m_nb_elements);
            ...
            void insert_at_offset_no_realloc(allocator_type &alloc, size_type offset,
                                             Args &&...value_args) {
                flare_sh_assert(offset <= m_nb_elements);
                flare_sh_assert(m_nb_elements < m_capacity);
            ...
            void insert_at_offset_realloc(allocator_type &alloc, size_type offset,
                ...
                flare_sh_assert(nb_new_values == m_nb_elements + 1);
            ...
            void erase_at_offset(allocator_type &alloc, size_type offset) {
                ...
                flare_sh_assert(m_nb_elements > 1);
 ...
 * `flare::sh::exception_safety::strong`.
 *
 ...
 */
        template<class ValueType, class KeySelect, class ValueSelect, class Hash,
                ...
                flare::sh::exception_safety ExceptionSafety, flare::sh::sparsity Sparsity,
                flare::sh::probing Probing>
        ...
        private:
            using sparse_array =
            flare::detail_sparse_hash::sparse_array<ValueType, Allocator, Sparsity>;
        ...
        public:
            ...
            public:
                ...
                sparse_iterator &operator++() {
                    flare_sh_assert(m_sparse_array_it != nullptr);
                    ++m_sparse_array_it;
        ...
        public:
            sparse_hash(size_type bucket_count, const Hash &hash, const KeyEqual &equal,
                ...
                if (m_bucket_count > 0) {
                    ...
                    flare_sh_assert(!m_sparse_buckets_data.empty());
            ...
            void insert(InputIt first, InputIt last) {
                if (std::is_base_of<
                    ...
                    flare_sh_assert(m_load_threshold_rehash >= size());
            ...
            iterator erase(iterator pos) {
                flare_sh_assert(pos != end() && m_nb_elements > 0);
            ...
            void swap(sparse_hash &other) {
                ...
                if (std::allocator_traits<Allocator>::propagate_on_container_swap::value) {
                    ...
                    flare_sh_assert(static_cast<Allocator &>(*this) ==
                                  static_cast<Allocator &>(other));
                }
            ...
            void max_load_factor(float ml) {
                ...
                flare_sh_assert(max_load_factor_with_deleted_buckets > 0.0f &&
        ...
        private:
            ...
            size_type bucket_for_hash(std::size_t hash) const {
                ...
                flare_sh_assert(sparse_array::sparse_ibucket(bucket) <
            ...
            size_type next_bucket(size_type ibucket, size_type iprobe) const {
                (void) iprobe;
                if (Probing == flare::sh::probing::linear) {
                    return (ibucket + 1) & this->m_mask;
                } else {
                    flare_sh_assert(Probing == flare::sh::probing::quadratic);
            ...
            void clear_deleted_buckets() {
                ...
                flare_sh_assert(m_nb_deleted_buckets == 0);
            }

            template<flare::sh::exception_safety U = ExceptionSafety,
                    typename std::enable_if<U == flare::sh::exception_safety::basic>::type
                    * = nullptr>
            ...
            template<flare::sh::exception_safety U = ExceptionSafety,
                    typename std::enable_if<
                            U == flare::sh::exception_safety::strong>::type * = nullptr>
            void rehash_impl(size_type count) {
                sparse_hash new_table(count, static_cast<Hash &>(*this),
                                      static_cast<KeyEqual &>(*this),
                                      static_cast<Allocator &>(*this), m_max_load_factor);
...
#endif  // FLARE_CONTAINER_SPARSE_HASH_H_
