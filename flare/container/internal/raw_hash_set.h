...
#ifndef FLARE_CONTAINER_INTERNAL_RAW_HASH_SET_H_
#define FLARE_CONTAINER_INTERNAL_RAW_HASH_SET_H_
...
#include "flare/container/internal/map_fwd_decl.h"
#include "flare/hash/hash.h"
#include "flare/base/math.h"
#include "flare/base/endian.h"
#include "flare/container/internal/map_base.h"
...
namespace flare {
    ...
    namespace priv {
        ...
        struct IsDecomposable<
                flare::void_t<decltype(
                Policy::apply(RequireUsableKey<typename Policy::key_type, Hash, Eq>(),
                              std::declval<Ts>()...))>,
                Policy, Hash, Eq, Ts...> : std::true_type {
        };

        // TODO: Switch to std::is_nothrow_swappable when gcc/clang supports it.
        // --------------------------------------------------------------------------
        template<class T>
        ...
        int TrailingZeros(T x) {
            if constexpr(sizeof(T) == 8)
                return base_internal::CountTrailingZerosNonZero64(
        ...
        int LeadingZeros(T x) {
            if constexpr(sizeof(T) == 8)return base_internal::CountLeadingZeros64(static_cast<uint64_t>(x));
            else
        ...
        */
        ...
        //
        // For example:
        //   for (int i : bit_mask<uint32_t, 16>(0x5)) -> yields 0, 2
        //   for (int i : bit_mask<uint64_t, 8, 3>(0x0000000080800000)) -> yields 2, 3
        // --------------------------------------------------------------------------
        ...
        public:
            ...
            using iterator = bit_mask;
            using const_iterator = bit_mask;

            explicit bit_mask(T mask) : mask_(mask) {}
            ...
            int LowestBitSet() const {
                return flare::base::countr_zero(mask_) >> Shift;
            }

            int HighestBitSet() const {
                return (sizeof(T) * CHAR_BIT - flare::base::countl_zero(mask_) -
                        1) >>
                           Shift;
            }
            ...
            int leading_zeros() const {
                ...
                return flare::base::countl_zero(mask_ << extra_bits) >> Shift;
            }

        private:

            friend bool operator==(const bit_mask &a, const bit_mask &b) {
        ...
        static_assert(~kEmpty & ~kDeleted & kSentinel & 0x7F,
                      ...
                      "by kSentinel to make the scalar test for match_empty_or_deleted() "
                      "efficient");
        static_assert(kDeleted == -2,
                      "kDeleted must be -2 to make the implementation of "
                      "convert_special_to_empty_and_full_to_deleted efficient");
...
#ifdef FLARE_MAP_NON_DETERMINISTIC
...
// --------------------------------------------------------------------------
// --------------------------------------------------------------------------
        struct group_sse2_impl {
            enum {
                kWidth = 16
            };  // the number of slots per group

            explicit group_sse2_impl(const ctrl_t *pos) {
                ctrl = _mm_loadu_si128(reinterpret_cast<const __m128i *>(pos));
            }

            // Returns a bitmask representing the positions of slots that match hash.
            // ----------------------------------------------------------------------
            bit_mask<uint32_t, kWidth> match(h2_t hash) const {
                auto match = _mm_set1_epi8((char) hash);
                return bit_mask<uint32_t, kWidth>(
                        _mm_movemask_epi8(_mm_cmpeq_epi8(match, ctrl)));
            }

            // Returns a bitmask representing the positions of empty slots.
            // ------------------------------------------------------------
            bit_mask<uint32_t, kWidth> match_empty() const {
#if FLARE_HAVE_SSSE3
            ...
            // -----------------------------------------------------------------------
            bit_mask<uint32_t, kWidth> match_empty_or_deleted() const {
            ...
            uint32_t count_leading_empty_or_deleted() const {
                auto special = _mm_set1_epi8(kSentinel);
                return flare::base::countr_zero(
                        _mm_movemask_epi8(_mm_cmpgt_epi8_fixed(special, ctrl)) + 1);
            }

            // ----------------------------------------------------------------------
            void convert_special_to_empty_and_full_to_deleted(ctrl_t *dst) const {
...
#endif  // FLARE_HAVE_SSE2

        // --------------------------------------------------------------------------
        // --------------------------------------------------------------------------
        struct group_portable_impl {
            enum {
                kWidth = 8
            };

            explicit group_portable_impl(const ctrl_t *pos)
                    : ctrl(flare::base::little_endian::load64(pos)) {}

            bit_mask<uint64_t, kWidth, 3> match(h2_t hash) const {
                ...
                return bit_mask<uint64_t, kWidth, 3>((x - lsbs) & ~x & msbs);
            }

            bit_mask<uint64_t, kWidth, 3> match_empty() const {
                constexpr uint64_t msbs = 0x8080808080808080ULL;
                return bit_mask<uint64_t, kWidth, 3>((ctrl & (~ctrl << 6)) & msbs);
            }

            bit_mask<uint64_t, kWidth, 3> match_empty_or_deleted() const {
                constexpr uint64_t msbs = 0x8080808080808080ULL;
                return bit_mask<uint64_t, kWidth, 3>((ctrl & (~ctrl << 7)) & msbs);
            }

            uint32_t count_leading_empty_or_deleted() const {
                constexpr uint64_t gaps = 0x00FEFEFEFEFEFEFEULL;
                return (uint32_t) ((flare::base::countr_zero(((~ctrl & (ctrl >> 7)) | gaps) + 1) + 7) >> 3);
            }
            ...
            void convert_special_to_empty_and_full_to_deleted(ctrl_t *dst) const {
                ...
                auto x = ctrl & msbs;
                auto res = (~x + (x >> 7)) & ~lsbs;
                flare::base::little_endian::store64(dst, res);
            }
...
#if FLARE_HAVE_SSE2
        using Group = group_sse2_impl;
#else
        using Group = group_portable_impl;
#endif
        ...
        inline void ConvertDeletedToEmptyAndFullToDeleted(
            ...
            for (ctrl_t *pos = ctrl; pos != ctrl + capacity + 1; pos += Group::kWidth) {
                Group{pos}.convert_special_to_empty_and_full_to_deleted(pos);
            }
...
// --------------------------------------------------------------------------
        inline size_t NormalizeCapacity(size_t n) {
            return n ? ~size_t{} >> flare::base::countl_zero(n) : 1;
        }
...
// ----------------------------------------------------------------------------
        ...
        namespace memory_internal {
            ...
            void ConstructFromTupleImpl(Alloc *alloc, T *ptr, Tuple &&t,
                                        flare::index_sequence<I...>) {
                flare::allocator_traits<Alloc>::construct(
            ...
            decltype(std::declval<F>()(std::declval<T>())) WithConstructedImpl(
                    Tuple &&t, flare::index_sequence<Is...>, F &&f) {
                return WithConstructedImplF<T, F>{std::forward<F>(f)}(
                        std::get<Is>(std::forward<Tuple>(t))...);
            }

            template<class T, size_t... Is>
            auto TupleRefImpl(T &&t, flare::index_sequence<Is...>)
...
// ----------------------------------------------------------------------------
        ...
        public:
            ...
            using pointer = typename flare::allocator_traits<
                    allocator_type>::template rebind_traits<value_type>::pointer;
            using const_pointer = typename flare::allocator_traits<
        ...
        private:
            ...
            using Layout = flare::priv::Layout<ctrl_t, slot_type>;
            ...
            using AllocTraits = flare::allocator_traits<allocator_type>;
            using SlotAlloc = typename flare::allocator_traits<
                    allocator_type>::template rebind_alloc<slot_type>;
            using SlotAllocTraits = typename flare::allocator_traits<
            ...
            using RequiresInsertable = typename std::enable_if<
                    flare::disjunction<std::is_convertible<T, init_type>,
        ...
        public:
            ...
            public:
                ...
                using reference =
                flare::conditional_t<PolicyTraits::constant_iterators::value,
                        const value_type &, value_type &>;
                using pointer = flare::remove_reference_t<reference> *;
        ...
        iterator operator--(int) {
            ...
            private:
                ...
                void skip_empty_or_deleted() {
                    while (IsEmptyOrDeleted(*ctrl_)) {
                        ...
                        uint32_t shift = Group{ctrl_}.count_leading_empty_or_deleted();
                        ctrl_ += shift;
                        slot_ += shift;
                    }
                }
            ...
            //   // copies the strings into the set.
            //   flare::flat_hash_set<std::string> s = {"abc", "def"};
            //
            ...
            std::is_nothrow_copy_constructible<allocator_type>::value)
                    : ctrl_(flare::exchange(that.ctrl_, EmptyGroup())),
                      slots_(flare::exchange(that.slots_, nullptr)),
                      size_(flare::exchange(that.size_, 0)),
                      capacity_(flare::exchange(that.capacity_, 0)),
                      infoz_(flare::exchange(that.infoz_, HashtablezInfoHandle())),
            ...
            flare::allocator_traits<allocator_type>::is_always_equal::value &&
            ...
            template<class InputIt, typename flare::enable_if_t<has_difference_operator<InputIt>::value, int> = 0>
...
#if !defined(FLARE_MAP_NON_DETERMINISTIC)

            template<typename OutputArchive>
            bool flare_map_dump(OutputArchive &) const;

            template<typename InputArchive>
            bool flare_map_load(InputArchive &);
            ...
            hasher
            hash_function() const { return hash_ref(); } // warning: doesn't match internal hash - use hash() member function
        ...
        private:
            ...
            friend
            struct flare::priv::hashtable_debug_internal::HashtableDebugAccess;
            ...
            bool find_impl(const key_arg<K> &key, size_t hashval, size_t &offset) {
                ...
                while (true) {
                    ...
                    for (int i : g.match((h2_t) H2(hashval))) {
                        ...
                        if (FLARE_LIKELY(PolicyTraits::apply(
                                EqualElement < K > {key, eq_ref()},
                                PolicyTraits::element(slots_ + offset))))
                            return true;
                    }
                    if (FLARE_LIKELY(g.match_empty()))
                        return false;
                    seq.next();
                }
            }
            ...
            struct HashElement {
                ...
                size_t operator()(const K &key, Args &&...) const {
                    return flare_mix<sizeof(size_t)>()(h(key));
                }
            ...
            void erase_meta_only(const_iterator it) {
                ...
                const auto empty_after = Group(it.inner_.ctrl_).match_empty();
                const auto empty_before = Group(ctrl_ + index_before).match_empty();
                ...
                bool was_never_full =
                        ...
                        static_cast<size_t>(empty_after.trailing_zeros() +
                                            empty_before.trailing_zeros()) < Group::kWidth;
            ...
            void drop_deletes_without_resize() FLARE_NO_INLINE {
                ...
                for (size_t i = 0; i != capacity_; ++i) {
                    ...
                    if (FLARE_LIKELY(probe_index(new_i) == probe_index(i))) {
            ...
            bool has_element(const value_type &elem, size_t hashval) const {
                ...
                while (true) {
                    Group g{ctrl_ + seq.offset()};
                    for (int i : g.match((h2_t) H2(hashval))) {
                        if (FLARE_LIKELY(PolicyTraits::element(slots_ + seq.offset((size_t) i)) ==
                                         elem))
                            return true;
                    }
                    if (FLARE_LIKELY(g.match_empty())) return false;
                    seq.next();
        ...
        protected:
            ...
            std::pair<size_t, bool> find_or_prepare_insert(const K &key, size_t hashval) {
                ...
                while (true) {
                    ...
                    if (FLARE_LIKELY(g.match_empty())) break;
                    seq.next();
                }
            ...
            size_t prepare_insert(size_t hashval) FLARE_NO_INLINE {
                auto target = find_first_non_full(hashval);
                if (FLARE_UNLIKELY(growth_left() == 0 &&
        ...
        private:
            ...
            flare::priv::CompressedTuple<size_t /* growth_left */, hasher,
...
// --------------------------------------------------------------------------
        ...
        public:
            ...
            MappedReference<P> at(const key_arg<K> &key) {
                ...
                if (it == this->end())
                    flare::base_internal::ThrowStdOutOfRange("flat_hash_map at(): lookup non-existent key");
...
// ----------------------------------------------------------------------------
        ...
        protected:
            using Lockable = flare::LockableImpl<Mtx_>;
        ...
        public:
            ...
            explicit parallel_hash_set(size_t bucket_cnt,
                    ...
                    parallel_hash_set(typename Inner::Params{bucket_cnt, hash_param, eq, alloc},
                                      flare::make_index_sequence<num_tables>{}) {}

            template<std::size_t... i>
            parallel_hash_set(typename Inner::Params const &p,
                              flare::index_sequence<i...>) : sets_{((void) i, p)...} {}
        ...
        for (auto& inner : sets_)
            ...
            // --------------------------------------------------------------------
            // flat_hash_map extension: emplace_with_hash
            // ----------------------------------
            ...
            // --------------------------------------------------------------------
            // end of flat_hash_map expension
            // --------------------------------------------------------------------
            ...
            bool if_contains_unsafe(const key_arg<K> &key, F &&f) const {
                return const_cast<parallel_hash_set *>(this)->template
                        modify_if_impl<K, F, LockableBaseImpl<flare::null_mutex>::DoNothing>(key, std::forward<F>(f));
            }
            ...
            template<typename OutputArchive>
            bool flare_map_dump(OutputArchive &ar) const;

            template<typename InputArchive>
            bool flare_map_load(InputArchive &ar);
        ...
        public:
            ...
            // ----------- end of flat_hash_map extensions --------------------------
        ...
        struct flat_hash_set_policy {
            ...
            static void construct(Allocator *alloc, slot_type *slot, Args &&... args) {
                flare::allocator_traits<Allocator>::construct(*alloc, slot,
                                                              std::forward<Args>(args)...);
            }

            template<class Allocator>
            static void destroy(Allocator *alloc, slot_type *slot) {
                flare::allocator_traits<Allocator>::destroy(*alloc, slot);
            }
            ...
            template<class F, class... Args>
            static decltype(flare::priv::DecomposeValue(
                    std::declval<F>(), std::declval<Args>()...))
            apply(F &&f, Args &&... args) {
                return flare::priv::DecomposeValue(
        ...
        struct flat_hash_map_policy {
            ...
            template<class F, class... Args>
            static decltype(flare::priv::DecomposePair(
                    std::declval<F>(), std::declval<Args>()...))
            apply(F &&f, Args &&... args) {
                return flare::priv::DecomposePair(std::forward<F>(f),
                                                  std::forward<Args>(args)...);
            }
        ...
        // --------------------------------------------------------------------------
        template<class T>
        struct node_hash_set_policy
                : flare::priv::node_hash_policy<T &, node_hash_set_policy<T>> {
            using key_type = T;
            using init_type = T;
            using constant_iterators = std::true_type;
            ...
            static T *new_element(Allocator *alloc, Args &&... args) {
                using ValueAlloc =
                typename flare::allocator_traits<Allocator>::template rebind_alloc<T>;
                ValueAlloc value_alloc(*alloc);
                T *res = flare::allocator_traits<ValueAlloc>::allocate(value_alloc, 1);
                flare::allocator_traits<ValueAlloc>::construct(value_alloc, res,
                                                               std::forward<Args>(args)...);
                return res;
            }
            ...
            static void delete_element(Allocator *alloc, T *elem) {
                ...
                typename flare::allocator_traits<Allocator>::template rebind_alloc<T>;
                ValueAlloc value_alloc(*alloc);
                flare::allocator_traits<ValueAlloc>::destroy(value_alloc, elem);
                flare::allocator_traits<ValueAlloc>::deallocate(value_alloc, elem, 1);
            }
...
// --------------------------------------------------------------------------
        ...
        class node_hash_map_policy
                : public flare::priv::node_hash_policy<
                        std::pair<const Key, Value> &, node_hash_map_policy<Key, Value>> {
        ...
        public:
            ...
            static value_type *new_element(Allocator *alloc, Args &&... args) {
                using PairAlloc = typename flare::allocator_traits<
                        Allocator>::template rebind_alloc<value_type>;
                PairAlloc pair_alloc(*alloc);
                value_type *res =
                        flare::allocator_traits<PairAlloc>::allocate(pair_alloc, 1);
                flare::allocator_traits<PairAlloc>::construct(pair_alloc, res,
                                                              std::forward<Args>(args)...);
                return res;
            }
            ...
            static void delete_element(Allocator *alloc, value_type *pair) {
                using PairAlloc = typename flare::allocator_traits<
                        Allocator>::template rebind_alloc<value_type>;
                PairAlloc pair_alloc(*alloc);
                flare::allocator_traits<PairAlloc>::destroy(pair_alloc, pair);
                flare::allocator_traits<PairAlloc>::deallocate(pair_alloc, pair, 1);
            }
        ...
        template<class CharT>
        struct string_hash_eq_t {
            using hash = string_hash_t<CharT>;
        ...
        template<>
        struct hash_eq<std::string> : string_hash_eq_t<char> {
        };

        template<>
        struct hash_eq<std::string_view> : string_hash_eq_t<char> {
        };

        // char16_t
        template<>
        struct hash_eq<std::u16string> : string_hash_eq_t<char16_t> {
        };

        template<>
        struct hash_eq<std::u16string_view> : string_hash_eq_t<char16_t> {
        };

        // wchar_t
        template<>
        struct hash_eq<std::wstring> : string_hash_eq_t<wchar_t> {
        };

        template<>
        struct hash_eq<std::wstring_view> : string_hash_eq_t<wchar_t> {
        };
        ...
        struct hash_eq<T *> {
            struct hash {
                ...
                size_t operator()(const U &ptr) const {
                    return flare::hash<const T *>{}(hash_eq::ToPtr(ptr));
                }
            };
            ...
            struct eq {
                ...
                bool operator()(const A &a, const B &b) const {
                    return hash_eq::ToPtr(a) == hash_eq::ToPtr(b);
                }
            };
        ...
        template <>
        struct case_ignored_hasher<std::string> {
        ...
        template <>
        struct case_ignored_hasher<std::string_view> {
            size_t operator()(const std::string_view &s) const {
                std::size_t result = 0;
                for (std::string_view::const_iterator i = s.begin(); i != s.end(); ++i) {
                    result = result * 101 + flare::ascii::to_lower(*i);
        ...
        template <>
        struct case_ignored_equal<std::string> {
            bool operator()(const std::string_view &s1, const std::string_view &s2) const {
                return flare::equal_case(s1, s2);
            }
        };

        template <>
        struct case_ignored_equal<std::string_view> {
            bool operator()(const std::string_view &s1, const std::string_view &s2) const {
                return flare::equal_case(s1, s2);
            }
        };
        ...
        namespace hashtable_debug_internal {
            ...
            struct HashtableDebugAccess<Set, flare::void_t<typename Set::raw_hash_set>> {
                ...
                static size_t GetNumProbes(const Set &set,
                    ...
                    while (true) {
                        ...
                        if (g.match_empty()) return num_probes;
                        seq.next();
                        ++num_probes;
                    }
                }
...
#endif  // FLARE_CONTAINER_INTERNAL_RAW_HASH_SET_H_
