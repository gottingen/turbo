...
#ifndef FLARE_CONTAINER_INTERNAL_MAP_BASE_H_
#define FLARE_CONTAINER_INTERNAL_MAP_BASE_H_
...
namespace flare {
    ...
    //
    // NOTE: `flare::void_t` does not use the standard-specified implementation so
...
// ---------------------------------------------------------------------------
    ...
    struct is_trivially_copy_assignable
            : std::integral_constant<
                    bool, __has_trivial_assign(typename std::remove_reference<T>::type) &&
                          flare::is_copy_assignable<T>::value> {
    };
    ...
    namespace type_traits_internal {
        ...
        struct IsHashable<Key,
                flare::enable_if_t<std::is_convertible<
        ...
        private:
            ...
            static std::nullptr_t DoIt() {
                ...
                static_assert(flare::is_copy_assignable<std::hash<Key>>::value,
...
namespace flare {
    namespace priv {
        ...
        private:
            ...
            template<class P>
            struct ConstantIteratorsImpl<P, flare::void_t<typename P::constant_iterators>>
...
#define FLARE_MAP_INTERNAL_EXTERN_DECL(type, name) \
      extern const ::flare::internal::identity_t<type> name;
#else  // Otherwise, just define the macro to do nothing.
#define FLARE_MAP_INTERNAL_EXTERN_DECL(type, name)
#endif  // defined(__clang__)

// See above comment at top of file for details.
#define FLARE_MAP_INTERNAL_INLINE_CONSTEXPR(type, name, init) \
  FLARE_MAP_INTERNAL_EXTERN_DECL(type, name)                  \
  inline constexpr ::flare::internal::identity_t<type> name = init
...
//   types, etc..
#define FLARE_MAP_INTERNAL_INLINE_CONSTEXPR(var_type, name, init)                  \
  template <class /*flare_map_internal_dummy*/ = void>                               \
  struct map_internal_inline_variable_holder##name {                             \
    static constexpr ::flare::internal::identity_t<var_type> kInstance = init; \
  };                                                                          \
                                                                              \
  template <class flare_map_internal_dummy>                                          \
  constexpr ::flare::internal::identity_t<var_type>                            \
      map_internal_inline_variable_holder##name<flare_map_internal_dummy>::kInstance;   \
                                                                              \
  static constexpr const ::flare::internal::identity_t<var_type>&              \
      name = /* NOLINT */                                                     \
      map_internal_inline_variable_holder##name<>::kInstance;                    \
...
namespace flare {
    ...
    // working with C++11 variadic templates). `flare::integer_sequence` is designed
    ...
    // `flare::index_sequence` is designed to be a drop-in replacement for C++14's
    // `std::index_sequence`.
    ...
    // for C++14's `std::move()`.
    template<typename T>
    constexpr flare::remove_reference_t<T> &&move(T &&t) noexcept {
        return static_cast<flare::remove_reference_t<T> &&>(t);
    }
    ...
    template<typename T>
    constexpr T &&forward(
            flare::remove_reference_t<T> &t) noexcept {  // NOLINT(runtime/references)
    ...
    namespace utility_internal {
        ...
        -> decltype(flare::base_internal::Invoke(
                flare::forward<Functor>(functor),
                std::get<Indexes>(flare::forward<Tuple>(t))...)) {
            return flare::base_internal::Invoke(
                    flare::forward<Functor>(functor),
                    std::get<Indexes>(flare::forward<Tuple>(t))...);
        }
    ...
    // pointer. `flare::apply` is designed to be a drop-in replacement for C++17's
    ...
    //       flare::apply(&user_function1, tuple1);
    //
    //       std::tuple<std::unique_ptr<Foo>> tuple2(flare::make_unique<Foo>());
    //       // Invokes the user function that takes ownership of the unique
    //       // pointer.
    //       flare::apply(&user_function2, std::move(tuple2));
    //
    //       auto foo = flare::make_unique<Foo>();
    //       std::tuple<Foo*, int> tuple3(foo.get(), 42);
    //       // Invokes the method Bar on foo with one argument, 42.
    //       flare::apply(&Foo::Bar, tuple3);
    //
    //       std::tuple<int, int> tuple4(8, 9);
    //       // Invokes a lambda.
    //       flare::apply(user_lambda, tuple4);
    //   }
    template<typename Functor, typename Tuple>
    auto apply(Functor &&functor, Tuple &&t)
    -> decltype(utility_internal::apply_helper(
            flare::forward<Functor>(functor), flare::forward<Tuple>(t),
            flare::make_index_sequence<std::tuple_size<
    ...
    // `obj`.  `flare::exchange` is designed to be a drop-in replacement for C++14's
    // `std::exchange`.
    //
    // Example:
    //
    //   Foo& operator=(Foo&& other) {
    //     ptr1_ = flare::exchange(other.ptr1_, nullptr);
    //     int1_ = flare::exchange(other.int1_, -1);
    //     return *this;
    //   }
    template<typename T, typename U = T>
    T exchange(T &obj, U &&new_value) {
        T old_value = flare::move(obj);
        obj = flare::forward<U>(new_value);
        return old_value;
    }
...
namespace flare {
    ...
    namespace memory_internal {
        ...
        // Traits to select proper overload and return type for `flare::make_unique<>`.
        template<typename T>
...
// ExtractOr<E, O, D>::type evaluates to E<O> if possible. Otherwise, D.
    ...
    struct allocator_traits {
        ...
        // flare::pointer_traits<pointer>::rebind<const value_type>
        using const_pointer =
        memory_internal::ExtractOrT<memory_internal::GetConstPointer, Alloc,
                typename flare::pointer_traits<pointer>::
                template rebind<const value_type>>;

        // void_pointer:
        // Alloc::void_pointer if present, otherwise
        // flare::pointer_traits<pointer>::rebind<void>
        using void_pointer = memory_internal::ExtractOrT<
                memory_internal::GetVoidPointer, Alloc,
                typename flare::pointer_traits<pointer>::template rebind<void>>;

        // const_void_pointer:
        // Alloc::const_void_pointer if present, otherwise
        // flare::pointer_traits<pointer>::rebind<const void>
        using const_void_pointer = memory_internal::ExtractOrT<
                memory_internal::GetConstVoidPointer, Alloc,
                typename flare::pointer_traits<pointer>::template rebind<const void>>;

        // difference_type:
        // Alloc::difference_type if present, otherwise
        // flare::pointer_traits<pointer>::difference_type
        using difference_type = memory_internal::ExtractOrT<
                memory_internal::GetDifferenceType, Alloc,
                typename flare::pointer_traits<pointer>::difference_type>;
        ...
        // rebind_traits:
        // flare::allocator_traits<rebind_alloc<T>>
        template<typename T>
        using rebind_traits = flare::allocator_traits<rebind_alloc<T>>;
    ...
    namespace memory_internal {
        ...
        void ConstructRange(Allocator &alloc, Iterator first, Iterator last,
            ...
            for (Iterator cur = first; cur != last; ++cur) {
                ...
                FLARE_INTERNAL_CATCH_ANY {
                    while (cur != first) {
                        --cur;
                        std::allocator_traits<Allocator>::destroy(alloc, std::addressof(*cur));
                    }
                    FLARE_INTERNAL_RETHROW;
                }
            }
        }
...
namespace flare {
    ...
    namespace priv {
        ...
        template<class T>
        struct IsTransparent<T, flare::void_t<typename T::is_transparent>>
                : std::true_type {
        };
        ...
        private:
            flare::optional<allocator_type> alloc_;
            mutable flare::aligned_storage_t<sizeof(slot_type), alignof(slot_type)> slot_space_;
        };
...
// ---------
        ...
        class node_handle<Policy, PolicyTraits, Alloc,
                flare::void_t<typename Policy::mapped_type>>
...
// Detection idioms for size() and data().
        template<typename C>
        using HasSize =
        std::is_integral<flare::decay_t<decltype(std::declval<C &>().size())>>;
...
// which returns a reference.
        ...
        using HasData =
        std::is_convertible<flare::decay_t<decltype(GetData(std::declval<C &>()))> *,
                T *const *>;

// Extracts value type from a Container
        template<typename C>
        struct ElementType {
            using type = typename flare::remove_reference_t<C>::value_type;
        };
...
// https://bugs.llvm.org/show_bug.cgi?id=27538.
    ...
    public:
        using value_type = flare::remove_cv_t<T>;
        using pointer = T *;
        ...
        //
        //   void Process(flare::Span<const int> x);
        //   Process({1, 2, 3});
        //
        ...
        //   void Process(flare::Span<const int> ints);
        //
        ...
        //   flare::Span<const int> ints = { 17, 19 };
        //   Process(ints);
        //
        ...
        constexpr reference front() const noexcept {
            return FLARE_ASSERT(size() > 0), *data();
        }
        ...
        constexpr reference back() const noexcept {
            return FLARE_ASSERT(size() > 0), *(data() + size() - 1);
        }
        ...
        //   flare::MakeSpan(vec).subspan(1, 2);  // {11, 12}
        //   flare::MakeSpan(vec).subspan(2, 8);  // {12, 13}
        //   flare::MakeSpan(vec).subspan(1);     // {11, 12, 13}
        //   flare::MakeSpan(vec).subspan(4);     // {}
        //   flare::MakeSpan(vec).subspan(5);     // throws std::out_of_range
        ...
        //
        //   std::vector<int> vec = {10, 11, 12, 13};
        //   flare::MakeSpan(vec).first(1);  // {10}
        //   flare::MakeSpan(vec).first(3);  // {10, 11, 12}
        //   flare::MakeSpan(vec).first(5);  // throws std::out_of_range
        ...
        //
        //   std::vector<int> vec = {10, 11, 12, 13};
        //   flare::MakeSpan(vec).last(1);  // {13}
        //   flare::MakeSpan(vec).last(3);  // {11, 12, 13}
        //   flare::MakeSpan(vec).last(5);  // throws std::out_of_range
...
// Examples:
//
//   void MyRoutine(flare::Span<MyComplicatedType> a) {
//     ...
//   };
...
//   MyRoutine(flare::Span<MyComplicatedType>(my_vector));
//
//   // Use MakeSpan() to make an flare::Span<T>
//   MyRoutine(flare::MakeSpan(my_vector));
//
//   // Construct a span from an array ptr+size
//   flare::Span<T> my_span() {
//     return flare::MakeSpan(&array[0], num_elements_);
//   }
//
    ...
    Span<T> MakeSpan(T *begin, T *end) noexcept {
        return FLARE_ASSERT(begin <= end), Span<T>(begin, end - begin);
    }

    template<int &... ExplicitArgumentBarrier, typename C>
    constexpr auto MakeSpan(C &c) noexcept  // NOLINT(runtime/references)
    -> decltype(flare::MakeSpan(span_internal::GetData(c), c.size())) {
...
// Examples:
//
//   void ProcessInts(flare::Span<const int> some_ints);
//
//   // Call with a pointer and size.
//   int array[3] = { 0, 0, 0 };
//   ProcessInts(flare::MakeConstSpan(&array[0], 3));
//
//   // Call with a [begin, end) pair.
//   ProcessInts(flare::MakeConstSpan(&array[0], &array[3]));
//
//   // Call directly with an array.
//   ProcessInts(flare::MakeConstSpan(array));
//
//   // Call with a contiguous container.
//   std::vector<int> some_ints = ...;
//   ProcessInts(flare::MakeConstSpan(some_ints));
//   ProcessInts(flare::MakeConstSpan(std::vector<int>{ 0, 0, 0 }));
//
    template<int &... ExplicitArgumentBarrier, typename T>
    ...
    Span<const T> MakeConstSpan(T *begin, T *end) noexcept {
        return FLARE_ASSERT(begin <= end), Span<const T>(begin, end - begin);
    }
...
#define FLARE_MAP_INTERNAL_HAS_CXA_DEMANGLE
#endif

#ifdef FLARE_MAP_INTERNAL_HAS_CXA_DEMANGLE
...
// Does `Ts...` contain `T`?
            ...
            using Contains = flare::disjunction<std::is_same<T, Ts>...>;

            template<class From, class To>
            using CopyConst =
            typename std::conditional<std::is_const<From>::value, const To, To>::type;

// Note: We're not qualifying this with flare:: because it doesn't compile under
// MSVC.
            template<class T>
...
// ---------------------------------------------------------------------------
            ...
            class LayoutImpl<std::tuple<Elements...>, flare::index_sequence<SizeSeq...>,
                    flare::index_sequence<OffsetSeq...>> {
            private:
                static_assert(sizeof...(Elements) > 0, "At least one field is required");
                static_assert(flare::conjunction<IsLegalElementType<Elements>...>::value,
        ...
        if (ElementAlignment<N - 1>::value % ElementAlignment<N>::value) {
            ...
            using LayoutType = LayoutImpl<
                    std::tuple<Ts...>, flare::make_index_sequence<NumSizes>,
                    flare::make_index_sequence<adl_barrier::Min(sizeof...(Ts), NumSizes + 1)>>;
...
// ---------------------------------------------------------------------------
        ...
        public:
            ...
            static_assert(
                    flare::conjunction<internal_layout::IsLegalElementType<Ts>...>::value,
                    "Invalid element type (see IsLegalElementType)");
            ...
            static constexpr PartialType<sizeof...(Sizes)> Partial(Sizes &&... sizes) {
                ...
                return PartialType<sizeof...(Sizes)>(flare::forward<Sizes>(sizes)...);
            }
...
// ------------------------------------------------
            ...
            struct Storage {
                ...
                explicit constexpr Storage(T &&v) : value(flare::forward<T>(v)) {}

                constexpr const T &get() const &{ return value; }

                T &get() &{ return value; }

                constexpr const T &&get() const &&{ return flare::move(*this).value; }
            ...
            struct Storage<D, I, true>
                ...
                explicit constexpr Storage(T &&v) : T(flare::forward<T>(v)) {}

                constexpr const T &get() const &{ return *this; }

                T &get() &{ return *this; }

                constexpr const T &&get() const &&{ return flare::move(*this); }
            ...
            struct CompressedTupleImpl<CompressedTuple<Ts...>, flare::index_sequence<I...>>
                ...
                explicit constexpr CompressedTupleImpl(Ts &&... args)
                        : Storage<CompressedTuple<Ts...>, I>(flare::forward<Ts>(args))... {}
            };
...
//
// Eg:
//   flare::priv::CompressedTuple<int, T1, T2, T3> value(7, t1, t2,
//                                                                    t3);
...
//   ...
//
// https://en.ccreference.com/w/cpp/language/ebo
// ---------------------------------------------------------------------------
        template<typename... Ts>
        class CompressedTuple
                : private internal_compressed_tuple::CompressedTupleImpl<
                        CompressedTuple<Ts...>, flare::index_sequence_for<Ts...>> {
        private:
            template<int I>
        ...
        public:
            ...
            explicit constexpr CompressedTuple(Ts... base)
                    : CompressedTuple::CompressedTupleImpl(flare::forward<Ts>(base)...) {}
...
namespace flare {
    namespace priv {
        ...
        void *Allocate(Alloc *alloc, size_t n) {
            ...
            using A = typename flare::allocator_traits<Alloc>::template rebind_alloc<M>;
            using AT = typename flare::allocator_traits<Alloc>::template rebind_traits<M>;
            A mem_alloc(*alloc);
...
namespace flare {
    namespace thread_safety_analysis {
        ...
        // reference.
        template<typename T>
        inline const T &ts_unchecked_read(const T &v) FLARE_NO_THREAD_SAFETY_ANALYSIS {
            return v;
        }

        template<typename T>
        inline T &ts_unchecked_read(T &v) FLARE_NO_THREAD_SAFETY_ANALYSIS {
            return v;
        }
        ...
        public:
            ...
            static void construct(Allocator *alloc, slot_type *slot, Args &&... args) {
                ...
                if (kMutableKeys::value) {
                    flare::allocator_traits<Allocator>::construct(*alloc, &slot->mutable_value,
                                                                  std::forward<Args>(args)...);
                } else {
                    flare::allocator_traits<Allocator>::construct(*alloc, &slot->value,
                                                                  std::forward<Args>(args)...);
                }
            }
            ...
            static void construct(Allocator *alloc, slot_type *slot, slot_type *other) {
                emplace(slot);
                if (kMutableKeys::value) {
                    flare::allocator_traits<Allocator>::construct(
            ...
            static void destroy(Allocator *alloc, slot_type *slot) {
                if (kMutableKeys::value) {
                    flare::allocator_traits<Allocator>::destroy(*alloc, &slot->mutable_value);
                } else {
                    flare::allocator_traits<Allocator>::destroy(*alloc, &slot->value);
                }
            }
            ...
            static void transfer(Allocator *alloc, slot_type *new_slot,
                ...
                if (kMutableKeys::value) {
                    ...
                    flare::allocator_traits<Allocator>::construct(*alloc, &new_slot->value,
            ...
            static void swap(Allocator *alloc, slot_type *a, slot_type *b) {
                if (kMutableKeys::value) {
                    ...
                    flare::allocator_traits<Allocator>::destroy(*alloc, &a->value);
                    flare::allocator_traits<Allocator>::construct(*alloc, &a->value,
                                                                  std::move(b->value));
                    flare::allocator_traits<Allocator>::destroy(*alloc, &b->value);
                    flare::allocator_traits<Allocator>::construct(*alloc, &b->value,
                                                                  std::move(tmp));
                }
            }
            ...
            static void move(Allocator *alloc, slot_type *src, slot_type *dest) {
                if (kMutableKeys::value) {
                    ...
                    flare::allocator_traits<Allocator>::destroy(*alloc, &dest->value);
                    flare::allocator_traits<Allocator>::construct(*alloc, &dest->value,
                                                                  std::move(src->value));
                }
            }
...
// ------------------------ lockable object used internally -------------------------
    ...
    public:
        ...
        struct DoNothing {
            ...
            DoNothing(mutex_type &, flare::adopt_lock_t) noexcept {}

            DoNothing(mutex_type &, flare::defer_lock_t) noexcept {}

            DoNothing(mutex_type &, flare::try_to_lock_t) {}
    ...
    // -----------------------------------
    // use as:
    //    using Lockable = flare::LockableImpl<mutex_type>;
    //    Lockable m;
    //
...
#endif  // FLARE_CONTAINER_INTERNAL_MAP_BASE_H_
