...
#include "flare/debugging/internal/elf_mem_image.h"

#ifdef FLARE_HAVE_ELF_MEM_IMAGE  // defined in elf_mem_image.h
...
int ElfBind(const ElfW(Sym) *) {
  FLARE_LOG(FATAL)<<"Unexpected word size";
  return 0;
}
...
ElfMemImage::ElfMemImage(const void *base) {
  FLARE_CHECK(base != kInvalidBase)<< "bad pointer";
  Init(base);
}
...
const ElfW(Sym) *ElfMemImage::GetDynsym(int index) const {
  FLARE_CHECK(index < GetNumSymbols())<< "index out of range";
...
const ElfW(Phdr) *ElfMemImage::GetPhdr(int index) const {
  FLARE_CHECK(index < ehdr_->e_phnum)<< "index out of range";
...
const char *ElfMemImage::GetDynstr(ElfW(Word) offset) const {
  FLARE_CHECK(offset < strsize_)<< "offset out of range";
...
const void *ElfMemImage::GetSymAddr(const ElfW(Sym) *sym) const {
  ...
  FLARE_CHECK(link_base_ < sym->st_value)<< "symbol out of range";
  return GetTableElement<char>(ehdr_, 0, 1, sym->st_value - link_base_);
}

const ElfW(Verdef) *ElfMemImage::GetVerdef(int index) const {
  FLARE_CHECK(0 <= index && static_cast<size_t>(index) <= verdefnum_)<<
                 "index out of range";
...
void ElfMemImage::SymbolIterator::Update(int increment) {
  ...
  FLARE_DCHECK(image->IsPresent() || increment == 0);
  if (!image->IsPresent()) {
    return;
  }
  index_ += increment;
  ...
  FLARE_DCHECK(symbol && version_symbol);
...
#endif  // FLARE_HAVE_ELF_MEM_IMAGE
