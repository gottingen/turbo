...
void hierarchical_condition(unsigned w) {
  ...
  turbo::Executor executor(w);
  turbo::Taskflow tf0("c0");
  turbo::Taskflow tf1("c1");
  turbo::Taskflow tf2("c2");
  turbo::Taskflow tf3("top");
...
TEST_CASE("MultiCond.Types") {
  ...
  auto explicit_mc = [](){
    turbo::SmallVector<int> v;
    return v;
  };

  auto implicit_mc = []() -> turbo::SmallVector<int> {
    return {1, 2, 3, 9};
  };
...
void multiple_loops(unsigned W) {
  ...
  auto B = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> turbo::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto C = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> turbo::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto D = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> turbo::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto E = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> turbo::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
...
void binary_tree(unsigned w) {
  ...
  for(int i=1; i<(1<<N); i++) {
    tasks.emplace_back(taskflow.emplace([&counter]() -> turbo::SmallVector<int>{
      counter.fetch_add(1, std::memory_order_relaxed);
      return {0, 1};
    }));
  }
