...
TEST_CASE("Runtime.Schedule.ModuleTask" * doctest::timeout(300)) {
  ...
  auto schedule = tf.emplace([&](turbo::Runtime& runtime) {
    value++;
...
TEST_CASE("Runtime.ExternalGraph.Simple" * doctest::timeout(300)) {
  ...
  std::vector<turbo::Taskflow> graphs(N);
  ...
  for (size_t subtask = 0; subtask <= subtasks; subtask = subtask == 0 ? subtask + 1 : subtask*2) {
    ...
    for (size_t i = 0; i < runtime_tasks_per_line * lines; ++i) {
      ...
      rts.emplace_back(taskflow.emplace([&sums, &subtask](turbo::Runtime& rt) {
        rt.corun([&sums, &subtask](turbo::Subflow& sf) {
    ...
    turbo::Pipeline pl(
      ...
      turbo::Pipe{
        turbo::PipeType::PARALLEL, [subtask, &sums](turbo::Pipeflow&, turbo::Runtime& rt) {
          rt.corun([subtask, &sums](turbo::Subflow& sf) {
  ...
  turbo::Pipeline pl(
    ...
    turbo::Pipe{
      turbo::PipeType::PARALLEL, [&sums](turbo::Pipeflow&, turbo::Runtime& rt) {
        rt.corun([&sums](turbo::Subflow& sf) {
