...
TEST_CASE("Type" * doctest::timeout(300)) {
  ...
  turbo::Taskflow taskflow, taskflow2;
  ...
  auto t5 = taskflow.emplace([](){ return turbo::SmallVector{1, 2}; });
  auto t6 = taskflow.emplace([](turbo::Runtime&){});
  auto t7 = taskflow.emplace([](turbo::Runtime&){ return 1; });
  auto t8 = taskflow.emplace([](turbo::Runtime&){ return turbo::SmallVector{1, 2}; });
  ...
  REQUIRE(t6.type() == turbo::TaskType::STATIC);
  REQUIRE(t7.type() == turbo::TaskType::CONDITION);
  REQUIRE(t8.type() == turbo::TaskType::CONDITION);
}
...
void sequential_runs(unsigned W) {
  ...
  SUBCASE("MultipleRuns") {
    ...
    turbo::Taskflow tf1, tf2, tf3, tf4;
...
void parallel_runs(unsigned w) {
  ...
  auto make_taskflow = [&] (turbo::Taskflow& tf) {
  ...
  SUBCASE("RunAndWaitForAll") {
    turbo::Executor executor(w);
    counter = 0;
    std::vector<std::unique_ptr<turbo::Taskflow>> taskflows(32);
    std::atomic<size_t> barrier(0);
    for(int t=0; t<32; t++) {
      threads.emplace_back([&, t=t] () {
        taskflows[t] = std::make_unique<turbo::Taskflow>();
