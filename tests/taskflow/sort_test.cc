...
TEST_CASE("BubbleSort" * doctest::timeout(300)) {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=10; end <= 1000; end += 200) {
      ...
      turbo::Taskflow taskflow("BubbleSort");
...
TEST_CASE("SelectionSort" * doctest::timeout(300)) {
  ...
  std::function<
    void(turbo::Subflow& sf, std::vector<int>&, int, int, int&)
  > spawn;
  ...
  spawn = [&] (
    ...
    auto SL = sf.emplace(
      [&spawn, &data, beg, m, l=minl] (turbo::Subflow& sf) mutable {
    ...
    auto SR = sf.emplace(
      [&spawn, &data, m, end, r=minr] (turbo::Subflow& sf) mutable {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=16; end <= 512; end <<= 1) {
      turbo::Taskflow taskflow("SelectionSort");
...
TEST_CASE("MergeSort" * doctest::timeout(300)) {
  ...
  spawn = [&] (turbo::Subflow& sf, std::vector<int>& data, int beg, int end) mutable {
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=10; end <= 10000; end = end * 10) {
      turbo::Taskflow taskflow("MergeSort");
      ...
      taskflow.emplace([&spawn, &data, end](turbo::Subflow& sf){
        spawn(sf, data, 0, end);
      }).name(std::string("[0")
            + ":"
            + std::to_string(end) + ")");
...
TEST_CASE("QuickSort" * doctest::timeout(300)) {
  ...
  std::function<void(turbo::Subflow& sf, std::vector<int>&, itr_t, itr_t)> spawn;
  ...
  for(unsigned w=1; w<=9; w+=2) {
    ...
    for(int end=16; end <= 16384; end <<= 1) {
      ...
      turbo::Taskflow taskflow("QuickSort");
      ...
      taskflow.emplace([&spawn, &data](turbo::Subflow& sf){
