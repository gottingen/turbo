...
struct zip_test : zip_base<typename B::value_type, B::size>
{
    ...
    void zip_low()
    {
        ...
        B b_res_lo = turbo::simd::zip_lo(b_lhs, b_rhs);
    ...
    void zip_hi()
    {
        ...
        B b_res_hi = turbo::simd::zip_hi(b_lhs, b_rhs);
        CHECK_BATCH_EQ(b_res_hi, b_exp_hi);
    }
};

#if !TURBO_WITH_AVX512F || TURBO_WITH_AVX512BW
#define ZIP_BATCH_TYPES BATCH_TYPES
#else
#define ZIP_BATCH_TYPES turbo::simd::batch<float>, turbo::simd::batch<double>, turbo::simd::batch<int32_t>, turbo::simd::batch<int64_t>
#endif
...
struct slide_test : public init_slide_base<typename B::value_type, B::size>
{
    ...
    void slide_left()
    {
        ...
        B b_res_left0 = turbo::simd::slide_left<0>(b_in);
        INFO("slide_left 0");
        CHECK_BATCH_EQ(b_res_left0, b_left0);

        B b_res_left_full = turbo::simd::slide_left<full_slide>(b_in);
        INFO("slide_left full");
        CHECK_BATCH_EQ(b_res_left_full, b_left_full);

        B b_res_left_half = turbo::simd::slide_left<half_slide>(b_in);
        INFO("slide_left half_slide");
        CHECK_BATCH_EQ(b_res_left_half, b_left_half);

        B b_res_left_one = turbo::simd::slide_left<sizeof(value_type)>(b_in);
        INFO("slide_left one_slide");
        CHECK_BATCH_EQ(b_res_left_one, b_left_one);

        if (activate_above_below_checks)
        {
            B b_res_left_above_half = turbo::simd::slide_left<above_half_slide>(b_in);
            INFO("slide_left above_half_slide");
            CHECK_BATCH_EQ(b_res_left_above_half, b_left_above_half);

            B b_res_left_below_half = turbo::simd::slide_left<below_half_slide>(b_in);
    ...
    void slide_right()
    {
        ...
        B b_res_right0 = turbo::simd::slide_right<0>(b_in);
        INFO("slide_right 0");
        CHECK_BATCH_EQ(b_res_right0, b_right0);

        B b_res_right_full = turbo::simd::slide_right<full_slide>(b_in);
        INFO("slide_right full");
        CHECK_BATCH_EQ(b_res_right_full, b_right_full);

        B b_res_right_half = turbo::simd::slide_right<half_slide>(b_in);
        INFO("slide_right half_slide");
        CHECK_BATCH_EQ(b_res_right_half, b_right_half);

        B b_res_right_one = turbo::simd::slide_right<sizeof(value_type)>(b_in);
        INFO("slide_right one_slide");
        CHECK_BATCH_EQ(b_res_right_one, b_right_one);

        if (activate_above_below_checks)
        {
            B b_res_right_above_half = turbo::simd::slide_right<above_half_slide>(b_in);
            INFO("slide_right above_half_slide");
            CHECK_BATCH_EQ(b_res_right_above_half, b_right_above_half);

            B b_res_right_below_half = turbo::simd::slide_right<below_half_slide>(b_in);
