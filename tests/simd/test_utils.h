...
struct precision_t
{
#if defined(__apple_build_version__) && (TURBO_WITH_SSE4_1 || TURBO_WITH_NEON64)
...
namespace detail
{
    ...
    bool expect_batch_near(const ::turbo::simd::batch<T, A>& lhs, const std::array<T, N>& rhs)
    {
        std::array<T, N> tmp;
        lhs.store_unaligned(tmp.data());
    ...
    template <class T, size_t N, class A>
    bool expect_batch_near(const std::array<T, N>& lhs, const ::turbo::simd::batch<T, A>& rhs)
    {
        std::array<T, N> tmp;
    ...
    template <class T, class A>
    bool expect_batch_near(const ::turbo::simd::batch<T, A>& lhs, const ::turbo::simd::batch<T, A>& rhs)
    {
        constexpr auto N = turbo::simd::batch<T, A>::size;
        std::array<T, N> tmp;
        lhs.store_unaligned(tmp.data());
    ...
    template <class T, size_t N, class A>
    bool expect_batch_near(const ::turbo::simd::batch_bool<T, A>& lhs, const std::array<bool, N>& rhs)
    {
    ...
    template <class T, size_t N, class A>
    bool expect_batch_near(const std::array<bool, N>& lhs, const ::turbo::simd::batch_bool<T, A>& rhs)
    {
        std::array<bool, N> tmp;
        rhs.store_unaligned(tmp.data());
        return expect_array_near(lhs, tmp);
    ...
    template <class T, class A>
    bool expect_batch_near(const ::turbo::simd::batch_bool<T, A>& lhs, const ::turbo::simd::batch_bool<T, A>& rhs)
    {
        constexpr auto N = turbo::simd::batch<T, A>::size;
        std::array<bool, N> tmp;
        lhs.store_unaligned(tmp.data());
...
#define BATCH_INT_TYPES turbo::simd::batch<uint8_t>,  \
                        turbo::simd::batch<int8_t>,   \
                        turbo::simd::batch<uint16_t>, \
                        turbo::simd::batch<int16_t>,  \
                        turbo::simd::batch<uint32_t>, \
                        turbo::simd::batch<int32_t>,  \
                        turbo::simd::batch<uint64_t>, \
                        turbo::simd::batch<int64_t>

#if TURBO_WITH_NEON64 || !TURBO_WITH_NEON
#define BATCH_FLOAT_TYPES turbo::simd::batch<float>, turbo::simd::batch<double>
#else
#define BATCH_FLOAT_TYPES turbo::simd::batch<float>
#endif
#if TURBO_WITH_NEON64 || !TURBO_WITH_NEON
#define BATCH_COMPLEX_TYPES turbo::simd::batch<std::complex<float>>, turbo::simd::batch<std::complex<double>>
#else
#define BATCH_COMPLEX_TYPES turbo::simd::batch<std::complex<float>>
#endif

#define BATCH_TYPES BATCH_INT_TYPES, BATCH_FLOAT_TYPES
#define BATCH_MATH_TYPES turbo::simd::batch<int32_t>, BATCH_FLOAT_TYPES

#if !TURBO_WITH_AVX || TURBO_WITH_AVX2
#define BATCH_SWIZZLE_TAIL , turbo::simd::batch<uint32_t>, turbo::simd::batch<int32_t>, turbo::simd::batch<uint64_t>, turbo::simd::batch<int64_t>
#else
...
#define CONVERSION_TYPES conversion_param<sizeof(turbo::simd::types::simd_register<int, turbo::simd::default_arch>) / sizeof(double), turbo::simd::default_arch::alignment()>

#endif // XTURBO_SIMD_TEST_UTILS_H_
