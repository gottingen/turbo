...
struct batch_cast_test
{
    ...
    using int8_batch = turbo::simd::batch<int8_t>;
    using uint8_batch = turbo::simd::batch<uint8_t>;
    using int16_batch = turbo::simd::batch<int16_t>;
...
#if TURBO_SIMD_X86_INSTR_SET >= TURBO_SIMD_X86_AVX_VERSION
...
#if TURBO_SIMD_X86_INSTR_SET >= TURBO_SIMD_X86_AVX512_VERSION
    template <size_t Align = A>
...
private:
    ...
    void test_bool_cast_impl(const std::string& name) const
    {
        ...
        using B_common_out = turbo::simd::batch_bool<T_out>;

        B_common_in all_true_in(true);
        B_common_out all_true_res = turbo::simd::batch_bool_cast<T_out>(all_true_in);
        INFO(name);
        CHECK_SCALAR_EQ(all_true_res.get(0), true);

        B_common_in all_false_in(false);
        B_common_out all_false_res = turbo::simd::batch_bool_cast<T_out>(all_false_in);
        INFO(name);
...
#if 0 && TURBO_SIMD_X86_INSTR_SET >= TURBO_SIMD_X86_AVX512_VERSION
