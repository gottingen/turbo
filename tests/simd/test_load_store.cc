...
struct load_store_test
{
    ...
    using index_type = typename turbo::simd::as_integer_t<batch_type>;
    template <class T>
    using allocator = turbo::simd::default_allocator<T, typename B::arch_type>;
...
private:
    ...
    {
        std::copy(v.cbegin(), v.cend(), expected.begin());
        index_type index = turbo::simd::detail::make_sequence_as_batch<index_type>();
