...
struct select_test
{
    ...
    void test_select_dynamic()
    {
        ...
        for (size_t i = 0; i < nb_input; i += size)
        {
            ...
            out = turbo::simd::select(lhs_in > value_type(3), lhs_in, rhs_in);
    ...
    {
        constexpr auto mask = turbo::simd::make_batch_bool_constant<batch_type, pattern>();
        ...
        for (size_t i = 0; i < nb_input; i += size)
        {
            detail::load_batch(lhs_in, lhs_input, i);
            detail::load_batch(rhs_in, rhs_input, i);
            out = turbo::simd::select(mask, lhs_in, rhs_in);
            detail::store_batch(out, res, i);
        }
        size_t diff = detail::get_nb_diff(res, expected);
        CHECK_EQ(diff, 0);
