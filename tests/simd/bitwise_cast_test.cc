...
struct bitwise_cast_test
{
    ...
    void test_to_int32()
    {
        ...
        {
            int32_batch i32bres = turbo::simd::bitwise_cast<int32_t>(f_input());
            i32bres.store_aligned(i32vres.data());
            INFO("to_int32(float)");
            CHECK_VECTOR_EQ(i32vres, ftoi32_res);
        }
        {
            int32_batch i32bres = turbo::simd::bitwise_cast<int32_t>(d_input());
            i32bres.store_aligned(i32vres.data());
    ...
    void test_to_int64()
    {
        ...
        {
            int64_batch i64bres = turbo::simd::bitwise_cast<int64_t>(f_input());
            i64bres.store_aligned(i64vres.data());
            INFO("to_int64(float)");
            CHECK_VECTOR_EQ(i64vres, ftoi64_res);
        }
        {
            int64_batch i64bres = turbo::simd::bitwise_cast<int64_t>(d_input());
            i64bres.store_aligned(i64vres.data());
    ...
    void test_to_float()
    {
        ...
        {
            float_batch fbres = turbo::simd::bitwise_cast<float>(i32_input());
            fbres.store_aligned(fvres.data());
            INFO("to_float(int32_t)");
            CHECK_VECTOR_EQ(fvres, i32tof_res);
        }
        {
            float_batch fbres = turbo::simd::bitwise_cast<float>(i64_input());
            fbres.store_aligned(fvres.data());
            INFO("to_float(int64_t)");
            CHECK_VECTOR_EQ(fvres, i64tof_res);
        }
        {
            float_batch fbres = turbo::simd::bitwise_cast<float>(d_input());
            fbres.store_aligned(fvres.data());
    ...
    void test_to_double()
    {
        ...
        {
            double_batch dbres = turbo::simd::bitwise_cast<double>(i32_input());
            dbres.store_aligned(dvres.data());
            INFO("to_double(int32_t)");
            CHECK_VECTOR_EQ(dvres, i32tod_res);
        }
        {
            double_batch dbres = turbo::simd::bitwise_cast<double>(i64_input());
            dbres.store_aligned(dvres.data());
            INFO("to_double(int64_t)");
            CHECK_VECTOR_EQ(dvres, i64tod_res);
        }
        {
            double_batch dbres = turbo::simd::bitwise_cast<double>(f_input());
            dbres.store_aligned(dvres.data());
