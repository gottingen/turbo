...
static_assert(!turbo::type_traits_internal::IsOwner<turbo::string_view>::value &&
                  turbo::type_traits_internal::IsView<turbo::string_view>::value,
              "string_view is a view, not an owner");

static_assert(turbo::type_traits_internal::IsLifetimeBoundAssignment<
                  turbo::string_view, std::string>::value,
...
TEST(StringViewTest, Ctor) {
...

  {
    ...
    turbo::string_view s20(hello);
    EXPECT_TRUE(s20.data() == hello);
    EXPECT_EQ(5u, s20.length());

    // const char* with length.
    turbo::string_view s21(hello, 4);
    EXPECT_TRUE(s21.data() == hello);
    EXPECT_EQ(4u, s21.length());

    // Not recommended, but valid C++
    turbo::string_view s22(hello, 6);
...

  {
    ...
    turbo::string_view s1(str1);
    s1.remove_prefix(strlen("BUNGIE-JUM"));

    turbo::string_view s2(str2);
  ...
  // TODO(mec): turbo::string_view(const turbo::string_view&);
}

TEST(StringViewTest, Swap) {
  turbo::string_view a("a");
  turbo::string_view b("bbb");
...
TEST(StringViewTest, STLComparator) {
  ...
  turbo::string_view p1(s1);
  turbo::string_view p2(s2);
  turbo::string_view p3(s3);

  typedef std::map<turbo::string_view, int> TestMap;
  TestMap map;
...
#define COMPARE(result, op, x, y)                                      \
  EXPECT_EQ(result, turbo::string_view((x)) op turbo::string_view((y))); \
  EXPECT_EQ(result, turbo::string_view((x)).compare(turbo::string_view((y))) op 0)

TEST(StringViewTest, ComparisonOperators) {
  COMPARE(true, ==, "",   "");
  COMPARE(true, ==, "", turbo::string_view());
  COMPARE(true, ==, turbo::string_view(), "");
  COMPARE(true, ==, "a",  "a");
  COMPARE(true, ==, "aa", "aa");
  COMPARE(false, ==, "a",  "");
...
// turbo::string_view::npos; So much so that we test here that they are the same.
...
TEST(StringViewTest, NposMatchesStdStringView) {
  EXPECT_EQ(turbo::string_view::npos, std::string::npos);

  EXPECT_TRUE(is_type<size_t>::same(turbo::string_view::npos));
  EXPECT_FALSE(is_type<size_t>::same(""));

  // Make sure turbo::string_view::npos continues to be a header constant.
  char test[turbo::string_view::npos & 1] = {0};
  EXPECT_EQ(0, test[0]);
}

TEST(StringViewTest, STL1) {
  const turbo::string_view a("abcdefghijklmnopqrstuvwxyz");
  const turbo::string_view b("abc");
  const turbo::string_view c("xyz");
  const turbo::string_view d("foobar");
  const turbo::string_view e;
  std::string temp("123");
  temp += '\0';
  temp += "456";
  const turbo::string_view f(temp);
...
TEST(StringViewTest, STL2) {
  ...
  const turbo::string_view b("abc");
  const turbo::string_view c("xyz");
  turbo::string_view d("foobar");
  const turbo::string_view e;
  const turbo::string_view f(
      "123"
      "\0"
      "456",
      7);
  ...
  EXPECT_EQ(a.find(b, 1), turbo::string_view::npos);
  EXPECT_EQ(a.find(c), 23u);
  EXPECT_EQ(a.find(c, 9), 23u);
  EXPECT_EQ(a.find(c, turbo::string_view::npos), turbo::string_view::npos);
  EXPECT_EQ(b.find(c), turbo::string_view::npos);
  EXPECT_EQ(b.find(c, turbo::string_view::npos), turbo::string_view::npos);
  EXPECT_EQ(a.find(d), 0u);
  EXPECT_EQ(a.find(e), 0u);
  EXPECT_EQ(a.find(d, 12), 12u);
  EXPECT_EQ(a.find(e, 17), 17u);
  turbo::string_view g("xx not found bb");
  EXPECT_EQ(a.find(g), turbo::string_view::npos);
  // empty string nonsense
  EXPECT_EQ(d.find(b), turbo::string_view::npos);
  EXPECT_EQ(e.find(b), turbo::string_view::npos);
  EXPECT_EQ(d.find(b, 4), turbo::string_view::npos);
  EXPECT_EQ(e.find(b, 7), turbo::string_view::npos);
  ...
  EXPECT_EQ(a.find('$'), turbo::string_view::npos);
  EXPECT_EQ(a.find('\0'), turbo::string_view::npos);
  ...
  EXPECT_EQ(a.find('b', 5), turbo::string_view::npos);
  // empty string nonsense
  EXPECT_EQ(d.find('\0'), turbo::string_view::npos);
  EXPECT_EQ(e.find('\0'), turbo::string_view::npos);
  EXPECT_EQ(d.find('\0', 4), turbo::string_view::npos);
  EXPECT_EQ(e.find('\0', 7), turbo::string_view::npos);
  EXPECT_EQ(d.find('x'), turbo::string_view::npos);
  EXPECT_EQ(e.find('x'), turbo::string_view::npos);
  EXPECT_EQ(d.find('x', 4), turbo::string_view::npos);
  EXPECT_EQ(e.find('x', 7), turbo::string_view::npos);

  EXPECT_EQ(a.find(b.data(), 1, 0), 1u);
  EXPECT_EQ(a.find(c.data(), 9, 0), 9u);
  EXPECT_EQ(a.find(c.data(), turbo::string_view::npos, 0),
            turbo::string_view::npos);
  EXPECT_EQ(b.find(c.data(), turbo::string_view::npos, 0),
            turbo::string_view::npos);
  // empty string nonsense
  EXPECT_EQ(d.find(b.data(), 4, 0), turbo::string_view::npos);
  EXPECT_EQ(e.find(b.data(), 7, 0), turbo::string_view::npos);

  EXPECT_EQ(a.find(b.data(), 1), turbo::string_view::npos);
  EXPECT_EQ(a.find(c.data(), 9), 23u);
  EXPECT_EQ(a.find(c.data(), turbo::string_view::npos), turbo::string_view::npos);
  EXPECT_EQ(b.find(c.data(), turbo::string_view::npos), turbo::string_view::npos);
  // empty string nonsense
  EXPECT_EQ(d.find(b.data(), 4), turbo::string_view::npos);
  EXPECT_EQ(e.find(b.data(), 7), turbo::string_view::npos);

  EXPECT_EQ(a.rfind(b), 0u);
  EXPECT_EQ(a.rfind(b, 1), 0u);
  EXPECT_EQ(a.rfind(c), 23u);
  EXPECT_EQ(a.rfind(c, 22), turbo::string_view::npos);
  EXPECT_EQ(a.rfind(c, 1), turbo::string_view::npos);
  EXPECT_EQ(a.rfind(c, 0), turbo::string_view::npos);
  EXPECT_EQ(b.rfind(c), turbo::string_view::npos);
  EXPECT_EQ(b.rfind(c, 0), turbo::string_view::npos);
  ...
  EXPECT_EQ(a.rfind(g), turbo::string_view::npos);
  EXPECT_EQ(d.rfind(b), turbo::string_view::npos);
  EXPECT_EQ(e.rfind(b), turbo::string_view::npos);
  EXPECT_EQ(d.rfind(b, 4), turbo::string_view::npos);
  EXPECT_EQ(e.rfind(b, 7), turbo::string_view::npos);
  // empty string nonsense
  ...
  EXPECT_EQ(g.rfind('q'), turbo::string_view::npos);
  EXPECT_EQ(g.rfind('o', 8), 8u);
  EXPECT_EQ(g.rfind('o', 7), 4u);
  EXPECT_EQ(g.rfind('o', 3), turbo::string_view::npos);
  EXPECT_EQ(f.rfind('\0'), 3u);
  EXPECT_EQ(f.rfind('\0', 12), 3u);
  EXPECT_EQ(f.rfind('3'), 2u);
  EXPECT_EQ(f.rfind('5'), 5u);
  // empty string nonsense
  EXPECT_EQ(d.rfind('o'), turbo::string_view::npos);
  EXPECT_EQ(e.rfind('o'), turbo::string_view::npos);
  EXPECT_EQ(d.rfind('o', 4), turbo::string_view::npos);
  EXPECT_EQ(e.rfind('o', 7), turbo::string_view::npos);
...
TEST(StringViewTest, STL2FindFirst) {
  ...
  EXPECT_EQ(a.find_first_of(b, 3), turbo::string_view::npos);
  ...
  EXPECT_EQ(a.find_first_of(c, 26), turbo::string_view::npos);
  EXPECT_EQ(g.find_first_of(b), 13u);
  EXPECT_EQ(g.find_first_of(c), 0u);
  EXPECT_EQ(a.find_first_of(f), turbo::string_view::npos);
  EXPECT_EQ(f.find_first_of(a), turbo::string_view::npos);
  // empty string nonsense
  EXPECT_EQ(a.find_first_of(d), turbo::string_view::npos);
  EXPECT_EQ(a.find_first_of(e), turbo::string_view::npos);
  EXPECT_EQ(d.find_first_of(b), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_of(b), turbo::string_view::npos);
  EXPECT_EQ(d.find_first_of(d), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_of(d), turbo::string_view::npos);
  EXPECT_EQ(d.find_first_of(e), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_of(e), turbo::string_view::npos);

  EXPECT_EQ(a.find_first_not_of(b), 3u);
  EXPECT_EQ(a.find_first_not_of(c), 0u);
  EXPECT_EQ(b.find_first_not_of(a), turbo::string_view::npos);
  EXPECT_EQ(c.find_first_not_of(a), turbo::string_view::npos);
  ...
  EXPECT_EQ(a.find_first_not_of(d, a.size()), turbo::string_view::npos);
  EXPECT_EQ(a.find_first_not_of(e, a.size()), turbo::string_view::npos);
  EXPECT_EQ(a.find_first_not_of(d, turbo::string_view::npos),
            turbo::string_view::npos);
  EXPECT_EQ(a.find_first_not_of(e, turbo::string_view::npos),
            turbo::string_view::npos);
  EXPECT_EQ(d.find_first_not_of(a), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_not_of(a), turbo::string_view::npos);
  EXPECT_EQ(d.find_first_not_of(d), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_not_of(d), turbo::string_view::npos);
  EXPECT_EQ(d.find_first_not_of(e), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_not_of(e), turbo::string_view::npos);

  turbo::string_view h("====");
  EXPECT_EQ(h.find_first_not_of('='), turbo::string_view::npos);
  EXPECT_EQ(h.find_first_not_of('=', 3), turbo::string_view::npos);
  ...
  // empty string nonsense
  EXPECT_EQ(d.find_first_not_of('x'), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_not_of('x'), turbo::string_view::npos);
  EXPECT_EQ(d.find_first_not_of('\0'), turbo::string_view::npos);
  EXPECT_EQ(e.find_first_not_of('\0'), turbo::string_view::npos);
}
...
TEST(StringViewTest, STL2FindLast) {
  ...
  turbo::string_view g("xx not found bb");
  turbo::string_view h("====");
  turbo::string_view i("56");

  d = turbo::string_view();
  EXPECT_EQ(h.find_last_of(a), turbo::string_view::npos);
  ...
  EXPECT_EQ(a.find_last_of('b', 0), turbo::string_view::npos);
  EXPECT_EQ(a.find_last_of('z', 25), 25u);
  EXPECT_EQ(a.find_last_of('z', 24), turbo::string_view::npos);
  EXPECT_EQ(f.find_last_of(i, 5), 5u);
  EXPECT_EQ(f.find_last_of(i, 6), 6u);
  EXPECT_EQ(f.find_last_of(a, 4), turbo::string_view::npos);
  // empty string nonsense
  EXPECT_EQ(f.find_last_of(d), turbo::string_view::npos);
  EXPECT_EQ(f.find_last_of(e), turbo::string_view::npos);
  EXPECT_EQ(f.find_last_of(d, 4), turbo::string_view::npos);
  EXPECT_EQ(f.find_last_of(e, 4), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_of(d), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_of(e), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_of(d), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_of(e), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_of(f), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_of(f), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_of(d, 4), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_of(e, 4), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_of(d, 4), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_of(e, 4), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_of(f, 4), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_of(f, 4), turbo::string_view::npos);

  EXPECT_EQ(a.find_last_not_of(b), a.size() - 1);
  EXPECT_EQ(a.find_last_not_of(c), 22u);
  EXPECT_EQ(b.find_last_not_of(a), turbo::string_view::npos);
  EXPECT_EQ(b.find_last_not_of(b), turbo::string_view::npos);
  EXPECT_EQ(f.find_last_not_of(i), 4u);
  EXPECT_EQ(a.find_last_not_of(c, 24), 22u);
  EXPECT_EQ(a.find_last_not_of(b, 3), 3u);
  EXPECT_EQ(a.find_last_not_of(b, 2), turbo::string_view::npos);
  // empty string nonsense
  EXPECT_EQ(f.find_last_not_of(d), f.size() - 1);
  EXPECT_EQ(f.find_last_not_of(e), f.size() - 1);
  EXPECT_EQ(f.find_last_not_of(d, 4), 4u);
  EXPECT_EQ(f.find_last_not_of(e, 4), 4u);
  EXPECT_EQ(d.find_last_not_of(d), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_not_of(e), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of(d), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of(e), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_not_of(f), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of(f), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_not_of(d, 4), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_not_of(e, 4), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of(d, 4), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of(e, 4), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_not_of(f, 4), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of(f, 4), turbo::string_view::npos);

  EXPECT_EQ(h.find_last_not_of('x'), h.size() - 1);
  EXPECT_EQ(h.find_last_not_of('='), turbo::string_view::npos);
  EXPECT_EQ(b.find_last_not_of('c'), 1u);
  EXPECT_EQ(h.find_last_not_of('x', 2), 2u);
  EXPECT_EQ(h.find_last_not_of('=', 2), turbo::string_view::npos);
  EXPECT_EQ(b.find_last_not_of('b', 1), 0u);
  // empty string nonsense
  EXPECT_EQ(d.find_last_not_of('x'), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of('x'), turbo::string_view::npos);
  EXPECT_EQ(d.find_last_not_of('\0'), turbo::string_view::npos);
  EXPECT_EQ(e.find_last_not_of('\0'), turbo::string_view::npos);
}
...
TEST(StringViewTest, UTF8) {
  ...
  EXPECT_EQ(utf8_len, turbo::string_view(utf8_twice).find_first_of(" "));
  EXPECT_EQ(utf8_len, turbo::string_view(utf8_twice).find_first_of(" \t"));
}
...
TEST(StringViewTest, FindConformance) {
  ...
  for (const auto& s : specs) {
    ...
    turbo::string_view sp = s.haystack;
    for (size_t i = 0; i <= sp.size(); ++i) {
      size_t pos = (i == sp.size()) ? turbo::string_view::npos : i;
      SCOPED_TRACE(pos);
...
TEST(StringViewTest, Set) {
  turbo::string_view a("foobar");
  turbo::string_view empty;
  turbo::string_view b;

  // set
  b = turbo::string_view("foobar", 6);
  EXPECT_EQ(b, a);
  b = turbo::string_view("foobar", 0);
  EXPECT_EQ(b, empty);
  b = turbo::string_view("foobar", 7);
  EXPECT_NE(b, a);

  b = turbo::string_view("foobar");
  EXPECT_EQ(b, a);
}

TEST(StringViewTest, FrontBack) {
  static const char arr[] = "abcd";
  const turbo::string_view csp(arr, 4);
...
TEST(StringViewTest, FrontBackSingleChar) {
  ...
  const turbo::string_view csp(&c, 1);
...
TEST(StringViewTest, Comparisons2) {
  ...
  turbo::string_view abc("abcdefghijklmnopqrstuvwxyz");

  // check comparison operations on strings longer than 4 bytes.
  EXPECT_EQ(abc, turbo::string_view("abcdefghijklmnopqrstuvwxyz"));
  EXPECT_EQ(abc.compare(turbo::string_view("abcdefghijklmnopqrstuvwxyz")), 0);

  EXPECT_LT(abc, turbo::string_view("abcdefghijklmnopqrstuvwxzz"));
  EXPECT_LT(abc.compare(turbo::string_view("abcdefghijklmnopqrstuvwxzz")), 0);

  EXPECT_GT(abc, turbo::string_view("abcdefghijklmnopqrstuvwxyy"));
  EXPECT_GT(abc.compare(turbo::string_view("abcdefghijklmnopqrstuvwxyy")), 0);

  // The "substr" variants of `compare`.
  turbo::string_view digits("0123456789");
  auto npos = turbo::string_view::npos;

  // Taking string_view
  EXPECT_EQ(digits.compare(3, npos, turbo::string_view("3456789")), 0);  // 2
  EXPECT_EQ(digits.compare(3, 4, turbo::string_view("3456")), 0);        // 2
  EXPECT_EQ(digits.compare(10, 0, turbo::string_view()), 0);             // 2
  EXPECT_EQ(digits.compare(3, 4, turbo::string_view("0123456789"), 3, 4),
            0);  // 3
  EXPECT_LT(digits.compare(3, 4, turbo::string_view("0123456789"), 3, 5),
            0);  // 3
  EXPECT_LT(digits.compare(0, npos, turbo::string_view("0123456789"), 3, 5),
            0);  // 3
  // Taking const char*
...
TEST(StringViewTest, StartsWith) {
  ...
  constexpr turbo::string_view kFooBar("foobar");
  constexpr turbo::string_view kFoo("foo");
  constexpr turbo::string_view kBar("bar");
...
TEST(StringViewTest, ExplicitConversionOperator) {
  turbo::string_view sp = "hi";
  EXPECT_EQ(sp, std::string(sp));
}

TEST(StringViewTest, NullSafeStringView) {
  {
    turbo::string_view s = turbo::NullSafeStringView(nullptr);
    EXPECT_EQ(nullptr, s.data());
    EXPECT_EQ(0u, s.size());
    EXPECT_EQ(turbo::string_view(), s);
  }
  {
    static const char kHi[] = "hi";
    turbo::string_view s = turbo::NullSafeStringView(kHi);
    EXPECT_EQ(kHi, s.data());
    EXPECT_EQ(strlen(kHi), s.size());
    EXPECT_EQ(turbo::string_view("hi"), s);
  }
}

TEST(StringViewTest, ConstexprNullSafeStringView) {
  {
    constexpr turbo::string_view s = turbo::NullSafeStringView(nullptr);
    EXPECT_EQ(nullptr, s.data());
    EXPECT_EQ(0u, s.size());
    EXPECT_EQ(turbo::string_view(), s);
  }
  ...
  {
    constexpr turbo::string_view s = turbo::NullSafeStringView("hello");
    EXPECT_EQ(s.size(), 5u);
    EXPECT_EQ("hello", s);
  }
}
...
#define TURBO_HAVE_CONSTEXPR_STRING_VIEW_COMPARISON 1
  ...
  constexpr turbo::string_view::iterator const_begin_empty = sp.begin();
  ...
  constexpr turbo::string_view::iterator const_end = cstr_len.end();
  constexpr turbo::string_view::size_type const_size = cstr_len.size();
  constexpr turbo::string_view::size_type const_length = cstr_len.length();
...
constexpr char ConstexprMethodsHelper() {
  ...
  turbo::string_view str("123", 3);
  str.remove_prefix(1);
  str.remove_suffix(1);
  turbo::string_view bar;
  str.swap(bar);
  return bar.front();
#else
  return '2';
...
TEST(StringViewTest, ConstexprMethods) {
  ...
  // substr
  constexpr turbo::string_view foobar("foobar", 6);
  constexpr turbo::string_view foo = foobar.substr(0, 3);
  constexpr turbo::string_view bar = foobar.substr(3);
  EXPECT_EQ(foo, "foo");
  EXPECT_EQ(bar, "bar");
}

TEST(StringViewTest, Noexcept) {
  EXPECT_TRUE((std::is_nothrow_constructible<turbo::string_view,
                                             const std::string&>::value));
  EXPECT_TRUE((std::is_nothrow_constructible<turbo::string_view,
                                             const std::string&>::value));
  EXPECT_TRUE(std::is_nothrow_constructible<turbo::string_view>::value);
  constexpr turbo::string_view sp;
...
TEST(ComparisonOpsTest, HeterogeneousStringViewEquals) {
  EXPECT_EQ(turbo::string_view("hello"), std::string("hello"));
  EXPECT_EQ("hello", turbo::string_view("hello"));
}

TEST(FindOneCharTest, EdgeCases) {
  turbo::string_view a("xxyyyxx");
  ...
  EXPECT_EQ(turbo::string_view::npos, a.find('x', 5));
  ...
  EXPECT_EQ(turbo::string_view::npos, a.find('x'));
...
TEST(LenExceedsMaxSizeTest, LenExceedsMaxSize) {
  auto max_size = turbo::string_view().max_size();
