...
template <typename T>
turbo::optional<std::string> StrPrintChar(T c) {
  return StrPrint("%c", static_cast<int>(c));
}
template <>
turbo::optional<std::string> StrPrintChar(wchar_t c) {
...
TYPED_TEST_P(TypedFormatConvertTest, Char) {
  ...
  for (const T c : vals) {
    ...
    turbo::optional<std::string> result = StrPrintChar(c);
    if (result.has_value()) {
...
void TestWithMultipleFormatsHelper(Floating tested_float) {
  ...
  for (const char *fmt : kFormats) {
    for (char f : {'f', 'F',  //
      ...
      if (fmt == turbo::string_view("%.5000") && f != 'f' && f != 'F' &&
          f != 'a' && f != 'A') {
...
// machinery.
struct NullSink {
  friend void TurboFormatFlush(NullSink *, string_view) {}
};
