...
TEST(NumbersTest, Atoi) {
  // simple_atoi(turbo::string_view, int32_t)
  ...
  // simple_atoi(turbo::string_view, uint32_t)
  ...
  // simple_atoi(turbo::string_view, int64_t)
  ...
  // simple_atoi(turbo::string_view, uint64_t)
  ...
  // simple_atoi(turbo::string_view, turbo::uint128)
  ...
  // simple_atoi(turbo::string_view, turbo::int128)
...
TEST(NumbersTest, HexAtoi) {
  // simple_hex_atoi(turbo::string_view, int32_t)
  ...
  // simple_hex_atoi(turbo::string_view, uint32_t)
  ...
  // simple_hex_atoi(turbo::string_view, int64_t)
  ...
  // simple_hex_atoi(turbo::string_view, uint64_t)
  ...
  // simple_hex_atoi(turbo::string_view, turbo::uint128)
...
void test_random_integer_parse_base(bool (*parse_func)(turbo::string_view,
                                                       IntType* value,
                                                       int base)) {
...
TEST(stringtest, safe_strtou32_base_length_delimited) {
  for (int i = 0; strtouint32_test_cases()[i].str != nullptr; ++i) {
    ...
    EXPECT_EQ(e.expect_ok,
              safe_strtou32_base(turbo::string_view(tmp.data(), strlen(e.str)),
                                 &value, e.base))
        << "str=\"" << e.str << "\" base=" << e.base;
    if (e.expect_ok) {
      EXPECT_EQ(e.expected, value) << "i=" << i << " str=" << e.str
...
TEST(stringtest, safe_strtou64_base_length_delimited) {
  for (int i = 0; strtouint64_test_cases()[i].str != nullptr; ++i) {
    ...
    uint64_t value;
    EXPECT_EQ(e.expect_ok,
              safe_strtou64_base(turbo::string_view(tmp.data(), strlen(e.str)),
                                 &value, e.base))
        << "str=\"" << e.str << "\" base=" << e.base;
    if (e.expect_ok) {
      EXPECT_EQ(e.expected, value) << "str=\"" << e.str << "\" base=" << e.base;
    ...
    for (int exponent = -324; exponent <= 308; ++exponent) {
      ...
      if (kFloatNumCases >= 1e9) {
        ...
        LOG(INFO) << "Exp " << exponent << " powten=" << powten << "(" << powten
                  << ") ("
                  << turbo::string_view(buf, SixDigitsToBuffer(powten, buf))
                  << ")";
      }
...
TEST(StrToInt32, Partial) {
  ...
  for (const Int32TestLine& test_line : int32_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = -2;
    status = safe_strto32_base(turbo::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToUint32, Partial) {
  ...
  for (const Uint32TestLine& test_line : uint32_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = 2;
    status = safe_strtou32_base(turbo::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToInt64, Partial) {
  ...
  for (const Int64TestLine& test_line : int64_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = -2;
    status = safe_strto64_base(turbo::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToUint64, Partial) {
  ...
  for (const Uint64TestLine& test_line : uint64_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = 2;
    status = safe_strtou64_base(turbo::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToInt32Base, PrefixOnly) {
  ...
  for (const Int32TestLine& line : int32_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strto32_base(turbo::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
...
TEST(StrToUint32Base, PrefixOnly) {
  ...
  for (const Uint32TestLine& line : uint32_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strtou32_base(turbo::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
...
TEST(StrToInt64Base, PrefixOnly) {
  ...
  for (const Int64TestLine& line : int64_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strto64_base(turbo::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
...
TEST(StrToUint64Base, PrefixOnly) {
  ...
  for (const Uint64TestLine& line : uint64_test_line) {
    for (const int base : base_array) {
      ...
      EXPECT_EQ(line.value, value) << line.input << " " << base;
      value = 2;
      status = safe_strtou64_base(turbo::string_view(line.input), &value, base);
      EXPECT_EQ(line.status, status) << line.input << " " << base;
      EXPECT_EQ(line.value, value) << line.input << " " << base;
    }
  }
