...
TEST(RecorderTest, test_complement) {
    ...
    for (int a = -10000000; a < 10000000; ++a) {
        const uint64_t complement = turbo::IntRecorder::_get_complement(a);
        const int64_t b = turbo::IntRecorder::_extend_sign_bit(complement);
        ASSERT_EQ(a, b);
    }
}
...
TEST(RecorderTest, test_compress) {
    ...
    const uint64_t compressed = turbo::IntRecorder::_compress(num, sum);
    ASSERT_EQ(num, turbo::IntRecorder::_get_num(compressed));
    ASSERT_EQ(sum, turbo::IntRecorder::_get_sum(compressed));
}

TEST(RecorderTest, test_compress_negtive_number) {
    for (int a = -10000000; a < 10000000; ++a) {
        const uint64_t sum = turbo::IntRecorder::_get_complement(a);
        const uint64_t num = 123456;
        const uint64_t compressed = turbo::IntRecorder::_compress(num, sum);
        ASSERT_EQ(num, turbo::IntRecorder::_get_num(compressed));
        ASSERT_EQ(a, turbo::IntRecorder::_extend_sign_bit(turbo::IntRecorder::_get_sum(compressed)));
    }
}
...
TEST(RecorderTest, sanity) {
    {
        ...
        ASSERT_EQ("2", turbo::Variable::describe_exposed("var1"));
...
TEST(RecorderTest, window) {
    turbo::IntRecorder c1;
    ASSERT_TRUE(c1.valid());
    turbo::Window<turbo::IntRecorder> w1(&c1, 1);
    turbo::Window<turbo::IntRecorder> w2(&c1, 2);
    turbo::Window<turbo::IntRecorder> w3(&c1, 3);
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        if (now - last_time >= 1000000L) {
            last_time = now;
            std::cout << "c1=" << c1 << " w1=" << w1 << " w2=" << w2 << " w3=" << w3<<std::endl;
...
static void *thread_counter(void *arg) {
    turbo::IntRecorder *recorder = (turbo::IntRecorder *)arg;
...
TEST(RecorderTest, perf) {
    ...
    std::cout << "Recorder takes " << totol_time / (OPS_PER_THREAD * TURBO_ARRAY_SIZE(threads))
              << "ns per sample with " << TURBO_ARRAY_SIZE(threads)
              << " threads"<<std::endl;
}
*/
    TEST(RecorderTest, latency_recorder) {
        turbo::IntRecorder recorder;
        turbo::Window<turbo::IntRecorder, turbo::SERIES_IN_SECOND> RecorderWindow(&recorder, 10);
        recorder.expose("recorder");
        RecorderWindow.expose("RecorderWindow");

        turbo::Fiber f;
        auto func = [](void *args) -> void*{
            turbo::IntRecorder *recorder = (turbo::IntRecorder *)args;
            for ( int i = 0; i < 100; ++i) {
                *recorder << i;
        ...
        f.start(std::move(func) , (void*)&recorder);
        for(int i = 0; i < 10; ++i) {
            auto s = RecorderWindow.get_value();
            TLOG_INFO("recorder: sum {}, num {}", s.sum, s.num);
            turbo::sleep_for(turbo::Duration::milliseconds(1000));
        }
        f.join();
    }

TEST(RecorderTest, latency_recorder_qps_accuracy) {
    turbo::LatencyRecorder lr1(2); // set windows size to 2s
    turbo::LatencyRecorder lr2(2);
    turbo::LatencyRecorder lr3(2);
    turbo::LatencyRecorder lr4(2);
    usleep(3000000); // wait sampler to sample 3 times

    auto write = [](turbo::LatencyRecorder& lr, int times) {   
        for (int i = 0; i < times; ++i) {
            lr << 1;
        }
    };
        TLOG_INFO("lr1 qps:{}", lr1.qps());
        TLOG_INFO("lr2 qps:{}", lr2.qps());
        TLOG_INFO("lr3 qps:{}", lr3.qps());
        TLOG_INFO("lr4 qps:{}", lr4.qps());
    write(lr1, 10);
    write(lr2, 11);
    write(lr3, 3);
    write(lr4, 1);
    ...
    auto read = [](turbo::LatencyRecorder& lr, double exp_qps, int window_size = 0) {
        int64_t qps_sum = 0;
