...
TURBO_DECLARE_FLAG(bool, var_log_dumpped);
...
// overloading for operator<< does not work for gflags>=2.1
    ...
    TEST_F(VariableTest, status) {
        ...
        ASSERT_EQ("9", turbo::Variable::describe_exposed("var1"));
        ASSERT_EQ("10", turbo::Variable::describe_exposed("var2"));
        turbo::Variable::list_exposed(&vars);
        ...
        ASSERT_EQ("", turbo::Variable::describe_exposed("Var2 Again"));
        ASSERT_EQ("10", turbo::Variable::describe_exposed("var2_again"));
        turbo::Variable::list_exposed(&vars);
        ASSERT_EQ(2UL, vars.size());
        ASSERT_EQ("var1", vars[0]);
        ...
        turbo::StatusVar<int> st3("var3", 11);
        ASSERT_EQ("var3", st3.name());
        ASSERT_EQ(3UL, turbo::Variable::count_exposed());
        ASSERT_EQ("11", turbo::Variable::describe_exposed("var3"));
        turbo::Variable::list_exposed(&vars);
        ...
        turbo::StatusVar<int> st4("var4", 12);
        ASSERT_EQ("var4", st4.name());
        ASSERT_EQ(4UL, turbo::Variable::count_exposed());
        ASSERT_EQ("12", turbo::Variable::describe_exposed("var4"));
        turbo::Variable::list_exposed(&vars);
    ...
    class MyDumper : public turbo::Dumper {
    public:
    ...
    TEST_F(VariableTest, dump) {
        ...
        // Nothing to dump yet.
        turbo::set_flag(&FLAGS_var_log_dumpped,true);
        ...
        ASSERT_EQ(5, turbo::Variable::dump_exposed(&d, NULL));
        ...
        d._list.clear();
        turbo::DumpOptions opts;
        opts.white_wildcards = "foo_bar_*";
        opts.black_wildcards = "*var5";
        ASSERT_EQ(2, turbo::Variable::dump_exposed(&d, &opts));
        ...
        d._list.clear();
        opts = turbo::DumpOptions();
        opts.white_wildcards = "*?rot*";
        ASSERT_EQ(1, turbo::Variable::dump_exposed(&d, &opts));
        ...
        ASSERT_EQ(3, turbo::Variable::dump_exposed(&d, &opts));
        ...
        ASSERT_EQ(0, turbo::Variable::dump_exposed(&d, &opts));
    ...
    TEST_F(VariableTest, latency_recorder) {
        ...
        turbo::Variable::list_exposed(&names);
