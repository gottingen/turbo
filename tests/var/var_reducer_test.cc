...
namespace {
    ...
    static void *thread_counter(void *arg) {
        turbo::Counter<uint64_t> *reducer = (turbo::Counter<uint64_t> *) arg;
    ...
    static long start_perf_test_with_adder(size_t num_thread) {
        turbo::Counter<uint64_t> reducer;
    ...
    TEST_F(ReducerTest, Min) {
        turbo::MinerGauge<uint64_t> reducer;
    ...
    TEST_F(ReducerTest, max) {
        turbo::MaxerGauge<uint64_t> reducer;
    ...
    void ReducerTest_window() {
        turbo::Counter<int> c1;
        turbo::MaxerGauge<int> c2;
        turbo::MinerGauge<int> c3;
        turbo::Window<turbo::Counter<int> > w1(&c1, 1);
        turbo::Window<turbo::Counter<int> > w2(&c1, 2);
        turbo::Window<turbo::Counter<int> > w3(&c1, 3);
        turbo::Window<turbo::MaxerGauge<int> > w4(&c2, 1);
        turbo::Window<turbo::MaxerGauge<int> > w5(&c2, 2);
        turbo::Window<turbo::MaxerGauge<int> > w6(&c2, 3);
        turbo::Window<turbo::MinerGauge<int> > w7(&c3, 1);
        turbo::Window<turbo::MinerGauge<int> > w8(&c3, 2);
        turbo::Window<turbo::MinerGauge<int> > w9(&c3, 3);
    ...
    static void *string_appender(void *arg) {
        turbo::Counter<std::string> *cater = (turbo::Counter<std::string> *) arg;
        int count = 0;
    ...
    TEST_F(ReducerTest, non_primitive_mt) {
        turbo::Counter<std::string> cater;
        pthread_t th[8];
        g_stop = false;
    ...
    TEST_F(ReducerTest, simple_window) {
        ...
        turbo::Counter<int64_t> a;
        turbo::Window<turbo::Counter<int64_t> > w(&a, 10);
        a << 100;
        sleep(3);
