...
public:
    ...
    void schedule(turbo::Timerfd *timer_thread) {
        _task_id = timer_thread->run_at(TimeKeeper::routine, this, _expect_run_time);
        TLOG_INFO("schedule name:{} task_id={}", _name, _task_id);
    }
...
TEST_CASE("TimerThreadTest, RunTasks") {
    turbo::Timerfd timer_thread;
    REQUIRE_EQ(timer_thread.start(turbo::TimerOptions()).ok(), true);
    ...
    TLOG_INFO("Sleep 1s");
    sleep(1);
    timer_thread.cancel(keeper2._task_id);
    timer_thread.cancel(keeper4._task_id);
...
class TestTask {
public:
    TestTask(turbo::Timerfd *timer_thread, TimeKeeper *keeper1,
    ...
    void run() {
        ...
        REQUIRE_EQ(_expected_unschedule_result,
                   _timer_thread->cancel(_keeper1->_task_id).code());
...
private:
    turbo::Timerfd *_timer_thread;  // not owned.
...
TEST_CASE("TimerThreadTest, schedule_and_unschedule_in_task") {
    ...
    TURBO_UNUSED(timer_thread.run_at(TestTask::routine, &test_task1, past_time));

    TestTask test_task2(&timer_thread, &keeper3, &keeper4, turbo::kESTOP);
    TURBO_UNUSED(timer_thread.run_at(TestTask::routine, &test_task2, past_time));
    ...
    REQUIRE_EQ(timer_thread.cancel(keeper5._task_id).code() , turbo::kEBUSY);
