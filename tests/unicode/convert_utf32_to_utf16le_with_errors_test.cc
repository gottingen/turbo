...
TEST_CASE("convert_into_2_UTF16_bytes") {
    for (size_t trial = 0; trial < trials; trial++) {
        ...
        auto procedure = [](const char32_t *utf32, size_t size, char16_t *utf16) -> size_t {
            turbo::UnicodeResult res = turbo::convert_utf32_to_utf16le_with_errors(utf32, size, utf16);
...
TEST_CASE("convert_fails_if_there_is_surrogate") {
    ...
    for (char32_t surrogate = 0xd800; surrogate <= 0xdfff; surrogate++) {
        for (size_t i = 0; i < size; i++) {
            auto procedure = [&i](const char32_t *utf32, size_t size, char16_t *utf16) -> size_t {
                turbo::UnicodeResult res = turbo::convert_utf32_to_utf16le_with_errors(utf32, size, utf16);
