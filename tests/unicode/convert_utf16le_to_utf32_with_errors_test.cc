...
TEST_CASE("convert_2_UTF16_bytes") {
  for(size_t trial = 0; trial < trials; trial ++) {
    ...
    auto procedure = [](const char16_t* utf16, size_t size, char32_t* utf32) -> size_t {
      turbo::UnicodeResult res = turbo::convert_utf16le_to_utf32_with_errors(utf16, size, utf32);
...
TEST_CASE("convert_fails_if_there_is_sole_low_surrogate") {
  ...
  for (char16_t low_surrogate = 0xdc00; low_surrogate <= 0xdfff; low_surrogate++) {
    for (size_t i=0; i < size; i++) {
      auto procedure = [ &i](const char16_t* utf16, size_t size, char32_t* utf32) -> size_t {
        turbo::UnicodeResult res = turbo::convert_utf16le_to_utf32_with_errors(utf16, size, utf32);
