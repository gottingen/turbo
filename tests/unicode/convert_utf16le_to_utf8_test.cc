...
TEST_CASE("issue_a73") {
    ...
    size_t expected_length = turbo::utf8_length_from_utf16le(utf16, len);
    ...
    const char expected[] =
            ...
            "\xad";
    size_t utf8size =
            turbo::convert_utf16le_to_utf8(utf16, len, output.data());
    REQUIRE(utf8size == expected_length);
...
TEST_CASE("issue_a72") {
    ...
    char utf8[96];
    size_t utf8size = turbo::convert_utf16le_to_utf8(utf16, 32, utf8);
    REQUIRE(utf8size == 96);
    for (size_t i = 0; i < 96; i++) {
        REQUIRE(utf8[i] == expected[i]);
    }
}
...
TEST_CASE("convert_pure_ASCII") {
    ...
    auto procedure = [](const char16_t *utf16, size_t size, char *utf8) -> size_t {
        return turbo::convert_utf16le_to_utf8(utf16, size, utf8);
    };
...
TEST_CASE("convert_into_1_or_2_UTF8_bytes") {
    for (size_t trial = 0; trial < trials; trial++) {
        ...
        for (size_t size: input_size) {
            transcode_utf16_to_utf8_test_base test([&random]() {
                return random();
            }, size);
            REQUIRE(test(procedure));
            REQUIRE(test.check_size(size_procedure));
...
TEST_CASE("convert_fails_if_there_is_low_surrogate_followed_by_another_low_surrogate") {
    auto procedure = [](const char16_t *utf8, size_t size, char *utf16) -> size_t {
        return turbo::convert_utf16le_to_utf8(utf8, size, utf16);
    };
