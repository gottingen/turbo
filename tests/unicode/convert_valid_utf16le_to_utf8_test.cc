...
TEST_CASE("issue_a73") {
  ...
  const char expected[] =
      ...
      "\xad";
  size_t utf8size =
      turbo::convert_valid_utf16le_to_utf8(utf16, len, output.data());
...
TEST_CASE("issue_a72") {
  ...
  char utf8[96];
  size_t utf8size = turbo::convert_valid_utf16le_to_utf8(utf16, 32, utf8);
  REQUIRE(utf8size == 96);
...
TEST_CASE("convert_pure_ASCII") {
  ...
  auto procedure = [](const char16_t* utf16, size_t size, char* utf8) -> size_t {
    return turbo::convert_valid_utf16le_to_utf8(utf16, size, utf8);
  };
