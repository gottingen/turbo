...
TEST(VariantTest, GetIndex) {
...

  {
    ...
    EXPECT_EQ(turbo::get<0>(std::move(const_v)), 0);
  }

  {
    ...
    using RValueGetType = decltype(turbo::get<1>(std::move(v)));
    ...
    EXPECT_EQ(turbo::get<1>(std::move(v)), "Hello");

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<1>(const_v));
    using ConstRValueGetType = decltype(turbo::get<1>(std::move(const_v)));
    ...
    EXPECT_EQ(turbo::get<1>(std::move(const_v)), "Hello");
  }

  {
    ...
    using RValueGetType = decltype(turbo::get<2>(std::move(v)));
    ...
    EXPECT_EQ(turbo::get<2>(std::move(v)), 2.);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<2>(const_v));
    using ConstRValueGetType = decltype(turbo::get<2>(std::move(const_v)));
    ...
    EXPECT_EQ(turbo::get<2>(std::move(const_v)), 2.);
  }

  {
    ...
    using RValueGetType = decltype(turbo::get<3>(std::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(turbo::get<3>(v), 1);
    EXPECT_EQ(turbo::get<3>(std::move(v)), 1);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<3>(const_v));
    using ConstRValueGetType = decltype(turbo::get<3>(std::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(turbo::get<3>(const_v), 1);
    EXPECT_EQ(turbo::get<3>(std::move(const_v)), 1);  // NOLINT
  }
}
...
TEST(VariantTest, GetType) {
...

  {
    ...
    using RValueGetType = decltype(turbo::get<int>(std::move(v)));

    EXPECT_TRUE((std::is_same<LValueGetType, int&>::value));
    EXPECT_TRUE((std::is_same<RValueGetType, int&&>::value));
    EXPECT_EQ(turbo::get<int>(v), 1);
    EXPECT_EQ(turbo::get<int>(std::move(v)), 1);

    const Var& const_v = v;
    using ConstLValueGetType = decltype(turbo::get<int>(const_v));
    using ConstRValueGetType = decltype(turbo::get<int>(std::move(const_v)));
    EXPECT_TRUE((std::is_same<ConstLValueGetType, const int&>::value));
    EXPECT_TRUE((std::is_same<ConstRValueGetType, const int&&>::value));
    EXPECT_EQ(turbo::get<int>(const_v), 1);
    EXPECT_EQ(turbo::get<int>(std::move(const_v)), 1);
  }

  {
    ...
    EXPECT_EQ(turbo::get<std::string>(std::move(v)), "Hello");
    ...
    EXPECT_EQ(turbo::get<std::string>(std::move(const_v)), "Hello");
  }

  {
    ...
    EXPECT_EQ(turbo::get<double>(std::move(v)), 2.);
    ...
    EXPECT_EQ(turbo::get<double>(std::move(const_v)), 2.);
  }
}
...
TEST(VariantTest, VisitRValue) {
  ...
  EXPECT_TRUE(turbo::visit(Visitor{}, std::move(v)));

  // Also test the variadic overload.
  EXPECT_EQ(0, turbo::visit(Visitor{}, v, v));
  EXPECT_EQ(1, turbo::visit(Visitor{}, v, std::move(v)));
  EXPECT_EQ(2, turbo::visit(Visitor{}, std::move(v), v));
  EXPECT_EQ(3, turbo::visit(Visitor{}, std::move(v), std::move(v)));
}
...
TEST(VariantTest, VisitResultTypeDifferent) {
  ...
  EXPECT_TRUE(
      (std::is_same<RValue_LValue,
                    decltype(turbo::visit(visitor, std::move(v)))>::value));
  EXPECT_TRUE((
