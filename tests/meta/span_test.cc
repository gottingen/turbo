...
namespace {
    ...
    MATCHER_P(DataIs, data,
              turbo::Format("data() {} {}", negation ? "isn't " : "is ",
                            testing::PrintToString(data))) {
        return arg.data() == data;
    }

    template<typename T>
    auto SpanIs(T data, size_t size) -> decltype(testing::AllOf(DataIs(data), testing::SizeIs(size))) {
...
// A struct supplying shallow data() const.
    ...
    void TestInitializerList(turbo::Span<const int> s, const std::vector<int> &v) {
        EXPECT_TRUE(turbo::equal(s.begin(), s.end(), v.begin(), v.end()));
    }
