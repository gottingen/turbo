...
namespace turbo {
    ...
    bool test(T &&arg) {
        ...
        return result(forward_sequence<shape_type, T>(arg));
    }
    ...
    R test_different_array(T &array) {
        return forward_sequence<R, T>(array);
    }
    ...
    TEST(xsequence, forward)
    {
        ...
        std::vector<int> resa = forward_sequence<std::vector<int>, decltype(a)>(a);
        EXPECT_EQ(resa, b);

        std::array<int, 2> resb = forward_sequence<std::array<int, 2>, decltype(b)>(b);
        EXPECT_EQ(resb, a);
    }
    ...
    TEST(xsequence, make_sequence)
    {
        ...
        vector_type v0 = make_sequence<vector_type>(size);
        EXPECT_EQ(v0.size(), size);

        array_type a0 = make_sequence<array_type>(size);
        EXPECT_EQ(a0.size(), size);

        vector_type v1 = make_sequence<vector_type>(size, init_value);
        EXPECT_EQ(v1.size(), size);
        EXPECT_EQ(v1[0], init_value);
        EXPECT_EQ(v1[1], init_value);
        EXPECT_EQ(v1[2], init_value);

        array_type a1 = make_sequence<array_type>(size, init_value);
        EXPECT_EQ(a1.size(), size);
        EXPECT_EQ(a1[0], init_value);
        EXPECT_EQ(a1[1], init_value);
        EXPECT_EQ(a1[2], init_value);

        vector_type v2 = make_sequence<vector_type>(init_list);
        EXPECT_EQ(v2.size(), size);
        EXPECT_EQ(v2[0], 3);
        EXPECT_EQ(v2[1], 2);
        EXPECT_EQ(v2[2], 4);

        array_type a2 = make_sequence<array_type>(init_list);
    ...
    TEST(xsequence, are_equivalent_sequences)
    {
        ...
        EXPECT_TRUE(are_equivalent_sequences(a, a) == true);
        EXPECT_TRUE(are_equivalent_sequences(a, b) == false);
        EXPECT_TRUE(are_equivalent_sequences(a, c) == false);
    }
}
