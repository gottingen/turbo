...
namespace turbo::simd {
    ...
    void mandelbrot(float x0,
        ...
        // float_batch_type programIndex(&arange[0], turbo::simd::aligned_mode());

        auto programIndex = float_batch_type::load(&arange[0], turbo::simd::aligned_mode());
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i += N) {
                ...
                // turbo::simd::store_aligned(result, output + base_index, active);
...
void run_arch(
        ...
        int height,
        int maxIters,
        std::vector<int, turbo::simd::aligned_allocator<int, Align>> &buffer) {
    std::fill(buffer.begin(), buffer.end(), 0);
    auto stats = bencher([&]() { turbo::simd::mandelbrot<arch>(x0, y0, x1, y1, width, height, maxIters, buffer.data()); });
...
// of archs
template<class... Arch>
struct run_archlist<turbo::simd::arch_list<Arch...>> {
...
int main() {
    ...
    std::vector<int, turbo::simd::aligned_allocator<int, turbo::simd::default_arch::alignment()>> buf(width * height);
    ...
    run_archlist<turbo::simd::supported_architectures>::run(bencher, x0, y0, x1, y1, width, height, maxIters, buf);
