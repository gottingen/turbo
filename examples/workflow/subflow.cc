// This example demonstrates how to use Workflow to create
...
int main(int argc, char* argv[]) {
  ...
  // Create a workflow graph with three regular tasks and one subflow task.
  turbo::Executor executor(4);
  turbo::Workflow workflow("Dynamic Tasking Demo");

  // Task A
  auto A = workflow.emplace([] () { std::cout << "TaskA\n"; });
  auto B = workflow.emplace(
    // Task B
    [cap=std::vector<int>{1,2,3,4,5,6,7,8}, detached] (turbo::Subflow& subflow) {
  ...
  executor.run(workflow).get();  // block until finished
