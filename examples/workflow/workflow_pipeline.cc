// This program demonstrates how to propagates a sequence of input tokens through
// linearly dependent workflows to implement complex parallel algorithms.
// Parallelism exhibits both inside and outside these workflows, combining
// both task graph parallelism and pipeline parallelism.

#include <turbo/workflow/algorithm/pipeline.h>
#include <turbo/workflow/workflow.h>

// workflow on the first pipe
void make_workflow1(turbo::Workflow& tf) {
  auto [A1, B1, C1, D1] = tf.emplace(
    [](){ printf("A1\n"); },
    [](){ printf("B1\n"); },
    [](){ printf("C1\n"); },
    [](){ printf("D1\n"); }
  );
  A1.precede(B1, C1);
  D1.succeed(B1, C1);
}

// workflow on the second pipe
void make_workflow2(turbo::Workflow& tf) {
  auto [A2, B2, C2, D2] = tf.emplace(
    [](){ printf("A2\n"); },
    [](){ printf("B2\n"); },
    [](){ printf("C2\n"); },
    [](){ printf("D2\n"); }
  );
  tf.linearize({A2, B2, C2, D2});
}

// workflow on the third pipe
void make_workflow3(turbo::Workflow& tf) {
  auto [A3, B3, C3, D3] = tf.emplace(
    [](){ printf("A3\n"); },
    [](){ printf("B3\n"); },
    [](){ printf("C3\n"); },
    [](){ printf("D3\n"); }
  );
  A3.precede(B3, C3, D3);
}

int main() {

  turbo::Workflow workflow("workflow processing pipeline");
  turbo::Executor executor;

  const size_t num_lines = 2;
  const size_t num_pipes = 3;

  // define the workflow storage
  // we use the pipe dimension because we create three 'serial' pipes
  std::array<turbo::Workflow, num_pipes> workflows;

  // create three different workflows for the three pipes
  make_workflow1(workflows[0]);
  make_workflow2(workflows[1]);
  make_workflow3(workflows[2]);

  // the pipeline consists of three serial pipes
  // and up to two concurrent scheduling tokens
  turbo::Pipeline pl(num_lines,

    // first pipe runs workflow1
    turbo::Pipe{turbo::PipeType::SERIAL, [&](turbo::Pipeflow& pf) {
      if(pf.token() == 5) {  // we only handle five scheduling tokens
        pf.stop();
        return;
      }
      printf("begin token %zu\n", pf.token());
      executor.run_and_wait(workflows[pf.pipe()]);
    }},

    // second pipe runs workflow2
    turbo::Pipe{turbo::PipeType::SERIAL, [&](turbo::Pipeflow& pf) {
      executor.run_and_wait(workflows[pf.pipe()]);
    }},

    // third pipe calls workflow3
    turbo::Pipe{turbo::PipeType::SERIAL, [&](turbo::Pipeflow& pf) {
      executor.run_and_wait(workflows[pf.pipe()]);
    }}
  );

  // build the pipeline graph using composition
  turbo::Task init = workflow.emplace([](){ std::cout << "ready\n"; })
                          .name("starting pipeline");
  turbo::Task task = workflow.composed_of(pl)
                          .name("pipeline");
  turbo::Task stop = workflow.emplace([](){ std::cout << "stopped\n"; })
                          .name("pipeline stopped");

  // create task dependency
  init.precede(task);
  task.precede(stop);

  // dump the pipeline graph structure (with composition)
  workflow.dump(std::cout);

  // run the pipeline
  executor.run(workflow).wait();

  return 0;
}
