...
public:
    ...
    bool Int(int i) {
        char buffer[12];
        return WriteNumberElement(buffer, snprintf(buffer, 10, "%d", i));
    }
    
    bool Uint(unsigned i) {
        char buffer[11];
        return WriteNumberElement(buffer, snprintf(buffer, 10, "%u", i));
    }
    ...
    bool Double(double d) {
        char buffer[30];
        return WriteNumberElement(buffer, snprintf(buffer, 29,"%.17g", d));
    }
