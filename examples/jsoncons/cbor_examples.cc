...
namespace {
    ...
    */
    ...
    void working_with_cbor1()
    {
        ...
        {
            std::cout << row[1].as<turbo::byte_string>() << " (" << row[1].tag() << ")\n";
        }
        std::cout << "\n";
    ...
    {
        // Parse the string of data into a std::vector<std::tuple<std::string,turbo::byte_string,std::string>> value
        auto val = cbor::decode_cbor<std::vector<std::tuple<std::string,turbo::byte_string,std::string>>>(data);
        ...
        for (; !cursor.done(); cursor.next())
        {
            ...
            switch (event.event_type())
            {
                ...
                case staj_event_type::key:
                    ...
                    std::cout << event.event_type() << ": " << event.get<turbo::string_view>() << " " << "(" << event.tag() << ")\n";
                    break;
                ...
                case staj_event_type::byte_string_value:
                    std::cout << event.event_type() << ": " << event.get<turbo::byte_string_view>() << " " << "(" << event.tag() << ")\n";
                    break;
