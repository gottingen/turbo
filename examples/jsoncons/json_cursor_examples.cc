...
#include "turbo/jsoncons/json.h" // json_decoder and json
#include <string>
#include <sstream>
#include <fstream>
        ...
        for (; !cursor.done(); cursor.next())
        {
            ...
            switch (event.event_type())
            {
                ...
                case staj_event_type::key:
                    ...
                    std::cout << event.event_type() << ": " << event.get<turbo::string_view>() << "\n";
                    break;
    ...
    void filtering_a_json_stream()
    {
        ...
        auto filter = [&](const staj_event& event, const ser_context&) -> bool
        {
            if (event.event_type() == staj_event_type::key &&
                event.get<turbo::string_view>() == "author")
            {
                author_next = true;
                return false;
            }
        ...
        for (; !filtered_c.done(); filtered_c.next())
        {
            ...
            {
                case staj_event_type::string_value:
                    std::cout << event.get<turbo::string_view>() << "\n";
                    break;
                default:
                    std::cout << "Unhandled event type: " << event.event_type() << " " << "\n";
                    break;
