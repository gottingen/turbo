...
#include "turbo/metrics/recorder.h"
#include "turbo/metrics/latency_recorder.h"
#include "turbo/metrics/gauge.h"
#include "turbo/strings/str_join.h"

namespace {
    TEST(RecorderTest, test_complement) {
        TURBO_LOG(INFO) << "sizeof(LatencyRecorder)=" << sizeof(turbo::LatencyRecorder)
                        << " " << sizeof(turbo::metrics_detail::percentile)
                        << " " << sizeof(turbo::max_gauge<int64_t>)
                        << " " << sizeof(turbo::IntRecorder)
                        << " " << sizeof(turbo::window<turbo::IntRecorder>)
                        << " " << sizeof(turbo::window<turbo::metrics_detail::percentile>);
    ...
    TEST(RecorderTest, sanity) {
        {
            ...
            ASSERT_EQ("2", turbo::variable_base::describe_exposed("var1"));
