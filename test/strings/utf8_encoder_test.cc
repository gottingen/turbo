...
#include "turbo/strings/utf8/greedy_encoder.h"
...
namespace turbo::utf8_detail {
    ...
    namespace {
        ...
        void naive_encode(const char32_t *s_ptr, size_t len, std::basic_string<char> &ret) {
            ...
            auto dst_len = naive_encoder((uint32_t *) s_ptr, (uint32_t *) s_ptr + len, p_dst);
            ret.resize(dst_len);
        }

        void greedy_encode(const char32_t *s_ptr, size_t len, std::basic_string<char> &ret) {
            auto *p_dst = (unsigned char *) (ret.data());
            auto dst_len = greedy_encoder((uint32_t *) s_ptr, (uint32_t *) s_ptr + len, p_dst);
            ret.resize(dst_len);
        }
    ...
    TEST(UTF8Lib, Encoder) {
        ...
        for (uint32_t i = 0; i <= 0x10FFFF; ++i) {
            auto *s_ptr = &i;
            auto c1 = naive_count_bytes_size(s_ptr, s_ptr + 1);
            auto c2 = naive_encoder(s_ptr, s_ptr + 1, buffer1);
            EXPECT_EQ(c1, c2);

            auto c3 = greedy_count_bytes_size(s_ptr, s_ptr + 1);
            auto c4 = greedy_encoder(s_ptr, s_ptr + 1, buffer2);
            EXPECT_EQ(c3, c4);
