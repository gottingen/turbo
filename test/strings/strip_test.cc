...
namespace {
    ...
    TEST(Strip, trim_inplace_complete) {
        ...
        turbo::trim_inplace_complete(&zero_string);
    ...
    TEST(Strip, trim_inplace_all) {
        std::string test2 = "\t  \f\r\n\vfoo \t\f\r\v\n";
        turbo::trim_inplace_all(&test2);
        EXPECT_EQ(test2, "foo");
        std::string test3 = "bar";
        turbo::trim_inplace_all(&test3);
        EXPECT_EQ(test3, "bar");
        std::string test4 = "\t  \f\r\n\vfoo";
        turbo::trim_inplace_all(&test4);
        EXPECT_EQ(test4, "foo");
        std::string test5 = "foo \t\f\r\v\n";
        turbo::trim_inplace_all(&test5);
        EXPECT_EQ(test5, "foo");
