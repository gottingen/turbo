...
namespace {
    ...
    static const int seed = turbo::time_now().to_time_t();
...
void clause11_21_4_6_3_a(String &test) {
    ...
    if (std::is_same<String, tbstring>::value) {
...
TEST(TBString, testAllClauses) {
    EXPECT_TRUE(1) << "Starting with seed: " << seed;
    std::string r;
    turbo::tbstring c;
#if TURBO_HAVE_WCHAR_SUPPORT
    std::wstring wr;
      turbo::basic_tbstring<wchar_t> wc;
#endif
    int count = 0;

    auto l = [&](const char *const clause,
                 void (*f_string)(std::string &),
                 void (*f_tbstring)(turbo::tbstring &),
                 void (*f_wtbstring)(turbo::basic_tbstring<wchar_t> &)) {
        do {
            if (true) {
            } else {
          ...
          wc = turbo::basic_tbstring<wchar_t>(wr.c_str());
#endif
...
#define TEST_CLAUSE(x)             \
  l(#x,                            \
    ...
    clause11_##x<turbo::tbstring>, \
    clause11_##x<turbo::basic_tbstring<wchar_t>>);
...
sed nisl. In diam lacus, lobortis ut posuere nec, ornare id quam.";
    ...
    vector<string> v = turbo::string_split(s1, "\n");
    {
...
TEST(TBString, testMoveCtor) {
    ...
    tbstring s(size, 'a');
    tbstring test = std::move(s);
    EXPECT_TRUE(s.empty());
    EXPECT_EQ(size, test.size());
}

TEST(TBString, testMoveAssign) {
// Move constructor. Make sure we allocate a large string, so the
// small string optimization doesn't kick in.
    auto size = random(100, 2000);
    tbstring s(size, 'a');
...
TEST(TBString, testMoveOperatorPlusLhs) {
...
TEST(TBString, testMoveOperatorPlusRhs) {
// Make sure we allocate a large string, so the
// small string optimization doesn't kick in.
    auto size1 = random(100, 2000);
    auto size2 = random(100, 2000);
...
TEST(TBString, testConstructionFromLiteralZero) {
    EXPECT_THROW(tbstring s(nullptr), std::logic_error);
}

TEST(TBString, testFixedBugs_D479397) {
    tbstring str(1337, 'f');
    tbstring cp = str;
    cp.clear();
    cp.c_str();
...
TEST(TBString, testFixedBugs_D481173) {
    tbstring str(1337, 'f');
    for (int i = 0; i < 2; ++i) {
        tbstring cp = str;
        cp[1] = 'b';
...
TEST(TBString, testFixedBugs_D580267_push_back) {
    tbstring str(1337, 'f');
    tbstring cp = str;
    cp.push_back('f');
}

TEST(TBString, testFixedBugs_D580267_operator_add_assign) {
    tbstring str(1337, 'f');
    tbstring cp = str;
    cp += "bb";
}

TEST(TBString, testFixedBugs_D661622) {
    turbo::basic_tbstring<wchar_t> s;
    EXPECT_EQ(0, s.size());
}

TEST(TBString, testFixedBugs_D785057) {
    tbstring str(1337, 'f');
    std::swap(str, str);
    EXPECT_EQ(1337, str.size());
}

TEST(TBString, testFixedBugs_D1012196_allocator_malloc) {
    tbstring str(128, 'f');
    str.clear(); // Empty medium string.
    tbstring copy(str); // Medium string of 0 capacity.
    copy.push_back('b');
    EXPECT_GE(copy.capacity(), 1);
}

TEST(TBString, testFixedBugs_D2813713) {
    tbstring s1("a");
...
TEST(TBString, testFixedBugs_D3698862) {
    EXPECT_EQ(tbstring().find(tbstring(), 4), tbstring::npos);
}


TEST(TBString, findWithNpos) {
    tbstring fbstr("localhost:80");
    EXPECT_EQ(tbstring::npos, fbstr.find(":", tbstring::npos));
}

TEST(TBString, testHash) {
    tbstring a;
    tbstring b;
    a.push_back(0);
    a.push_back(1);
    b.push_back(0);
    b.push_back(2);
    std::hash<tbstring> hashfunc;
    EXPECT_NE(hashfunc(a), hashfunc(b));
}

#if TURBO_HAVE_WCHAR_SUPPORT
TEST(TBString, testHashChar16) {
  using u16tbstring = turbo::basic_tbstring<char16_t>;
  u16tbstring a;
  u16tbstring b;
  a.push_back(0);
  a.push_back(1);
  b.push_back(0);
  b.push_back(2);
  std::hash<u16tbstring> hashfunc;
  EXPECT_NE(hashfunc(a), hashfunc(b));
}
#endif

TEST(TBString, testFrontBack) {
    tbstring str("hello");
...
TEST(TBString, noexcept) {
    EXPECT_TRUE(noexcept(tbstring()));
    tbstring x;
    EXPECT_FALSE(noexcept(tbstring(x)));
    EXPECT_TRUE(noexcept(tbstring(std::move(x))));
    tbstring y;
    EXPECT_FALSE(noexcept(y = x));
    EXPECT_TRUE(noexcept(y = std::move(x)));
}

TEST(TBString, iomanip) {
    stringstream ss;
    tbstring fbstr("Hello");
...
TEST(TBString, moveTerminator) {
// The source of a move must remain in a valid state
    tbstring s(100, 'x'); // too big to be in-situ
    tbstring k;
    k = std::move(s);
 ...
 *   explicit basic_tbstring(const A& a) noexcept;
 */

    struct TestStructDefaultAllocator {
        turbo::basic_tbstring<char> stringMember;
    };
...
TEST(TBStringCtorTest, DefaultInitStructDefaultAlloc) {
    TestStructDefaultAllocator t1{};
    EXPECT_TRUE(t1.stringMember.empty());
}

TEST(TBStringCtorTest, NullZeroConstruction) {
    char *p = nullptr;
    int n = 0;
    turbo::tbstring f(p, n);
    EXPECT_EQ(f.size(), 0);
}
...
TEST(TBString, compareToStdString) {
    using turbo::tbstring;
...
TEST(U16TBString, compareToStdU16String) {
    using turbo::basic_tbstring;
    using namespace std::string_literals;
    auto stdA = u"a"s;
    auto stdB = u"b"s;
    basic_tbstring<char16_t> fbA(u"a");
    basic_tbstring<char16_t> fbB(u"b");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(U32TBString, compareToStdU32String) {
    using turbo::basic_tbstring;
    using namespace std::string_literals;
    auto stdA = U"a"s;
    auto stdB = U"b"s;
    basic_tbstring<char32_t> fbA(U"a");
    basic_tbstring<char32_t> fbB(U"b");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(WTBString, compareToStdWString) {
    using turbo::basic_tbstring;
    using namespace std::string_literals;
    auto stdA = L"a"s;
    auto stdB = L"b"s;
    basic_tbstring<wchar_t> fbA(L"a");
    basic_tbstring<wchar_t> fbB(L"b");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(TBString, compareToStdStringLong) {
    using turbo::tbstring;
    using namespace std::string_literals;
    auto stdA = "1234567890a"s;
    auto stdB = "1234567890ab"s;
    tbstring fbA("1234567890a");
    tbstring fbB("1234567890ab");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
TEST(U16TBString, compareToStdU16StringLong) {
    using turbo::basic_tbstring;
    using namespace std::string_literals;
    auto stdA = u"1234567890a"s;
    auto stdB = u"1234567890ab"s;
    basic_tbstring<char16_t> fbA(u"1234567890a");
    basic_tbstring<char16_t> fbB(u"1234567890ab");
    EXPECT_TRUE(stdA == fbA);
    EXPECT_TRUE(fbB == stdB);
    EXPECT_TRUE(stdA != fbB);
    EXPECT_TRUE(fbA != stdB);
...
#define TURBO_HAVE_WCHAR_SUPPORT 1
#if TURBO_HAVE_WCHAR_SUPPORT
TEST(U32TBString, compareToStdU32StringLong) {
  using turbo::basic_tbstring;
  using namespace std::string_literals;
  auto stdA = U"1234567890a"s;
  auto stdB = U"1234567890ab"s;
  basic_tbstring<char32_t> fbA(U"1234567890a");
  basic_tbstring<char32_t> fbB(U"1234567890ab");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(WTBString, compareToStdWStringLong) {
  using turbo::basic_tbstring;
  using namespace std::string_literals;
  auto stdA = L"1234567890a"s;
  auto stdB = L"1234567890ab"s;
  basic_tbstring<wchar_t> fbA(L"1234567890a");
  basic_tbstring<wchar_t> fbB(L"1234567890ab");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(TBString, convertToStringView) {
  turbo::tbstring s("foo");
  std::string_view sv = s;
  EXPECT_EQ(sv, "foo");
  turbo::basic_tbstring<char, custom_traits> s2("bar");
  std::basic_string_view<char, custom_traits> sv2 = s2;
  EXPECT_EQ(sv2, "bar");
}
#endif

TEST(TBString, Format) {
    EXPECT_EQ("  foo", fmt::format("{:>5}", turbo::tbstring("foo")));
}

TEST(TBString, OverLarge) {
    EXPECT_THROW(
            tbstring().reserve((size_t) 0xFFFF'FFFF'FFFF'FFFF), std::length_error);
    EXPECT_THROW(
            tbstring_core<char32_t>().reserve((size_t) 0x4000'0000'4000'0000),
            std::length_error);
}

TEST(TBString, to_std) {
    turbo::tbstring tbs("foo");
    std::string a = tbs;
    std::string_view b = tbs;
    std::string c(tbs);
    std::string_view d(tbs);
    EXPECT_EQ(a, "foo");
    EXPECT_EQ(b, "foo");
    EXPECT_EQ(a, b);
    EXPECT_EQ(c, d);
