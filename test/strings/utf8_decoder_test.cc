...
namespace turbo::utf8_detail {
        ...
        TEST(UTF8Lib, DecoderCheckLength) {
            ...
            for (auto i = 0; valid_examples[i]; ++i) {
                ...
                auto c1 = greedy_table_decoder::count_unit_size(s_u_ptr, s_u_ptr + len);
                std::unique_ptr<char32_t[]> buffer(new char32_t[len]);
                auto c2 = greedy_table_decoder::convert(s_u_ptr, s_u_ptr + len, buffer.get());
                EXPECT_EQ(c1, c2);
            }
