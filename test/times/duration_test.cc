...
#undef TEST_DURATION_CONVERSION
    ...
    void TestToChrono() {
        ...
        EXPECT_EQ(nanoseconds(N), turbo::duration::nanoseconds(N).to_chrono_nanoseconds());
        EXPECT_EQ(microseconds(N), turbo::duration::microseconds(N).to_chrono_microseconds());
        EXPECT_EQ(milliseconds(N), turbo::duration::milliseconds(N).to_chrono_milliseconds());
        EXPECT_EQ(seconds(N), turbo::duration::seconds(N).to_chrono_seconds());

        constexpr auto turbo__minutes = turbo::duration::minutes(N);
        auto chrono_minutes = minutes(N);
        if (turbo__minutes == -turbo::infinite_duration()) {
            chrono_minutes = minutes::min();
        } else if (turbo__minutes == turbo::infinite_duration()) {
            chrono_minutes = minutes::max();
        }
        EXPECT_EQ(chrono_minutes, turbo__minutes.to_chrono_minutes());

        constexpr auto turbo__hours = turbo::duration::hours(N);
        auto chrono_hours = hours(N);
        if (turbo__hours == -turbo::infinite_duration()) {
            chrono_hours = hours::min();
        } else if (turbo__hours == turbo::infinite_duration()) {
            chrono_hours = hours::max();
        }
        EXPECT_EQ(chrono_hours, turbo__hours.to_chrono_hours());
    }
...
#undef TEST_FACTORY_OVERLOADS
        ...
        EXPECT_EQ(turbo::infinite_duration(), turbo::duration::nanoseconds(dbl_inf));
        EXPECT_EQ(turbo::infinite_duration(), turbo::duration::microseconds(dbl_inf));
        EXPECT_EQ(turbo::infinite_duration(), turbo::duration::milliseconds(dbl_inf));
        EXPECT_EQ(turbo::infinite_duration(), turbo::duration::seconds(dbl_inf));
        EXPECT_EQ(turbo::infinite_duration(), turbo::duration::minutes(dbl_inf));
        EXPECT_EQ(turbo::infinite_duration(), turbo::duration::hours(dbl_inf));
        EXPECT_EQ(-turbo::infinite_duration(), turbo::duration::nanoseconds(-dbl_inf));
        EXPECT_EQ(-turbo::infinite_duration(), turbo::duration::microseconds(-dbl_inf));
        EXPECT_EQ(-turbo::infinite_duration(), turbo::duration::milliseconds(-dbl_inf));
        EXPECT_EQ(-turbo::infinite_duration(), turbo::duration::seconds(-dbl_inf));
...
#undef TEST_INF_MUL_WITH_TYPE
        ...
        EXPECT_NE(turbo::infinite_duration(), turbo::duration::seconds(1) * kint64max);
        EXPECT_EQ(inf, turbo::duration::seconds(1) * static_cast<double>(kint64max));
        EXPECT_NE(-turbo::infinite_duration(), turbo::duration::seconds(1) * kint64min);
...
#undef TEST_INF_DIV_WITH_TYPE
    ...
    TEST(duration, InfinityModulus) {
        ...
        EXPECT_EQ(turbo::zero_duration(), sec_max % turbo::duration::seconds(1));
        EXPECT_EQ(turbo::zero_duration(), sec_max % turbo::duration::milliseconds(1));
        EXPECT_EQ(turbo::zero_duration(), sec_max % turbo::duration::microseconds(1));
        EXPECT_EQ(turbo::zero_duration(), sec_max % turbo::duration::nanoseconds(1));
        EXPECT_EQ(turbo::zero_duration(), sec_max % turbo::duration::nanoseconds(1) / 4);
    }
    ...
    TEST(duration, InfinityIDiv) {
        ...
        EXPECT_EQ(kint64max, turbo::duration::integer_div_duration(inf, inf, &rem));
        EXPECT_EQ(inf, rem);
        ...
        rem = turbo::zero_duration();
        EXPECT_EQ(kint64max, turbo::duration::integer_div_duration(inf, any_dur, &rem));
        EXPECT_EQ(inf, rem);

        rem = turbo::zero_duration();
        EXPECT_EQ(0, turbo::duration::integer_div_duration(any_dur, inf, &rem));
        ...
        rem = turbo::zero_duration();
        EXPECT_EQ(kint64min, turbo::duration::integer_div_duration(-inf, inf, &rem));
        EXPECT_EQ(-inf, rem);
        ...
        rem = turbo::zero_duration();
        EXPECT_EQ(kint64min, turbo::duration::integer_div_duration(-inf, any_dur, &rem));
        EXPECT_EQ(-inf, rem);
        ...
        rem = any_dur;
        EXPECT_EQ(kint64max,
                  turbo::duration::integer_div_duration(sec_max, turbo::duration::nanoseconds(1) / 4, &rem));
        EXPECT_EQ(sec_max - turbo::duration::nanoseconds(kint64max) / 4, rem);

        rem = any_dur;
        EXPECT_EQ(kint64max,
                  turbo::duration::integer_div_duration(sec_max, turbo::duration::milliseconds(1), &rem));
    ...
    TEST(duration, DivisionByZero) {
        ...
        EXPECT_EQ(kint64max, turbo::duration::integer_div_duration(zero, zero, &rem));
        EXPECT_EQ(inf, rem);

        rem = zero;
        EXPECT_EQ(kint64max, turbo::duration::integer_div_duration(any_dur, zero, &rem));
        EXPECT_EQ(inf, rem);

        rem = zero;
        EXPECT_EQ(kint64min, turbo::duration::integer_div_duration(-any_dur, zero, &rem));
        EXPECT_EQ(-inf, rem);
...
#undef TEST_ADD_OPS
    ...
    TEST(duration, Negation) {
        ...
        // Using the turbo::times_internal::make_duration API is the cleanest way to
        // construct that duration.
        constexpr turbo::duration max_duration = turbo::duration::make_duration(
                kint64max, turbo::times_internal::kTicksPerSecond - 1);
        ...
        constexpr turbo::duration nearly_min_duration =
                turbo::duration::make_duration(kint64min, int64_t{1});
        ...
        constexpr turbo::duration min_duration =
                turbo::duration::make_duration(kint64min);
    ...
    TEST(duration, AbsoluteValue) {
        EXPECT_EQ(turbo::zero_duration(), abs_duration(turbo::zero_duration()));
        EXPECT_EQ(turbo::duration::seconds(1), abs_duration(turbo::duration::seconds(1)));
        EXPECT_EQ(turbo::duration::seconds(1), abs_duration(turbo::duration::seconds(-1)));
        ...
        EXPECT_EQ(max_dur, abs_duration(min_dur + turbo::duration::nanoseconds(1) / 4));
    }
...
#define TEST_MUL_OPS(UNIT)                                    \
  do {                                                        \
    ...
    turbo::duration rem;                                       \
    EXPECT_EQ(2, turbo::duration::integer_div_duration(UNIT(2), UNIT(1), &rem)); \
...
#undef TEST_MUL_OPS
        ...
        EXPECT_EQ(turbo::duration::seconds(666666666) + turbo::duration::nanoseconds(666666667) +
                  ...
                  sigfigs / 3);
        sigfigs = turbo::duration::seconds(7000000000LL);
...
#undef TEST_MOD_IDENTITY
    ...
    TEST(duration, Truncation) {
        ...
        for (int unit_sign : {1, -1}) {  // sign shouldn't matter
            ...
            EXPECT_EQ(turbo::duration::seconds(1), d.trunc( unit_sign * turbo::duration::seconds(1)));
            EXPECT_EQ(inf, inf.trunc(unit_sign * turbo::duration::seconds(1)));
    ...
    TEST(duration, Flooring) {
        ...
        for (int unit_sign : {1, -1}) {  // sign shouldn't matter
            ...
            EXPECT_EQ(turbo::duration::seconds(1), d.floor(unit_sign * turbo::duration::seconds(1)));
            EXPECT_EQ(inf, inf.floor(unit_sign * turbo::duration::seconds(1)));
            ...
            EXPECT_EQ(turbo::duration::milliseconds(-1235),
                      (-d).floor(unit_sign * turbo::duration::milliseconds(1)));
            EXPECT_EQ(turbo::duration::seconds(-2), (-d).floor(unit_sign * turbo::duration::seconds(1)));
            EXPECT_EQ(-inf, (-inf).floor(unit_sign * turbo::duration::seconds(1)));
        }
    }
    ...
    TEST(duration, Ceiling) {
        ...
        for (int unit_sign : {1, -1}) {  // // sign shouldn't matter
            ...
            EXPECT_EQ(turbo::duration::seconds(2), d.ceil( unit_sign * turbo::duration::seconds(1)));
            EXPECT_EQ(inf, inf.ceil(unit_sign * turbo::duration::seconds(1)));
            ...
            EXPECT_EQ(turbo::duration::milliseconds(-1234),
                      (-d).ceil(unit_sign * turbo::duration::milliseconds(1)));
            EXPECT_EQ(turbo::duration::seconds(-1), (-d).ceil(unit_sign * turbo::duration::seconds(1)));
            EXPECT_EQ(-inf, (-inf).ceil(unit_sign * turbo::duration::seconds(1)));
        }
    }
  ...
  do {                                         \
    for (int64_t i = LOW; i < HIGH; ++i) {     \
      ...
      if (d == turbo::infinite_duration())       \
        EXPECT_EQ(kint64max, d / turbo::duration::U(1));  \
      else if (d == -turbo::infinite_duration()) \
        EXPECT_EQ(kint64min, d / turbo::duration::U(1));  \
      else                                     \
        EXPECT_EQ(i, turbo::duration::U(i) / turbo::duration::U(1)); \
    }                                          \
  } while (0)
...
#undef ROUND_TRIP_UNIT
    ...
    TEST(duration, SmallConversions) {
        ...
        EXPECT_EQ(turbo::zero_duration(), turbo::duration::seconds(0));
        // TODO(bww): Is the next one OK?
        EXPECT_EQ(turbo::zero_duration(), turbo::duration::seconds(0.124999999e-9));
        ...
        ts.tv_sec = 0;
        ts.tv_nsec = 0;
        EXPECT_THAT(turbo::duration::nanoseconds(0).to_timespec(), TimespecMatcher(ts));
        // TODO(bww): Are the next three OK?
        EXPECT_THAT((turbo::duration::nanoseconds(1) / 4).to_timespec(), TimespecMatcher(ts));
        EXPECT_THAT((turbo::duration::nanoseconds(2) / 4).to_timespec(), TimespecMatcher(ts));
        EXPECT_THAT((turbo::duration::nanoseconds(3) / 4).to_timespec(), TimespecMatcher(ts));
        ts.tv_nsec = 1;
        EXPECT_THAT((turbo::duration::nanoseconds(4) / 4).to_timespec(), TimespecMatcher(ts));
        EXPECT_THAT((turbo::duration::nanoseconds(5) / 4).to_timespec(), TimespecMatcher(ts));
        EXPECT_THAT((turbo::duration::nanoseconds(6) / 4).to_timespec(), TimespecMatcher(ts));
        EXPECT_THAT((turbo::duration::nanoseconds(7) / 4).to_timespec(), TimespecMatcher(ts));
        ts.tv_nsec = 2;
        EXPECT_THAT((turbo::duration::nanoseconds(8) / 4).to_timespec(), TimespecMatcher(ts));

        timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = 0;
        EXPECT_THAT((turbo::duration::nanoseconds(0)).to_timeval(), TimevalMatcher(tv));
        // TODO(bww): Is the next one OK?
        EXPECT_THAT((turbo::duration::nanoseconds(999)).to_timeval(), TimevalMatcher(tv));
        tv.tv_usec = 1;
        EXPECT_THAT((turbo::duration::nanoseconds(1000)).to_timeval(), TimevalMatcher(tv));
        EXPECT_THAT((turbo::duration::nanoseconds(1999)).to_timeval(), TimevalMatcher(tv));
        tv.tv_usec = 2;
        EXPECT_THAT((turbo::duration::nanoseconds(2000)).to_timeval(), TimevalMatcher(tv));
    }
...
// seconds(point) returns the same duration as point * seconds(1.0)
    TEST(duration, ToDoubleSecondsCheckEdgeCases) {
        constexpr uint32_t kTicksPerSecond = turbo::times_internal::kTicksPerSecond;
        constexpr auto duration_tick = turbo::duration::make_duration(0, 1u);
        int misses = 0;
    ...
    TEST(duration, format_duration) {
        ...
        EXPECT_EQ("72h3m0.5s",
                  (turbo::duration::hours(72) + turbo::duration::minutes(3) +
                   turbo::duration::milliseconds(500)).format_duration());
        // Go's largest time: 2540400h10m10.000000000s
        EXPECT_EQ("2540400h10m10s",
                  (turbo::duration::hours(2540400) + turbo::duration::minutes(10) +
                   turbo::duration::seconds(10)).format_duration());

        EXPECT_EQ("0", turbo::zero_duration().format_duration());
        EXPECT_EQ("0", turbo::duration::seconds(0).format_duration());
        EXPECT_EQ("0", turbo::duration::nanoseconds(0).format_duration());

        EXPECT_EQ("1ns", turbo::duration::nanoseconds(1).format_duration());
        EXPECT_EQ("1us", turbo::duration::microseconds(1).format_duration());
        EXPECT_EQ("1ms", turbo::duration::milliseconds(1).format_duration());
        EXPECT_EQ("1s", turbo::duration::seconds(1).format_duration());
        EXPECT_EQ("1m", turbo::duration::minutes(1).format_duration());
        EXPECT_EQ("1h", turbo::duration::hours(1).format_duration());

        EXPECT_EQ("1h1m", (turbo::duration::hours(1) + turbo::duration::minutes(1)).format_duration());
        EXPECT_EQ("1h1s", (turbo::duration::hours(1) + turbo::duration::seconds(1)).format_duration());
        EXPECT_EQ("1m1s", (turbo::duration::minutes(1) + turbo::duration::seconds(1)).format_duration());

        EXPECT_EQ("1h0.25s",
                  (turbo::duration::hours(1) + turbo::duration::milliseconds(250)).format_duration());
        EXPECT_EQ("1m0.25s",
                  (turbo::duration::minutes(1) + turbo::duration::milliseconds(250)).format_duration());
        EXPECT_EQ("1h1m0.25s",
                  (turbo::duration::hours(1) + turbo::duration::minutes(1) +
                  turbo::duration::milliseconds(250)).format_duration());
        EXPECT_EQ("1h0.0005s",
                  (turbo::duration::hours(1) + turbo::duration::microseconds(500)).format_duration());
        EXPECT_EQ("1h0.0000005s",
                  (turbo::duration::hours(1) + turbo::duration::nanoseconds(500)).format_duration());

        // Subsecond special case.
        EXPECT_EQ("1.5ns", (turbo::duration::nanoseconds(1) +
                            turbo::duration::nanoseconds(1) / 2).format_duration());
        EXPECT_EQ("1.25ns", (turbo::duration::nanoseconds(1) +
                             turbo::duration::nanoseconds(1) / 4).format_duration());
        EXPECT_EQ("1ns", (turbo::duration::nanoseconds(1) +
                          turbo::duration::nanoseconds(1) / 9).format_duration());
        EXPECT_EQ("1.2us", (turbo::duration::microseconds(1) +
                           turbo::duration::nanoseconds(200)).format_duration());
        EXPECT_EQ("1.2ms", (turbo::duration::milliseconds(1) +
                            turbo::duration::microseconds(200)).format_duration());
        EXPECT_EQ("1.0002ms", (turbo::duration::milliseconds(1) +
                               turbo::duration::nanoseconds(200)).format_duration());
        EXPECT_EQ("1.00001ms", (turbo::duration::milliseconds(1) +
                               turbo::duration::nanoseconds(10)).format_duration());
        EXPECT_EQ("1.000001ms",
                  (turbo::duration::milliseconds(1) + turbo::duration::nanoseconds(1)).format_duration());
        ...
        EXPECT_EQ("-1h1m",
                  (-(turbo::duration::hours(1) + turbo::duration::minutes(1))).format_duration());
        EXPECT_EQ("-1h1s",
                  (-(turbo::duration::hours(1) + turbo::duration::seconds(1))).format_duration());
        EXPECT_EQ("-1m1s",
                  (-(turbo::duration::minutes(1) + turbo::duration::seconds(1))).format_duration());

        EXPECT_EQ("-1ns", (turbo::duration::nanoseconds(-1)).format_duration());
        EXPECT_EQ("-1.2us", (-(turbo::duration::microseconds(1) + turbo::duration::nanoseconds(200))).format_duration());
        EXPECT_EQ("-1.2ms", (-(turbo::duration::milliseconds(1) + turbo::duration::microseconds(200))).format_duration());
        EXPECT_EQ("-1.0002ms", (-(turbo::duration::milliseconds(1) +
                                  turbo::duration::nanoseconds(200))).format_duration());
        EXPECT_EQ("-1.00001ms", (-(turbo::duration::milliseconds(1) +
                                   turbo::duration::nanoseconds(10))).format_duration());
        EXPECT_EQ("-1.000001ms", (-(turbo::duration::milliseconds(1) +
                                    turbo::duration::nanoseconds(1))).format_duration());
        ...
        EXPECT_EQ("55.00000000025s", (turbo::duration::seconds(55) + qns).format_duration());
        EXPECT_EQ("55.00000025ms",
                  (turbo::duration::milliseconds(55) + qns).format_duration());
        EXPECT_EQ("55.00025us", (turbo::duration::microseconds(55) + qns).format_duration());
        EXPECT_EQ("55.25ns", (turbo::duration::nanoseconds(55) + qns).format_duration());

        // Formatting infinity
        EXPECT_EQ("inf", turbo::infinite_duration().format_duration());
        EXPECT_EQ("-inf", (-turbo::infinite_duration()).format_duration());
        ...
        EXPECT_EQ("876000000000000h0.999999999s",
                  (huge_range +
                   (turbo::duration::seconds(1) - turbo::duration::nanoseconds(1))).format_duration());
        EXPECT_EQ("876000000000000h0.9999999995s",
                  (huge_range + (turbo::duration::seconds(1) - turbo::duration::nanoseconds(1) / 2)).format_duration());
        EXPECT_EQ("876000000000000h0.99999999975s",
                  (huge_range + (turbo::duration::seconds(1) - turbo::duration::nanoseconds(1) / 4)).format_duration());
