...
namespace {
    TEST(WaitableEventTest, wait_on_already_timedout_event) {
        ...
        *event = 1;
        ASSERT_EQ(-1, turbo::fiber_internal::waitable_event_wait(event, 1, &now));
        ASSERT_EQ(ETIMEDOUT, errno);
    }

    void *sleeper(void *arg) {
        turbo::fiber_sleep_for((uint64_t) arg);
        return nullptr;
    }
    ...
    void *waiter(void *arg) {
        ...
        const long t1 = turbo::get_current_time_micros();
        const int rc = turbo::fiber_internal::waitable_event_wait(
    ...
    TEST(WaitableEventTest, sanity) {
        ...
        *b1 = 1;
        ASSERT_EQ(0, turbo::fiber_internal::waitable_event_wake(b1));
        ...
        for (size_t i = 0; i < 2 * N; ++i) {
            ASSERT_EQ(1, turbo::fiber_internal::waitable_event_wake(b1));
        }
        ASSERT_EQ(0, turbo::fiber_internal::waitable_event_wake(b1));
        sleep(1);
        turbo::fiber_internal::waitable_event_destroy(b1);
    ...
    void *wait_event(void *void_arg) {
        ...
        int rc = turbo::fiber_internal::waitable_event_wait(arg->event, arg->expected_val, &ts);
    ...
    TEST(WaitableEventTest, stop_after_running) {
        ...
        *event = 7;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            tm.start();
            ASSERT_EQ(0, fiber_start_urgent(&th, &attr, wait_event, &arg));
            ASSERT_EQ(0, turbo::fiber_sleep_for(SLEEP_MSEC * 1000L));
            ASSERT_EQ(0, fiber_stop(th));
            ASSERT_EQ(0, fiber_join(th, nullptr));
            tm.stop();

            ASSERT_LT(labs(tm.m_elapsed() - SLEEP_MSEC), 25);
            // ASSERT_TRUE(turbo::fiber_internal::get_task_control()->
    ...
    TEST(WaitableEventTest, join_cant_be_wakeup) {
        ...
        *event = 7;
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            ASSERT_EQ(0, turbo::fiber_sleep_for(WAIT_MSEC / 2 * 1000L));
            ASSERT_TRUE(turbo::fiber_internal::fiber_worker::exists(th));
            ASSERT_TRUE(turbo::fiber_internal::fiber_worker::exists(th2));
            ASSERT_EQ(0, turbo::fiber_sleep_for(WAIT_MSEC / 2 * 1000L));
            ASSERT_EQ(0, fiber_stop(th));
    ...
    TEST(WaitableEventTest, stop_after_slept) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            ASSERT_EQ(0, turbo::fiber_sleep_for(WAIT_MSEC * 1000L));
            ASSERT_EQ(0, fiber_stop(th));
            ASSERT_EQ(0, fiber_join(th, nullptr));
            tm.stop();
