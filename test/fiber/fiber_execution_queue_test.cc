...
#include <turbo/fiber/internal/execution_queue.h>
...
namespace {
    ...
    struct LongIntTask {
        long value;
        turbo::fiber_latch *event;

        LongIntTask(long v)
                : value(v), event(nullptr) {}

        LongIntTask(long v, turbo::fiber_latch *e)
                : value(v), event(e) {}

        LongIntTask() : value(0), event(nullptr) {}
    };

    int add(void *meta, turbo::fiber_internal::TaskIterator<LongIntTask> &iter) {
    ...
    TEST_F(ExecutionQueueTest, single_thread) {
        ...
        stopped = false;
        turbo::fiber_internal::ExecutionQueueId<LongIntTask> queue_id;
        turbo::fiber_internal::ExecutionQueueOptions options;
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_start(&queue_id, &options,
                                                                  add, &result));
        for (int i = 0; i < 100; ++i) {
            expected_result += i;
            ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, i));
        }
        TURBO_LOG(INFO) << "stop";
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_stop(queue_id));
        ASSERT_NE(0, turbo::fiber_internal::execution_queue_execute(queue_id, 0));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_join(queue_id));
        ASSERT_EQ(expected_result, result);
        ASSERT_TRUE(stopped);
    }

    struct PushArg {
        turbo::fiber_internal::ExecutionQueueId<LongIntTask> id;
    ...
    void *push_thread(void *arg) {
        ...
        while (turbo::fiber_internal::execution_queue_execute(pa->id, t) == 0) {
            sum += num;
            t.value = ++num;
            if (pa->wait_task_completed) {
                e.wait();
    ...
    void *push_thread_which_addresses_execq(void *arg) {
        ...
        timer.start();
        int num = 0;
        turbo::fiber_internal::ExecutionQueue<LongIntTask>::scoped_ptr_t ptr
                = turbo::fiber_internal::execution_queue_address(pa->id);
        EXPECT_TRUE(ptr);
    ...
    TEST_F(ExecutionQueueTest, performance) {
        pthread_t threads[8];
        turbo::fiber_internal::ExecutionQueueId<LongIntTask> queue_id = {0}; // to suppress warnings
        turbo::fiber_internal::ExecutionQueueOptions options;
        int64_t result = 0;
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_start(&queue_id, &options,
                                                                  add, &result));
...
#define BENCHMARK_BOTH
    ...
    int add_with_suspend(void *meta, turbo::fiber_internal::TaskIterator<LongIntTask> &iter) {
        int64_t *result = (int64_t *) meta;
    ...
    TEST_F(ExecutionQueueTest, execute_urgent) {
        ...
        for (int i = 0; i < 100; ++i) {
            ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, -100));
            while (!g_suspending) {
                usleep(100);
            }
            ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(
                    queue_id, -1, &turbo::fiber_internal::TASK_OPTIONS_URGENT));
            g_suspending = false;
            usleep(100);
        }
        usleep(500 * 1000);
    ...
    TEST_F(ExecutionQueueTest, urgent_task_is_the_last_task) {
        ...
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, -1,
                                                                    &turbo::fiber_internal::TASK_OPTIONS_URGENT));
        usleep(100);
        g_suspending = false;
    ...
    void *push_thread_with_id(void *arg) {
        turbo::fiber_internal::ExecutionQueueId<LongIntTask> id = {(uint64_t) arg};
        int thread_id = num_threads.fetch_add(1, std::memory_order_relaxed);
        TURBO_LOG(INFO) << "Start thread" << thread_id;
        for (int i = 0; i < 100000; ++i) {
            turbo::fiber_internal::execution_queue_execute(id, ((long) thread_id << 32) | i);
        }
        return nullptr;
    }

    int check_order(void *meta, turbo::fiber_internal::TaskIterator<LongIntTask> &iter) {
        for (; iter; ++iter) {
    ...
    int check_running_thread(void *arg, turbo::fiber_internal::TaskIterator<LongIntTask> &iter) {
        if (iter.is_queue_stopped()) {
            return 0;
        }
        for (; iter; ++iter) {}
    ...
    TEST_F(ExecutionQueueTest, in_place_task) {
        ...
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_start(&queue_id, &options,
                                                                  ...
                                                                  (void *) thread_id));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(
                queue_id, 0, &turbo::fiber_internal::TASK_OPTIONS_INPLACE));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_stop(queue_id));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_join(queue_id));
    }
    ...
    void *run_first_tasks(void *arg) {
        turbo::fiber_internal::ExecutionQueueId<InPlaceTask> queue_id = {(uint64_t) arg};
        InPlaceTask task;
        task.first_task = true;
        task.thread_id = pthread_self();
        EXPECT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, task,
                                                                    &turbo::fiber_internal::TASK_OPTIONS_INPLACE));
        return nullptr;
    }

    int stuck_and_check_running_thread(void *arg, turbo::fiber_internal::TaskIterator<InPlaceTask> &iter) {
        if (iter.is_queue_stopped()) {
            return 0;
        }
        ...
        if (iter->first_task) {
            ...
            futex->store(1);
            turbo::fiber_internal::futex_wake_private(futex, 1);
            while (futex->load() != 2) {
                turbo::fiber_internal::futex_wait_private(futex, 1, nullptr);
            }
            ++iter;
            EXPECT_FALSE(iter);
        } else {
    ...
    TEST_F(ExecutionQueueTest, should_start_new_thread_on_more_tasks) {
        turbo::fiber_internal::ExecutionQueueId<InPlaceTask> queue_id = {0};
        turbo::fiber_internal::ExecutionQueueOptions options;
        ...
        while (futex.load() != 1) {
            turbo::fiber_internal::futex_wait_private(&futex, 0, nullptr);
        }
        for (size_t i = 0; i < 100; ++i) {
            InPlaceTask task;
            task.first_task = false;
            task.thread_id = pthread_self();
            ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, task,
                                                                        &turbo::fiber_internal::TASK_OPTIONS_INPLACE));
        }
        futex.store(2);
        turbo::fiber_internal::futex_wake_private(&futex, 1);
    ...
    TEST_F(ExecutionQueueTest, size_of_task_node) {
        TURBO_LOG(INFO) << "sizeof(TaskNode)=" << sizeof(turbo::fiber_internal::TaskNode);
    }

    int add_with_suspend2(void *meta, turbo::fiber_internal::TaskIterator<LongIntTask> &iter) {
        int64_t *result = (int64_t *) meta;
        if (iter.is_queue_stopped()) {
            stopped = true;
            return 0;
    ...
    TEST_F(ExecutionQueueTest, cancel) {
        ...
        g_suspending = false;
        turbo::fiber_internal::TaskHandle handle0;
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, -100, nullptr, &handle0));
        while (!g_suspending) {
            usleep(10);
        }
        ASSERT_EQ(1, turbo::fiber_internal::execution_queue_cancel(handle0));
        ASSERT_EQ(1, turbo::fiber_internal::execution_queue_cancel(handle0));
        turbo::fiber_internal::TaskHandle handle1;
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, 100, nullptr, &handle1));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_cancel(handle1));
        g_suspending = false;
        ASSERT_EQ(-1, turbo::fiber_internal::execution_queue_cancel(handle1));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_stop(queue_id));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_join(queue_id));
        ASSERT_EQ(0, result);
    }
    ...
    struct CancelSelf {
        std::atomic<turbo::fiber_internal::TaskHandle *> handle;
    };

    int cancel_self(void * /*meta*/, turbo::fiber_internal::TaskIterator<CancelSelf *> &iter) {

        for (; iter; ++iter) {
            while ((*iter)->handle == nullptr) {
                usleep(10);
            }
            EXPECT_EQ(1, turbo::fiber_internal::execution_queue_cancel(*(*iter)->handle.load()));
            EXPECT_EQ(1, turbo::fiber_internal::execution_queue_cancel(*(*iter)->handle.load()));
            EXPECT_EQ(1, turbo::fiber_internal::execution_queue_cancel(*(*iter)->handle.load()));
        }
        return 0;
    ...
    TEST_F(ExecutionQueueTest, cancel_self) {
        turbo::fiber_internal::ExecutionQueueId<CancelSelf *> queue_id = {0}; // to suppress warnings
        turbo::fiber_internal::ExecutionQueueOptions options;
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_start(&queue_id, &options,
                                                                  cancel_self, nullptr));
        CancelSelf task;
        task.handle = nullptr;
        turbo::fiber_internal::TaskHandle handle;
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, &task, nullptr, &handle));
    ...
    int add_with_cancel(void *meta, turbo::fiber_internal::TaskIterator<AddTask> &iter) {
        if (iter.is_queue_stopped()) {
            return 0;
        }
        ...
        for (; iter; ++iter) {
            if (iter->cancel_task) {
                const int rc = turbo::fiber_internal::execution_queue_cancel(iter->handle);
                if (rc == 0) {
    ...
    TEST_F(ExecutionQueueTest, random_cancel) {
        turbo::fiber_internal::ExecutionQueueId<AddTask> queue_id = {0};
        AddMeta m;
        m.sum = 0;
        m.expected.store(0);
        ...
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_start(&queue_id, nullptr,
                                                                  add_with_cancel, &m));
        int64_t expected = 0;
        for (int i = 0; i < 100000; ++i) {
            turbo::fiber_internal::TaskHandle h;
            AddTask t;
            t.value = i;
            t.cancel_task = false;
            ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, t, nullptr, &h));
            const int r = turbo::base::fast_rand_less_than(4);
            expected += i;
            if (r == 0) {
                if (turbo::fiber_internal::execution_queue_cancel(h) == 0) {
                    expected -= i;
                }
            } else if (r == 1) {
                t.cancel_task = true;
                t.cancel_value = i;
                t.handle = h;
                ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, t, nullptr));
            } else if (r == 2) {
                t.cancel_task = true;
                t.cancel_value = i;
                t.handle = h;
                ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, t,
                                                                            &turbo::fiber_internal::TASK_OPTIONS_URGENT));
            } else {
                // do nothing;
            }
    ...
    int add2(void *meta, turbo::fiber_internal::TaskIterator<LongIntTask> &iter) {
        if (iter) {
            int64_t *result = (int64_t *) meta;
            *result += iter->value;
            if (iter->event) { iter->event->signal(); }
    ...
    int add_with_suspend3(void *meta, turbo::fiber_internal::TaskIterator<LongIntTask> &iter) {
        int64_t *result = (int64_t *) meta;
        if (iter.is_queue_stopped()) {
            stopped = true;
            return 0;
    ...
    TEST_F(ExecutionQueueTest, cancel_unexecuted_high_priority_task) {
        ...
        turbo::fiber_internal::TaskHandle h;
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(
                queue_id, -100, &turbo::fiber_internal::TASK_OPTIONS_URGENT, &h));
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_cancel(h));

        // Resume executor
        g_suspending = false;

        // Push a normal task
        ASSERT_EQ(0, turbo::fiber_internal::execution_queue_execute(queue_id, 12345));
