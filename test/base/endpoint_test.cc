...
#include "turbo/base/endpoint_extended.h"
#include "turbo/log/logging.h"
#include "turbo/container/flat_map.h"

namespace {

    using turbo::detail::extended_end_point;

    TEST(EndPointTest, comparisons) {
        turbo::end_point p1(turbo::int2ip(1234), 5678);
        turbo::end_point p2 = p1;
    ...
    TEST(EndPointTest, ip_t) {
        TURBO_LOG(INFO) << "INET_ADDRSTRLEN = " << INET_ADDRSTRLEN;
    ...
    TEST(EndPointTest, show_local_info) {
        TURBO_LOG(INFO) << "my_ip is " << turbo::my_ip() << std::endl
    ...
    TEST(EndPointTest, endpoint) {
        ...
        turbo::end_point p2(turbo::IP_NONE, -1);
        ...
        ASSERT_EQ(0, hostname2endpoint("localhost:65535", &p5)) << turbo_error();
        ASSERT_EQ(0, hostname2endpoint("localhost:0", &p5));

    }

    TEST(EndPointTest, hash_table) {
        turbo::container::hash_map<turbo::end_point, int> m;
        turbo::end_point ep1(turbo::IP_ANY, 123);
        turbo::end_point ep2(turbo::IP_ANY, 456);
        ++m[ep1];
    ...
    TEST(EndPointTest, flat_map) {
        turbo::container::FlatMap<turbo::end_point, int> m;
        ASSERT_EQ(0, m.init(1024));
        uint32_t port = 8088;

        turbo::end_point ep1(turbo::IP_ANY, port);
        turbo::end_point ep2(turbo::IP_ANY, port);
        ++m[ep1];
        ++m[ep2];
        ASSERT_EQ(1u, m.size());
        ...
        for (int i = 0; i < 1023; ++i) {
            turbo::end_point ep(turbo::int2ip(++ip), port);
            ++m[ep];
        }

        turbo::container::BucketInfo info = m.bucket_info();
        TURBO_LOG(INFO) << "bucket info max long=" << info.longest_length
    ...
    static void test_parse_and_serialize(const std::string &instr, const std::string &outstr) {
        ...
        turbo::end_point_str s = turbo::endpoint2str(ep);
    ...
    TEST(EndPointTest, unix_socket) {
        ...
        ASSERT_EQ(-1, turbo::str2endpoint("unix:tooloooooooooooooooooooooooooooooooooooooooooooooooooo"
                                          "ooooooooooooooooooooooooooooooooooooooooooooooong.sock", &point));
#ifdef TURBO_PLATFORM_LINUX
        ASSERT_EQ(0, turbo::str2endpoint(" unix:looooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooong.sock", &point));
        ASSERT_EQ(std::string("unix:loooooooooooooooooooooooooooooooooooooooooooooooooooo"
                  ...
                  turbo::endpoint2str(point).c_str());
#endif
