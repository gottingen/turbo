...
if [[ ${USE_BAZEL_CACHE:-0} -ne 0 ]]; then
  ...
  BAZEL_EXTRA_ARGS="--remote_http_cache=https://storage.googleapis.com/turbo-bazel-remote-cache/${container_key} --google_credentials=/keystore/73103_absl-bazel-remote-cache ${BAZEL_EXTRA_ARGS:-}"
fi
      ...
      time docker run \
        ...
        /bin/sh -c "
          ...
          if [ -n \"${ALTERNATE_OPTIONS:-}\" ]; then
            cp ${ALTERNATE_OPTIONS:-} turbo/base/options.h || exit 1
          fi
          /usr/local/bin/bazel test ... \
            ...
            --test_env=\"TZDIR=/abseil-cpp/turbo/time/internal/cctz/testdata/zoneinfo\" \
            --test_output=errors \
